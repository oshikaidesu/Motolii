...
- **発注はreturn後の再選定まで閉じる**: 利用者成果の背骨から一契約境界をcompileし、実装できない時は検索場所、候補、採否、不適合理由、exact gap、再入場条件を`RESEARCH_RETURN`として返す。主担当は古い`next`へ戻らずcurrent codeから次edgeを再選定する。詳細は[発注コンパイルと調査返却loop](docs/reviews/2026-08-07-outcome-order-compilation-and-research-return-loop.md)
