...
- **2026-07-23運用改訂**: Terra固定を撤回し、Codexが`mechanical / standard / rapid / complex / cross-boundary`から`TASK_CLASS`を選ぶ。対応する完全model IDは順に`gpt-5.4-mini-none` / `gpt-5.6-luna-none-fast` / `gpt-5.6-terra` / `gpt-5.6-sol-none-fast` / `gpt-5.6-sol-none-fast`とし、aliasは使わない。価格・利用倍率・期間は固定せず、同一課題比較と利用可能性で見直す
