...
- **M3 RN製品source凍結**: `ui/motolii-rn/`がRN、Rerun、Skiaを束ねる唯一の製品source兼write targetである。`ui/motolii-rn-legacy/`は利用者がexact path付きで解凍するまでread-onlyの旧製品shellとし、ここへのwrite、製品codeのcopy／移植、第二RN app／shell／rootを作らない
