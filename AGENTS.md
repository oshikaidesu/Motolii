...
- **M3 RN製品targetは一つだけ**: 通常の「RN」「RN Stage」「現UI」はsource `ui/motolii-rn/`、app `MotoliiRn`、workspace `ui/motolii-rn/macos/MotoliiRn.xcworkspace`、scheme `MotoliiRn-macOS`、build入口 `scripts/build-macos-app.sh`へ解決する。通常の実装・確認では`ui/motolii-rn-legacy/`、旧worktree、DerivedData、リポジトリ外artifactを検索・候補化・起動しない。current targetが使えなければ旧版へfallbackせず、exact gapを返す。歴史調査は利用者が明示した時だけ別taskで行う
- **凍結copyを増やさない**: 過去版の保全はGit履歴に任せ、新しい`legacy`／`frozen`／`archive` source tree、別RN app／shell／rootを作らない。既存`ui/motolii-rn-legacy/`は通常routeから除外された履歴物であり、read／write／build／launch targetにしない
