...
- **状態を先取りしない**: WIP、fixture、probe、test green、外部review、main統合、通常製品route、製品完成を証拠なしに読み替えない。これはartifactを別pathへ移す規則ではない。実入力接続と該当製品oracleを通した同じartifactは、code locationを変えず次の状態へ更新できる。LLMの賛同をauthorityや採用資格にしない
...
- **RN製品面は2026-08-16に撤去済み**: `rn_product_host`、`product_runtime`(winit窓＋wry WebView host)、`ui/motolii-rn/native-renderer`(RN staticlib)、旧egui app(`app/`、`timeline_egui`、`motolii_ui_shell`)はmainに無い。`scripts/build-macos-app.sh`と`ui/motolii-rn/macos/MotoliiRn.xcworkspace`はfileとして残っているが**native-rendererを必要とするので動かない**。復元、検索、build、launchしない。詳細と帰結は[Web窓とRN製品面の畳み込み](docs/reviews/2026-08-16-web-window-and-rn-product-fold.md)
- **`ui/motolii-rn/src`のTypeScriptは移植元として残す**: 色・寸法・構造の唯一の出所は`productStyles.ts`と各`.tsx`であり、Blitzパネルはそこからfile:line付きで写す。**Rustのbuildには入っていない**(TSなので)。値をここで決めない
- **現在mainで実行できるのは`motolii-blitz-shell`だけ**: eframe/eguiがhost、`egui_tiles`がドッキング、StageはRerun Spatial Viewerのegui widget、他paneはBlitzテクスチャ。**入力は未配線**(Stageのorbit/zoomのみ副産物として通る)。絵の確認は`motolii-blitz-dump`。current targetが使えなければ旧版へfallbackせず、exact gapを返す。歴史調査は利用者が明示した時だけ別taskで行う。**(2026-08-19注記: 上記は2026-08-16時点の記述。2026-08-18裁定でホストをicedへ移行中、`motolii-shell-iced`もmainで並走中。詳細は[CANON.md](docs/CANON.md))**
- **ホストがeguiなのは好みではなく構造上の帰結**: Rerun Spatial Viewerは`show(ui, ..)`で`&mut egui::Ui`を要求する**eguiウィジェット**である。したがって**ホストがeguiでないとStageが動かない**。ここからドッキングもeguiの責任になる(`egui_tiles`)。**CSSやBlitzへドッキングを移す案は成立しない** — Blitzのcustom widgetへ渡るのはwgpuの描画コールバックだけで`egui::Ui`ではなく、Stageを入れるには**eguiをもう1つ立てる**ことになる。それは2026-08-16に18.6→2.1ms/frameのために撤去したものであり、戻すと入力も落ちる。**Stageは`egui_wgpu::Callback`ではなく`show(ui)`で挿すこと**
- **Timelineの実行時基盤はegui**(2026-08-16再選定。同日のBlitz裁定は撤回済み): HTML/CSSモック(`docs/mocks-ui`)はUXの台本、Blitzはビルド時に計算済みの値を取り出すコンパイラで、実行時には出てこない。現在の実装は`crates/motolii-ui/src/timeline_editor/`(`DocumentWriter`/D2直結、計9,059行)で、**shellへ結線済み**(`blitz_shell`のTimeline paneが直接呼ぶ。`examples/timeline_egui_lab.rs`は同じ実装を呼ぶ薄殻として残る)。`timeline_egui.rs`という名のファイルは存在しない(旧`timeline_egui/`は本行の再選定と同日2026-08-16に削除)。`timeline_projection`/`*_gesture`/`timeline_viewport_state`はrenderer非依存で本決定の外側。`timeline_skia_raster`は**意味・hit・oracleの源として残すが製品経路ではない**(呼び手は無い)。詳細は[Timeline実行時基盤の再選定](docs/reviews/2026-08-16-timeline-runtime-reselection-to-egui.md)
- **過去UIをmainへ置かない**: 過去版の保全はGit履歴に任せ、`legacy`／`frozen`／`archive` source tree、別RN app／shell／rootをmainへ作らない。旧R0 UI sourceは2026-08-11にmainから削除済みであり、通常作業で復元、検索、build、launchしない
- **Rerun wrapper既定**: M3 Stage／M5 spatial runtimeは固定Rerun Spatial Viewer subsystemへ相乗りし、MotoliiはDocument／D2、creator向け意味、snapshot／time／identity翻訳、admission、terminal intent、Preview／Export policyだけを薄く持つ。現行製品artifactを別targetへcopyせず、固定fixtureを実Document入力へ順次置き換える。`re_renderer`直結の独自scene／view／query／camera／picking再構築、Rectangle専用frame、第二runtimeを作らない。Rerun機構を外すのはnamed product oracleとの実測衝突を示し、利用者が例外を決定した時だけとする
...
- 通常channelはOpenAI系をCodex direct、Anthropic系をClaude direct、Cursor first-partyのGrok 4.6／Composer 2.5をCursorとする。Cursor上のthird-party modelは対応direct channelがlimit／capacity／障害で実際に利用不能な時だけ、fresh runへ代用理由とexact IDを記録して使う。Cursor `auto`や黙ったfallbackは使わない
- 総監督はauthority、次粒、owner、scope、oracle、finding処分、最終統合を所有する。order未閉鎖かつ探索範囲boundedならTerraで候補compile、極小closed施工はfresh Spark、通常のclosed施工はGrok 4.6 medium non-fast、複雑な一境界はhigh、長時間agenticなbounded施工やvisual／interactive成果はxhighを候補にする。Composer 2.5 standardは価格／capacity／task実測の明示理由がある代替施工にする。Grok 4.6の能力がSol級でもauthorityや最終採否は移譲しない。既にclosedならTerraを省略し、同じoutcome／owner／scope／oracleの短wave内でもreturn後に次edgeを選び直す。固定直列route、自動fallback、同じ施工modelの自己reviewを作らない。詳細は[役割再配置](docs/reviews/2026-08-07-terra-grok-composer-role-reallocation-decision.md)と[LLM役割選択](docs/reviews/2026-08-03-history-calibrated-llm-role-selection-decision.md)
