...
## 最初にこれを実行する(会話の文脈から始めない)

```bash
python3 scripts/plan_steps.py     "$(git rev-parse --show-toplevel)"   # 次にやる段階。ここから選ぶ
...
**現在地はこの4つが出す。文書ではない。** 引き継ぎ文書は無い(2026-08-23 に廃止 —
状態が生成される形になったので運ぶ物が無くなった)。**どの時点からでもここで再開できる。**
