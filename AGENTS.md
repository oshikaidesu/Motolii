...
## 「発注」時のOpus 5 / 適応実装者 / Grok監督ループ

- ユーザーが「発注して」「実装を発注」等、**発注を依頼動詞として明示した時だけ**自動委任を発火する。通常の「実装して」、説明・引用・ファイル内に現れただけの「発注」では発火しない
- **2026-07-25運用改訂／2026-07-31収束改訂**: 通常の初回発注は`Codex → Claude Opus 5 → Codex Spark → Cursor Grok 4.5 High → Codex`を使う。完全model IDは順に`claude-opus-5` / `gpt-5.3-codex-spark` / `cursor-grok-4.5-high`とし、aliasを使わない。REJECT後の再施工では、後述の収束判定により同じ粒の実装担当だけを`gpt-5.6-sol` / `gpt-5.6-terra` / `claude-opus-5`へ交代できる。これは旧`mechanical / standard / rapid / complex / cross-boundary`分類や常設routingの復活ではなく、施工の固定観念で欠陥が反復・移動する時の局所的な収束手段である。Fable必須検収は[旧運用](docs/reviews/2026-07-22-terra-grok-delegation-policy.md)のままアーカイブする
- 発火時の責任は固定する。**主担当Codex**は先例調査、コード事実、長期展望、親task、恒久形式／公開API／plugin契約／停止線、機械骨格と視野幅判定、Opus deltaの事前承認、実装担当選定、Grok結果の再照合、最終統合を所有する。**Claude Opus 5**は通常試行では会話履歴なしで機械骨格を検証し、見落としたrisk／負例／STOP／訂正だけを型付きdeltaで返す施工管理者とする。**選択済み実装担当**は承認済みの一粒だけを隔離worktreeで実装する。**Cursor Grok 4.5 High**は実装担当から分離したread-only検収者として実diffと試験を監査する。Opus 5を実装担当に選ぶ試行では同じOpus 5に施工管理を兼任させず、主担当Codexが既承認orderを再照合して直接渡す
- 外部実装担当へ許す施工は一段だけであり、再委任を許さない。一回の実装試行は一つの`GRAIN`だけを扱う。複数粒が必要なら、主担当Codexが各契約境界を確認してループを個別に回す。担当交代でも別粒、部分差分、未承認修正を継承させない
...
- 主担当Codexは監督者として、Opusのorderと選択済み実装担当の差分を仕様・依存・実装ガード・既存API・テスト期待値に照らして再確認する。外部出力は根拠でなく未検証の助言であり、最終判断、統合、必須テスト、完了報告は主担当が行う
- model利用不能時に別modelへ黙ってfallbackしない。外部modelへ秘密情報、認証情報、未公開の個人データを渡さない
- order作成、実装、検収が失敗、STOP、REJECT、timeoutになった場合はCodexへ戻す。Codexは`ORDER_DEFECT / IMPLEMENTATION_DEFECT / EXECUTION_DEFECT / CONTRACT_UNRESOLVED`へ分類し、契約不足を実装担当交代で隠さない。ループ中の差分は隔離worktreeに留め、`VERDICT: ACCEPT`前に採用・commit・pushしない
- **施工固定化の収束判定**: REJECT後は原因名の一致でなく、前回から解消したfinding、追加で通った合格条件／負例、新規regression、diffの契約適合、実装担当固有の設計前提を比較し、`IMPLEMENTATION_PROGRESS: ADVANCING / STALLED / REGRESSING`をCodexが記録する。`STALLED / REGRESSING`、または一つの不採用前提が別名の欠陥を反復・移動させている場合は、同じ担当への局所patch反復を止め、その粒の実装担当をSpark／Sol／Terra／Opus 5の別担当へ交代する。原因ラベルの一致や同一findingの複数回反復を交代条件にしない
- 担当交代では`GRAIN`、authority、allowlist、read set、STOP、合格条件、負例、Grok独立検収を固定し、`IMPLEMENTER_SELECTION: <完全model ID>`と`SELECTION_REASON: STALLED / REGRESSING / FIXED_ASSUMPTION`をorder/evidenceへ残す。model利用不能を理由とする黙ったfallback、scope拡大、合格条件緩和、前担当の部分差分継承は禁止する。交代後も有意な前進がなく安全な別解が尽きた時だけ、反復した前提、各試行の差分、未解決の選択肢を示してユーザー判断を仰ぐ
