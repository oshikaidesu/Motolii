...
- 実装発注は隔離worktreeで行う。Codexが対象仕様ID、目的、現状、変更許可ファイル、非目標、再利用箇所、STOP条件、必須負例、実行コマンドを含むclosed orderを作り、`ORDER: READY`、task hash、`CODEX PRECHECK: APPROVED`を明示してから`./scripts/delegate-cursor-supervised.sh execute <worktree> <order-file> "<task>"`でTerraを起動する。`prepare`はGrokのread-only助言が有用な時だけ使い、その出力を自動承認しない
