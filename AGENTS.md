...
- **M3 RN製品source凍結**: 現在の接続targetは`spikes/motolii-rn-probe/`であり、RN、Rerun、Skiaが共存する同じartifactをその場で`PROBE`から`PRODUCT_SOURCE`へ繰り上げる。`ui/motolii-rn/`は利用者がexact path付きで解凍するまでread-onlyの旧製品shellとし、ここへのwrite、probeからのcopy／移植、第二RN app／shell／rootを作らない。`probe`は成熟状態であって廃棄先ではなく、接続成功はcode locationを変えずproduct stateを変える
- **Rerun wrapper既定**: M3 Stage／M5 spatial runtimeは固定Rerun Spatial Viewer subsystemへ相乗りし、MotoliiはDocument／D2、creator向け意味、snapshot／time／identity翻訳、admission、terminal intent、Preview／Export policyだけを薄く持つ。既存probe artifactはcopyせずその場で製品化し、固定fixtureは実Document入力へ順次置き換える。`re_renderer`直結の独自scene／view／query／camera／picking再構築、Rectangle専用frame、第二runtimeを作らない。Rerun機構を外すのはnamed product oracleとの実測衝突を示し、利用者が例外を決定した時だけとする
