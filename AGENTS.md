...
- **追いつきターンの波運転(裁定189、2026-08-22)**: 意味既決の消化フェーズでは、レーンの検収線を `cargo check --tests -p <crate>` まで(テストは書くが実行は後送 — 落ちるテスト先行の「書く」は維持)。supervisor が波単位で merge を束ね**一括 cargo test を消費点の直前に1回**(消費点= 実窓を建てる時・push 区切り・引き継ぎ — merge が何本溜まったかは走らせる理由にしない。その間の main の赤は誰も踏まない)。赤は fix-forward、帰属が曖昧なら bisect(log₂N 回で有界 — 赤レートが上がったら束を縮める)。レーン毎の cold テスト税(〜10分)が波1回に集約される。新しい意味論・store/engine 跨りの束は従来の即検収に戻す
