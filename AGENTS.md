...
- **現在mainで実行できるのは`motolii-blitz-shell`だけ**: eframe/eguiがhost、`egui_tiles`がドッキング、StageはRerun Spatial Viewerのegui widget、他paneはBlitzテクスチャ。**入力は未配線**(Stageのorbit/zoomのみ副産物として通る)。絵の確認は`motolii-blitz-dump`。current targetが使えなければ旧版へfallbackせず、exact gapを返す。歴史調査は利用者が明示した時だけ別taskで行う
