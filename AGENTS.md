...
- **利用者成果の背骨を先に立てる**: 複数粒にまたがる新規挙動は、通常製品routeの操作列、stable identity、成功出口、失敗回復、自動oracle、external gateを先に固定する。実装は一契約境界のまま、調査不足を`REUSE / REMAP / REDUCE / 再調査 / WAIT_TARGET`へ局所処分し、古い`next`や粒数でなく現行codeから次edgeを再計測する。詳細は[利用者成果の背骨と調査不足粒の自律再接続決定](docs/reviews/2026-08-04-outcome-spine-autonomous-gap-research-decision.md)
...
- Sol mediumはauthority衝突、粒分割／STOP、複数契約統合、最終採否／main統合を保持する。閉じたpreflight、次手選定、bounded施工、同一境界修正、diff／test照合はfreshなLuna Maxへ委譲でき、単純施工・修正はfreshなSparkも選べる。固定routeや独立review資格にはしない。詳細は[LLM役割選択](docs/reviews/2026-08-03-history-calibrated-llm-role-selection-decision.md)
