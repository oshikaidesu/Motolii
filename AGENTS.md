...
- **Timelineの正本は`timeline_blitz`**(2026-08-16裁定): 描画・行・clip・keyはBlitz(HTML/CSS＋custom widget)側で持つ。`timeline_skia_raster`は**意味・hit・oracleの源として残すが製品経路ではない**(呼び手は無い)。詳細は[BlitzがTimelineの正本](docs/reviews/2026-08-16-blitz-timeline-authority.md)
