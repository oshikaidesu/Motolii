...
- **追いつきターンの波運転(裁定189、2026-08-22)**: 意味既決の消化フェーズでは、レーンの検収線を `cargo check --tests -p <crate>` まで(テストは書くが実行は後送 — 落ちるテスト先行の「書く」は維持)。supervisor が波単位で merge を束ね**一括 cargo test を波の閉包時に1回**(閉包= その波の全レーンが merge され、**次波の発注の前提になる直前** — 未検証の main を前提に次を積まない、が内在的な理由。利用者起点の消費点(実窓・push・引き継ぎ)は追加の関門であって主ゲートではない)。もう1つの内在理由= 帰属の賞味期限: 赤の bisect が安いのは波の worktree と RETURN が生きているうちだけ。赤は fix-forward(log₂N 回で有界 — 赤レートが上がったら束を縮める)。レーン毎の cold テスト税(〜10分)が波1回に集約される。新しい意味論・store/engine 跨りの束は従来の即検収に戻す
