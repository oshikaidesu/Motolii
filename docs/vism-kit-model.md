...
Kitの責任は決まっており、構想から消えたわけではない。未決なのは公開schema、container、typed consumer方式、atomic materializeの実装である。現行コードがまだKitを実行しないことを、責任未定義と読み替えない。

| 概念 | 固定する責任 | 現在の状態 |
|---|---|---|
| **Vism Kit** | provider／consumer、型付き接続、初期値、素材要求を用途へ組み、Projectへmaterializeする | 設計原則決定、VSM-B0〜B2/B2I待ち |
| **Vism package** | 一つの持ち運べる表現のidentity、実装、要求能力、互換責任 | コンセプト決定、manifest／payload未決 |
| **curator list／feed** | 複数のVism／Kitを紹介する外部の発見情報。意味論artifactではない | 外部キュレーションを使う方向だけ決定、共通schema／UI未決 |
| **Project Lock**（旧仮称Plugin Lock） | 作品再現に必要なpackage、version、source／artifactの解決結果 | 歴史から回収した比較候補、schema未決 |
| **catalog／分散index** | 作者のsourceやartifactを発見する索引 | Motolii運営server非依存は決定、protocol／署名／失効未決 |
| **install store／runtime loader** | 検査済みartifactの端末配置と実行 | v2、未実装 |

Kitは接続済みの用途を人へ渡す**構成兼共有単位**、curator list／feedは複数のVism／Kitを紹介する**外部の発見情報**、Lockは作品の**再現**、catalogは**発見地図**である。旧仮称`Plugin Set`を別artifactとして残さない。Motolii projectが常時稼働する配布backendを運営しない原則も、Kitの実行機構ではない。作者GitHub等を使う具体topologyは比較中である。用語統合は[Kit / Plugin Set統合決定](reviews/2026-07-23-vism-kit-rack-unification-decision.md)、配布系譜の処分と再入場条件は[歴史回収](reviews/2026-07-23-historical-vism-kit-distribution-lineage-recovery.md)を参照する。
...
KitはAbleton Rackに似た構造上の類比として、複数の能力を接続済みの一単位へまとめ、用途に必要な初期値と公開controlを作者が設計する。受け手は一つの完成した入口として使え、展開後は内部のVismと接続をinspection、調整、forkできる。

この類比はAbletonのfile format、device runtime、UI、license、音楽中心の製品像を引き継ぐ意味ではない。MotoliiのKitは映像表現の型付き構成であり、次を満たさない任意の導入bundleをKitと呼ばない。

- 複数Vismまたはprovider／consumerの具体的な選択。
- 型付き接続。
- 初期parameterと利用者へ公開するcontrol。
- 必要assetと利用不能時の診断。
- 一つの目的、look、workflowとして読めるpreview／example。

無関係なVism／Kitのおすすめ集合はcurator list／feedであり、Kitではない。単一Vismの不可分なlookは一Vism、単一能力の値だけならPresetを使う。
...
Beat Map Vismのprovider候補:

- Projectの固定BPM／tempo map。
- 音声解析Vism。
- MIDI import。
- 手動tapでmaterializeしたevent列。
...
最初の検証では現行BPMをHost内蔵providerとして読み、Beat位置またはevent列へ変換するpre-Vism参照実装を作る。次を比較してから処分する。

1. 固定BPMだけなら現行`Bpm`をCore primitiveとして残す方が小さいか。
2. tempo／meter mapをVism providerへ置く方が意味とfork拡張を分離できるか。
3. `DataTrack<Value>`で十分か、typed event domainが必要か。
4. Beat Gridは汎用Time Guide投影へ畳めるか。
5. 旧Projectの追加migrationなしでproviderへ投影できるか。
