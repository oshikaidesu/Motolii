...
**重さの実測(2026-09-13、1080p、M4、release、`examples/feedback_cost.rs`)**: 実写 720p を 1.5 倍、RGB Trail(板の道)+
Background Delay(画面の道 + 別時刻の合成)。

| 場面 | 実写だけ | + RGB Trail | + Background Delay |
|---|---|---|---|
| 順再生(ms/frame) | 8.6 | 10.0 | 17.2 |
| 飛び 48→60(写し 30 から 29 歩) | 10 | 115 | 424 |
| 戻り 48→40(写し 30 から 10 歩) | 12 | 9 | 77 |
| 遠く 40→110(写し 60 から 49 歩) | 55 | 230 | 1946 |
| 欄を回す @110(入点から 110 歩) | 13 | 232 | 860 |

順再生は 60 fps 圏。飛びは「直近の写しからの歩数 × 1 歩の重さ」で、板の道は 1 歩 4 ms、画面の道(フレーム丸ごと)は
1 歩 15〜40 ms。効くつまみは 2 つ: 写しの間隔(`FEEDBACK_CHECKPOINT_EVERY`、30 → 15 で飛びが半分、RAM 倍)と、
編集時に捨てる範囲(今は全部 → 変わった層だけ、は未実装)。Draft の近似(辿り直しを粗く)は 6-3 の通り許される。
