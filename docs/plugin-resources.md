...
作成日: 2026-07-10
ステータス: **凍結ゲートで確定**(契約シグネチャ。GpuAssetCache結線・Importer実装はM2/将来)
...
プラグイン契約には現在、両立不能な2つの規律がある:

- 「隠れた可変状態を持たない」(純関数契約。[plugin-authoring.md](plugin-authoring.md)§3-3)
- 「ループ内でGPUリソースを毎フレーム新規生成しない」(同§3-6)

参照プラグイン(Clear系)はリソースを持たないため矛盾が露呈していないが、**パイプラインを1本持つ普通のグローFilterを書いた瞬間に両立機構がないことが露呈する**。コア側は `RenderSession`(motolii-render)がノードのパイプラインと中間ターゲットをフレーム間で使い回して解決済みなのに、プラグインには相当する口がない。
...
### D2. アセット: Importer種別 + ホスト所有GpuAssetCache

- **Importer種別を新設**(予約済み`PluginKind::Input`の具体化): `ファイル(バイト列) → Assetペイロード(opaque blob + type文字列 + 内容ハッシュ)`。CPU側の変換であり、GPU・時刻・パラメータ評価に触らない
...
`ValueType`/`Value`に**AssetRef(アセットID参照)を予約**する(必要なら汎用Stringも同時に判断)。点群に限らず「ファイルを食うプラグイン」全般の前提。パラメータスキーマの互換性(F-9/G-1のparam同一性)に波及するため、**変種の予約だけは凍結ゲートで行う**。実装(ドキュメントモデルのAssetテーブルとの結線)はM2 D1。

- M2 D1のAsset定義は「動画・SVG素材」から「**opaqueペイロード+type文字列を持つ一般アセット**」に広げる(動画・SVGはその特殊ケース)

### D4. レジデンシと決定性: residency = f(Quality)

ストリーミングLODは`render_frame(t, Quality)`の決定性(B-4)と衝突する。先人に倣い:

- **FINAL**: 要求LODのロード完了まで待つ(ブロック)。出力は入力とtだけで決まる=決定的
- **DRAFT**: ポイントバジェット内で「今あるもの」を描く。プレビューのみの近似として許容
...
|---|---|
| PipelineCacheの最小実装+パイプラインを持つ参照Filter 1個での実証 | Importer種別の実装(契約定義と`PluginKind`の具体化だけ先行) |
| `ValueType::AssetRef`の予約 | 点群本体(八分木・バジェット・compute rasterizer) |
| M2 D1のAsset一般化(opaque+type) | LODストリーミング、residency=f(Quality)の実装 |
| GpuAssetCacheの契約定義(trait口) | 構成要素別ハッシュ分割 |

## 5. 凍結ゲートとの関係(並列化の入場条件)

[pitfalls-and-roadmap.md](pitfalls-and-roadmap.md) G-1の入場条件に従い、**紙のまま凍結しない**。この文書の決定を凍結ゲートに通すための実証:

1. **グラフ実行器のプラグインディスパッチ**: `RenderStep`に一般ステップ(プラグインID+params+入出力)を追加し、参照FilterがPluginRegistry経由でグラフ内から呼ばれてゴールデンが通る(現状はParamDriverのみが製品経路でレジストリを通っており、Filter/Composite/LayerSourceは登録できるが呼ばれる口がない)
...
3. 1と2が通った時点で、trait+PipelineCache+GpuAssetCacheの**契約シグネチャ**を凍結する。AssetRef/Importerは契約の予約のみで凍結に含める(実装はM2/将来)

この3点(1=Filterディスパッチ、2=TintFilter+PipelineCache)は2026-07-10に達。残りのG-1入場条件(Composite Pluginゴールデン・TimeMap製品経路・F-2骨格・param移行・Draft/Final一致・口の型予約)は[reviews/2026-07-10-freeze-gate-remaining.md](reviews/2026-07-10-freeze-gate-remaining.md)。**FG-C1〜C6全緑まで**、M2のD1(Assetスキーマ)とD3(doc→グラフ変換)を並列レーンに出さない。
