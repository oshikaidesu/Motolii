...
| `M3-R1-BROWSER-RECTANGLE` | 現RN Browserの通常Create catalogでRectangleを選び、double clickでStage中央、D&Dで任意のStage位置へ出せる | `PARTIAL / UI ACTIVATION LANDED`。Effectsを親にした共通Browser view、Media由来のサムネイルのみ表示、single click選択、double click中央Rerun投影、D&DのStage正規化座標投影をcurrent product sourceへ着地。D2 terminalは`R1-SHELL`の共通Host接続待ち | `ui/motolii-rn/App.tsx`のBrowser catalog／viewと既存Fabric Rerun Stage。Document writer 0 | RN pointer lifecycle、既存Fabric props更新、Rerun `LineDrawableBuilder`を`REUSE`。専用event bus、React overlay、mock Documentを作らない。製品接続時は既存Host platform capture／native latest-epoch hit-testへ置換する | Jestでsingle click時Stage不変／double click中央／drop座標正規化／Stage外0、Rustでdrop中心と不正座標拒否。未達はProduct Host／D2 terminal、source identity、Undo/Redo |
...
| `DEV-RN-MAC-WARM-BUILD` | RN製品UIの反復buildを、worktreeを分けても短くする | `ADOPTED`。`scripts/dev-macos-app.sh`を通常入口とし、Rust依存のworktree間cache、worktree固有target、Metro Fast Refreshを組み合わせる | 開発tooling owner。Cargo target、Xcode DerivedData、RN codegen、Pods。製品M4 cacheとは別seat | Cargo公式が案内する`sccache`を`RUSTC_WRAPPER`で`ADOPT`し、0.17のclient-side path正規化を使う。Cargo target／DerivedDataは共有しない。既存RN codegen、Pods、Xcode増分build、Metroを`REUSE`。APFS copy-on-write複製は採らない | `sccache`不在を明示errorにし、Rust source変更は通常のCargo invalidationへ従う。UI変更はMetro Fast Refresh、native変更はworktree固有のDebug buildを通す。Release受入と製品M4 cacheへ状態を繰り上げない |
