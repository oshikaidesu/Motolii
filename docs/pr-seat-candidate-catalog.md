...
| `M3-R1-BROWSER-RECTANGLE` | 現RN Browserの通常Create catalogからRectangleを選べる | `PARTIAL / UI ENTRY LANDED`。表示／選択はcurrent product sourceへ着地。terminalは`R1-SHELL`の共通Host接続待ち | `ui/motolii-rn/App.tsx`のBrowser catalog。Document writer 0 | `DiscoveryBrowserCandidate`の情報階層を`REUSE`し、Rectangleを同じ配列へ置く。専用bridge、mock Document、固定中央配置を作らない | JestでCREATE→Rectangle表示／選択。未達は明示project path、Product Host staticlib、共通intent bridge、source identity、terminal一回 |
