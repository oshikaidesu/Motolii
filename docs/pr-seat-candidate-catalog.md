...
| `M3-R1-BROWSER-RECTANGLE` | 現RN Browserの通常Create catalogでRectangleを選び、double clickでStage中央へ出せる | `PARTIAL / UI ACTIVATION LANDED`。Effectsを親にした共通Browser view、Media由来のサムネイルのみ表示、single click選択、double click中央Rerun投影をcurrent product sourceへ着地。D2 terminalは`R1-SHELL`の共通Host接続待ち | `ui/motolii-rn/App.tsx`のBrowser catalog／viewと既存Fabric Rerun Stage。Document writer 0 | RN macOS `onDoubleClick`、既存Fabric props更新、Rerun `LineDrawableBuilder`を`REUSE`。共通item IDだけを渡し、専用event bus、React overlay、mock Documentを作らない | Jestでsingle click時Stage不変／double click時item ID一回、Rustで矩形中心が正準原点。これは明示操作であり、旧DnDの暗黙fallbackとして棄却した既定中央配置を復活させない。未達はProduct Host／D2 terminal、source identity、Undo/Redo |
