...
- Plugin BrowserはProject Explorerと同じ`Search → Sources / Collections → Results → 必要時Filter / Detail`のBrowser shellへ統合する。`All / Installed / Project Used / Missing`はHost source、Favorites等はUser Collection、用途・雰囲気・kindは横断Filter、確定使用履歴はRecent sourceとして役割を分け、folder select・label chip・history stripを狭いpanelへ同時常設しない。Collectionはstable package identityを参照する仮想User libraryであり、install先、package内部path、`.vism`のfilesystem階層を反映しない。
- Plugin Resultsの既定は視覚結果を最大面積にしたthumbnail gridとする。通常cardはthumbnail、短い名前、kind iconを基本とし、正常な`READY / AVAILABLE`、説明、tag列、由来を反復表示しない。`MISSING / UNAVAILABLE`だけを逸脱状態としてthumbnailへ残す。選択した候補の短い説明と用途は同じBrowser内の単一Effect Detailへ置き、由来・capability・providerはさらにInfo / Developer infoへ下げる。適用はcardを対象Objectへdrag/dropする操作を基本とし、選択中Objectが有効ならdouble clickまたは`Enter`を同じCommit Intentへ接続する。反復`Apply`ボタンは置かない。grid/list切替とthumbnail寸法は表示だけのUser settingで、Collection、選択、Documentを変えない。
