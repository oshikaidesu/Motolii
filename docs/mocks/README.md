...
- Plugin Resultsの既定は視覚結果を最大面積にしたthumbnail gridとする。通常cardはthumbnail、短い名前、kind iconを基本とし、正常な`READY / AVAILABLE`、説明、tag列、由来を反復表示しない。`MISSING / UNAVAILABLE`だけを逸脱状態としてthumbnailへ残し、由来・capability・providerは選択後のDetail / Info / Developer infoへ下げる。grid/list切替とthumbnail寸法は表示だけのUser settingで、Collection、選択、Documentを変えない。
