...
- **開発原則 — 利用者成果の背骨と既知実装採択**: Motoliiは通常製品routeの利用者成果を先に背骨として固定し、一般機構を第一原理から発明しない。M3〜M5は利用者成果→機構class→既知実装調査→`REUSE / ADOPT / WRAP / PORT / PATTERN / EXTERNAL`裁定→採択地図→薄い接続→同一oracleで旧route退役、の順で進める。調査不足の粒は`REUSE / REMAP / REDUCE / 再調査 / WAIT_TARGET`へ局所処分し、別の接続可能edgeを止めない。既存task列や投入工数を実装・維持理由にせず、M4/M5も採択地図が閉じる前に独自cache／scheduler／3D engine／scene framework等を作らない。正本は[既知実装採択・置換開発モデル](known-implementation-adoption-model.md)と[自律再接続決定](reviews/2026-08-04-outcome-spine-autonomous-gap-research-decision.md)
- **開発方式とM3現在地**: 仕様書駆動の並列AIエージェント開発。Coreは[制御されたMicrokernel](reviews/2026-07-25-controlled-microkernel-host-module-parallelism-decision.md)としてauthorityとtyped protocolへ細くし、[並列Human Response Frontier](reviews/2026-07-25-parallel-human-response-frontier-execution-decision.md)として通常製品routeで触る・見る・測る地点へrolling waveで返す。[M2基盤再締結](reviews/2026-07-15-m2-foundation-reclosure-gate.md)はmainで解除済み。M3は[縦slice実行方針](reviews/2026-07-24-m3-vertical-slice-execution-decision.md)に従い、通常製品routeから利用者成果までの接続で進捗を測る。VS-1「Rectangle配置と三面投影、Undo/Redo」、VS-2の`U4a-1` / `CU-202` parameter control model、accepted React routeのproduct token consumer `CU-0B02R`、component state direct promotion `CU-0B02C-P`、common feedback direct promotion `CU-203P`、private carry最終処分`CU-0B02C-V`は`DONE`。Browser catalog decoder `CU-0A08BP`は`DONE`、Direct Browser connection親`CU-0A08BT`は`SPLIT`。`CU-204S/A`で診断投影の契約と5 reason × 4 densityのprivate純粋adapterを閉じた。`CU-204P`は実在通常source待ちを維持し、[CU-205S](reviews/2026-07-31-cu-205s-opacity-direct-route-split-decision.md)でfirst-party Opacity正常系をB1G/B1I/B2/T/P/W/Eへ分割して先行可能にした。`CU-205B1G/B1I/B2`と親`CU-205B`、`CU-205T/P/W/E`は`DONE`。正常系は通常Mac製品windowのreopenまで完走したが、親`CU-205` / `U4a-2`は`CU-204P`が閉じるまで`SPLIT / WAIT`。詳細な現行状態は[実装台帳](implementation-ledger.md)を正とする。Motolii Studio Previewはまだ未実装、G0-9DはDistribution Ready用hardware gate、plugin UI公開契約はG0-3 / GAP-13の別審判まで停止する
- **現在のUI接続粒（2026-08-04）**: [CU-108](reviews/2026-07-30-cu-108-rectangle-product-spine-e2e-decision.md)で実Mac通常製品のBrowser Rectangle→Stage / Timeline / Inspector→Undo→Redoを完走し、同じ`LayerId`のjournal Add/Remove/Addを確認した。`U4a-1`はtoolkit非依存のparameter mapping、全first-party保存parameter conformance、既存`SetProperty` / `EffectParam` routeまでを閉じた。`CU-0B02R`はcommit `782ffa3b`でaccepted routeをgenerated CSS consumerへ反転した。`CU-0B02C-P`は固定primitive 2ファイルと7 exportをproduct ownerへ直接移管し、mockをproduct re-export consumerへ反転、generation `ee3c1a2d44fd-ead41d4d6562`の30 PNG byte同一まで閉じた。`CU-203M`で確立した共通feedbackの固定JSX/CSSを`CU-203P`でproduct ownerへbyte同一移管し、mock re-export consumer、package `Feedback` export、copy 0、9-state matrix不変、generation `0d972253d868-ead41d4d6562`の30 PNG byte同一を閉じた。[CU-0B02C-V](reviews/2026-07-31-cu-0b02cv-private-carry-final-disposition.md)は第二consumer比較後もprivate carryを維持した。[CU-204S/A](reviews/2026-07-31-cu-204-staged-diagnostic-projection-split-decision.md)は診断投影を閉じ、`CU-205E`までfirst-party Opacity正常系を通常Mac製品windowで完走した。[CU-204P再確認](reviews/2026-08-01-cu-204p-normal-source-readiness-recheck.md)は通常source 0を固定した。[CU-201S](reviews/2026-08-01-cu-201-u3b-move-trim-snap-responsibility-split-decision.md)でU3bを分割し、[HOST-INPUT実装受入](reviews/2026-08-04-cu-201p-host-input-implementation-acceptance.md)でraw ownerとcancelを再締結しMOVEをtechnical reclose、[TRIM実装受入](reviews/2026-08-04-cu-201p-trim-implementation-acceptance.md)で既知handle hitから既存trim writer、[CU-201R受入](reviews/2026-08-04-cu-201r-random-move-trim-oracle-acceptance.md)で2,048-step random oracleを閉じた。[CU-201E](reviews/2026-08-04-cu-201e-normal-product-route-e2e-receipt.md)状態は`DONE` / `PRODUCT_E2E_PASS` / `EXTERNAL_POINTER_GATE_PENDING` / `HUMAN_DEFERRED`。通常製品MOVE/TRIM/Undo/Redo/reopenは`PRODUCT_E2E_PASS`。pointer-lossは`EXTERNAL_POINTER_GATE_PENDING`のまま。残余親CU-201Pは`SPLIT` / `WAIT_TARGET`。M3最終HUMANチェックリストでユーザー目視を収集する。
- **M3の実装分解**: [既知技術採択・並列実装地図](m3-parallel-implementation-map.md)の12親・33子、共有境界の衝突表、4実装waveを検索入口にする。旧152粒はoracle来歴であり、現行dispatchは実装台帳の`DO`行だけを使う
- **M3の発注可能性**: [実行可能発注地図](m3-executable-dispatch-map.md)で33子をtyped stateへコンパイルし、exact target、React実コードから監査した動的UIのevent/owner/intent/stale rule、正負oracle、利用者出口が閉じない子を実装担当へ送らない
...
4. M3〜M5の計画・実装に着手する時: [known-implementation-adoption-model.md](known-implementation-adoption-model.md)(利用者成果→既知実装調査→採択地図→接続→退役の横断順序) → [自律再接続決定](reviews/2026-08-04-outcome-spine-autonomous-gap-research-decision.md)(背骨→調査不足の局所処分→次edge再計測)
...
| [m3-parallel-implementation-map.md](m3-parallel-implementation-map.md) | M3の既知技術供給route、12の検索親、33の詳細子、共有file衝突、並列wave、旧責任の置換条件 | **M3実装分解の現行入口**(意味はspec、dispatch状態はimplementation ledger) |
| [m3-executable-dispatch-map.md](m3-executable-dispatch-map.md) | 33子の現在コンパイル結果、typed state遷移、発注擬似コード、exact target不足、Local Alphaまでの依存IR | **M3施工前コンパイル正本**(実装許可はledgerの`DO`で発効) |
