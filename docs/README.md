...
- **開発方式（G0-6H判定直前の履歴snapshot）**: 仕様書駆動の並列AIエージェント開発。Coreは[制御されたMicrokernel](reviews/2026-07-25-controlled-microkernel-host-module-parallelism-decision.md)としてauthorityとtyped protocolへ細くし、締結済みseat上のHost capability module／provider／consumer／表現pluginを製品全体の完成待ちから解放する。全seat inventoryやmilestone完了をbarrierにせず、[並列Human Response Frontier](reviews/2026-07-25-parallel-human-response-frontier-execution-decision.md)として通常製品routeで触る・見る・測る地点へrolling waveで返す。Fableは共有境界の非同期反対側reviewへ限定し、全leafの待ち行列にしない。[M2基盤再締結](reviews/2026-07-15-m2-foundation-reclosure-gate.md)はmainで解除済み。M3は[縦slice実行方針](reviews/2026-07-24-m3-vertical-slice-execution-decision.md)に従い、通常製品routeから利用者成果までの接続で進捗を測る。現在はVS-1「Rectangle配置と三面投影、Undo/Redo」。U0e-2R/U0e-2、固定MacのG0-9L platform prerequisite evidence、React直接移管R0〜R4C、CU-0A08IS/CU-0A08IP、`U3a-1S`/`U3a-1I`、`CU-G03D`/`CU-G03R`、親`CU-G03`、`CU-101`、`CU-102`、`CU-G09`／`CU-G09O`／`CU-G09R`、`CU-0A08BP`、`CU-104`、`U2h-1S`、`CU-104E`、`U2h-1I`、docs-only `U2h-1PR`/`CU-105R`/`CU-106S`/`U3a-2S0`、docs-only `U3a-2S`、docs-only `U3a-2R`、docs-only `U3a-2Z`、docs-only `U3a-2A`、docs-only `U3a-2P`、docs-only `U3a-2Q`、docs-only `U3a-2Q-P`、docs-only `U3a-2Q-P2`、docs-only `U3a-2Q-P3`、docs-only `U3a-2Q-P4`、docs-only `CU-109S0`、docs-only `CU-109S`、docs-only `CU-109SP`、docs-only `CU-G04S0`、docs-only `CU-G04S`、docs-only `CU-G04SC0`、docs-only `CU-G04SC`、実装粒`CU-109`（[#425](https://github.com/oshikaidesu/Motolii/pull/425)）、docs-only `CU-110S`まで`DONE`。CU-105とCU-106親は`SPLIT`し、`U2h-1P`/`CU-106P`/`CU-106F`は実consumer surfaceまで`WAIT`。docs-only `CU-110D`、docs-only `CU-107S`、docs-only `CU-107D`、docs-only `CU-107R`、docs-only `CU-107N`／`CU-107W`は`DONE`、`CU-0A08BTR`は`DONE`、親`CU-0A08BT`は`SPLIT`。`CU-0A08BTP`も`DONE`。`CU-0A08ITP-P`は`DONE`、親`CU-0A08IT`は`SPLIT`、`CU-0A08ITP`は`DONE`、`CU-0A08ITI`は`WAIT`。R6親`CU-0A09`はsurface別へ`SPLIT`し、Browser子粒`CU-0A09B`とdocs-only閉鎖境界`CU-0A09S`は`DONE`。Inspector/KEYS-LAYERS/Easing triggerは各R5交換待ちで、eligibleなR6実装粒は0件。次PRODUCT-ASSET `DO`は未選定（0件）。R6はdevelopment observer分離、最初のnon-mock callerはH1b、通常製品windowはW0bが所有する。Motolii Studio Previewはまだ未実装。G0-6HはU0e-3を止める並行人間審判、G0-9DはDistribution Ready用hardware gateとして分離する。plugin UI公開契約はG0-3 / GAP-13の別審判まで停止する
- **現在のUI接続粒（2026-07-29）**: Dark token authority後、token後続を`WAIT`へ戻した。[CU-0A08BTI](reviews/2026-07-29-cu-0a08bti-browser-place-typed-intent-implementation-decision.md)でRectangle Browser dragをtyped Host callbackへ接続し、[CU-0B03H](reviews/2026-07-29-cu-0b03h-browser-host-contract-offline-mount-decision.md)と[CU-0B03](reviews/2026-07-29-cu-0b03-native-browser-host-codec-inbox-implementation-decision.md)でBrowser一面のclosed codec、offline product mount、native bounded inboxを完了した。実MacでWebView跨ぎreleaseがegui Stageへ届かない停止点を再現し、[CU-0B04S](reviews/2026-07-29-cu-0b04s-browser-native-place-terminal-ownership-reclosure-decision.md)でHost platform capture所有へ再締結、[CU-0B04P](reviews/2026-07-29-cu-0b04p-host-platform-pointer-capture-implementation-decision.md)でcapture seamを実装した。[CU-0B04N](reviews/2026-07-29-cu-0b04n-native-stage-surface-layout-implementation-decision.md)で同一device/queue・一つのtop-level Surfaceへnative Stage/Timelineを表示し、最新layout epochのStage hit-test/Y-up変換まで接続した。[CU-0B04R](reviews/2026-07-29-cu-0b04r-browser-island-focus-geometry-implementation-decision.md)でopaque Browser islandのstale geometry拒否、requested focus owner、single-shot initial projection readyを閉じ、[CU-0B05](reviews/2026-07-29-cu-0b05-browser-lifecycle-reprojection-implementation-decision.md)でnew epoch再投影、old callback拒否、一回のprocess自動回復と二回目Browser-only degradedを実Macで閉じた。[CU-107PV](reviews/2026-07-29-cu-107pv-place-preview-delivery-implementation-decision.md)でproduction Movedをprivate Host Transient previewへ非terminal配送し、[CU-107TC](reviews/2026-07-29-cu-107tc-place-terminal-cause-classification-implementation-decision.md)で候補terminalをEscape / outside Stage / capture loss / noneへ排他網羅分類し、[CU-107AD](reviews/2026-07-29-cu-107ad-place-terminal-admission-implementation-decision.md)でstale / duplicate / replayを拒否して一drag高々一件へadmitし、[CU-107TD](reviews/2026-07-29-cu-107td-place-terminal-delivery-implementation-decision.md)でadmit済みterminalを単一下流境界へ一回配送し、[CU-107親閉鎖](reviews/2026-07-29-cu-107-place-coordinator-parent-closure-decision.md)を完了した。次PRODUCT-ASSET `DO`は`CU-110`
...
| [reviews/2026-07-22-m3-comfortable-use-granulation.md](reviews/2026-07-22-m3-comfortable-use-granulation.md) | 快適利用大地図を仕様判断・実装・E2E・実機審判の粒へ分け、依存とSTOPを固定 | **履歴snapshot／dispatch mirror**（[縦slice実行方針](reviews/2026-07-24-m3-vertical-slice-execution-decision.md) §8。機械dispatchは[implementation-ledger.md](implementation-ledger.md)のみ） |
