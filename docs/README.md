...
- **M3現在地**: R0製品seatは`DONE`。現在はDocument評価をRerun入力へ渡し、既存RN Stageへmountするwrapper seamを閉じる。旧VS-1、parameter、Timeline move／trim、Undo／Redoは意味・command・oracleとして再利用し、旧rendererは再実装しない。詳細は[実装台帳](implementation-ledger.md)
- **Rerun wrapperとM3 bring-up（2026-08-11）**: 製品UIは`ui/motolii-rn/`、Stage runtimeは固定Rerun Spatial Viewerへ凍結する。Document評価をRerun入力へ薄く翻訳し、結果を既存RN Stageへmountし、確定操作だけD2へ戻す。`spikes/motolii-rn-probe/`は`PROBE ONLY`で、direct `re_renderer` scene、shape別frame、第二Host／writer／GPU／UIを作らない。正本は[Rerun採択再締結](reviews/2026-08-10-m5-rerun-spatial-viewer-adoption-reclosure-decision.md)、[共同開発bring-up](reviews/2026-08-10-m3-collaborative-bringup-decision.md)、[Stage Hero](reviews/2026-08-11-m3-m5-stage-hero-projection-root-decision.md)
