...
- **shell現在地(2026-08-19追記)**: 2026-08-18裁定によりホストtoolkitはegui→**iced**へ移行中。egui(`motolii-blitz-shell`)が現行既定、iced(`motolii-shell-iced`)が移行先(M-0〜M-4完了・`cargo test --workspace`2131 passed/0 failed済み、M-5=既定bin切替は未実施で並走中)。詳細・各面の視覚正本・撮影器具は[CANON.md](CANON.md)、裁定は[iced移行決定](reviews/2026-08-18-iced-host-migration-decision.md)
- **M3現在地(2026-08-16更新)**: **RN製品面は撤去済み**。`rn_product_host`／`product_runtime`／`ui/motolii-rn/native-renderer`／旧egui appはmainに無く、実行できるのは`motolii-blitz-shell`だけ（**入力は未配線**）。`ui/motolii-rn/src`のTypeScriptは**移植元**として残る（色・寸法の出所。Rustのbuildには入らない）。撤去の理由と帰結は[Web窓とRN製品面の畳み込み](reviews/2026-08-16-web-window-and-rn-product-fold.md)。旧VS-1、parameter、Timeline move／trim、Undo／Redoは意味・command・oracleとして再利用し、旧rendererは再実装しない。詳細は[実装台帳](implementation-ledger.md)
- **Rerun wrapperとM3 bring-up（2026-08-11）**: （**2026-08-16に失効**: ここが指すRN起動経路は撤去済みで、`scripts/dev-macos-app.sh`と`scripts/build-macos-app.sh`はfileとして残るが動かない。現在の入口は`cargo build --profile fast -p motolii-ui --bins`と`motolii-blitz-shell`／`motolii-blitz-dump`。）旧R0 UI sourceはmainから削除済みで、過去版はGit履歴だけに置く。旧worktree、DerivedData、リポジトリ外artifactを検索・起動・fallback候補にせず、RN、Rerun、Skiaが共存する同じartifactへDocument評価を接続する。別UI／第二Host／writer／GPUを作らない。正本は[Rerun採択再締結](reviews/2026-08-10-m5-rerun-spatial-viewer-adoption-reclosure-decision.md)、[共同開発bring-up](reviews/2026-08-10-m3-collaborative-bringup-decision.md)、[Stage Hero](reviews/2026-08-11-m3-m5-stage-hero-projection-root-decision.md)
