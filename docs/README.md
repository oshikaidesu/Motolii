...
- **stage4(2026-08-29裁定・最優先で読む)**: 世界は`motolii/`(四代目)へ切り替わった。入口は[motolii/AGENTS.md](../motolii/AGENTS.md)——一行の憲法「rerunを軸に持ったAE(Lottie)インタフェースのソフト」+三本柱+背骨3本の1ページ。front=Dioxus Native(Blitz)、技術層=Rerunフォークの部品(再発明は罪)、意味=Lottieが審判。これにより下記「リセット」「技術スタック」「shell現在地」「M3現在地」および「読む順序」のM3〜M5系は**stage4以前の記述**になった(歴史記録として残す)。stage4で生きて引き続けるのは意味の正本([concept.md](concept.md)・[CANON.md](CANON.md)・[決定逆引き台帳](decision-index.md)・[references.md](references.md))。正本=[stage4裁定](decision-index.md)
