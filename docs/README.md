...
- **現在のUI接続粒（2026-07-30）**: [CU-110](reviews/2026-07-29-cu-110-product-place-d2-commit-implementation-decision.md)からStage / Timeline / Inspector投影、[CU-106P](reviews/2026-07-30-cu-106p-native-timeline-primary-selection-implementation-decision.md)、[CU-111](reviews/2026-07-30-cu-111-product-undo-redo-implementation-decision.md)まで接続した。[CU-108](reviews/2026-07-30-cu-108-rectangle-product-spine-e2e-decision.md)で実Mac通常製品のBrowser Rectangle→三面→Undo→Redoを完走し、同じ`LayerId`のjournal Add/Remove/Addを確認した。VS-1と製品spineは`DONE`。次PRODUCT-ASSET `DO`は未選定
...
| [reviews/2026-07-27-vism-authoring-journey-decision.md](reviews/2026-07-27-vism-authoring-journey-decision.md) | WGSL／Text／Path／Motion等の作者入口をpayload classとsemantic seatから分離し、推奨utility、typed identity、shader／asset closure、既知資産のauthoring時import、代表Vismを独立probeへ分ける反例を比較 | **比較中**(公開API・package形式・runtimeの実装許可ではない) |
