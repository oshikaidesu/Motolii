...
- **現在のUI接続粒（2026-07-29）**: [CU-0A08BTI](reviews/2026-07-29-cu-0a08bti-browser-place-typed-intent-implementation-decision.md)から[CU-110](reviews/2026-07-29-cu-110-product-place-d2-commit-implementation-decision.md)までを接続し、通常製品BrowserのRectangle dropがsession-backed D2、journal、published snapshot / primaryまで到達した。実MacでjournalのAddTrackItem一件を確認済み。[CU-110P分割](reviews/2026-07-29-cu-110p-product-published-snapshot-projection-split-decision.md)で次を`CU-110PS → CU-110PT → CU-110PI → CU-106P → CU-111 → CU-108`へ固定した。現在の唯一のPRODUCT-ASSET `DO`は、latest published snapshotを同じVRAM display slotへ再投影する`CU-110PS`。token後続は`WAIT`
