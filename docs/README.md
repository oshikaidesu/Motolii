...
- **技術スタック**: Rerun Spatial Viewer（Stage／spatial runtime）/ React Native（製品shell、Browser、Inspector、通常panel）/ rust-skia（Timeline、Curve、Stage overlay）/ Rust Host（Document、D2、media、resource policy）/ FFmpeg（media／export）。MotoliiはRerunのcreator-facing wrapperとして、永続意味と薄いidentity／time／asset翻訳だけを持ち、scene／view／query／camera／picking／rendererを再構築しない。全surfaceは同じrevision付きsnapshotを読む
...
- **M3現在地**: `ui/motolii-rn/`が唯一のRN製品sourceで、Document評価を既存Rerun入力へ渡すwrapper seamを閉じる。別UIへ移植しない。旧VS-1、parameter、Timeline move／trim、Undo／Redoは意味・command・oracleとして再利用し、旧rendererは再実装しない。詳細は[実装台帳](implementation-ledger.md)
- **Rerun wrapperとM3 bring-up（2026-08-11）**: 通常のRN sourceは`ui/motolii-rn/`一つ、開発入口は`sccache`でworktree間のRust依存buildを再利用し、Debug増分buildとMetroを起動する`scripts/dev-macos-app.sh`、Release受入入口は`scripts/build-macos-app.sh`とする。旧R0 UI sourceはmainから削除済みで、過去版はGit履歴だけに置く。旧worktree、DerivedData、リポジトリ外artifactを検索・起動・fallback候補にせず、RN、Rerun、Skiaが共存する同じartifactへDocument評価を接続する。別UI／第二Host／writer／GPUを作らない。正本は[Rerun採択再締結](reviews/2026-08-10-m5-rerun-spatial-viewer-adoption-reclosure-decision.md)、[共同開発bring-up](reviews/2026-08-10-m3-collaborative-bringup-decision.md)、[Stage Hero](reviews/2026-08-11-m3-m5-stage-hero-projection-root-decision.md)
