...
- **Rerun wrapperとM3 bring-up（2026-08-11）**: 通常のRN作業は`ui/motolii-rn/`と`scripts/build-macos-app.sh`だけを入口にする。旧worktree、DerivedData、リポジトリ外artifact、`ui/motolii-rn-legacy/`は検索・起動・fallback候補にしない。過去版のための新しい凍結copyも作らず、保全はGit履歴に任せる。RN、Rerun、Skiaが共存する同じartifactへDocument評価を接続し、別UI／第二Host／writer／GPUを作らない。正本は[Rerun採択再締結](reviews/2026-08-10-m5-rerun-spatial-viewer-adoption-reclosure-decision.md)、[共同開発bring-up](reviews/2026-08-10-m3-collaborative-bringup-decision.md)、[Stage Hero](reviews/2026-08-11-m3-m5-stage-hero-projection-root-decision.md)
