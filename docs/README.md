...
- **現在のUI接続粒（2026-08-01）**: [CU-108](reviews/2026-07-30-cu-108-rectangle-product-spine-e2e-decision.md)で実Mac通常製品のBrowser Rectangle→Stage / Timeline / Inspector→Undo→Redoを完走し、同じ`LayerId`のjournal Add/Remove/Addを確認した。`U4a-1`はtoolkit非依存のparameter mapping、全first-party保存parameter conformance、既存`SetProperty` / `EffectParam` routeまでを閉じた。`CU-0B02R`はcommit `782ffa3b`でaccepted routeをgenerated CSS consumerへ反転した。`CU-0B02C-P`は固定primitive 2ファイルと7 exportをproduct ownerへ直接移管し、mockをproduct re-export consumerへ反転、generation `ee3c1a2d44fd-ead41d4d6562`の30 PNG byte同一まで閉じた。`CU-203M`で確立した共通feedbackの固定JSX/CSSを`CU-203P`でproduct ownerへbyte同一移管し、mock re-export consumer、package `Feedback` export、copy 0、9-state matrix不変、generation `0d972253d868-ead41d4d6562`の30 PNG byte同一を閉じた。[CU-0B02C-V](reviews/2026-07-31-cu-0b02cv-private-carry-final-disposition.md)は第二consumerを比較し、非color値と`#ffffff30`を全件component-privateに維持、公開token追加0で閉じた。[CU-204S](reviews/2026-07-31-cu-204-staged-diagnostic-projection-split-decision.md)は現行5診断、4密度、copy、Host Transient owner、既存Inspector surfaceを固定した。[CU-204A](reviews/2026-07-31-cu-204a-diagnostic-projection-adapter-implementation-decision.md)は同じtyped identityと順序を保持するcrate-private純粋adapterを閉じた。[CU-205S](reviews/2026-07-31-cu-205s-opacity-direct-route-split-decision.md)は実在first-party `core.filter.opacity`を既存Browser / effect prepare / Inspector / parameter control / single writerへ接続する正常系を7粒へ固定し、`CU-205E`まで通常Mac製品windowのreopenを完走した。[CU-204P再確認](reviews/2026-08-01-cu-204p-normal-source-readiness-recheck.md)で通常source 0を固定し、新source成立まで再監査・実装を反復しない。[CU-201S](reviews/2026-08-01-cu-201-u3b-move-trim-snap-responsibility-split-decision.md)でU3bを分割し、[CU-201M-S](reviews/2026-08-01-cu-201m-s-clip-start-command-contract-decision.md)とcommit `729953d3`でsame-lane `SetClipStart`契約・実装を閉じた。[CU-201T-S](reviews/2026-08-01-cu-201t-s-clip-trim-timemap-contract-decision.md)でin/out trimとTimeMapの厳密意味を閉じ、次はCORE `CU-201T-C`
...
4. M3実装に着手する時: [m3-parallel-implementation-map.md](m3-parallel-implementation-map.md)(親検索)→ [m3-executable-dispatch-map.md](m3-executable-dispatch-map.md)(exact delta/擬似コード/oracle/出口)→ [implementation-ledger.md](implementation-ledger.md)(実際にdispatchする`DO`)→ [specs/M3-ui-integration.md](specs/M3-ui-integration.md)(意味と実装ガード)
5. M3以外の実装に着手する時: [implementation-ledger.md](implementation-ledger.md)(NOW/NEXT/WAIT)→ [specs/README.md](specs/README.md)(プロセスとステータス表)→ 対象`specs/M*.md`(タスク表と**末尾の「実装ガード」節**の両方を読む)
