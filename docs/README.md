...
3. **プラグイン純関数契約**: プラグインの出力は時刻tと入力だけで決まる。隠れた可変状態の禁止。**ランタイム物理/逐次シミュレーションもここに含む**(2D/3D問わず。[concept.md](concept.md))。これがフレーム並列(マルチコア)の前提で、破るとAEと同じ「後付け不能」になる([performance-model.md](performance-model.md)§6)
