...
- **M3現在地**: 現在の接続テストは`spikes/motolii-rn-probe/`で、Document評価を既存Rerun入力へ渡すwrapper seamを閉じる。成功時は同じartifactを`PRODUCT_SOURCE`へ繰り上げ、別UIへ移植しない。旧VS-1、parameter、Timeline move／trim、Undo／Redoは意味・command・oracleとして再利用し、旧rendererは再実装しない。詳細は[実装台帳](implementation-ledger.md)
- **Rerun wrapperとM3 bring-up（2026-08-11）**: RN、Rerun、Skiaが共存する`spikes/motolii-rn-probe/`をその場で製品化する。Document評価をRerun入力へ薄く翻訳し、結果を同じStageへ載せ、確定操作だけD2へ戻す。`ui/motolii-rn/`はread-onlyの旧製品shellであり、probeからのcopy、direct `re_renderer` scene、shape別frame、第二Host／writer／GPU／UIを作らない。正本は[Rerun採択再締結](reviews/2026-08-10-m5-rerun-spatial-viewer-adoption-reclosure-decision.md)、[共同開発bring-up](reviews/2026-08-10-m3-collaborative-bringup-decision.md)、[Stage Hero](reviews/2026-08-11-m3-m5-stage-hero-projection-root-decision.md)
