...
- **長期の北極星**: 映像表現を、時刻・入力・型付きparameterから決まる**映像表現モジュール（Visual Module）**として演奏・保存・配布できる共通実行環境にする。音声用VST pluginの読み込みやVST APIの実装ではない([concept.md](concept.md#長期の北極星-映像表現を演奏再利用配布できる単位にする))
...
これらは個別の最適化規則ではなく、Visual Moduleの共通実行環境を成立させる下部構造でもある。新しい公開境界は、表現単位・再現性・可搬性・作品の持続性・Host一貫性・作者体験・制作者体験の[7審判](concept.md#設計と実装の審判)を通す。

## 用語の最短定義

- **映像表現モジュール（Visual Module）**: 時間・入力・型付きparameterから、映像または制御値を生成する再利用可能な表現単位。Filter、Composite、LayerSource、ParamDriver等を含む
