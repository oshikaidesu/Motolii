...
- **技術スタック**: Rust + wgpu（render/media/GPU core、Stage preview）/ rust-skia（Timeline、Curve Editor、Stage overlay）/ React Native（shell、Browser、Inspector、通常panel、IME/a11y）/ ffmpeg sidecar / Cargo workspaceへ再基線化した。native canvasのgestureはRust headless interaction、編集はD2 single writer、全surfaceは同じrevision付きsnapshotを読む。旧WebView islands、direct-wgpu/Vello UI、egui shellは新規製品UIを追加せず、移行oracleとして保持する。詳細は[UI runtime責任境界](ui-runtime-architecture.md)と[2026-08-07再基線決定](reviews/2026-08-07-m3-react-native-rust-skia-runtime-rebaseline.md)
...
- **M3共同開発bring-up（2026-08-10、2026-08-11 target凍結）**: R0受入と並列開始gateは完了した。RN製品UIの唯一の接続先は`ui/motolii-rn/`へ凍結し、`spikes/motolii-rn-probe/`は`PROBE ONLY`の検証・visual oracleに限定する。別RN app／shell／app root、既存panelの縮約copy、probe側への製品機能追加は行わない。変更には利用者判断と移行／cutover／退役を閉じた解凍decisionが必要である。以後は[共同開発bring-up決定](reviews/2026-08-10-m3-collaborative-bringup-decision.md)に従い、この起動可能なRN製品artifactへStage、Timeline、Browser、Inspectorの小さい製品出口を継続統合する。Stageは製品の主役surfaceとし、M5 spatial主部には[Rerun Spatial Viewerをsubsystem採択](reviews/2026-08-10-m5-rerun-spatial-viewer-adoption-reclosure-decision.md)、Timelineには既存Skia fixtureの製品昇格を入力とする。第二Document／Host／writer／GPU ownerは作らない
- **Stage Hero projection root（2026-08-11）**: PR前の根本は[Stage Heroとprojection root決定](reviews/2026-08-11-m3-m5-stage-hero-projection-root-decision.md)を正とする。M5由来のRerun Stageを第一consumerとするが、意味正本はDocument、変更はD2、翻訳根はaccepted snapshot／revision／primary／generation／Host時刻に置く。Stage入力を先に閉じ、Timelineを時間面、Inspectorを意味面として同じidentityからboundedに投影する
- **クリエイター翻訳機構と叩き台PR（2026-08-10）**: M3〜M5は一発の完成回答を要求せず、制作意図→型付き意味→採択済み実装→Stageの結果という理論を通す叩き台をmainへ継続統合する。Rerun等は良い塊として採択し、PRは一つの利用者成果を運ぶlanding envelopeとする。機械的conflictはintegration ownerが解消し、意味owner／single writer／GPU owner／公開・永続contractのsemantic conflictだけを止める。正本は[クリエイター翻訳機構・叩き台PR統合決定](reviews/2026-08-10-creator-translation-working-draft-pr-integration-decision.md)。次にこちらから開けるIssue候補は[PR席候補一覧](pr-seat-candidate-catalog.md)で管理するが、同表は発注authorityや第二ledgerではない
