...
- **現在のUI接続粒（2026-07-29）**: [CU-110](reviews/2026-07-29-cu-110-product-place-d2-commit-implementation-decision.md)で通常製品BrowserのRectangle dropをsession-backed D2、journal、published snapshot / primaryまで接続した。[CU-110P分割](reviews/2026-07-29-cu-110p-product-published-snapshot-projection-split-decision.md)後、[CU-110PS](reviews/2026-07-29-cu-110ps-native-stage-published-snapshot-projection-implementation-decision.md)でlatest snapshotを同じVRAM display slotへ再投影し、実Macでdrop直後・再起動なしのRectangle表示を確認した。現在の唯一のPRODUCT-ASSET `DO`はnative Timeline投影`CU-110PT`。後続は`CU-110PI → CU-106P → CU-111 → CU-108`、token後続は`WAIT`
