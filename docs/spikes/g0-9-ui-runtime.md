...
2. **Timelineの高頻度同期領域はnative所有へ絞る**。Canvas 2D/browser WebGPUでvisible rangeを
   単一面へ描けることは先例証拠として残すが、製品枝はdirect wgpuを第一候補、Velloをpath/textだけに
   局所利用する。Reactは外側のtoolbar/menu/parameter編集を所有する。
3. **browser WebGPUはWeb所有panel内の局所visualization候補だけに残る**。Stage/Timeline ownerにはしない。
   今回のadapterはheadless ChromiumのSwiftShaderであり、native Rust/wgpuのApple M4 Metal deviceとは別物だった。
...
Vello＝native側の複雑path/text局所renderer**という責任分担をhybrid第一候補として次段へ送る。
native実装方式の比較とFable向け反証点は
[native surface renderer再選定](../reviews/2026-07-21-native-surface-renderer-reselection.md)を正とする。
...
| 候補 | 既成範囲 | 現時点の扱い |
|---|---|---|
| [Konva](https://konvajs.org/docs/overview.html) | DOM状のStage/Layer/Group/Shape、hidden hit Canvas、event、drag、transform。公式に[20,000 interactive node](https://konvajs.org/docs/sandbox/20000_Nodes.html)例 | **今回実測・先例限定**。interaction fixture/oracleとして残し、製品Pen/Roto/handle rendererにしない |
| [PixiJS](https://pixijs.com/8.x/guides/components/scene-objects) | GPU scene graph、DOM風event、[100,000 Particle](https://pixijs.com/8.x/guides/components/scene-objects/particle-container)例、static/dynamic upload | **今回実測・先例限定**。dense batchの比較証拠で、製品Timeline rendererにしない |
| [@pixi/react](https://react.pixijs.io/getting-started) / [pixi-viewport](https://viewport.pixijs.io/jsdoc/index.html) | React binding、drag/pinch/wheel/clamp/snap viewport | pan/zoom/edge-panのinteraction oracle。製品runtime依存候補から外す |
| [Fabric.js](https://fabricjs.com/docs/why-fabric/) | Canvas object model、hit detection、control、event、serialization | 先例のみ。native Stage ownerとMotolii Documentを置換しない |
...
| [Canvas Timeline](https://github.com/techsquidtv/canvas-timeline) | headless timeline、snap/history、React interaction、worker Canvas | 入力・worker分離の先例のみ。0.1.0、MPL-2.0、独自historyを製品依存にしない |
...
8. direct wgpu + Vello局所passのwindowed Timelineを、egui baselineと同じfixture/操作列で反証する
