...
> **既知の陳腐化(2026-08-19追記)**: 下表・下記の`motolii_ui_shell`は2026-08-16に撤去済み。現在実行できる製品shellは`motolii-blitz-shell`(egui、既定)と`motolii-shell-iced`(iced、2026-08-18着手の移行先。M-0〜M-4完了・M-5=既定bin切替は未実施)。用語の分類方法(実行場所/役割/結合段階)自体は有効だが、「現在の実体」欄の`motolii_ui_shell`は現物が無い。現行の正本一覧は[docs/CANON.md](CANON.md)を見よ。
