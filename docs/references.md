...
| [Rerun](https://github.com/rerun-io/rerun) | MIT OR Apache-2.0 | **egui実運用パターンの第一学習源(仮判定・比較中)**。0.34.1(commit `4efb18f`)へ固定して読む。egui 0.35 / egui_tiles 0.16 / wgpu 29 / winit 0.30.13とMotolii実測組が一致。blueprint(layout正本のデータ化→egui_tiles投影)、time panel、`.ron` theme、re_video(ffmpeg-sidecar収束)が主対象。**`re_*` viewer系crateへは依存しない**("Expect breaking changes!"宣言・高頻度churn)。[方向決定](reviews/2026-07-20-rerun-prior-art-direction.md)・[inventory](reviews/2026-07-20-rerun-pinned-commit-source-inventory.md)・[転移計画](reviews/2026-07-20-rerun-learning-transfer-plan.md) |
| [egui_table](https://github.com/rerun-io/egui_table) | MIT OR Apache-2.0 | rerun-io保守の大量行table widget(0.9.0、egui 0.35対応、sticky header・可変行高)。Browser大量行表示の**未評価候補**(登録のみ、採否は必要が生じたチケットで審査。[方向決定§3.3](reviews/2026-07-20-rerun-prior-art-direction.md)) |
