...
| [Rerun](https://github.com/rerun-io/rerun) | repository: MIT OR Apache-2.0、`re_ui`: `(MIT OR Apache-2.0) AND OFL-1.1` | **M3 Stage／M5 spatial runtimeの採択subsystem**。固定commit `954bf95a`の`re_view_spatial`／view query／visualizer／camera／picking／outline／`re_renderer` compositeを`ADOPT / WRAP`し、Motoliiはcreator意味と薄い翻訳だけを持つ。RN shell／通常panelは`spikes/motolii-rn-probe/`内の既存artifactをその場昇格し、`re_ui`／`egui_tiles`を製品chromeにはしない。[採択再締結](reviews/2026-08-10-m5-rerun-spatial-viewer-adoption-reclosure-decision.md) |
