...
| [rust-skia (skia-safe)](https://github.com/rust-skia/rust-skia) / [Skia](https://skia.org/) | rust-skia: MIT / Skia: BSD-3-Clause | **`ADOPT`(2026-08-08)。ただし2026-07-21再選定§3と`U3a-2A`で`REJECT`されていたものを[2026-08-08裁定](reviews/2026-08-08-skia-reject-to-adopt-authority-reconciliation.md)で撤回した経緯がある**(旧`REJECT`理由「既存wgpu/Velloと重複」はVello退役により前提消滅。ただし**alpha・色の懸念は撤回せず維持**し、Skiaの色処理を第二の色変換点にしない)。[2026-08-07再基線決定](reviews/2026-08-07-m3-react-native-rust-skia-runtime-rebaseline.md)がTimeline／Curve Editor／Stage overlay の標準に定めた2D canvas renderer。`skia-safe 0.99.0` + `wgpu 29` はリポジトリ外の隔離probe `skia-timeline-probe` で実動、Windows targetのcheckも別probeで実施済み([棚卸し](reviews/2026-08-08-out-of-repository-asset-inventory.md))。**用途はcanonical出力外のoverlay描画のみ**(handle／gizmo／outline／grid／safe area／path)。base previewとfinal compositeはwgpuが所有し、overlayはdirty時だけraster/uploadしCPU readbackを使わない。**skia型をdomain／公開plugin契約／Document schemaへ漏らさない**。配布物へSkia(BSD-3-Clause)とrust-skia(MIT)のcopyright・license・disclaimerを再掲する。退役対象の`vello`版`native_timeline_renderer.rs`は新route出口を同じoracleで確認するまで削除しない。ゲート記録は[N-OVERLAY依存ゲート](reviews/2026-08-08-n-overlay-dependency-gate.md) |
| [wgpu](https://github.com/gfx-rs/wgpu) | Apache-2.0/MIT | レンダリングコアの土台(採用決定済み) |
| [Vello](https://github.com/linebender/vello) | Apache-2.0/MIT | **採用決定(2026-07-10、S3スパイク合格)**。vello 0.9=wgpu29依存で本体と同一device同居を実測確認。条件: Renderer長寿命保持(初期化~900ms)・出力straight alpha→境界でpremul化。`vello_svg 0.10.0`はP13-C1でpath/group/fill/stroke、unsupported callback、typed parse errorを4 fixture、3 target cross-buildで確認し`REMAP / VERIFIED`。missing external fileはusvg段で無言dropするためnetwork/file typed preflightはMotolii owner。独自walkerは作らない。詳細は[spikes/s3-vello.md](spikes/s3-vello.md)、[M4調査](reviews/2026-08-02-m4-known-implementation-survey.md) |
...
| [Rerun](https://github.com/rerun-io/rerun) | repository: MIT OR Apache-2.0、`re_ui`: `(MIT OR Apache-2.0) AND OFL-1.1` | **M3 Stage／M5 spatial runtimeの採択subsystem**。固定commit `954bf95a`の`re_view_spatial`／view query／visualizer／camera／picking／outline／`re_renderer` compositeを`ADOPT / WRAP`し、Motoliiはcreator意味と薄い翻訳だけを持つ。RN shell／通常panelは`ui/motolii-rn/`内の既存artifactをその場昇格し、`re_ui`／`egui_tiles`を製品chromeにはしない。[採択再締結](reviews/2026-08-10-m5-rerun-spatial-viewer-adoption-reclosure-decision.md)。**★2026-08-29: Motoliiの依存先は上流ではなく利用者自身のフォーク`https://github.com/oshikaidesu/rerun`(比喩ではなく`Cargo.toml`の実際の参照先)。Documentは既に`re_entity_db`/`re_chunk_store`の上に建っており、Undoも`re_viewer_context/src/undo.rs`(Blueprintストア)の機構を踏襲している——技術基盤はRerunフォーク側に足し、Motoliiは足りていない編集体系だけを出す、という役割分担の実測的裏付け。今後のコンセプト検討の参考文献として要参照**: [Rerun部品目録調査](reviews/2026-08-29-rerun-parts-catalog-survey.md) |
| [`gltf`](https://github.com/gltf-rs/gltf) | MIT OR Apache-2.0 | **M5-P1のDEPEND第一候補**。private importer seamの内側だけで使い、crate APIをDocument／公開plugin契約へ出さない。採択order前にversion、feature、license、maintenance、malformed input、3 OSを固定する。[M5 import／render決定](reviews/2026-08-01-m5-3d-import-rendering-boundary-decision.md) |
| [`tobj`](https://github.com/Twinklebear/tobj) | MIT | **M5-P1のDEPEND候補**。OBJをglTFと同じprivate faithful assetへlowerする用途だけ。maintenance／MTL coverage／3 OSの再確認前は採択済みと扱わず、OBJ専用rendererや永続変換formatを作らない。[M5 import／render決定](reviews/2026-08-01-m5-3d-import-rendering-boundary-decision.md) |
| [Khronos glTF Sample Assets](https://github.com/KhronosGroup/glTF-Sample-Assets) / [Validator](https://github.com/KhronosGroup/glTF-Validator) / [Sample Viewer](https://github.com/KhronosGroup/glTF-Sample-Viewer) | Apache-2.0ほか(assetごとに確認) | **M5-P1/P2のEXTERNAL ORACLE／PATTERN**。format validation、positive／negative fixture、PBR／encoding比較へ使う。reference画像類似をMotolii全体のacceptanceにせず、Validatorのblocking CI化は別decision |
| [renderling](https://github.com/schell/renderling) | MIT OR Apache-2.0 | **M5比較spikeだけ**。wgpu、glTF、PBR／IBL、headlessの比較対象だがalpha段階でrust-gpu／SPIR-V build chainを伴う。隔離crate／worktreeだけで測り、採択decision前にworkspace lockfile／通常buildへ入れない |
| [rend3](https://github.com/BVE-Reborn/rend3) | MIT OR Apache-2.0 | **M5新規dependencyは棄却**。2025-06-07 archive／read-onlyのため、過去のrender graph／PBR設計を履歴参考以上へ昇格しない |
...
| CPU cache | [foyer-memory](https://docs.rs/foyer-memory/0.22.3/foyer_memory/) `REMAP / VERIFIED` (`moka`は比較候補) | M4-P03でweight／handle／resize／filter／並行操作を4 fixture、3 target cross-buildで確認。default `foyer-tokio` runtime closureを含む。remove後の外部handle生存量はprivate ownerで加算する | VRAM texture寿命、外部生存量込みhard admission、invalidation意味、永続key |
| disk artifact | [Blender VSE](https://docs.blender.org/manual/en/4.5/editors/video_sequencer/introduction.html)／[After Effects](https://helpx.adobe.com/after-effects/desktop/memory-storage-performance/memory-and-storage/memory-storage1.html)／[Premiere](https://helpx.adobe.com/premiere/desktop/troubleshooting/media-issues/manage-media-cache.html)／[Resolve](https://documents.blackmagicdesign.com/UserManuals/DaVinci_Resolve_15_Reference_Manual.pdf)／[Final Cut Pro](https://support.apple.com/guide/final-cut-pro/manage-render-files-ver68a8c250/mac)の再生成media `PATTERN` + workspace `sha2/std::fs` `REUSE` + [sccache local](https://github.com/mozilla/sccache/blob/main/src/lru_disk_cache/mod.rs)／D1 atomic persist `PATTERN` + [`tempfile 3.27.0`](https://docs.rs/tempfile/3.27.0/tempfile/) `VERIFIED (V1 BASELINE ONLY)` | M4-P05 V1 baselineで通常artifact file、same-dir publication、FFmpeg temp、integrity、stale tempを確認。[cacache](https://docs.rs/cacache/13.1.0/cacache/)はdormant/fork推奨とWindows破損により`REJECT` | process kill、ENOSPC、同一recipe競合、Windows runtime visibility、V2/V3/V4、完全key、disk hard budget、LRU/pin、artifact codec。global content dedupは実測再入場 |
| disk watermark | [`fs4 1.1.0`](https://docs.rs/fs4/1.1.0/fs4/) `VERIFIED (observation only)` | M4-P04-C4のfree-space／allocation-granularity観測。3 fixtureと3 target cross-buildを確認済み | hard budget、admission、eviction、Document/Project failure |
| 区間集合 | [rangemap](https://docs.rs/rangemap/1.7.1/rangemap/) | M4-P06のhalf-open invalidation/coverage採択probe。1.7.1はadjacent/overlap/coalesce/gapsを確認済み。empty range raw insertがpanicするためprivate guardを`REMAP`採用 | Document変異の意味、generation、scheduler priority、raw RangeSet型／panic |
| priority collection | [priority-queue](https://docs.rs/priority-queue/2.7.0/priority_queue/) | M4-P07のreprioritize/remove/popとcomposite tie-breakを4 fixtureで確認。bounded admission／generation filterはprivate ownerへ`REMAP` | worker lifecycle、cancel、Document/cache writer、queue上限そのもの |
