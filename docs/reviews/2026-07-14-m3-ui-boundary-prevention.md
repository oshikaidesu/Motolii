...
- GPUデバイスはコアが作り、Slintは借りる
- render/decodeはworkerで行い、Slint componentの更新はevent-loop threadへ戻す
...
- 再生クロックをvsync/Slint描画callbackへ従属させない
...
- Slintの論理px/物理px、DPI scale、window座標: UI adapter内だけ

scale変更の自動審判は注入したscaleで同一操作から同一domain command/Documentが得られること。実モニタ移動は人間実機審判として分ける。

### GR-UI-5. Slintをadapter境界へ封じ込める

製品UIクレート名は`motolii-ui`とする。

- `motolii-ui`はSlint callbackをSlint非依存のdomain intent/commandへ変換してよい
- `motolii-ui`以外の製品クレートは`slint`へ依存しない
- Slint型をdomain intent、Document command、core/eval/render/pluginの公開APIへ出さない
- timeline layout/hit-test/render modelはウィンドウなしでテスト可能なSlint非依存moduleに置く

審判はCargo metadataの依存検査と公開型走査。callback adapterそのものをUIクレート外へ追い出すことは要求しない。

### GR-UI-6. 負荷と測定方法を先に固定する

timeline/波形/keyframeを項目ごとのSlint elementで作らず、単一wgpu面へ描画する。負荷データは「clips 1,000 + keyframes 100,000」を固定する。
...
- GAP-13決着前は同梱`.slint`、plugin所有の自由wgpu UI、`ParamDef::WidgetHint`、DPI/Slint型を渡すAPIへ着手しない
...
5. Slint依存と型が`motolii-ui`の外へ漏れていないか
