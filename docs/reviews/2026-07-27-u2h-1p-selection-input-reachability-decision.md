...
docs-only `U3a-2S`とdocs-only `U3a-2R`とdocs-only `U3a-2Z`とdocs-only `U3a-2A`とdocs-only `U3a-2P`は`DONE`で、現行の次PRODUCT-ASSET判断は`U3a-2Q`の`DO`である。実consumer surfaceとproduction入力が成立するまで
