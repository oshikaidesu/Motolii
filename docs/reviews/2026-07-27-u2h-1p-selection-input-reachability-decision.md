...
docs-only `U3a-2S`とdocs-only `U3a-2R`とdocs-only `U3a-2Z`とdocs-only `U3a-2A`とdocs-only `U3a-2P`とdocs-only `U3a-2Q`とdocs-only `U3a-2Q-P`は`DONE`で、現行の次PRODUCT-ASSET判断は`U3a-2Q-P2`の`DECIDE`である。実consumer surfaceとproduction入力が成立するまで
