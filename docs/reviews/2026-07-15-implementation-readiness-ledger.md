...
## AUT-0 — Automation / Scripting boundary（将来予約）

AUT-0は実装タスクではなく、Motoliiを手作業だけの編集ソフトへ閉じないための設計留保である。JSON・CSV・MIDI・外部データからの動画生成、一括編集、定型処理、AI生成スクリプトによる操作へ拡張できる余地を保持する。

### 現時点の固定事項

- スクリプトパネルと公開Automation APIは、現マイルストーンでは実装・Issue化しない
- 将来の利用体験は **JavaScript-first / TypeScript-friendly** を第一候補とする。JavaScriptを貼り付けて実行でき、TypeScriptは型定義・補完・必要に応じた変換で支援する
- UI、CLI、将来のScriptは、Document編集について同じCommand意味論を使う
- CLIによる外部バッチ処理を、開いているDocument・選択・現在時刻へ作用する対話型スクリプトの代替とはみなさない

### 保持する不変条件

- 意味のあるDocument編集をUIイベント内だけに閉じ込めず、共通Command境界を通す
- 複数操作を1 Undo単位として実行できるmacro / transaction境界を閉ざさない
- 保存・再読込・Undo後も対象を追跡できるstable IDを維持する
- 項目の列挙・検索・選択取得を提供できるQuery境界を閉ざさない
- UI、CLI、Script、将来の外部Automationで編集意味論を分岐させない
- Document JSONの無検証な直接編集を公開APIにしない

### 想定する利用

- JSONから曲紹介、ランキング、歌詞、商品紹介などの定型動画を生成する
- CSVやMIDIからClip、Text、Marker、Animationを生成する
- 選択Clipへの一括操作、規則配置、命名、差し替えを行う
- テンプレートへデータを流し込み、preview、save、exportを起動する
- AIが生成した短いJavaScriptを、開いているDocumentへ1 Undo単位で適用する

### 着手条件と非目標

Document、Command、Undo、stable identity、selection / query境界が十分に安定した後、runtime選定と公開API設計を別spikeで行う。現時点ではruntime、sandbox、権限、ファイルアクセス、公開object model、保存形式を決めず、M2〜M5の実装列やIssue昇格順へ追加しない。
