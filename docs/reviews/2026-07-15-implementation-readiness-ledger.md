...
|---|---|---|---|---|
| GAP-14 | `SPIKE` | 比較対象、反例、決定すべき操作、審判 | reject/cascade/copy-local/orphan GCの採否 | [#166](https://github.com/oshikaidesu/Motolii/issues/166) |
| D1l | `BLOCKED` | Definition/Use分離、ordered stack、inline migration、非隣接共有 | GAP-14 lifecycle | GAP-14決定後 |
| D3e | `WAIT` | 各Use位置で個別評価、Groupは子合成後1回、source非消費 | D1l、D3 | D1l schema merge後 |

### GAP-14の出口

- 判定語付きで`Delete Definition while used`、`Unlink Use`、`Copy Local`、`Delete last Use`を全て決める
- 各操作のDocument前後、Undo/Redo、未知plugin、再保存後を表にする
- D1lのmigrationとvalidationへ必要な不変条件を列挙する
- schema/API/production codeは変更しない

## M3 — UI projection

| ID | 状態 | 固定済み | 可変UI | 依存/Issue化 |
|---|---|---|---|---|
| U1f | `READY` + `UI-PROTOTYPE` | Final frame不変、off-frame objectを表示/選択、K0を待たない、UI thread readback禁止 | scrim濃度、outline、full-pixel表示、境界装飾 | U1b/U0e/D1k/D3後。[#169](https://github.com/oshikaidesu/Motolii/issues/169) |
| U2f | `READY` + `UI-PROTOTYPE` | 全Position Const/keyへ同じEdit-Space差分、時刻/補間/接線不変、1 Undo、Cancel 0、Bake/offset/helperなし | modifier物理key、HUD、ghost、path表示 | U0c/U0d/U2a/U2c/D2後。[#168](https://github.com/oshikaidesu/Motolii/issues/168) |
| U2g | `WAIT` + `UI-PROTOTYPE` | Effect Definition out→Layer Use in、非隣接、1 drag=1 Use、Group/Explicit意味差 | gutter、routing、bundle/stub、socket形状 | D1l/D3e/U3a後。Issue化しない |
...
```text
作成済み: GAP-14 #166, K0 #167, U2f #168, U1f #169, P0I #170

GAP-14 decision
  -> D1l schema/migration
  -> D3e evaluation
  -> U2g UI prototype
  -> K2 invalidation
