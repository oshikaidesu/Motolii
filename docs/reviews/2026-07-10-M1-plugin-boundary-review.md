...
- [x] **7. T8残(中間バッファのピンポン再利用)** — `RenderSession` が中間RTを2枚ピンポンで確保。Solid単色もキャッシュ。`session_reuses_two_ping_pong_targets_across_frames` で generations=1・len=2 を確認。
  M1仕様書に記載済みの残タスク。所見1のRenderStep改修と同じ箇所を触るため、同一PRか直後に片付けるのが安い。
