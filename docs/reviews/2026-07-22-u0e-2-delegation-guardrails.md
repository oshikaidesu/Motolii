...
1. `CODEX PRECHECK: APPROVED` が marker の存在だけで、main 正本、対象 worktree の base、粒の
...
- 対象 worktree の `HEAD`、発注書の `BASE_SHA`、authority file の path + SHA-256 が一致しない
- main 正本で対象粒が `DO` でない、または依存粒が `DONE` でない
...
発注実行前に、発注書と正本を機械照合する。

- `BASE_SHA` と対象 worktree `HEAD` の完全一致
- `AUTHORITY path sha256` の全件が対象 worktree 内に存在し、byte hash が一致
- `GRAIN` を main の粒度化台帳から引き、状態 `DO` と依存 `DONE` を確認
