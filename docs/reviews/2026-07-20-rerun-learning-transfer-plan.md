...
ステータス: **決定**。Rerunを高密度UI、時間面、GPU viewport、選択、実行系、試験系の主要先例として層別に学ぶ。2026-07-24の[egui製品不採用](../ui-runtime-architecture.md)により、`re_ui`、`egui_tiles`、egui callback等を製品へ`DEPEND/VENDOR/PORT`しない。toolkit横断の観察と`PATTERN`裁定、既存egui比較・診断baselineの反証は有効だが、製品転移へ戻すにはUI runtime決定の明示改訂を要する。本文書は「何を、どの順で、どの境界まで習うか」の運用正本であり、個別crateの依存追加・vendoring・forkを単独では許可しない。
...
RR-1の出口は「Rerun風画面」ではない。React product catalogをMotolii fixtureで検査し、Rerun componentは情報階層、状態、contrast、密度の`PATTERN`反証に使う。既存egui catalogがある場合も比較baselineであり、製品componentの到達点にしない。
...
| [`re_viewport_blueprint`](https://github.com/rerun-io/rerun/tree/954bf95a4e1a01de4cb67e0e92b8a5e059ee2b8e/crates/viewer/re_viewport_blueprint) | 安定したView/Container記述から毎frame `egui_tiles::Tree`を構築し、変更をdeferred commandで反映 | Motolii所有layout modelからruntime placementを投影する`PATTERN`。`egui_tiles`製品転移はしない |
...
以下の二方式比較は2026-07-18採用時の歴史的baseline調査として保持する:

1. Motoliiの長寿命display textureを`register_native_texture`でsample
2. egui paint callback内でMotolii/Rerun型rendererを直接composite

`register_native_texture`は既存TextureViewをsampleするためCPU copyもGPU texture copyも前提にしない。この事実はbaselineとして保持するが、Callback方式を製品候補へ昇格しない。offscreen target、overlay、clipping、lifecycle、エラー隔離、計測容易性はdirect wgpu／Vello局所枝の反証項目へ翻訳する。
...
- packed Timelineの遠景density、中景bar、近景key（Motolii語では遠景density / 中景cluster / 近景individual）
...
| M3 UI | component、shell、panel投影、時間密度、selection、viewport接合のtoolkit横断pattern | `PATTERN`裁定とMotolii fixture。egui固有assetは既存baseline反証だけ | Rerun viewerまたはegui assetを編集UIの仕様・製品runtimeにする |
