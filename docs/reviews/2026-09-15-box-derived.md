...
## 関係を読み取る(提案 2026-09-15)

利用者: 3D の格子は「つまんない」。「グリッドから離れよう。tracy が人気な理由は? 簡単に関係性が可視化されているように見えるからだ」。Tracery は関係を推し量って線と札を付ける。Motolii は書類が本当の関係を持つので、それを Tracery の見た目で出す。最初の 1 本は押し合い(間合いの法)。

- **Trace = Push**(なぞる形の新しい種類): 押された物の、書いた場所の箱(いたかった所)と、そこから今の中心への矢印。押されていなければ箱は今の箱に重なり、矢印は長さと一緒に 0 へ縮む。先例: Figma の寸法の赤線、AE の親を指す線
- **文字の Readout / Readout Of**: 文字の `#` を相手の値(Push = 押された px、Width、Height)に置き換える(`#` が無ければ全部)。先例: AE の Source Text の expression、Figma の寸法の札
