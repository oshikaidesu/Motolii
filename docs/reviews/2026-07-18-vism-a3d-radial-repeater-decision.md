...
状態: **設計決定／VSM-A3実装完了**。本書が固定した意味を[A3S](2026-07-18-vism-a3s-layersource-lowering-spec.md)の一般loweringと分割ticketで実装済み。製品Parameter Panel適合はM3-U4aへ分離したままである。

本書は[VSM-A3R](2026-07-18-vism-a3-external-expression-survey.md)が推薦した第一候補を、**決定論的 2D Radial Repeater** として採用・縮小・延期を確定した設計正本である。本書自身の当初ticketは実装許可、公開 API 変更、Document schema 変更、crate 追加を含まず、後続A3Sで境界を閉じてから実装した。調査の正本は A3R を改変せず維持する。
...
| ID | 依存 | 内容 |
|---|---|---|
| VSM-A3S | 本決定 | **完了**: §11 の handoff 質問を仕様化。仕様ticketではコードを触らない |
| VSM-A3 | VSM-A3S | **完了**: 本 v1 identity を A3S 一般経路と公開 API だけで外部 crate実装。製品Panel適合はU4aへ分離したまま |
