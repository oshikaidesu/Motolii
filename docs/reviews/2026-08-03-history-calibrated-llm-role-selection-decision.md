...
状態: **決定**（main統合後発効、2026-08-04 Luna担当範囲拡張）

日付: 2026-08-03

## 目的

Solを総監督としてauthority衝突、粒分割／STOP、複数契約統合、最終採否／main統合へ残す。authority、owner、scope、oracleが
閉じた粒では、preflight、次手選定、bounded施工、同一境界のreview finding修正、diff／test照合をfreshなLuna Maxへ委譲できる。
単純施工・修正にはSparkも選べる。Sol、Luna、Spark、Grok、Claudeを全task共通の固定順へせず、taskの判定対象から役割を選ぶ。
transport、監督責任、採用資格は増やさない。
...
1. 総監督は完全model ID `gpt-5.6-sol`、通常`model_reasoning_effort=medium`を基準とし、authority衝突、粒分割／STOP、複数契約の
   統合、外部model選択、最終採否／main統合を所有する。閉じた粒のpreflight、次手選定、bounded施工、同一境界finding処分、
   diff／test照合はLunaへ委譲できるが、Sol自身もLunaも独立検収票に数えない
...
5. authority、owner、scope、oracleが閉じた粒では、完全model ID `gpt-5.6-luna`、`model_reasoning_effort=max`のfresh sessionへ
   preflight、次手選定、bounded施工、同一境界finding修正、diff／test照合を一つまたは同一短waveとして委譲できる。単純な
   初回機械施工・修正は完全model ID `gpt-5.3-codex-spark`のfresh sessionも選べる。Luna Maxでもcapsule外read、複数契約施工、
   最終採否を移さない
...
Lunaの閉じたpreflight、次手選定、bounded施工、同一契約内finding修正、diff／test照合は`model_reasoning_effort=max`を
標準とする。ただしmaxは思考深度の選択だけであり、capsule外read、repo横断探索、複数契約施工、無制限tool turnを許さない。
担当範囲拡張は既存の検証結果で決定済みであり、運用開始前の再benchmarkを要求しない。
...
|---|---|---|---|
| authority衝突、粒分割／STOP、複数契約統合、最終採否／main統合 | Sol medium以上 | 総監督、境界変更、最終採否 | SolはOpenAI family施工の独立reviewを兼ねない |
| 閉じた粒のpreflight、次手選定、diff／test照合 | Luna Max | bounded capsule内の照合と次の一契約境界選定 | Lunaは独立reviewer、最終採否者を兼ねない |
...
| 閉じた粒のbounded施工、同一境界のreview finding修正 | Luna Max。単純施工・修正はfresh Spark可 | 指定pathの変更と指定試験 | 施工・設計へ未関与の別family |
...
1. Codexが先にauthority、base/cwd、worktree、scope、oracle、実装担当候補を確認する。閉じたcapsuleならpreflightと次手選定を
   Luna Maxへ委譲できる
2. Luna Maxは閉じたpreflight、次手選定、bounded施工、同一境界finding修正、diff／test照合を担える。単純施工・修正はSparkへ
   送れる。全modelを通すことや再benchmarkを完了条件にしない
...
9. authority衝突、粒分割／STOP、複数契約統合、最終採否／main統合はLunaへ委譲しない。必要性を検出したLunaはfindingとして返す
...
- Lunaの担当範囲拡張を固定stage、Sol代替、独立review資格、main統合権限へ読み替えること
