...
|---|---|---|
| U1a-1 shell/static viewport | PV-1 | notifierなし。製品binaryの実resize/minimize/restore、scale-factor adapter不変条件、resource lifetimeまで。複数scale monitor上の実DPI移動は別window/monitorを扱うU1eへ送り、疑似eventで実証したと主張しない。worker/seekは混ぜない |
