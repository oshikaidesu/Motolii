...
| VSM-A3 | A3D採択済みのRadial Repeater LayerSourceを、A3Sの一般経路と公開APIだけで別workspace crateに実装する | VSM-A3S | VRAM常駐golden、Draft/Final同一関数、正準座標、private依存拒否。M3停止中は現行Contract列挙fixtureだけを審判し、製品Parameter Panel適合はU4aへ分離する。[A3S §10](2026-07-18-vism-a3s-layersource-lowering-spec.md#10-後続-vsm-a3-の契約境界分割発注表)の分割発注待ち | **A3S完了・分割発注待ち** |
...
今:
  VSM-A0（調査完了）
    ...
    ├─ VSM-A3R（調査完了）─ VSM-A3D（決定完了）─ VSM-A3S（仕様完了）─ VSM-A3
