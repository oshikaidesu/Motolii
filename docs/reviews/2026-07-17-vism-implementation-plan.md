...
| VSM-A1 | `core.filter.opacity`相当を**別workspace crate**へ移し、公開`motolii-plugin` APIだけで登録・評価する | VSM-A1G | façade経由のみ、normal/dev/build allowlist、既存ID／version／A1G pixel一致、assembled purity、ID parity、Host必須capability検査 | **進行中**: A1-1完了（façade公開面）／A1-2・A1-3未着手 |
