...
| VSM-A4I | A4Sに従い、同じ境界の外部crate作者scaffoldを実装する | VSM-A4S、§8.1の全体レビュー | (1)生成crateの通常依存は`motolii-plugin`だけ、dev/build依存・build.rs・private APIなし (2)Host側conformance/purity/golden入口を同時生成または型付き案内し、plugin crateからtestkitへ逆依存しない (3)first-party参照crateがHost内部crate／UI toolkit／OS・vendor APIへ依存するとCI赤 (4)package/install/manifestを生成せずVism完成を称さない (5)既存in-tree scaffoldの出力を壊さない | **実装・機械oracle・main統合完了／独立review待ち**: `ee05faea`。`scripts/new-plugin-crate.sh` と `external_plugin_scaffold` がRadial Repeater source fork、dependency／ambient authority拒否、temporary Host conformance／GPU purity／goldenを検査する。distribution／dynamic load／hot reloadは後続境界 |
...
1. **VSM-A4**: first-party無特権gateを仕様へ昇格し、外部crate証拠を個別testの慣習から共通CI契約へする。A4SとA4Iは実装・機械oracle・main統合まで完了し、独立reviewが残る。
