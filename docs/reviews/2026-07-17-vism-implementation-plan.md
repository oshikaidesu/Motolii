...
| VSM-A3S | 一般LayerSourceをprepared recipeからruntime render stepへ下ろす公開signatureと旧`clear`特例処分を仕様化する。コードは触らない | VSM-A3D, VSM-A0S | kind検査、executor欠落、contract-only、未来version、raw不変、built-in rect分離、ID allowlist禁止の正負fixtureが未決なし | **着手可**（A3D完了） |
| VSM-A3 | A3D採択済みの`Clear`より実用的なLayerSourceを、A3Sの一般経路と公開APIだけで別workspace crateに実装する | VSM-A3S | VRAM常駐golden、Draft/Final同一関数、正準座標、private依存拒否。M3停止中は現行Contract列挙fixtureだけを審判し、製品Parameter Panel適合はU4aへ分離する | VSM-A3S待ち |
...
今:
  VSM-A0（調査完了）
    ...
    ├─ VSM-A3R（調査完了）─ VSM-A3D（決定完了）─ VSM-A3S ─ VSM-A3
