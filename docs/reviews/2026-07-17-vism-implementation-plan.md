...
| VSM-A3D | A3R候補を採用／縮小／延期し、表現意味、parameter、UI投影、非目標を決定する。コードは触らない | VSM-A3R | 現行`ValueType`で閉じる最小parameter表、純関数式、Parameter Panel投影、Composite／Simulation／Authoring Toolへの延期線、反対側論点が未決なし | **次。設計ticket** |
| VSM-A3S | 一般LayerSourceをprepared recipeからruntime render stepへ下ろす公開signatureと旧`clear`特例処分を仕様化する。コードは触らない | VSM-A3D, VSM-A0S | kind検査、executor欠落、contract-only、未来version、raw不変、built-in rect分離、ID allowlist禁止の正負fixtureが未決なし | VSM-A3D待ち |
| VSM-A3 | A3D採択済みの`Clear`より実用的なLayerSourceを、A3Sの一般経路と公開APIだけで別workspace crateに実装する | VSM-A3S | VRAM常駐golden、Draft/Final同一関数、正準座標、private依存拒否。M3停止中はContract metadata／UI投影fixtureでParameter Panel適合を判定し、製品UIを先行実装しない | VSM-A3D／A3S待ち |
...
今:
  VSM-A0（調査完了）
    ...
    ├─ VSM-A3R（調査完了）─ VSM-A3D ─ VSM-A3S ─ VSM-A3
