...
長期的には、catalog上の全Vismを独立実装単位として並列発注できる状態を目指す。共有公開境界の変更まで各Vismへ分散させるのではなく、kind横断conformance、first-party無特権gate、typed port、Host所有resource／Bake／StateTrack等を先に独立境界で閉じる。その後のVism追加は、独立crate、固有contract、固有fixture、追加的composition registrationだけで完了し、他Vismのsource／test／期待値を変更しない。詳細な完成条件は[Vismプラグインカタログ §14](../vism-plugin-catalog.md#14-将来展望--全vismを安全に並列実装できる構造)を正本とする。

この並列性は内部開発の速度最適化だけではない。Phase B〜Eで第三者へ接続する時、互いに非同期な作者・version・Host forkを同じ分散並列モデルで受け止めるための事前証明である。first-partyだけが中央調整やprivate APIで成立し、第三者だけにmanifest／permission／conformance負担を課す二重契約を作らない。
...
| VSM-A1 | `core.filter.opacity`相当を**別workspace crate**へ移し、公開`motolii-plugin` APIだけで登録・評価する | VSM-A1G | façade経由のみ、normal/dev/build allowlist、既存ID／version／A1G pixel一致、assembled purity、ID parity、Host必須capability検査 | **完了**: A1-1(façade)・A1-2(composition root)・A1-3(Opacity外部crate化) |
| VSM-A2 | ParamDriver参照実装を別workspace crate化する | VSM-A0I-3, VSM-A1, [VSM-A2S](2026-07-17-vism-a2-legacy-project-migration-decision.md) | Sine v1→v2をA0Iのdeclarative renameだけでprepared解決可能。raw recipe不変、同一`t`+入力で同一値、非有限入力の型付き拒否、private依存拒否。旧ProjectV1互換はprivate declarative adapter | **完了** |
| VSM-A3R | AE／aescripts／Blenderの外部表現を責任分類し、Expressionの実験性とParameter Panel集約を両立するA3候補を比較する。コードは触らない | VSM-A0 | 値評価、生成、合成、Host mutation、simulation、bridgeを分類し、A3候補・延期先・現行GAP・反対側審判を記録 | **調査完了**: [A3R](2026-07-18-vism-a3-external-expression-survey.md)。決定論的Repeater／Particle Field LayerSourceを第一候補として推薦 |
| VSM-A3D | A3R候補を採用／縮小／延期し、表現意味、parameter、UI投影、非目標を決定する。コードは触らない | VSM-A3R | 現行`ValueType`で閉じる最小parameter表、純関数式、現行Contract列挙とM3-U4a製品Panelの責任分離、Composite／Simulation／Authoring Toolへの延期線、反対側論点が未決なし | **決定完了**: [A3D Radial Repeater採用](2026-07-18-vism-a3d-radial-repeater-decision.md) |
| VSM-A3S | 一般LayerSourceをprepared recipeからruntime render stepへ下ろす公開signatureと旧`clear`特例処分を仕様化する。コードは触らない | VSM-A3D, VSM-A0S | kind検査、executor欠落、contract-only、未来version、raw不変、built-in rect分離、ID allowlist禁止の正負fixtureが未決なし | **仕様完了**: [A3S lowering](2026-07-18-vism-a3s-layersource-lowering-spec.md) |
| VSM-A3S-F1 | A3-2着手前監査で判明した公開Host `PipelineCache`境界欠落（0-input+uniform64定型なし）を訂正し、`VSM-A3-0`をA3S §10へ挿入する。コードは触らない | VSM-A3S | Host cache GAPと`tex_sample_uniform4`流用不可が明記。Q9 U4a維持。§10依存が`A3-2←A3-0∧A3-1c`。許可3 docsのみ | **文書補正完了**（本票） |
| VSM-A3-1a | `PreparedDocumentPlugins`をGraphBuilderへ渡し、LayerSource slot取得とtyped不変違反を追加。clear paramsをprepared評価へ切り替える | VSM-A3S | P3 prepared rename/raw不変、`PreparedLayerSourceMissing` typed拒否。一般loweringはまだ行わない | **実装完了**: `71737bd` |
| VSM-A3-1b | `doc.layer_source.rect`のcatalog／registry衝突をcomposition rootで型付き拒否する | VSM-A3S | catalog-only／registry-onlyのN6を`ReservedBuiltinId`で拒否 | **実装完了**: `b55b421` |
| VSM-A3-1c | prepared LayerSourceをexecutor kindで一般loweringし、clear専用allowlistを撤去する | VSM-A3-1a | P1/P2、executor欠落N1/N2、allowlist再導入N7、P4 rect built-in経路。[A3S §10](2026-07-18-vism-a3s-layersource-lowering-spec.md#10-後続-vsm-a3-の契約境界分割発注表) | **実装完了**: `e5ef694` |
| VSM-A3-0 | `motolii-gpu::PipelineCache`に`get_or_create_fullscreen_uniform16`（`[f32;16]`=64 bytes、binding 0、fullscreen triangle、`Rgba8Unorm` replace）を追加し、façade経由で外部crateが利用可能にする | VSM-A3S, VSM-A3S-F1 | cache hitでpipeline/shader/buffer再生成なし。外部fixtureが`motolii-plugin`依存だけでmethod到達。[A3S §10](2026-07-18-vism-a3s-layersource-lowering-spec.md#10-後続-vsm-a3-の契約境界分割発注表) | **実装完了**: `4a264ac` |
| VSM-A3-2 | `core.layer_source.radial_repeater` v1 外部crate＋contract／executor＋firstparty登録＋parity | VSM-A3-1c, VSM-A3-0 | P7 parity、A3D正準意味のVRAM executor、private依存拒否。[A3S §10](2026-07-18-vism-a3s-layersource-lowering-spec.md#10-後続-vsm-a3-の契約境界分割発注表) | **実装完了**（本票） |
...
- 現行BPMを変更せず、既存DataTrack→parameter結線へ時系列意味を渡すfixtureがある。BPM／拍リズムの製品所有はVism providerへ決定済みだが、consumer pluginとtyped rhythm公開形はまだ公約しない。
- 単一passだけをFilterの暗黙上限にしない。package／payload固定前に、Host所有multi-pass resourceを使う外部crateの証拠`VSM-A8G3`を通す。
...
| ID | 内容 | 依存 | 完了条件 | 状態 |
|---|---|---|---|---|
| VSM-B0 | package／entry／Kit／Project instance／artifactのidentity fixture | VSM-A1, VSM-A2 | §3の4ケースについてrename、update、duplicate、missing、reinstall、fork差替え後のidentity期待値が表になる | **着手可／意味fixtureのみ** |
| VSM-B1 | Vism／Kit／Preset／Asset／Bake／外部job／Project／Delivery Adapterの境界fixture | VSM-B0 | 各成果物の正本、持ち運び、欠落、更新、実行可否を分類。Project openでinstallが発生しない。Delivery Adapterはv1映像Exportと混ぜず延期のまま、trait／schemaを予約しない | VSM-B0待ち |
...
| VSM-B4 | payload classとfork capabilityを分類する | VSM-A3, VSM-A6, VSM-A8G3, VSM-B2, VSM-B4W | Declarative、WGSL、source+Host build、WASM、nativeとBase／Optional／Fork capabilityの可搬性・権限・再現性・DXを別評価。B4WのWGSLだけの結果を他payloadへ一般化せず、multi-passを単一entryの偶然へ縮約しない | VSM-A6/A8G3/B4W待ち |
| VSM-B5 | headless compatible runnerでUI／Document漏れを反証し、ISF／OpenFX adapter範囲を判定する | VSM-B2, VSM-B4 | Motolii UI／Documentなしで最小provider→consumerを評価。GLSL／HLSL／ISF等はruntime外部依存でなくauthoring時importとして比較し、非対応能力を型付き診断する。変換、source copy、由来、license claim、近似拒否のloss表 | VSM-B4待ち |
...
V2-8のcustom UI延期を維持する。Vism製品UIはU4a等の個別依存に従う。

| ID | 内容 | 依存 | 完了条件 | 状態 |
|---|---|---|---|---|
| VSM-E0 | installed Vism一覧／検索／診断snapshot | VSM-D2, VSM-D5, M3入場 | UI非依存のread-only snapshot。同期I/Oやtoolkit型を公開しない | WAIT |
...
今:
  VSM-A0（調査完了）
    ...
    ├─ VSM-A3R（調査完了）─ VSM-A3D（決定完了）─ VSM-A3S（仕様完了）─ VSM-A3S-F1（文書補正）─ VSM-A3-0 ─ VSM-A3-2…
    ├─ VSM-A7（完了、既存DataTrack→paramだけ）
    └─ VSM-A8D（調査完了）─ VSM-A8G0（multi-pass仕様）

M4 K1/K7後:
  SIM-1 ─ VSM-A6

M4 K1a/K1b・M5中間形式決定後:
  VSM-A8G0 ─ VSM-A8G1 ─ VSM-A8G2 ─ VSM-A8G3
...
Phase Aの仕様化とM2基盤再締結作業は並列に検討できるが、M2 Documentテストを変更する`VSM-A5`は再締結責任者と衝突確認する。`VSM-A8G0`の仕様化は進められるが、`VSM-A8G1`以降はM4 K1a/K1bとM5のlinear/HDR中間形式決定を越えない。Phase EはM3入場前に着手しない。Phase C以降はSIM-1とmulti-passを含む責任の異なる実plugin証拠、Phase B反対側レビューを飛ばさない。
...
### 8.1 2026-07-23再基線の次の五手

調査から計画へ移す次の順序は以下とする。

1. **VSM-A4**: first-party無特権gateを仕様へ昇格し、外部crate証拠を個別testの慣習から共通CI契約へする。
2. **VSM-A5**: kind横断のmissing／future／unknown matrixをM2再締結と衝突確認して固定する。
3. **VSM-B0**: packageを直列化せず、五つのidentityをfixtureで確定する。
4. **VSM-B1**: Vism／Kit／Preset／Asset／Bake／外部job／Projectを分類し、Delivery Adapterはv1映像Exportと分離した延期席に置く。
5. **VSM-B2**: provider→consumerとmaterialize Kitを比較し、xCloner級macroが専用backdoorなしで構成できる意味を決める。
