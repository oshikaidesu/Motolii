...
| VSM-A2 | ParamDriver参照実装を別workspace crate化する | VSM-A0I-3, VSM-A1 | Sine v1→v2をA0Iのdeclarative renameだけでprepared解決可能。raw recipe不変、同一`t`+入力で同一値、非有限入力の型付き拒否、private依存拒否 | **次** |
