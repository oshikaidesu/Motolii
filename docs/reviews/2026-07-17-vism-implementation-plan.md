...
| VSM-A1 | `core.filter.opacity`相当を**別workspace crate**へ移し、公開`motolii-plugin` APIだけで登録・評価する | VSM-A1G | façade経由のみ、normal/dev/build allowlist、既存ID／version／A1G pixel一致、assembled purity、ID parity、Host必須capability検査 | **次。実装ticket** |
