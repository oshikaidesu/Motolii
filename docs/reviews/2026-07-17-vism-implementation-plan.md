...
| VSM-A3 | A3D採択済みのRadial Repeater LayerSourceを、**A3-0完了後の公開Host cache**とA3S一般経路で別workspace crateに実装する | VSM-A3S, VSM-A3-0 | VRAM常駐golden、Draft/Final同一関数、正準座標、private依存拒否。M3停止中は現行Contract列挙fixtureだけを審判し、製品Parameter Panel適合はU4aへ分離する。[A3S §10](2026-07-18-vism-a3s-layersource-lowering-spec.md#10-後続-vsm-a3-の契約境界分割発注表)の分割発注待ち | **A3-2発注可** |
