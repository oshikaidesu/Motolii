...
|---|---|---|---|---|---|
| PRODUCT-ASSET | `CU-0A08IP` | `DO` | [Inspector read-model inventory](2026-07-26-cu-0a08is-inspector-read-model-inventory.md)でCU-0A08IS閉包とCU-0A08IP着手境界（§8.1）を固定。product-owned・非export pure decoder module、fixture/testのみ | Host transport、typed intent、JSX binding、`S`行、Rust/schema/plugin変更、mock literalからの推測 | CU-0A08IP implementation closed order |
