...
状態: **Host cache GAP 訂正済み／A3-0 挿入済み（コードは A3-0 待ち）**。本書は [VSM-A3D §11](2026-07-18-vism-a3d-radial-repeater-decision.md#11-a3s-handoff-質問未決のまま残す) の 9 質問をすべて閉じ、一般 `LayerSource` を **ID allowlist なし**で prepared recipe → runtime `RenderStep::Plugin` へ下ろす契約を固定する。[VSM-A3S-F1](2026-07-17-vism-implementation-plan.md) で A3-2 着手前監査により判明した公開 Host `PipelineCache` 境界欠落を訂正し、`VSM-A3-0` を §10 に挿入した。`motolii-gpu` 実装は A3-0 の別発注へ残す。
...
**現状 GAP（コード事実・graph／A3-1a〜1c で解消済み）**: `motolii-doc/src/graph.rs::build_source` は `CLEAR_LAYER_SOURCE` だけを `RenderStep::Plugin` へ下ろし、他は `UnsupportedSourcePlugin` とする。`build_document_frame_graph` は `prepare_plugins`＋診断を行うが、**prepared recipe を `GraphBuilder` に渡さず** raw `clip.params` を `resolve_plugin_params` で評価している。不足は公開 signature ではなく graph 内部の prepared 参照欠落である（A3-1a〜1c 完了）。

**現状 GAP（コード事実・Host cache・A3-0 待ち）**: 公開 Host `PipelineCache`（`motolii-gpu`、façade は `motolii-plugin` 再 export）には **0-input + 64-byte uniform** 定型が無い。現行 `get_or_create_tex_sample_uniform4` は **input texture + sampler + 16-byte uniform（binding 0/1/2）** の Filter 専用であり、Radial Repeater（A3D v1）が必要とする **input なし + `[f32; 16]`（64 bytes、binding 0 のみ）** には流用不可。したがって A3-2（外部 0-input LayerSource crate）は、**現行公開 Host cache API だけでは** AGENTS「ループ内 GPU 生成禁止」を満たせない。毎 frame `create_render_pipeline`／`create_shader_module`／uniform buffer 再作成が必要になる。本 GAP は Q9 U4a Contract 投影の十分性とは無関係である（§9 維持）。
...
| ID | 依存 | 内容 |
|---|---|---|
| VSM-A3S-F1 | VSM-A3S | Host cache GAP 訂正・A3-0 挿入（文書のみ。本票） |
| VSM-A3-0 | VSM-A3S（+F1） | 公開 Host `get_or_create_fullscreen_uniform16` 定型（§10） |
| VSM-A3-1a／1b／1c／2／3／4 | 本仕様と§10の個別依存 | §10 分割発注表 |
| VSM-A3-2 | A3-1c **かつ** A3-0 | Radial Repeater 外部 crate（§10） |
| VSM-A3 | VSM-A3-1a／1b／1c／0／2／3／4 | A3D v1 identity の外部 crate 実装と審判 |
