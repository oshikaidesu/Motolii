...
**現状 GAP（コード事実・Host cache・A3-0 完了）**: 公開 Host `PipelineCache`（`motolii-gpu`、façade は `motolii-plugin` 再 export）に **0-input + 64-byte uniform** 定型 `get_or_create_fullscreen_uniform16` が到達済み。現行 `get_or_create_tex_sample_uniform4` は **input texture + sampler + 16-byte uniform（binding 0/1/2）** の Filter 専用であり、0-input LayerSource へは流用不可。A3-2（外部 0-input LayerSource crate）は本定型を利用する。本 GAP 解消は Q9 U4a Contract 投影の十分性とは無関係である（§9 維持）。
