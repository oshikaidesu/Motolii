...
ORDER: CLOSED（当初は文書のみのREADY。VSM-A3全分割ticketの実装・審判完了）
...
**解消記録（A3-1a〜1c 着手時のコード事実）**: 着手時の `motolii-doc/src/graph.rs::build_source` は `CLEAR_LAYER_SOURCE` だけを `RenderStep::Plugin` へ下ろし、他を `UnsupportedSourcePlugin` としていた。また `build_document_frame_graph` は `prepare_plugins`＋診断を行っても、prepared recipe を `GraphBuilder` へ渡さず raw `clip.params` を評価していた。A3-1a〜1cで `GraphBuilder` のprepared参照、catalog-backed LayerSourceのprepared params評価、ID allowlistなしの一般loweringへ置換済みである。

**解消記録（Host cache・A3-0）**: 公開 Host `PipelineCache`（`motolii-gpu`、façade は `motolii-plugin` 再 export）へ **0-input + 64-byte uniform** 定型 `get_or_create_fullscreen_uniform16` を追加済みである。`get_or_create_tex_sample_uniform4` は **input texture + sampler + 16-byte uniform（binding 0/1/2）** の Filter 専用として維持し、0-input LayerSourceへ流用しない。A3-2の外部crateはA3-0定型を利用する。この解消はQ9 U4a Contract投影の十分性とは無関係である（§9維持）。
...
| Host cache | **A3-0 完了後**の `get_or_create_fullscreen_uniform16`（§10 `VSM-A3-0`）。現行 `get_or_create_tex_sample_uniform4` は流用不可 |
...
| VSM-A3-3（**完了**） | VRAM golden／union・premul・純関数／Draft=Final | testkit／plugin shader | schema・Panel |
| VSM-A3-4（**完了**） | 非 UI Contract 列挙 fixture（U4a 手渡し十分性） | 試験のみ | Slint・U4a 本体 |
...
型紙は既存 `get_or_create_tex_sample_uniform4`／`CachedTexSampleUniform4`／`PipelineCacheKey`（`vs_main`／`fs_main` の WGSL）に倣う。`[f32; 16]` の意味フィールド割当は A3-2 以降（本仕様では決めない）。

| 欄 | 値 |
|---|---|
| uniform | `[f32; 16]` = 64 bytes |
| binding | **0 のみ**（texture／sampler 無し） |
| geometry | fullscreen triangle |
| 出力 | `Rgba8Unorm`、blend **replace** |
| cache key | `PipelineCacheKey { id, wgsl }`（既存と同型） |
| cache hit | pipeline／shader module／uniform buffer を**再生成しない**（miss=1→hit≥1 の hits／misses カウンタ審判） |
| 外部到達 | 外部相当 fixture が **`motolii-plugin` 依存だけ**で `get_or_create_fullscreen_uniform16` を呼べる閉集合審判（`motolii-gpu` 直接依存や private Host API は不可） |

**正例（A3-0 実装 ticket）**: 同一 key で miss=1→hit≥1、pipeline／shader／buffer 再生成なし。外部 fixture が `motolii-plugin` だけで method 到達。

**負例（A3-0 実装 ticket）**: 毎 frame `create_render_pipeline`／`create_shader_module`／uniform buffer 再作成。`motolii-gpu` 直接依存。generic layout／可変 size／raw 公開割当。
...
以下は仕様化ticketとF1文書補正へ実装を混ぜないための当時の境界であり、後続A3-0〜A3-4の完了を否定しない。恒久schema、公開raw API、CPU frame等の禁止は現在も有効である。

- コード・テスト・schema・crate・UI／Slint 実装（本発注および VSM-A3S-F1 文書補正）
- `VSM-A3-0` の `motolii-gpu` 実装（A3-0 別発注。F1 では ticket 挿入のみ）
- generic bind layout、任意 uniform size、compute 抽象、raw resource allocation 公開 API（A3-0 非目標と同型）
...
9. Q9 U4a 十分性を Host cache GAP 名目で撤回しようとする（U4a と Host cache は別論点）
10. A3-0 なしで A3-2 着手可と読める依存表記を残す
11. `get_or_create_tex_sample_uniform4` を 0-input LayerSource へ流用しようとする
12. A3-0 実装（`pipeline_cache.rs` 等）を F1 文書補正に混ぜようとする
13. uniform 意味レイアウト／WGSL／Repeater 本体を本票で決めようとする

---

## 14. 検証コマンド

### 本発注（文書のみ）および VSM-A3S-F1
...
# A3-0 挿入・Host cache GAP・F1 訂正の機械確認
rg -n 'VSM-A3-0|get_or_create_fullscreen_uniform16|A3S-F1|fullscreen_uniform16|Host cache|tex_sample_uniform4' \
  docs/reviews/2026-07-18-vism-a3s-layersource-lowering-spec.md \
  docs/reviews/2026-07-17-vism-implementation-plan.md \
  docs/README.md

rg -n 'A3-2.*A3-0|A3-0.*A3-2|A3-1c' \
  docs/reviews/2026-07-18-vism-a3s-layersource-lowering-spec.md
...
`cargo test` は本発注および F1 の完了条件にしない。

**F1 審判（正例）**: §10 に `VSM-A3-0` があり、依存が `A3-2 ← A3-0∧A3-1c`。Host cache GAP と `tex_sample_uniform4` 流用不可が明記。Q9／§9 U4a 文が実質維持。

**F1 審判（負例）**: `.rs` 等の非 docs 差分。A3-0 なしのまま A3-2 着手可と読める文が残る。
...
- egui U0a移行とA3-0〜A3-4は完了済み。A3実装前に要求した分割発注、A3-2 ← A3-0依存、反対側レビューの各gateは充足した。
- U4aは個別依存のまま。本A3完了を製品Parameter Panel適合の完了と読まない。
- GR-PV: 恒久 schema／新 field を仕様名目で先焼きしない
- 画素正本は A3D。A3S は lowering／拒否／coverage 数値意味と handoff のみ。A3-0 は Host cache 定型のみ
...
| ID | 依存 | 内容 |
|---|---|---|
| VSM-A3S-F1 | VSM-A3S | **完了**: Host cache GAP 訂正・A3-0 挿入（文書のみ） |
| VSM-A3-0 | VSM-A3S（+F1） | **完了**: 公開 Host `get_or_create_fullscreen_uniform16` 定型（§10） |
| VSM-A3-1a／1b／1c／2／3／4 | 本仕様と§10の個別依存 | **完了**: §10 分割発注表 |
| VSM-A3-2 | A3-1c **かつ** A3-0 | **完了**: Radial Repeater 外部 crate（§10） |
| VSM-A3 | VSM-A3-1a／1b／1c／0／2／3／4 | **完了**: A3D v1 identity の外部 crate 実装と審判 |
