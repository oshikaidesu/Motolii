...
歴史的な[INF-7g Opacity実演](2026-07-11-INF-7g-llm-plugin-demo.md)は、scaffold、conformance、purity、goldenを先に用意すると、LLMが一つのFilterを人間差し戻し0回で緑へ到達できた一例である。この証拠が支えるのは作者入口の摩擦を機械判定へ移せる可能性までであり、全pluginの自動採用、レビュー不要、第三者runtime／配布成立を意味しない。
