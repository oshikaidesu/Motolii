...
- 初回レビューが挙げた「抜け5点」のうち、Motoliiにとって本当に新しいP0問題は**GPU→CPUリードバックのパイプライン化**のみ。VRAM予算・プラグイン資源・色管理は既に設計へ入っており、「抜けている」は過大だった
...
- シェーダー初回コンパイルのヒッチは実在する未解決ギャップ(リードバックに次ぐ)
...
### B-1. GPU→CPUリードバックのパイプライン化 — **採用候補(P0提案・未採択)**

初回レビューの指摘どおり最重要。しかも**文書と実装が不一致**だった(確認済、2026-07-13リポジトリ実物)。
...
必要な作業: 2〜4本のリングバッファで `GPU render N+1 / GPU→staging copy N / CPU encode N-1` を重畳し、boundedなin-flight数でdGPU・ユニファイド双方を計測する。P1例外(確定出力の非同期コピーアウトによるキャッシュ充填)もこの実装が前提。

既存タスクとの関係(2026-07-13追記): **bounded in-flight自体は[M1実装ガードG7](../specs/M1-vertical-slice.md)に既にある**(bounded channelによるバックプレッシャ、増強チケット候補・未実装)。本レビューで新規なのは (1)**GPU stagingリングによる重畳**(G7はchannel上限の話で、GPU→CPUコピーの非同期重畳は含まない)と (2)**性能SLO**(§C-2)の2点であり、正式タスク化はまだされていない — 本文書はP0**提案**であって採択済みではない。採択判断はbacklog/仕様への反映時に行う。
...
### B-4. シェーダー初回コンパイルのヒッチ — **採用候補(P1昇格提案・ゲート判定待ち)**
...
4. 同一GPU・同一処理の最小Metal/Vulkan/DX12試作と比較
5. 直接APIではSLOを満たすのに、**wgpu固有**の抽象・ロック・変換・interop制約で一貫して満たせないことを確認

ここまで来て初めて「wgpu採用が技術的失敗だった」と言える。なお「一部CPUフォールバック」はVRAM常駐原則(memory-model)を壊し、多くの場合さらに遅くなるため、**早期リカバリー策にならない**(誤診による最悪の対処)。
...
## 優先順位(アクションは1セット、2026-07-13改訂)

1. **P0(1セットで採択判断する)**:
   - GPU stagingリング(重畳)の正式採択判断 — M1 G7(bounded channel)との差分を明確にした上でbacklog/仕様へ
   - 原因分離ベンチマーク(§C-2の4分割+2 sink)
   - 3〜5分代表MVのユーザー向けexport SLO(実時間の何倍か)
   - SLO未達時にwgpu固有原因かを判定する比較手順(§C-4)
2. **P0**: memory-model.md P1の過大記述の訂正 — **本レビューで実施済み**
3. **P1**: cold shader compilationのSLOとprewarm/fallback完了条件をINF-8へ(B-4)
4. **P1**: memory-model.md P3のVRAM API記述をwgpu 29へ更新 — **本レビューで実施済み**(allocator reportは診断補助扱い)
5. **P2**: bindlessは要求が発生するまで不採用。採用時はfeature matrixとfallback必須(A-6)
6. **v2**: 動的C ABI・プロセス外共有テクスチャ境界の専用スパイク(B-3、backlog V2-1の枠内)

この形なら「遅かったから即CPUフォールバック」という誤診を防ぎつつ、wgpuが本当に限界だった場合も早期に見切れる。v1の実測は開発主機(Apple M4/ユニファイド)のみで行い、dGPU実測はWindows対応時の再計測項目として予約する。
