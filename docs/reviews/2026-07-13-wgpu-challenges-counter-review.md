...
> 2026-07-23歴史監査: cutoff全3版と併読先例1版を[Unit 5C回収](2026-07-23-historical-wgpu-readback-cold-compile-lineage-recovery.md)で処分した。最新版の「計測前にring本数／優先度を決めない」訂正を維持する。同期export readbackの採択gateはGAP-29、dev hot reloadとは別のproduct cold pipeline admissionはGAP-30で追跡する。M1 G7のboundedな滞留、M4 K1c/K7aの将来copy-out、現行PipelineCacheの一部捕捉を重畳／prewarm実装済み証拠にしない。
