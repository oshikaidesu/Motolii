...
---
| **Intent 背骨そのものがスクリプト命令集合**(裁定2/47/48 の帰結、利用者洞察 2026-08-22: 「今行っていることがスクリプト操作」) | 自動化・バッチ操作・外部制御 | スクリプト/ExtendScript 系の map 行は「作る」対象でなく**薄い顔**(言語バインディング or JSON-Intent の口+query API+undo 束ね規約)だけで成立 | 境界: バインディング言語未裁定・StoreView の query 面の外部公開整備は未着手・式(per-property reactive)は別物で D5 未完のまま |
