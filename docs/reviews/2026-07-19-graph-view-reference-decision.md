...
日付: 2026-07-19
状態: **React prototype比較中／製品採択・M3 task化は未決**

## 比較境界

固定React baselineは、実時間×実値で複数key・複数区間を俯瞰して編集する
独立`Graph View`候補を比較する。既存のFlow / Alight Motion型区間editorとは
同じ名前、状態所有、表示範囲として扱わない。ただし、Graph Viewの製品採択、
`Interval Easing Editor`への改称、M3 task追加を本記録だけでは決めない。

次表はprototype内で比較対象を混同しないための区別であり、製品仕様ではない。
...
これらを製品契約へ採択する場合は、M3仕様、譜面UI構成モデル、状態所有、
command/Undo審判を先に改訂し、一意なtask IDを与える。
