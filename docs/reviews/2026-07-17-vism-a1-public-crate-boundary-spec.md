...
状態: **仕様決定／A1-3完了**。Opacityは `plugins/motolii-plugin-opacity` へ外部化済み。first-party組み立て・依存検査・公開API閉集合・assembled purityはVSM-A1-3で完成。

ここでいう「外部crate」は`motolii-plugin`実装crateの外、同一repository/workspace内のbundled first-party crateを指す。第三者packageのinstall/load、dynamic ABI、sandbox、署名、version併存を実証したという意味ではない。全9版と後続provenance決定の処分は[公開capability歴史回収](2026-07-23-historical-public-capability-provenance-lineage-recovery.md)を参照する。
...
外部から可変Registry／Catalogを受け取る登録関数、builder、raw GPU helperは公開しない。
...
`FirstPartyError`はcontract、plugin、runtimeの元errorをtransparentに保持し、必須Host capability欠落とDocument builtin予約ID混入も別variantで返す。callerから可変registryを受け取る公開登録関数を足さず、部分登録状態を外へ残さない。表示文字列化は最終adapter境界まで遅らせる。
...
このcomposition rootはbundled first-party既定集合だけを組み立てる。user-installed package集合、project lock解決、trust policy、marketplace registryを同じ関数へ混ぜない。
