...
状態: **A1-1完了／A1-2・A1-3未着手**。A0I-1〜3でContract Catalog、prepared resolution、製品実行入口を実装したが、現行`FilterPlugin`を別crateから実装するための公開依存、first-party組み立て点、検査器、移動前pixel基線が未確定だった。本書でそれらを固定し、A1実装者が公開APIや例外を発明しない状態にする。
