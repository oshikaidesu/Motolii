...
| [2026-08-30-ae-shortcut-backsolve.md](2026-08-30-ae-shortcut-backsolve.md) | **打鍵からの逆算**(測定・一次資料突き合わせ済み)。分割に共通打鍵は無く(Pr=Cmd+K/Resolve=Cmd+B/AE=Cmd+Shift+D)、ResolveはPr/FCP/Avidのpresetを読み込める — **土台は打鍵の交差集合ではなく操作の名簿、打鍵はpreset**。I/O・J/Kが世界をまたぐと別の意味になる衝突表。属性表示はU/UU/SS(述語)を引き継ぎP/S/R/T/A(1属性=1文字)は引き継がない — vismの任意パラメータで破綻するため |
| [2026-08-30-media-delegation.md](2026-08-30-media-delegation.md) | **motolii-mediaは全部スクラッチ**(裁定+測定)。2673行のうち2357行に委譲先が在り新規依存はゼロ — decode/probe/point_cloudはRerunフォーク、encode/muxはffmpeg-sidecar、waveformはsymphonia(後2つは既にCargo.lockに居る)。**re_renderer/src/video/が憲法の中に在り`frame_at`が時刻→GpuTexture2Dを返す**。境目は「読む/書く」ではなく「保守責務を自分が持つか」 |
| [2026-08-30-rerun-viewer-composition.md](2026-08-30-rerun-viewer-composition.md) | **借りられる核は5クレート**(測定)。re_renderer/re_video/re_chunk_store/re_entity_db/re_queryがegui依存ゼロで、**5つのうち4つは既に借りていた**。egui非依存の駆動手本は`re_renderer_examples/framework.rs`(winit+wgpuのみ)。**次の候補=ピッキング** — Stageのクリック選択は自前だが`PickingLayerProcessor`が上流に在りegui非依存の実証コード付き |
| [2026-08-30-agents-v4-first-draft.md](2026-08-30-agents-v4-first-draft.md) | **四代目AGENTS.md初版の全文**(歴史)。注意の全削・一からの裁定で退役し、憲法は13行へ書き直した |
| [2026-08-30-legacy-agents-canon.md](2026-08-30-legacy-agents-canon.md) | **三代目までの AGENTS.md の原文**(歴史)。リポ根の入口を正本(motolii/AGENTS.md)への案内だけへ畳んだ際に移設。当時の事実のまま保存し更新しない |
| [2026-08-30-upstream-conformance-ledger.md](2026-08-30-upstream-conformance-ledger.md) | **上流の契約を外れている所9件**(測定+利用者裁定2件)。欠陥の形はどれも同じ — 上流の前提を読まずに繋ぎ、Motolii が別の前提で書いた。`depth_offset` は層の重ね順ではなく**同一平面クラスタ内の同点処理**(`plane_clustering.rs`)で、背景を世界の板として置いたせいで z を持つ層が消えていた。premultiplied を `SeparateAlpha` で渡してα=0.5の画素が27%暗い(128→93、検算つき)。**export と Stage で premultiplied sRGB の規約が違う**(186→174、検算つき)— 背骨の preview=export はバイト水準で成り立っていなかった。**当日 12件へ更新** — 7件着地、7番は欠陥でなかった、12番「blend mode は特別ではない」(blend/matte が Vism の口を通っていないだけ)。**上流に blend mode が無いことは全文検索+公式ドキュメントで検証済み — 再調査しないこと** |
| [2026-08-30-handoff.md](2026-08-30-handoff.md) | **宿題**(引き継ぎ)。裁定待ち8件 / すぐ出せる4件 / 先に測る4件 / 見えている欠陥4件 / 近似で止めた1件 / 監督の失敗3件。**次のセッションはまずこれ** |
...
| [2026-08-23-handoff-evening.md](2026-08-23-handoff-evening.md) | **引き継ぎ(2026-08-23 夕)— 最終チェックに来る利用者へ**: 利用者が「開発者ではなく一ユーザーとして批判する」と宣言したことを受け、**批判の的を絞るための3点セット**(起動コマンド/触ってほしい順序=P1 手順書/**既知の穴リスト**)。§0= 今日の成果は全て静的検査だけで着地し**窓を一度も開けていない**(裁定219 の新語彙「結線済(実機未確認)」)。§4= 経路(台帳が枝の端しか示せていないことが数字になった→基準を変えたら測っていた物が消えた→正典が台帳と同じ病気だった→責任の集中が分割線になった)。§5= 裁定217/218/219。§6= 監督の失敗3件(worktree 非隔離・背景待ち自己停止13件・`--workspace` を回すまで取り残しに気づかず) | **引き継ぎ**(2026-08-23) |
| [2026-08-23-portability-and-user-settings.md](2026-08-23-portability-and-user-settings.md) | **User Settings 層と可搬性の設計案(F-1)**: P3後半「別マシンで開く/人に渡す」の【穴】意味が無い17件+C-1が自己申告した「User Settings層自体が未設計」を受けた設計案(コード無し)。**Document に入れる/入れない判定式**=軸A(Lottie出力の意味を変えるか)×軸B(他人へ運びたいか)の2軸、既存2先例(`AssetStatus`の`#[serde(skip)]`・`Session`の「Documentには乗らない」)が同じ論理の独立発見だったと確認。**探索範囲つき**で`dirs`/`directories`crateがワークスペースに皆無(直接・推移的とも grep 0件)と確定、C-1の手組み`user_settings_dir()`を畳んで単一JSON(`user_settings.json`)へ載せ替える案を提示。可搬性はAE Collect Files(フォント非対応)/Premiere Project Manager/Resolve Media Management/Blender Pack Resources/Figmaのmissing-fontダイアログの共通項(使用素材のみコピー・フォントは通知止まり・欠落は常時可視の帯)を出典つきで既定に提案。`WorkspaceBook`/`pane_layout::Layout`/C-1 sidecarは畳める、`auto_save`は畳まないと判定。write-set案6レーン(G-1直列→G-2〜G-4並列→G-5→G-6独立)。利用者裁定4点(crate追加可否・Session復元範囲・Collect Filesのスコープ・WorkspaceBook結線の要否)を明示、裁定候補5件を提示 | **設計案**(2026-08-23) |
| [2026-08-23-lessons.md](2026-08-23-lessons.md) | **教訓(2026-08-23)— 引き継ぎ文書が要らなくなった日**: 利用者の観察「もう構造で誰でも続きを書ける。なら行うことは教訓書き」を受け、**引き継ぎを書かず教訓だけを残した**。現在地は `plan_steps.py`/`plan_backlog.py`/`plan_waves.py`/`gen-inventory.sh` が出すので運ぶ物が無い。中身= (1) **台帳の腐り25件は全部「機械が出せる列を人が書き写していた」型**だった (2) **報酬の設計を2回間違えた**(実装が在る≠届く・確認する≠直す) (3) **勘で優先順位を決めていた**(選んだ A03 は実際には最下位) (4) **自前スクラッチ(syn 17,488行)は典型的に壊れた**(doc コメントを呼び手に数える)— rustdoc JSON へ置換で 1,737→5,134項目 (5) 並列は write-set で決まり**計算できる** (6) **監督も止まる**・階層を深くすると停止点が倍 (7) **手順書を階段にすると次の仕事が議論なしで決まる**(28段階) (8) **静通/実通**の分離で人の席が決まった (9) **意味は外部から。壁を上へ投げない**。裁定217〜222 と、残っている人手5件 | **教訓**(2026-08-23) |
| [2026-08-25-layer-attrs-vs-property-audit.md](2026-08-25-layer-attrs-vs-property-audit.md) | **layer 属性 vs property の棚卸し**(観察)。判定基準は1文「layer は property の集合しか持たない/値と補間は property が持つ/静的値は keyframe 0本の property である」(出典 = Lottie `ks`、裁定215 の「借りる」の形)。`next/core/motolii-store/` の layer 属性 28件を P/A/? で判定 = P 10・A 13・? 5。Speed の二重化(ATTRS 欄の `Intent::SetTiming` 経路が動き、`PropertyId::speed()` の track は器だけで配線未完)は特別ではなく「静的値の置き場が property の外にある」の1例。Inspector 側の帰結 = `src` 10,059行・手書き `*_section` 8本・`TransformField::EffectParam(EffectId, GlowParam)` の型シグネチャに Glow 1本が焼き込まれている。descriptor の持ち主は**未裁定** |
| [2026-08-25-execution-plan.md](2026-08-25-execution-plan.md) | **実行計画(2026-08-25)**(観察・**計画であって裁定ではない**)。裁定237/238/239/240/241 を発注へ落とす順序表。**発注前に閉じる穴3件**(descriptor の持ち主 / 姿勢表現+handedness / `motolii-store` を割るか)はレーンに判断させず裁定へ回す。**ボトルネック** = `next/core/motolii-store` が1 crate のため裁定241 の P群10件の移送が直列化する — 裁定240 の標準作業「並列を上げたい所を割る」の最初の適用先で、**割るなら移送の発注前**。波 = W0 分割判定+憲章起草 / W1 property 移送 / W2 Inspector=窓 / W3 camera 向き / 並行して裁定237 の切片6本。**既知の衝突1件** = 切片4(呼び手を chrome へ)と W2 が `motolii-inspector-pane` で衝突 → 切片4 から Inspector を外し W2 着地後へ。開いた項目 = `decision-index.md:23` の凍結宣言と加筆され続けている運用実態の矛盾(未裁定) |
| [2026-08-26-timeline-ordinary-ux-ledger.md](2026-08-26-timeline-ordinary-ux-ledger.md) | Timelineの機能名より下にある普通のUX契約30件を、W3C Pointer Events・WCAG・Apple・AE/Premiere/Resolve/CapCut公式資料から正規化。共有意味核はPASS 17/PARTIAL 13、MakepadホストはPASS 9/PARTIAL 10/FAIL 11。汎用gesture seamとmacOS native pinch、trackpad phase/axis/momentum/deltaは静的実装済み、他OS native producer・hit ownership・cancel・selection・snap feedback・drag代替は未適合として機械可読TSVへ固定 | **調査/台帳**(2026-08-26) |
| [2026-08-27-layer-ui-parity-defects.md](2026-08-27-layer-ui-parity-defects.md) | **レイヤー UI の当たり前が欠けている箇所**(採用・発注中)。並列監査5レーンが挙げた27件から「**AE なら30年前からできること**」に絞った21件の台帳。分類 = 虚報(操作できると表示しながら操作が無い — Q0直撃)/ 欠落 / 幾何充填 / 状態設計。主な物: クリップにトリムハンドルが無い・無修飾の縦スクロールが常に `0.0`・クリック選択の経路が無い・検索欄が `InkLabel` で打てない・インスペクタの数値が `InkLabel` なのにフッターが `"drag to scrub · click to type"` と約束している・`fitted_lane_height`(ペインのリサイズがタイムライン全体の画像ズームになる)。**なぜ本題か** = 「レイヤーは UI であって存在論ではない」という賭けは、レイヤー UI が実際に強い場合にしか成立しない。検収は**窓を叩いて**行う(テストを書かない — 裁定270) | **台帳/発注**(2026-08-27) |
...
- [環境の解像度 — 空は 4096、映り込みは mip](2026-09-08-environment-mip-chain.md)
- [ガラスは背後に描かれた層を屈折して通す](2026-09-08-transmission-backdrop.md)
- [Clip — 世界の平面で切る](2026-09-08-clip-plane.md)
- [環境(HDRI)と面の応え方 — 空・鏡面・Glass](2026-09-07-environment-and-surfaces.md)
