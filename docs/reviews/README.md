...
| [2026-08-22-zero-copy-seam-decision.md](2026-08-22-zero-copy-seam-decision.md) | **裁定171: ゼロコピーの合流設計(M4)** — engine は iced device へ遅延合流(Pipeline::new で clone→OnceLock 口→Engine::with_device 作り直し)・headless は器具/export の恒久フォールバック・blit 元は BL1b の main_target accessor・同一 queue の submission 順で同期。市松は当面 readback フォールバック | **決定**(2026-08-22) |
