...
区間Easingを開くGraph iconと、object・channel・pressed/disabledを表すaccessible stateだけをReact chromeが
所有する。visible summary chromeは固定source不在の
未決事項であり、本契約から発明しない。popup内のframe、preset shelf、
...
```text
React Graph icon / accessible object-channel and pressed-disabled state
...
| 対象 | owner | 禁止 |
|---|---|---|
| Graph icon、object・channel・pressed/disabledのaccessible state | React | visible summary chromeを発明せず、popup内容、preset thumbnail、curve stateを複製しない |
