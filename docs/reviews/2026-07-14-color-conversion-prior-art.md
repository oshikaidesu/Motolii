...
| 提案 | 対応する穴 | 粒度 |
|---|---|---|
| §3 GPU RGB→YUVエンコードパス | F-5ガードの例外解消+ユーザー方針 | 1PR(シェーダ+encode.rs+テスト3種) |
| §2 GAP-5測定マトリクス | 「書き出したら色が違う」の最終境界 | 実測レポート(backlog GAP-5そのもの、P2) |
| §4 sws_flags | §3延期時のみ | 1行+ゴールデン更新 |
