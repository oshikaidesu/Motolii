...
**→ 決着(2026-08-22・第5セッションで利用者確認済み)**: (a) が正解 — Lab という解釈が誤り。利用者の意図は「並列エージェントがタイムライン/インスペクター/ステージで毎回 main をビルドせず・コンフリクトしないための **pane crate 分割**」で、それは**裁定160 として実装済み**(`next/ui/` の 7 crate・[pane-split-survey](2026-08-21-pane-split-survey.md))。新しく作る物は無く、キューの「pane Lab 発注」は解消。
