...
ステータス: **調査完了・採択待ち**(仕様変更はユーザー決定事項。[レビュー規律](README.md)1/6により本文書単独を設計根拠にしない — ただし本調査は「現行仕様が根拠に挙げた先例(mpv/ffplay)の一次資料確認」を含むため、現行仕様側の帰属検証は済んでいる)
...
4. **採択案**: D5からクロックオーナー交代・適応リサンプリングを外し、「音声クロック常時主+映像フレームドロップ+**適応解像度降格**(Draft 1/2→1/4、[performance-model.md](../performance-model.md)の既定の実装)」へ。D5最大の難所だった「可聴アーティファクトの人間の耳審査」は、音声経路が恒等(リサンプラ無し)になることでクリティカルパスから消える。GPU性能が上がるほど降格頻度が下がるため、ハードウェア進化で自動的に品質が上がる(ユーザー指摘と整合)。
...
- **cpal**: `OutputCallbackInfo::timestamp()`の`playback - callback`で出力デバイスレイテンシを取得可能(ALSA/CoreAudio/WASAPI等ホスト別実装あり、PulseAudio/ASIOは粗い)。現行仕様6項(レイテンシ補償)の実装部品。総提示遅延=デバイスレイテンシ+リング充填量(+リサンプラ遅延があればその分)。[cpal timestamp.rs](https://github.com/RustAudio/cpal/blob/master/src/timestamp.rs)
- 現行D4の`pick_output_config`(`crates/motolii-audio/src/device.rs`)はデバイスが素材のサンプルレートを直接サポートしない場合に型付きエラーで拒否する。固定比リサンプルのフォールバック席が未整備(下記判定表)。
...
- 制御則はid Tech型二重閾値+UEの安全弁に縮小して輸入:
  ...
  - 計測はGPUフレーム時間(wgpuタイムスタンプクエリ)を正とし、CPUバウンド判定を最初から席として持つ(UEの教訓: CPUが犯人の時に解像度を下げても無意味)
- 発振(解像度パンピング)しないことをテスト完了条件に含める
...
> | D5 | Transport(音声クロック常時主): 再生位置=供給済みサンプル数由来。映像はフレームドロップで追従(常に最新の現在時刻のみレンダ)。低速時は適応解像度降格(Draft 1/2→1/4、二重閾値ヒステリシス+連続超過パニック降格+CPUバウンド判定)で実時間を回復し、なお不足なら音声正速のままドロップ継続。クロックオーナー交代・自動バリスピードは持たない(明示スクラブ/シャトルは別機能)。レイテンシ補償はcpalタイムスタンプ+リング充填量 | D3, D4 | 10分素材でA/Vドリフトなし(実測)。レンダを人工的に0.5xに律速した状態で**音声が無傷(正速・無欠落・ビット同一)**のまま映像がドロップ+解像度降格で追従するテスト。解像度切替が発振(パンピング)しないテスト。降格・復帰の切替点で音声グリッチが無いことの確認 |

完了条件の「人間の耳による可聴アーティファクト審査」は、音声経路にリサンプラが入らなくなるため**縮小**される(適応リサンプリングの品質審査→切替時に音声経路へ副作用が無いことの確認へ)。完全廃止ではない。
...
| rubato固定比変換(デバイスレート≠素材レート) | **縮小採用の余地** | 現行D4はデバイス非対応レートを型付きエラーで拒否(`pick_output_config`)。固定比フォールバックの席が要る。D5かD4-FUかは発注時判断 |
