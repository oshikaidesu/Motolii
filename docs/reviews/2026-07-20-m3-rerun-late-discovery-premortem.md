...
計器を製品型へ出したくなった時点でU1a-1bを停止し、負例の作り方を先に見直す。

## 5. 実装順への反映

本監査は新しい横断タスク列を増やさない。既存枝番へ吸収する。ただしU1a-1だけは、
「1チケット=1成果物」原則に対して受入面が過大になるため、表示成立と寿命負例を分ける。
分割後も初回Uシリーズの直列実行は変えず、順序はU1a-1→U1a-1b→U1a-2とする。

| 枝番 | 追加する審判 |
|---|---|
| U0e-2 | 単一fixture manifest、React/egui adapter同一identity、test-only font、layout安定化、manifest保存形式の確定 |
| U1a-1 | 表示が成立すること。色・alpha・寿命の負例はU1a-1bへ送る |
| U1a-1b | 色・alpha負例、native texture/pool寿命、resize/minimize/restoreのresource一定性、test-only計器 |
