...
- 本書作成後、mainではU0a〜U0e-1、U1a-1/2、U1b-1/2、U2a-0/1、U2b-1、
  U2c-1/4まで完了した。初回Uシリーズを1枝番ずつ直列実行する規律は維持する。
- `motolii-ui`の製品依存はegui 0.35 / egui-wgpu 0.35 / egui_tiles 0.16 /
  wgpu 29へ統一され、egui shell、native texture preview、panel layout投影、render workerは
  比較基準として成立済みである。今後のruntime固有実装はG0-9の再選定結果を待つ。
...
| LD-2 | React fixture、Rust fixture、snapshotが別々の入力を持ち、見た目が一致しても別の製品状態を検査する | Reactは安定IDを持つが、DOM/CSSは製品契約でなく、U0e-2の共有fixture所有が未明示だった | **決定**。1審判につきtoolkit非依存のfixture manifestを一つだけ正とし、content ID・component安定ID・state・locale・viewport条件を持つ。React/eguiはadapterで同じmanifestを読む。DOM、CSS、snapshot、Rerun fixtureから意味を逆生成しない。manifestの保存形式はU0e-2の成果物としてG0-6H**より前**に確定し、以後の枝番は形式を再発明せず参照する |
...
| LD-4 | native texture表示で色変換、sRGB解釈、premultiplied alphaをUI側が重ね、Exportと見た目がずれる | 現行は`Rgba8Unorm` viewをeguiへ登録する方針だが、静止表示の色・alpha負例がU1a-1枝番に明示されていなかった | **決定**。UIはrendererが作った表示textureを一度sampleするだけで色変換・premultiply/unpremultiplyを追加しない。opaque、半透明edge、color rampの固定fixtureでdouble transform、double premultiply、channel swapを拒否する責任は現行U1a-1契約へ吸収済みである |
| LD-5 | resize、pool再作成、panel detach時に`TextureId`とtexture slotの寿命がずれ、tearing、古いframe、GPU resource leakが後から出る | 「一度だけnative texture登録」は既決だが、retire/unregisterとsampling中slot再利用の審判が不足していた | **決定**。display slotはUI frameが参照中に再利用しない。登録はpool生成時一回、解除はpool retirement時だけとする。ここでの「登録し直さない」対象はframe更新と**pool extentが変わらないsurface resize**であり、extentが変わる場合はretirement→新pool生成→新規登録だけを正規経路とし、既存poolのviewを差し替える近道を禁じる。resize/minimize/restoreとpool交代の負例は現行U1a-1契約へ吸収済みである |
...
| LD-8 | Rerunのdensity表示は成立しても、semantic zoom境界で個別keyのhit targetと選択が消える | Rerun Time Panelは閲覧面で、Motoliiの編集hit-testを証明しない | **決定**。U3aは遠景density・中景cluster・近景individualを同じ時間rangeとstable ID projectionから作る。三段の呼称はこのMotolii語を正本とし、転移計画の「遠景density・中景bar・近景key」はRerun側の表現として対応付ける。zoom境界前後で選択identity、playhead、visible rangeを保ち、density pixelをDocument object identityに使わない |
...
manifestは製品状態ではないため、U0b-1の5所有層（Document / User settings / Workspace profile /
Project session / Transient）のいずれにも分類しない**試験資産**として扱う。所有層を与えたく
なった場合は、それは製品状態が紛れ込んだ兆候であり、manifestではなく対象specへ戻す。
保存形式の確定者と時点はU0e-2であり、実装担当が着手後に選ぶ余地を残さない。
...
registration数とGPU resource数を数える計器も同じ扱いとする。公開APIやDocumentへcounterを
足さず、`motolii-ui` crate内のtest-only accessor（`#[cfg(test)]`またはtest専用feature）に限る。
計器を製品型へ出したくなった時点でU1a-1を停止し、負例の作り方を先に見直す。

## 5. 実装順への反映

本監査は新しい横断タスク列を増やさず、既存枝番へ吸収する。当初検討したU1a-1b分割は
mainへ統合されず、表示成立と色・alpha・lifecycle負例を一つの現行U1a-1契約で閉じた。
したがって独立したU1a-1bは未完了タスクではなく、履歴上の検討案として扱う。

| 枝番 | 追加する審判 |
|---|---|
| U0e-2 | 単一fixture manifest、React/egui adapter同一identity、test-only font、layout安定化、manifest保存形式の確定 |
| U1a-1 | 表示成立、色・alpha負例、native texture/pool寿命、resize/minimize/restore、test-only計器 |
...
- fixture manifestを公開API、Document、plugin契約へ昇格したくなった、またはU0e-2で確定した
  保存形式を後続枝番で作り直したくなった
- registration/resource計器を公開APIまたはDocumentへ出したくなった
