...
- **理由1（供給側に閉包述語がない）**: `CU-107` の依存 `CU-0B05` は W0 表で `WAIT`（依存: `CU-0B04N/R`）であり未解決である。したがって `CU-107` 側には「何をもって閉じたと言えるか」の述語が採択済み authority に存在せず、狭い名前付き前提の個数を先に決めれば「もっともらしいデフォルト」を発明することになる。これは [AGENTS.md](../../AGENTS.md) ワークフロー「仕様書の未決事項に依存するタスクに着手しない／未決をもっともらしいデフォルトで埋めない」に反する。
