...
後続仕様改訂によりG0-9は段階化済みである。`G0-9L`は固定Macのplatform prerequisite evidenceだけを
限定確定して完了したが、W0b、H1b、Motolii Studio Preview、window結合を単独では解禁しない。
`G0-9D`はWindows・追加hardwareを含むDistribution Ready用の`WAIT / HARDWARE`として残る。
G0-6Hと製品粒の個別依存は引き続きM3仕様とimplementation ledgerを正とする。
...
| W0g platform gate段階化 | 固定Mac prerequisite evidenceとDistribution Readyの証跡が仕様上分かれる | G0-9L `DONE`、G0-9D `WAIT / HARDWARE`、M3 spec/ledger | 段階化済み。G0-9L単独では製品粒を解禁せず、G0-9DまでDistribution Readyを名乗らない |
...
W0g G0-9L DONE -> fixed-Mac prerequisite evidence
                         G0-9D WAIT / HARDWARE -> Distribution Ready
```

これは能力ごとの全タスクを強制的に直列化する図ではない。W4のうち実素材に依存しない選択・診断・keymap等は
W3と並行できる。Selected U seriesの一時点`DO`一粒はPRODUCT-ASSET lane内だけに残し、M4-K0、
M5-P0I、M2 narrow repair、Vism specを同じ待ち列へ入れない。M3内のRectangle D2やU2h等は、
現在のPRODUCT-ASSET粒を追い越さず、縦slice方針とledgerがrollingに解禁した順だけを使う。

### 4.1 2026-07-25 current Wave 0 overlay

本書のW0〜W6は利用者成果の地理であり、全phaseの発注列ではない。現在の同時着手可能面は次のように重ねる。
...
| M2-REPAIR | `GAP-23` | 背景の基盤修復。完了後に同laneの`GAP-24`を再判定 | M3成果地図を再直列化しない |
| ORACLE-GUARD | `GAP-25` | gate自己保護。GAP-23とのpath非重複確認後だけ起動 | 重複時はこのlaneだけGAP-23後へ移す |

変更許可file、STOP、Human Response Frontier、`WAIT`候補は
[並列レーン着手地図](2026-07-25-parallel-lane-readiness-map.md)を正とする。
...
| playhead | runtime ownerはHost coordinator。freshなHost coordinatorでprojectを再openした時は以前の値を復元せず、projectから決定的に定まる安全な初期位置へ戻す。具体値・五層owner・永続形式は未決（U3a-2Q-P2） |
...
1. G0-9段階化仕様決定とfixed-Mac platform prerequisite evidence gate合格（`G0-9L`完了済み）
2. G0-6Hと製品token導入
...
W0a〜W6の項目別分解後に行ったClaude Fableのread-only大局レビューは、2026-07-22時点の
粒度化証拠として完了済みである。Fableは実装担当や仕様authorityではなく、地図全体の欠落・循環・
過剰scopeを探した反対側助言者だった。

レビューでは少なくとも次を問い、粒度化時点の欠落を確認した。
...
レビューだけで既存STOP、公開契約、タスク順を解除しない。2026-07-25以後の通常発注は
[Opus 5 / Spark / Grok監督ループ](2026-07-25-opus-spark-grok-supervision-loop-decision.md)を使い、
Fableを全leafの必須gateにしない。大地図、共有公開境界、恒久契約等でCodexが必要と判断した場合だけ
ループ外からread-only相談する。
...
| G0-9段階化 | 完了 | G0-9Lは固定Mac prerequisite evidenceとして完了。製品粒の単独解禁には使わない |
| Distribution Ready matrix | `WAIT / HARDWARE` | G0-9DがWindows・追加hardware・AX等を引き継ぐ |
| Wave 0並列レーン | 確定commit待ち | lane地図、ledger、runner修復を同じ基線へ確定後、各laneを独立worktreeで起動する |
...
| 実装発注 | 明示依頼待ち | 本書だけでは起動しない。ユーザーが「発注」を依頼動詞として明示した粒だけ現行監督ループへ入れる |
