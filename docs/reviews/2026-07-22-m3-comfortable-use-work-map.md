...
状態: **粒度化・Fable全粒レビュー完了 / 発注書化は粒度化文書の停止線に従う**
...
- `G0-9L`候補: 主開発Mac上の製品統合を解禁するlocal platform gate
...
| W0g platform gate段階化 | Local AlphaとDistribution Readyの解禁条件が仕様上分かれる | G0-9、platform分類、M3 spec/ledger | local product integrationとdistributionの審判が別ID・別証跡で閉じる |
| W0b 製品window統合 | 通常起動で正しい製品UIが現れる | W0a、local platform gate、G0-6H、U0e-3、H1b | Browser、Inspector、KEYS/LAYERS、native Stage/Timelineが正しいownerから表示される |
...
```text
W0a 製品資産所有 ─────────────────────────────┐
W0g G0-9段階化仕様改訂 -> local platform gate ─┤
                                                v
                                      G0-6H -> U0e-3
                                                v
                                      W0b 製品window統合
...
- 現行G0-9の合格条件をlocal product integrationとdistributionへ仕様上分離する
...
- M3仕様、implementation ledger、UI runtime責任境界、React移管契約の解禁文言を同じ改訂で同期する
- parent G0-9の「全platform審判完了」と各段階の「限定解禁」を混同しない

この仕様改訂が反対側レビューを通るまでW0b以降を実装しない。
...
完走できることとする。製品入口のtask ID、Unsaved Changes、Save As等の未決UXは粒度化中に発明せず、仕様判断粒へ戻す。
...
次段階ではW0〜W6を一度に実装単位へ落とさない。各地点を次の順で分解する。
...
1. G0-9段階化仕様改訂とlocal platform gate合格
2. G0-6Hと製品token導入
...
| 対象 | 状態 | 次の行動 |
|---|---|---|
| 本ワークマップ | 初回レビュー反映済み / 粒度化中 | W0aから順にユーザー成果、依存、負例、審判へ分解する |
...
| Claude Fable全粒レビュー | WAIT | 項目別分解後にread-onlyで実行する |
