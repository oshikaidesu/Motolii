...
| 処分済み（Unit 1 + 2A + 2B + 2C + 3A + 3B-UI + 3B-runtime-A/B1/B2-A + 3C + 4A + 4B + 4C-1/2/3 + 4D + 4E + 4F + 4G + 4H + 4I + 4J + 4K + 4L + 4M + 4N + 4O + 4P + 4Q + 5A + 5B + 5C + 5D + 5E + 5F + 5G + 5H + 5I + 5J + 5K + 5L + 5M + 8A + 9A + 9B + 9C + 9D + 9E） | 425 |
| 未処分 | 1,372 |
...
| 5F | メディア可搬性／GPUベンダ差の再調査計画全1版。未実施仮説を所見化せず、GAP-3／7・K4とINF-3の狭い再入場gateへ正規化 | DISPOSITIONED（1 blob。調査未実施、現行の部分実装と恒久意味を分離） |
| 5G | Vello採否レビュー／spike結果全2版。同一device／texture成立性、長寿命Renderer、単一premul境界を回収し、局所renderer採択と製品未統合を分離 | DISPOSITIONED（2 blobs。K6／P6／U3a-2とCQ-7は未実装） |
| 5H | R9実素材／B-4書き出し受入全4版。M1歴史sign-offを維持し、private素材、旧preview、可変toleranceを現行release proofから分離 | DISPOSITIONED（4 blobs。製品後継はGAP-32） |
| 5I | M0-S2 decode pipeline全6版。`ffmpeg-sidecar`クレート不採用、自前pipe／CFR seekの成立と、VFR／process lifecycle／pool未成立を分離 | DISPOSITIONED（6 blobs。K4／GAP-26へ再入場） |
| 5J | M4 cache／analysis仕様全20版。Host専権、完全key、並行契約、StateTrack採択とsimulation全拒否敗北枝、現行未実装を再締結 | DISPOSITIONED（20 blobs。K0〜K8は未実装） |
| 5K | performance model全21版。帯域／VRAM規律、liveness-aware target poolを回収し、fp16／fusion／40-layer／DRSの実装境界を分離 | DISPOSITIONED（21 blobs。dynamic pool実装済み、最適化／製品性能未成立） |
| 5L | memory model全6版。VRAM／RAM／disk責任、hard budget、capacity／deadline分離を維持し、固定2枚／400MB／K7／K8の実装境界を分離 | DISPOSITIONED（6 blobs。K1／K7／K8未実装） |
| 5M | R3/DataTrack統合review全3版。未知param、export長、fallback、VideoSource、testkit集約の当時完了を現行意味へ再判定 | DISPOSITIONED（3 blobs。半開総尺へ置換、helper再発を分離） |
| 5N〜 | render、GPU、media、cache、analysis、color、export残lineage | 未着手 |
...
| 8A | M5 object／camera／depthの5版から、Host semantic seat、換装可能Provider、presentation分離を一般則へ回収 | DISPOSITIONED（5 blobs。Unit 8残余は未着手） |
