...
| 4 | Text addressable spans | M5 P6 cluster mappingの審判 | first-party plugin要件候補 |
| 5 | Anchor/Align/Distribute | M3 selection/bounds契約 | UI要件候補 |
| 6 | Search/Filter/Selection | Workspace-session所有決定 | UI要件候補 |
| 7 | Import/animated bounds | importとderived cacheの所有決定 | 監査待ち |
| 8 | Path batch operations | shape/path選択モデル | 監査待ち |
