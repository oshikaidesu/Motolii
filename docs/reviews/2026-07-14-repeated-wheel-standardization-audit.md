...
Adjustment Layer相当は、`Group Composite`と`Composition Output`で通常用途を覆い、AE式の「連続stackへ掛ける」はAdvanced `Stack Range`としてのみ残す。画面全体への作用はComposition Outputを明示選択した時だけ既定にする。
