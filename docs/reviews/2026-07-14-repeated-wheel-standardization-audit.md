...
|---|---|---|
| **3D** | 共通world、Transform、単一active cameraの席、Observation配布、depth policy、bounds／picking参加scope、object/world表現、GPU pass | Camera Provider、mesh generator、material、3D effect、importer |
...
特に3Dはplugin領域へ閉じられない。RGBA化後のEffectとして3Dを実装すると、layer間交差、depth、camera、shadow、Backdropとの処理段を正しく共有できない。M5の共有world／Observation／depth／bounds参加境界がHost根本で、pluginはその境界へCamera Provider、object、material、generatorを供給する側である。具体camera modelまでHostへ固定するという意味ではなく、観測所有の改訂正本は[Camera Object / Provider決定](2026-07-24-camera-object-provider-decision.md)とする。
