...
1. 網(mesh)か点群の層を選び、Effects 棚の **Turbulent Displace** を載せる
...
- doc: 既定と選択肢
- render(実 GPU): Space の変位で絵が変わり、Evolution でまた変わり、Normal の変位で陰影が変わる。点群は Amount を超えて動かず、Evolution で動く
- 棚: 並んでいて pass にならず、既定 Amount 50

## 宿題

- GPU と CPU の場が数値で一致することの検証(同じ写しだが、GPU 読み戻しの test は未作成)
- 2D 層のサンプル位置への適用(裁定 2026-09-02 の 3 つ目)
