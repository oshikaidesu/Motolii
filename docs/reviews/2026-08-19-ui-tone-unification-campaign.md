...
## 追記 — UIスケール%との接続(2026-08-19 夜)

利用者: Ableton で好みなのは**設定で全体UIの大きさを%で細かく変えられる**こと。
「トンマナ的な変数が随時にあるのではないか」— この仮説は正しい(Live の Zoom Display は
内部寸法が基準変数から導出されているから成立する。Blender も widget_unit=20px の1変数型)。

含意:
- ui-visual-language「UIスケール」節(2026-07-14決定)が既にこれを**機構要件**として持つ。
  実装だけが未着手
- **第3波の定数化がそのままスケール%の土台**: scale 集合と dims が単一 token 系に集約され、
  oracle がベタ書き混入を落とす状態になれば、UIスケールは「基準に係数を掛ける」薄い実装になる。
  iced にはアプリ全体の scale_factor フックが既製(fork でのAPI形は第4波で実機確認)
- 順序が本質: **第3波(定数化)→ 第4波(UIスケール%)**。逆順だとベタ書き部分だけ
  拡大されない「歯抜けズーム」になる
