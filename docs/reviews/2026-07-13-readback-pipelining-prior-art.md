...
- **Motoliiの現物との接続(接合条件あり)**: 現行[NodeDesc](https://github.com/oshikaidesu/Motolii/blob/fb8818db514c8de028e757109f5751870a1f3447/crates/motolii-plugin/src/lib.rs)にshader/pipeline宣言は無い。Filter系は[ホスト所有PipelineCache](https://github.com/oshikaidesu/Motolii/blob/fb8818db514c8de028e757109f5751870a1f3447/crates/motolii-gpu/src/pipeline_cache.rs)へ`PipelineCacheKey{id, wgsl}`で要求する(F-10実証済み)が、**捕捉は全面ではない** — [motolii-nodes](https://github.com/oshikaidesu/Motolii/blob/fb8818db514c8de028e757109f5751870a1f3447/crates/motolii-nodes/src/lib.rs)のOverlay/Composite系はPipelineCacheを介さず直接`create_render_pipeline`している(2026-07-13確認: 直接生成2箇所)。また現行キーは固定レイアウト1形式(texture+sampler+uniform4)の識別子であり、Fossilizeが記録するlayout・render pass等を含む**完全なpipeline記述ではない**([Fossilize公式README](https://github.com/ValveSoftware/Fossilize)は`CreateInfo`一式を記録する)。したがってキー列の記録だけではFossilize型replayにならない
