...
## 面白くなかった記録(2026-09-14)

| 作った物 | 利用者 | 読み |
|---|---|---|
| 3D スイス・ポスター(`swiss3d.js`) | 「あまり面白い画とは言えない、もう Web で何度も行っていることだからかな」 | Web の絵の描き直し |
| 渋谷の俯瞰 × Exclusions の格子(`shibuya.js`) | 「フレームごとに動いて連続性の意味が無い」「このような画はプログラムでいつでも出せる」「大事なのは箱でありグリッドではない」 | 解き方の絵で、時間の連続(同一性)が無い → [間合いの法](2026-09-14-margin-law.md) へ |
| 渋谷の俯瞰 × 間合いで譲り合うラベル(`margins.js`) | 「やはりこれもあまり面白くないなぁ、ドッグフーディングを再度確認する」 | 仕組みの見本で、作品の意図が無い |
