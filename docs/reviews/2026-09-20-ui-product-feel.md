...
### 案 — 色は「数の意味」に専有させ、層の識別は色以外へ逃がす

- **6 色は家族に固定**(spatial / amount / time / count / seed / angle)。
  glyph・track・handle のどれでも同じ意味。これは OP-1 の規則そのもので、既にそう書いてある。
- **層の識別は色をやめる**。任意の `id % 7` に意味は無く、色という希少資源を食っている。
  代わりに **種類の記号(形)** と **名前**で区別する。同じ種類が並ぶ時は位置と順で足りる。
- **種類の色は要るか** — `kindColor` は棚(browser)で効いている。
  種類は数える程度(File・Shape・Text・Group・Camera・Stage・Null・Particles)なので、
  **形の記号を主、色を従**にする。色を使うなら家族とは別の軸(彩度を落とした 1 段)にして、
  「強い色 = 数の意味」を守る。
...
1. **色の衝突を外す**(1 レーン)。`layerColor` の 6 箇所と `kindColor` の 3 箇所。
   家族の 6 色を専有させ、層の識別を形と名前へ逃がす。**最初にここ** — 土台なので。
