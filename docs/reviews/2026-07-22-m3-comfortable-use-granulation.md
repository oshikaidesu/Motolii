...
| CU-0G04 | `MEASURE / DO` | 100回resize/DPI/capture/lostとWebView crash復旧 | CU-G01、CU-0G03 | Document/selection不変、stale epoch拒否、resource leakなし | synthetic resizeだけで実機合格にしたくなる |
...
- CU-0G03はユーザーによる実windowの実IME／VoiceOver／復帰審判で合格した。直近の実行候補は現行ledgerどおり`CU-0G04`だけであり、責任最小化短票を通してから100回lifecycle／failure観測へ進む
