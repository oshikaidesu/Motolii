...
| CU-0G02A | `SPEC / DONE` | L1の候補stack対baseline二armとGPU rawを正本化 | CU-0G02、renderer再選定 | pure direct第三armを作らず、GPU timestamp/RSS/unified-memory非推測、同一session再実測を[修正決定](2026-07-24-m3-g0-9l-l1-measurement-amendment.md)へ固定。既存rawをL1 PASSへ昇格せず、Grok R2 P0/P1/P2=0でACCEPT | 既存DONEへ合わせてGPU rawを免除、または第三glyph rendererが必要 |
| CU-0G02BH | `CORE / DONE / FROZEN` | CU-0G02Bを固定commitで実行するbounded GPU timestamp harness | CU-0G02A | wgpu標準queryだけを`REUSE / WRAP`し、同一pass境界、typed provenance、測定後一括resolve/map、query上限・一意slotを固定。frame loopのwait/map/resolveとpixel readbackは0。35 tests、両arm各5回×500 frameと修正後各500 frameの実window診断、Grok R2 P0/P1=0でACCEPT。製品へimportしない | Metal固有counter、profiler framework、製品telemetry、期待値緩和が必要 |
| CU-0G02B | `MEASURE / DONE / FROZEN` | 既存二armへGPU timestamp rawを追補 | CU-0G02BH | 固定commit `7c3a590e`の同一instrumentation・session・digestで両armを各30秒再実行し、CPU/GPU/RSS/resource/readback/toolchain/commitを[typed raw](../spikes/g0-9-windowed-timeline.md#cu-0g02b-gpu-timestamp-raw-comparison2026-07-24)へ保存。両arm complete、pixel readback 0、query result readback 1、測定中resource生成0、Grok P0/P1=0でACCEPT。絶対閾値・勝者なし | profiler framework、新renderer、製品telemetry、frame-loop同期waitが必要 |
...
| CU-0G04 | `MEASURE / DONE` | 100回resize/DPI/capture/lostとWebView crash復旧 | CU-G01、CU-0G03 | 同じ実windowで102 resize、実DPI event、合成capture、注入Lost再present、実WebContent終了とwry callback/backoff/reload/再Ready、offline既定拒否、stale layout/WebView epoch拒否を[単一manifest](../spikes/g0-9-surface-host-evidence/report.json)へ保存。sentinel/resource/semantic write不変 | synthetic resizeだけで実機合格にしたくなる |
| CU-0G04B | `MEASURE / DONE / FROZEN` | L3同一実window閉包の不足観測だけを追補 | CU-0G04 | `REUSE / WRAP`。既存surface-hostの一続きの実window実行でminimize/restore、fullscreen往復、1×1一時不可用、native/WebView境界pointer capture/cancel/focus lossを、既存resize/stale/Lost/WebContent/offlineと同じmanifestへ保存。各操作後のlayout/bounds、sentinel、semantic write 0、readback 0、stale拒否後不変を検査。33 tests、実window exit 0、workspace/docs全緑、Grok R2 P0/P1=0でACCEPTし、追補を`FROZEN / DELETE-LATER`へ固定 | CU-0G03人間証跡またはCU-0G04凍結manifestを機械L3へ読み替える、新runner／監視framework／製品APIが必要 |
| CU-0G05L | `SPEC / DONE / FROZEN` | local gate判定とfixed-Mac platform prerequisite evidenceの限定確定を記録 | CU-0G02B、CU-0G03〜04B | [単一manifest](../spikes/g0-9-local-platform-evidence/manifest.json)でL1〜L3の8 retained path SHA、実行commit、toolchain、固定構成、未合格platformを束ね、Grok R2 P0/P1/P2=0でACCEPT。G0-9Lは固定Mac証拠だけPASS。W0b、H1b、Motolii Studio Preview、window結合を解禁しない | Windows/追加hardwareまで合格、または製品粒を解禁と書きたくなる |
...
| CU-205B | `PRODUCT-ASSET / SPLIT` | first-party Opacityを既存Effects Browserへtyped投影 | CU-202、CU-G09、CU-0B02C-P | B1G provenance guard→B1I source consumer→B2 shipped Host/bundle。既存PluginCard、exact `core.filter.opacity`、single click write 0、二重copy 0 | static card/label/thumbnailからidentity推測 |
| CU-205B1 | `PRODUCT-ASSET / SPLIT` | Web sourceのstrict catalog consumer | CU-205S | B1G provenance guard→B1I source/provenance。初回Spark/Grok REJECT差分は未採用 | source hash期待値を書換え、fixtureだけで親B DONE |
| CU-205B1G | `ORACLE-GUARD / DO` | Browser post-promotion共有hash-chain guard | CU-205S | fixed index 0、exact key、chain、last=liveを維持。Browser decoder guardへ共有し、Inspector guardのmanifest全体hash固定だけを外す。product code/provenance 0 | current hashだけ受理、空chain、guard重複 |
| CU-205B1I | `PRODUCT-ASSET / WAIT` | Web source strict catalog consumer + provenance append | CU-205B1G | codec→existing decoder→main→PluginCard、mock 3 card不変、Rust/bundle/intent 0 | fallback/default/Context、provenance欠落 |
| CU-205B2 | `PRODUCT-ASSET / WAIT` | Rust Host snapshotとshipped bundle接続 | CU-205B1I | first_party_catalog由来、正規bundle/manifest/include、通常製品表示 | contract複製、source意味変更、generatedだけ手修正 |
| CU-205T | `PRODUCT / WAIT` | Opacity double click/Enter→primaryへ新Definition+Use | CU-205B2、VS-1 D2 | `prepare_create_effect`、明示default、1 attach=1 Undo、primaryなしwrite 0 | LinkEffectUse共有、bare item ID、Stage全体をtarget化 |
