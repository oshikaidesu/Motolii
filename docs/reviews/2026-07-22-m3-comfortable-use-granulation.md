...
| CU-106 | `CORE / READY-RECHECK` | U2h selection kernelとessential focusをprimary/focusへ分割し、最小production callerを再確認 | CU-104 + CU-105Rのselection handoff | primary selectionはU2h-1P P5と実在callerを同じ差分で成立。focus/三surface/hidden件数は別粒 | surface別selection store、lint抑制、dummy callerが必要 |
