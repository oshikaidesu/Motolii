...
|---|---|
| 種類 / 状態 | `SPEC / DECIDE` |
| 一成果 | local product integration gateとdistribution gateを別ID・別証跡にする |
...
| 正例 | local gate合格で検証済みMac構成だけW0bを解禁し、parent G0-9とdistribution gateは未完了のまま残る |
...
## 6. W0g local platform gate

| ID | 種類 / 状態 | 一成果 | 依存 | 合格と必須負例 | STOP |
|---|---|---|---|---|---|
| CU-0G02 | `MEASURE / WAIT` | direct wgpu(+Vello局所)対eguiを同条件比較 | CU-G01、既存G0-9 fixtures | raw frame/input/memory結果を保存、絶対閾値を後付けしない | fixture・device・window条件が非同一 |
| CU-0G03 | `HUMAN / WAIT` | macOS IME/VoiceOver/focus/keyboard受入 | CU-G01 | preedit、Enter非奪取、bounded AX、focus往復。失敗を既知制約扱いしない | 実製品surfaceで試せない |
| CU-0G04 | `MEASURE / WAIT` | 100回resize/DPI/capture/lostとWebView crash復旧 | CU-G01 | Document/selection不変、stale epoch拒否、resource leakなし | synthetic resizeだけで実機合格にしたくなる |
| CU-0G05L | `SPEC / WAIT` | local gate判定と限定解禁を記録 | CU-0G02〜04 | P0/P1=0、対象Mac構成と未合格platformを明記 | Windows/追加hardwareまで合格と書きたくなる |

## 7. W0b 製品window統合

| ID | 種類 / 状態 | 一成果 | 依存 | 合格と必須負例 | STOP |
|---|---|---|---|---|---|
| CU-0B01 | `HUMAN / DO` | G0-6Hで5画面の階層・識別・馴染みを判定 | CU-0A02、CU-0G05L（local gate） | `docs/mocks-ui/reference-handoff.md`へ判定者/条件/採否理由を保存。agentが目視代行しない | reference画面欠落 |
...
| CU-0B03 | `PRODUCT / WAIT` | H1b codec/offline bundle/mountをHostへ接続 | CU-0A09、CU-0G05L（local gate） | CDN/HMR/fixture script依存0、invalid/stale codec拒否 | WebView transportを公開plugin APIへ一般化したくなる |
| CU-0B04N | `PRODUCT / WAIT` | native Stage/Timeline viewportを1 top-level Surfaceへ接続 | CU-0B02、CU-0G05L（local gate） | 同一device/queue、CPU readback 0、viewport resizeでsemantic不変 | native viewportごとに別正本Surfaceが必要 |
| CU-0B04R | `PRODUCT / WAIT` | opaque child WebView islandsをHost layoutへ接続 | CU-0B03、CU-0B04N | transparent overlay 0、focus/geometry epoch、React semantic state 0 | DOM/pxをnative/Document identityへ使いたくなる |
...
| CU-105 | `CORE / WAIT` | U3a layout/hit-test/dense Timeline projectionを閉じる | CU-0G05L（local gate）、既存U3a | 1000 clip/100k key、zoom境界でselection/playhead/range不変 | React TimelineまたはDOM identityが必要 |
...
CU-G01 G0-9段階化 ─> CU-G02 order ───────────────┐
        |                                         |
        +─> CU-0G02..05L local gate ──┬─> CU-105 U3a ─────────┤
...
- 直近の実行候補は現行ledgerどおり`CU-0A01 = U0e-2R`だけであり、本書の追加は発注再開ではない
