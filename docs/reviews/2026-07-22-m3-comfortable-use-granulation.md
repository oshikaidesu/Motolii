...
| CU-0G02B | `MEASURE / DO` | 既存二armへGPU timestamp rawを追補 | CU-0G02BH | CU-0G02BH固定commitの同一instrumentation・同一session・同一digestで両armを再実行し、CPU/GPU/RSS/resource/readback/toolchain/commitをtyped rawへ保存。絶対閾値・勝者なし | profiler framework、新renderer、製品telemetry、frame-loop同期waitが必要 |
...
| CU-0G05L | `SPEC / WAIT` | local gate判定とfixed-Mac platform prerequisite evidenceの限定確定を記録 | CU-0G02B、CU-0G03〜04B | P0/P1=0、対象Mac構成と未合格platformを明記。W0b、H1b、Motolii Studio Preview、window結合を解禁しない | Windows/追加hardwareまで合格、または製品粒を解禁と書きたくなる |
...
- CU-0G03はユーザーによる実windowの実IME／VoiceOver／復帰審判で合格し、CU-0G04Bまで責任最小化短票どおり既存winit/wgpu/wry/macOS標準経路を使う単一runのmanifestでL3を閉じた。CU-0G05L reviewでL1正本とGPU rawの不足を検出し、`CU-0G02A`で正本を修正した。`CU-0G02BH`はwgpu標準queryだけのbounded harnessを固定し、Grok R2 P0/P1=0でACCEPTした。直近はその実行commitを使う`CU-0G02B`同一session再実測とする。CU-0G05LはWAITのまま、固定Mac prerequisite evidenceの限定確定以外を解禁しない
