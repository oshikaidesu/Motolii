...
| CU-0A03 | `SPEC / DO` | R0 source inventory/provenance manifest | CU-0A01、固定SHA`56c318ed` | path/export/CSS/model/test closure全列挙、legacyとnative面を誤分類しない | 固定sourceまたはclosure不在 |
...
- U0e-2R/U0e-2は完了済みで再実行しない。現在の製品orderは`CU-0A03 / R0 source inventory`だけである
