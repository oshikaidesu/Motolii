...
| CU-103 | `CORE / DO` | `VectorRecipe::StandardShape::Rect`をD3/GPUへlower | M2 Vector意味、CU-101、[M2 D3 CU-103追補](../specs/M2-document-model.md) | 不透明白fill・strokeなしのRectだけを既存OverlayRectへlowerし、preview/export pixel同一、fixture rectで代用しない。非Rect Vectorはtyped reject | schema/style/public API追加、既存golden期待値変更、非Rectの黙示loweringが必要 |
