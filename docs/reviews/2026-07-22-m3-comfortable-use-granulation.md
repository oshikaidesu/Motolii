...
| CU-0A05B | `ASSET / READY-RECHECK` | R2B 同形React triggerをproduct ownerへ直接移管 | CU-0A05A | mock側copy 0、popup/modelはmockのnative oracle、Reactにcurve/Undo state 0 | popup全体またはHost projectionを同時に持込む必要がある |
...
- U0e-2R/U0e-2、CU-0A03/R0、CU-0A04/R1、CU-0A05A/R2Aは完了済みで再実行しない。唯一の現在の製品粒は`CU-0A05B / R2B product ownership`の`READY-RECHECK`であり、R2全体とMotolii Studio Previewは未実装である
