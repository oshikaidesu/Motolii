...
| CU-0G02B | `MEASURE / DO` | 既存二armへGPU timestamp rawを追補 | CU-0G02A | `REUSE / WRAP`。同一instrumentation・同一session・同一digestで両armを再実行し、CPU/GPU/RSS/resource/readback/toolchain/commitをtyped rawへ保存。絶対閾値・勝者なし | profiler framework、新renderer、製品telemetry、frame-loop同期waitが必要 |
...
- CU-0G03はユーザーによる実windowの実IME／VoiceOver／復帰審判で合格し、CU-0G04Bまで責任最小化短票どおり既存winit/wgpu/wry/macOS標準経路を使う単一runのmanifestでL3を閉じた。CU-0G05L reviewでL1正本とGPU rawの不足を検出したため、直近は`CU-0G02A`正本修正、次に`CU-0G02B`同一session再実測とする。CU-0G05LはWAITのまま、固定Mac prerequisite evidenceの限定確定以外を解禁しない
