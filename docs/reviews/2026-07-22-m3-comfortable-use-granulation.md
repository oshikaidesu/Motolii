...
| CU-0A07A | `ORACLE / DO` | 未変更legacy Inspectorへ構造・style・ARIA・操作oracleを固定 | CU-0A07S | 5状態と主要操作を再現し、source/threshold/golden変更0 | archived HTMLやvisual基準の変更が必要 |
| CU-0A07B | `ASSET / WAIT` | 固定mock内でInspectorを同形React化しlegacy adapterを封じる | CU-0A07A | exact-count containment、双方向store 0、skeleton代用0 | handlerを一方向adapterへ封じられない |
| CU-0A07C | `ASSET / WAIT` | R4B source/CSSをproduct ownerへbyte同一移管 | CU-0A07B | mock copy 0、legacy runtime import 0、state owner維持 | source closureや公開境界の変更が必要 |
...
- U0e-2R/U0e-2、CU-0A03/R0、CU-0A04/R1、CU-0A05A/R2A、CU-0A05B/R2B、CU-0A06A/R3A、CU-0A06B/R3B、CU-0A07S/R4Sは完了済みで再実行しない。現在粒は`CU-0A07A / R4A Inspector oracle`の`DO`で、R4B/R4C以降とMotolii Studio Previewは未実装である
