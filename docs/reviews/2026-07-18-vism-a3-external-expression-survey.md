...
状態: **調査完了／A3候補の推薦まで。採用は[A3D](2026-07-18-vism-a3d-radial-repeater-decision.md)で処分済み**。本書自体は設計採用・公開API変更・実装許可ではない。[レビュー文書の規律](README.md)に従い、外部製品の機能をMotoliiへそのまま移植せず、事実と転移仮説を分ける。
