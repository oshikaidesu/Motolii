...
利用者裁定: 窓が「ミチミチ」で、行 20 は詰まりすぎ。根拠は外部から取る。

| 値 | 根拠 | 確認 |
|---|---|---|
| **row 24 px**(行・欄・帯の基準) | WCAG 2.2 SC 2.5.8 — "The size of the target for pointer inputs is at least 24 by 24 CSS pixels". 例外の「間隔」(24 px の円が隣の標的と交わらない)は、行が隣と接する密な窓では成り立たない。よって行そのものを 24 にする | 2026-09-20 に原文を取得(4-4) |
| **control 28 px**(単独の押す部品) | Apple HIG Accessibility の表 — macOS の当たり判定は既定 28×28 pt、最小 20×20 pt | 台帳 4-1 の記録どおり。2026-09-20 は JS 描画で再取得できず、**未再確認** |
| 旧 row 20 | Apple の最小 20 pt には合うが、WCAG の 24 を割る。R6(当たり 24)と R10(row 20)が矛盾していた | — |

`section` 30 / `bar` 32 / `tall` 34 は、row 24 と control 28 の上に 1 段ずつ積んだ値で、独立の出典は無い(観察: 見た目の判定は利用者)。
