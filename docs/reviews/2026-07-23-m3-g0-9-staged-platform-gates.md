...
判定手順として、反対側レビューでP0/P1=0になった後にだけ`G0-9L: PASS`と
固定Macのplatform prerequisite evidence限定確定を別decisionへ記録する。

次の個別証拠をCU-0G05Lの[単一manifest](../spikes/g0-9-local-platform-evidence/manifest.json)へ
束ね、Grok R2 P0/P1/P2=0後に固定Macのplatform prerequisite evidenceだけを
`G0-9L: PASS`として限定確定した。
...
この採択とCU-0G05Lで確定したのはgate構造と固定Mac証拠の効力だけである。
parent G0-9、G0-9D、製品粒、Windows/追加hardwareは未合格のまま残る。
