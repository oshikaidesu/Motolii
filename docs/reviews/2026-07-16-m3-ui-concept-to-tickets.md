...
| U0d-1 | G0-2の閉じたGesture語彙でbuiltin baseへuser deltaを重ねる純粋keymap resolver | U0c-2 | Add、base exact GestureのReplace/Disable、複数割当。Primaryのplatform展開とKeyToggleのPress/Release展開後のEffectiveTrigger競合、Primary混在、未登録ID、OS予約を型付き診断し該当triggerを実行mapへ載せない | JSON/serde、原本保全、ファイルI/O、Context、設定画面を含めない |
