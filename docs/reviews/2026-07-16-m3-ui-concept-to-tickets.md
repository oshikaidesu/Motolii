...
main済み U0a
  ...
  → U1a-1 shell/static viewport → U1a-1b 色/alpha/寿命負例 → U1a-2 layout projection
...
`U2a-1`までを閉じてから製品shellの`U1a-1`へ入る。PR #184と旧`cursor/m3-u0b-1-night` /
...
| U0e-2 | 5 reference screenのtoolkit非依存な単一fixture manifestと同条件render手順 | U0e-1 | React/eguiが同じfixture identity・stable component ID・state・locale・viewportを読み、normal/lightness/grayscale/CVDを生成。manifestの保存形式を本枝番で確定する。test-only fontを固定し、有界なlayout安定化前のframeをsnapshotに採らない | DOM/CSS/snapshotをfixture正本にせず、見た目の良否をpixel testだけで決めない。manifestへU0b-1の所有層を与えない（製品状態ではなく試験資産） |
...
| ID | 1チケットの成果物 | 依存 | 自動審判 | STOP / 非目標 |
|---|---|---|---|---|
| U1a-1 | egui shell、既存device共有、静止viewport native texture、組み込み既定layout preset | U0a, U0b-2, G0-1, D3 | 同じDocument frameをCPU copyなしで表示。native texture登録はpool生成時一回で、UI threadから`render_frame`を直接呼べない | layout永続化、再生、連続seek、製品CPU readback、UI用別render経路を入れない。色/alpha/寿命の負例はU1a-1bへ送り、ここで先取りしない |
| U1a-1b | U1a-1の表示境界に対する色・alpha・寿命の負例一式 | U1a-1 | opaque/半透明edge/color ramp fixtureでUI側の追加色変換・二重premultiply・channel swapを拒否。sampling中slot再利用なし、解除はpool retirement時だけ、pool extent不変のresizeで再登録なし、extent変更はretirement→新pool生成のみ。resize/minimize/restore反復で旧extent表示とresource単調増加なし | 計器を公開API/Documentへ出さずtest-only accessorに限る。製品CPU readbackとGPU work強制cancelを要件にしない |
| U1a-2 | toolkit非依存panel layout intentとegui_tiles runtime投影 | U1a-1b, U0b-2 | split/tab/resize/hide/restore/resetを操作でき、Motolii所有のstable panel catalog IDから決定的runtime treeを作る。表示名・locale変更後もactive tab/split identity不変。操作でDocument serialize不変 | `egui_tiles::Tree`/`TileId`/serde形、表示名、翻訳文、挿入順をidentityまたは保存正本にしない。panel固有domain intentを作らない |
...
| U1b-2 | 完了順反転fixtureと古いresult破棄のE2E | U1b-1 | 最新の完全なrequest generationだけをevent-loopへ投影。resize/pool交代を挟んだ逆順完了も旧extent・旧pool generationとして破棄 | GPU work強制cancelや、別generationのsize/quality/document条件の混成を要件にしない |
...
3. `U0b-2`後も初回Uシリーズは並走させず、`U0c-1`→`U0c-2`→`U0d-1`→`U0d-2`→`U0d-3`→`U2a-1`の順に意味入口を閉じる
4. `U2a-1`完了後の最新mainから`U1a-1`を起票し、静止viewport以外を混ぜない
5. `U1a-1`→`U1a-1b`→`U1a-2`→`U1b-1/2`→`U2b-1`→`U2c-*`を直列に進める
