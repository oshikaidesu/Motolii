...
| U0e-2 | 5 reference screenのtoolkit非依存な単一fixture manifestと同条件render手順 | U0e-1 | React/eguiが同じfixture identity・stable component ID・state・locale・viewportを読み、normal/lightness/grayscale/CVDを生成。test-only fontを固定し、有界なlayout安定化前のframeをsnapshotに採らない | DOM/CSS/snapshotをfixture正本にせず、見た目の良否をpixel testだけで決めない |
...
| ID | 1チケットの成果物 | 依存 | 自動審判 | STOP / 非目標 |
|---|---|---|---|---|
| U1a-1 | egui shell、既存device共有、静止viewport native texture、組み込み既定layout preset | U0a, U0b-2, G0-1, D3 | 同じDocument frameをCPU copyなしで表示。opaque/半透明edge/color rampでUI側の追加色変換・二重alpha変換を拒否。登録はpool生成時一回、sampling中slot再利用なし、retirement時だけ解除し、resize/minimize/restore反復で旧extent表示とresource単調増加なし。UI threadから`render_frame`を直接呼べない | layout永続化、再生、連続seek、製品CPU readback、UI用別render経路を入れない |
| U1a-2 | toolkit非依存panel layout intentとegui_tiles runtime投影 | U1a-1, U0b-2 | split/tab/resize/hide/restore/resetを操作でき、Motolii所有のstable panel catalog IDから決定的runtime treeを作る。表示名・locale変更後もactive tab/split identity不変。操作でDocument serialize不変 | `egui_tiles::Tree`/`TileId`/serde形、表示名、翻訳文、挿入順をidentityまたは保存正本にしない。panel固有domain intentを作らない |
