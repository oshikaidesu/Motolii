...
main済み U0a
  ...
  → U3a-1I headless timeline foundation `DONE`（論理依存はU0a+U0bのみ。docs-only `CU-104`も[selection publish envelope決定](../reviews/2026-07-27-cu-104-selection-publish-envelope-decision.md)を`DONE`） → U4a-1 parameter mapping `DO` → U4a-2 generated panel `WAIT`
...
|---|---|---|---|---|
| U0e-1 | **完了**: [決定済みDTCG token schema](2026-07-21-m3-u0e-1-token-generator-contract.md)と決定的Rust/egui adapter generator | U0a。PR #184は証拠・抽出元 | 同じ入力からbyte一致、生成物の手編集差分を拒否 | 色、px、radius等の**製品値を確定しない** |
| U0e-2R | [固定React comparison baseline](2026-07-21-m3-u0e-2-reference-fixture-contract.md)を最新mainへ再結合 | U0e-1 | React build/43 tests、workspace gate、衝突10文書の後発決定保持 | 5画面実装、新しいUI判断、archive逆実装を混ぜない |
| U0e-2 | 5 reference screenの固定fixtureと同条件render手順 | U0e-2R | normal/lightness/grayscale/CVDを同じfixtureから生成 | 見た目の良否をpixel testだけで決めない |
...
5. `U1a-1/2`→`U1b-1/2`→`U2b-1`→`U2c-1`を直列に進める。U2c-1後は、実在する複数製品入口をまだ持たないU2c-2を完了扱いせず、独立した診断modelのU2c-4だけを先行する
6. PR #184から生成機構だけを`U0e-1`へ抽出し、固定React比較baselineを
