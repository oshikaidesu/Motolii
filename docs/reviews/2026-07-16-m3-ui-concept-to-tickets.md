...
日付: 2026-07-16
状態: **段階発注の正本**。意味は[UI操作言語](../ui-interaction-language.md)と[UI視覚言語](../ui-visual-language.md)、境界は[M3 UI境界汚染の予防](2026-07-14-m3-ui-boundary-prevention.md)に従う。2026-07-19のM3入場再翻訳後は、個別依存を満たす枝番だけを発注できる
...
U0a入場移行は完了した。次の順序を個別依存とSTOP条件を保ったまま適用する。
