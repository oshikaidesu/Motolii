...
**室は viewport ごとに数える(裁定261、2026-08-27 追記)。** 上の表は「窓の葉」を単数・
共有面1枚として書いてあるが、プレビューカメラ(第三者視点でアクションカメラの画角と
動きを外から見る)とアクションカメラ(実際に切り取ってレンダリングする)は**同じシーンを
同時に生描画する**ので、面は窓の数ではなく viewport の数だけ要る。窓に出すかドックの
セルに出すかはレイアウトの都合であって、面の数え方とは関係がない。
実装は段階が要求するまでやらない — ここで確定しているのは数え方だけ。
先例: After Effects の Active Camera / Custom View、Blender の Viewport / Camera view。
