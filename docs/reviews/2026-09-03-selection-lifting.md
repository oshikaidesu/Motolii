...
- 状態: **縮小採用**。出典は [PR #479](https://github.com/oshikaidesu/Motolii/pull/479)、head `05cdbd6119aab0526066de1efc002364e8979d7b`。以下の原文は `main c2c7a89` を対象にした設計材料として保存する。
- 今回の採否・施工・残余の正本は [PR #479 採用計画](2026-09-04-pr-479-adoption.md)。現行の [Inspector と机](2026-09-02-inspector-and-desk.md)の複数選択契約と [現行憲法](../../motolii/AGENTS.md)を優先する。

| 分類 | 取り込み判断 |
|---|---|
| 現行で先行済み | `Session.editable_selection`、選択全体の Delete/nudge、Inspector 共通 Transform の混在「—」・scrub 相対・入力絶対。原文 §1 の「消費する側が無い」は失効。rename も現行で実装済み。 |
| 今回の実装入力 | 対象解決を共有し、単体の編集を Intent 列へ展開して一度に確定する。Duplicate/Split と層行からの操作は現行の Document・選択・錠・キー所有を通す。面ごとの残余は採用計画に記録する。 |
| 採用する操作契約 | 一手一 Undo、scrub/drag は開始値からの相対・直接入力は絶対、preview と commit は同じ対象・同じ値を使う。既存のキー編集や主選択だけへ跳ぶ操作まで無条件に層操作へ置換しない。 |
| 原文の契約を訂正 | §2 の「apply_all 一回」と「層をまたいでは束ねない」は両立しない。対象ごとの適用可否を先に解決し、採用した Intent 列は既存の一回の atomic apply/Undo に通す。部分拒否の表示や複合操作の可否は各動詞で決める。 |

---

以下は取り込み元の原文。関数名・対象一覧・未実装の表記も当時の文脈に限る。
