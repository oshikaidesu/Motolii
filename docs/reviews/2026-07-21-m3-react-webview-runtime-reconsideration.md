...
ステータス: **責任境界・surface topology・egui製品不採用を決定 / platform受入比較中**。本調査と後続レビューを受け、React chrome + native Stage/Timeline + headless interactionを[UI runtime責任境界](../ui-runtime-architecture.md)へ、通常windowを[1 top-level wgpu Surface + 2 native viewport + opaque child WebView islands](2026-07-21-ui-surface-topology-decision.md)へ正本化した。2026-07-18のegui shell等は比較・診断baselineとして保持するが、2026-07-24に標準製品runtimeの候補から外した。G0-9実機spikeまでは製品統合と既存baseline撤去を停止する。plugin UI公開契約は2026-07-22追補2によりG0-3 / GAP-13へ分離した。Rust/wgpu core、M2 Document、D2 command、単一writer、正準座標、preview/export同一評価、UI toolkit隔離は変更しない。
...
2026-07-24追補: eguiは標準製品runtimeへ採用しない。既存shell、native texture preview、
layout投影、render worker、IME/lifecycle証拠は比較・回帰・診断baselineとして保持するが、新規の製品面を
eguiへ実装しない。G0-9はegui対hybridの採否比較ではなく、決定済みhybrid構成のplatform受入と、
direct wgpu／Vello局所枝を既存egui baselineへ回帰させない審判を行う。baseline撤去は受入と代替診断経路の
成立後に分離し、失敗時にeguiを自動復帰させない。
...
1. 現行egui + native wgpu（比較・回帰baseline。製品候補ではない）
...
- 現行mainのegui shell、native texture preview、layout投影、render worker、依存方向CIは比較・診断基準として保持する。G0-9の撤去条件成立前に削除せず、toolkit固有の製品面を拡張または公開型化しない
