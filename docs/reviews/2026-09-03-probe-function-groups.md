...
- 状態: **観察**。出典は [PR #479](https://github.com/oshikaidesu/Motolii/pull/479)、head `05cdbd6119aab0526066de1efc002364e8979d7b`。以下の原文は `main c2c7a89 + PR #479` の関数配置の記録であり、現行コードの全量台帳ではない。
- 今回の採否・施工・残余の正本は [PR #479 採用計画](2026-09-04-pr-479-adoption.md)。家は [現行憲法](../../motolii/AGENTS.md)の doc/render/ui/vism/tests を維持する。

| 分類 | 取り込み判断 |
|---|---|
| 現行で先行済み | `motolii/probe/src` は現行ではなく `motolii/src/ui`。複製・分割は `timeline_edit.rs`、状態は `session.rs`、意味は `crates/motolii-doc`、描画は `crates/motolii-render`。原文の関数数・行数・移動先から施工を発注しない。 |
| 今回の実装入力 | Duplicate/Split の同族処理、対象解決、操作と確定の分離を、今在る責任の中で共通化する。既存の `kurbo` / `glam` / doc 型を先に探す。 |
| 設計参考 | read / verb / gesture / control / paint / table / view / lens の分類は意味と副作用を読む補助線。8 群の directory、`shell/`、`atoms/`、文字列 registry、新 crate を増やす指示ではない。 |
| 原文の一般化を限定 | Split と Duplicate は区間だけでなく identity・複製対象・参照再写像・選択結果も異なる。「copy_body 一個で同じ」の採否は現行 Document の契約から決める。自作原子・test も既存実装と外部の定規を探した後に限る。 |

---

以下は取り込み元の原文。旧 file の全関数台帳・未決・移動順も当時の文脈に限る。
