...
| `re_video` / `re_mp4_reader` | video decode/presentation、MP4読取。0.34.1の`re_video` featureは`default = ["av1", "ffmpeg"]`で、**H.264は`ffmpeg-sidecar`(CLI)経由 — MotoliiのB-2本命と同一crate**。AV1は`dav1d`(re_rav1d、Linux ARM64除外、`nasm` featureで高速化)、webはWebCodecs(`web-sys`) | D5の反例・codec fixture調査。ffmpeg-sidecar実運用の独立収束事例 | 原則`PATTERN` |
