...
- 切片検収に `next/check.sh` を入れ忘れ、新 crate 5つの wraps/owns マーカー欠落を音声レーンの指摘まで見逃した(以後の検収ループに組込み済み)
- 切片7 merge の衝突解決で自作 dedup が閉じ括弧を食い、import 2つを落とした(レーン側 rebase 方式へ切替えて以後回避)
