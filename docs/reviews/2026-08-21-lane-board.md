...
| weblike-layout | 調査 | worktree(probe可・merge不可) | 利用者発注「web みたいに iced でレイアウトしたい — モック再現が勝負」: taffy / CSS サブセット interpreter / モック→データのコンパイラ / Blitz 再評価(先行裁定=不採用の覆し実測があるかのみ)。判定軸= 保守最低限>機械検証可能な忠実度>手癖不要化>ホットリロード |
