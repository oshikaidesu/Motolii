...
- **ゼロコピー presenter spike(裁定166 §5 訂正、利用者指摘 2026-08-21 深夜)**: iced master 0.15-dev は wgpu 29 宣言済み(一次資料確認)— 型障壁は消滅。spike 内容= iced 0.15-dev rev pin(既存 fork 線)の成立性+engine device 共有で readback 撤去。iced_test 線の揃えが同梱条件。急ぎではない(readback の実害は未実測 — 滑らかさは利用者合格済み)
