...
| **I-mock: Inspector 意味論モック(回収・merge `116a6cef` — **利用者の一瞥待ち**)** | 旧世界 inspector-library.*(899+524行)を構造逐語・色ロール別読み替えで next/reference/mocks/ へ+semantics 版(救出台帳14行: effect stack/mode tabs/selection summary/context menus/extension tabs 等が解禁束)。冒頭で ui-scale-and-z を scale 資料へ降格宣言=二重モック構造の解消。FINDING: way_inspector/way_plugins 相当の token ロール不在(旧候補色のまま) |
