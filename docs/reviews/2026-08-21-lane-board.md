...
| R7: Godot/Blender 逆算 | 調査 | web/clone | Godot=MIT コード可・Blender=意味のみ。モーダル編集の対比込み |
...
1. **監査統合パス**(R7 待ち): R6+R7+R8 の「抜け」を正典へ畳む — 借用既定(拘束7)・人口タイブレーク・キーは仮置き、の3ルールで機械処置
2. **トンマナ柵**(裁定142 仕上げ): pane の raw 色値・px 直書きを落とすテスト
3. **Timeline 第2波 第1束**: レーンバー実体+move/trim(正典+裁定147/148 が発注仕様)
