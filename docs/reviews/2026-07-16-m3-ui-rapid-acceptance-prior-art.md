...
ステータス: **仮説メモ**(受容側の先例集 — **設計根拠ではない**。同日のレビュー3巡を反映: ①「全事例が既存ユーザー基盤/界隈への着地であり不確定変数が多すぎる」→第一部の結論を格下げ(「参照クラスの偏り」節)、②「政治でなく根本的なUX/UIの話」→**第二部(操作パターン単位の収斂語彙とUX原理)を追補**、③「Abletonの答えは操作の直感性。MotoliiはAEのカウンターとしてそうありたい」→**第三部(後発の勝ち筋の分解)を追補**。文末の改訂記録)。U0a入場後も**設計根拠ではない**；転移は個別M3チケット採択時の仮説として台帳化するのみ。本文書はタスク・完了条件・契約を一切変更しない。
...
- **U1 統合既定preset**: AviUtl2のシングルウィンドウ統合が「ウィンドウ行方不明の解消」として受容された。既定は一画面へ復帰可能にしつつ、利用者がpanelをdock/tab/hideできる方針と整合
