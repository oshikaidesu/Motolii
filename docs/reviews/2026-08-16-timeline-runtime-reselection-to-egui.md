...
| **カーソルは1つの語彙に揃えた**。動かす=`Grab`/`Grabbing`(clip の体・キー・ロケータ・ループの中・並べ替え・ナビゲータ)、端を伸ばす=`ResizeHorizontal`(トリム・ループの端)、掃く=`Crosshair`(矩形選択・新しいループ)。**掴んでいるあいだの形は掴んだものが決める**(`hold_cursor` が hover の答えを後から上書き) — hover から毎フレーム決め直すと、ポインタが的から少し外れた瞬間に形が戻り「トリム中なのに掴み手」になる。bar では**絵・カーソル・掴みが同じ1つの判定**を読む(以前は3箇所が別々の入力から聞いていて、答えが食い違っていた) | 同上 |
