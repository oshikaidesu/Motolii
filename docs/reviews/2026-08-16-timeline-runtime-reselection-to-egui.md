...
| **再生**(`Space`)。終端で止まり、終端で押すと頭から。**再生中は面が流れ、playhead は窓の中の相対位置を保つ**(DAW型。AE のページ送りも「中央へ寄せ直す」も採らない — どちらも押した瞬間に面が滑り、playhead が瞬間移動して見える。中央は特別な場所ではなく、左に置いたなら左のまま流れる。窓の外へ出たときだけ中央へ連れ戻す)。スクラブは再生を止め、playhead もフレームに乗る | 同上 |
