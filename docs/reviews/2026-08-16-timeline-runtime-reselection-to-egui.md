...
| **表示名の変更**(名前をダブルクリック / `⏎` / メニュー)。D2 に `SetLayerName` を新設し、`LayerIdTable::rename` を足した。**動くのは台帳だけ** — ツリーも ID も参照(`transform.parent` / `LookAt` / journal)も動かない。空の名前は断る、ロック中は始まらない、その場が入力欄になる(別窓を出さない) | `cargo test -p motolii-ui --example timeline_egui_lab` + `-p motolii-doc --test d2_command renaming` |
| **マーカー(メモ)**。`M` で playhead に置き、その場で書ける。ルーラ下端に印、面には薄い縦線だけ。掴んで動かす(吸着つき)、右クリックで外す、**文を空にすると消える**。D2 は `Document.markers: Vec<Marker>{t, text}` と4つの command(Add/Remove/SetTime/SetText)。**識別子を持たず保持順が宛先**、`skip_serializing_if` で空なら書き出さないので既存文書のバイト列は不変、旧readerは未知キーとして往復するので**版は上げない** | `cargo test -p motolii-ui --example timeline_egui_lab` + `-p motolii-doc --test d2_command marker` |
