...
| **ループ再生**。ルーラの**上端9px**を引いて区間を作る(右→左でも同じ区間、端はフレームに乗る、最短1フレーム)。端6pxで伸縮、中を掴んで移動、`L`で効きの入り切り(帯は残る)。折り返しは**行き過ぎた分を頭へ足す**(捨てると毎周ぶん周期が伸びる)。区間の外から`Space`を押したら区間の頭から | `cargo test -p motolii-ui --example timeline_egui_lab` |
