...
- **ロック / 表示名の変更 / マーカーは概念か入口が無い。** `ItemEnvelope.lock` は**フィールドだけあって command が無い**(`schema.rs:238`)、表示名は台帳にあるが変える command が無い、マーカーは schema に概念が無い。3件とも「足す」判断は済んでいる(2026-08-16 利用者)
