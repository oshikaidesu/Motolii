...
| Browser catalog projection | CU-G09 | Rectangleを含むcardの型付きread modelとunknown/dangling拒否 | **DO / SPEC**。次PRODUCT-ASSET粒としてprojection契約だけを決定し、catalog ID/labelから意味を推測せず、CU-0A08B実装とHost接続を束ねない |
