...
| Browser catalog projection | CU-G09 | Rectangleを含むcardの型付きread modelとunknown/dangling拒否 | **DONE**。[CU-G09 Browser catalog projection契約決定](2026-07-26-cu-g09-browser-catalog-projection-contract-decision.md)でcarrier shape・identity・`B1`〜`B12`を閉じた。次PRODUCT-ASSET粒は未選定。CU-0A08B実装とHost接続は未着手のまま束ねない |
