...
| selection / Undo再投影 | CU-104 DO | commit/Undo/Redo後にTransient selectionと三面投影をどう更新するか | 次のdocs-only PRODUCT-ASSET粒として選定済み。owner・visibility・`projection_generation`更新条件・Apply/Undo/Redo後publish前reconcile時点だけを閉じ、selectionをDocumentへ保存せず、policy決定までVS-1 E2Eを閉じない |
