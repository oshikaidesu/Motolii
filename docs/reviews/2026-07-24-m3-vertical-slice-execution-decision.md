...
| journal durabilityとsnapshot publish順 | CU-G03 DONE / CU-109S DO / CU-109 WAIT | **CU-G03/D/R DONE**。単一command順序とcatalog未反映committed tail recovery guardを閉じた | [CU-G03決定](2026-07-26-cu-g03-edit-durability-ordering-decision.md)と[#369](https://github.com/oshikaidesu/Motolii/pull/369)を正とする。次PRODUCT-ASSET `DO`はdocs-only `CU-109S`（Undo/Redo prepared-action順序の再確認）。CU-109 runtime配線は未完了 |
