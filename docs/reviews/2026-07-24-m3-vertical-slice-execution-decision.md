...
| Browser catalog projection | CU-G09 DONE / CU-G09O DONE / CU-G09R DONE / CU-0A08BP DONE | Rectangleを含むcardの型付きread modelとunknown/dangling拒否 | [CU-G09 Browser catalog projection契約決定](2026-07-26-cu-g09-browser-catalog-projection-contract-decision.md)で受理入力・identity・`B1`〜`B12`を閉じ、[CU-G09O Browser decoder output契約決定](2026-07-26-cu-g09o-browser-decoder-output-contract-decision.md)でdecoder出力shape・`O1`〜`O6`・`B11`/`B12`改訂・`B13`/`B14`を閉じ、[CU-G09R Browser decoder拒否優先順決定](2026-07-26-cu-g09r-browser-decoder-rejection-precedence-decision.md)で拒否family優先順・`B15`・残余ID/ref境界を閉じた。`CU-0A08BP`は`DONE`（product-owned・非export の Browser catalog decoder、fixture/testのみ）。`CU-104`は[selection publish envelope決定](2026-07-27-cu-104-selection-publish-envelope-decision.md)を`DONE`とし、docs-only `U2h-1S`は[primary selection implementation split決定](2026-07-27-u2h-1-primary-selection-implementation-split-decision.md)を`DONE`とした。docs-only `CU-104E`は[projection generation枯渇境界決定](2026-07-27-cu-104e-projection-generation-exhaustion-decision.md)を`DONE`とした。PRODUCT-ASSET laneの次実装粒は`U2h-1I`（CU-104E決定により次実装粒へ戻した）。`CU-0A08BT`は`WAIT`（CU-0A08BP + `U4a-2` Direct製品入口待ち） |
...
| selection / Undo再投影 | CU-104 DONE / U2h-1S DONE / CU-104E DONE | commit/Undo/Redo後にTransient selectionと三面投影をどう更新するか | `CU-104`は[selection publish envelope決定](2026-07-27-cu-104-selection-publish-envelope-decision.md)を`DONE`とし、docs-only `U2h-1S`は[primary selection implementation split決定](2026-07-27-u2h-1-primary-selection-implementation-split-decision.md)を`DONE`とし、docs-only `CU-104E`は[projection generation枯渇境界決定](2026-07-27-cu-104e-projection-generation-exhaustion-decision.md)を`DONE`とした。PRODUCT-ASSET laneの次実装粒は`U2h-1I`（`DO`、[CU-104E projection generation枯渇境界決定](2026-07-27-cu-104e-projection-generation-exhaustion-decision.md)経由で再開）。owner・visibility・`projection_generation`更新条件・枯渇preflight・Apply/Undo/Redo後publish前reconcile時点だけを閉じ、selectionをDocumentへ保存せず、U2h-1I実装完了までVS-1 E2Eを閉じない。完了証拠: [U2h-1 primary selection implementation split決定](2026-07-27-u2h-1-primary-selection-implementation-split-decision.md)、[CU-104E projection generation枯渇境界決定](2026-07-27-cu-104e-projection-generation-exhaustion-decision.md) |
