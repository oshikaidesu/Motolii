...
| selection / Undo再投影 | CU-104 DONE / U2h-1S DONE | commit/Undo/Redo後にTransient selectionと三面投影をどう更新するか | `CU-104`は[selection publish envelope決定](2026-07-27-cu-104-selection-publish-envelope-decision.md)を`DONE`とし、docs-only `U2h-1S`は[primary selection implementation split決定](2026-07-27-u2h-1-primary-selection-implementation-split-decision.md)を`DONE`とした。PRODUCT-ASSET laneの次実装粒は`U2h-1I`。owner・visibility・`projection_generation`更新条件・Apply/Undo/Redo後publish前reconcile時点だけを閉じ、selectionをDocumentへ保存せず、U2h-1I実装完了までVS-1 E2Eを閉じない。完了証拠: [U2h-1 primary selection implementation split決定](2026-07-27-u2h-1-primary-selection-implementation-split-decision.md) |
