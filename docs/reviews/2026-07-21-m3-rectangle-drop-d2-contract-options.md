...
状態: **決定 / D0再採択・製品実装は未着手**。当初の反対側レビューP0=0・P1=6後に閉じたD0契約が
別lineageで決定されていたことを2026-07-23の[歴史回収](2026-07-23-historical-d2-selection-timeline-lineage-recovery.md#3-u2b-2-place-product-core再採択)で確認し、現行へ再採択した。BrowserからのRectangle dropを既存D2へ接続する選択肢と停止線を固定する。
...
| D0 | U2bのPlace専用private経路がwriter snapshotの`LayerIdTable` clone上で候補IDを作り、同じ同期call stackでlive-next照合→既存`AddTrackItem`をapplyする | 維持 | Dと同じ。汎用公開plannerを増やさない | **採択・未実装**。Place draftの閉じた意味と追加停止線は[歴史回収 §3](2026-07-23-historical-d2-selection-timeline-lineage-recovery.md#3-u2b-2-place-product-core再採択)を正とする |
...
単一active drag制約を本文へ反映した。その後historical-only contractでPlace draft、U2h-1、U3a-1まで閉じて
D0がdocs採択されていたことを2026-07-23に再確認した。製品コードは現在も未実装であり、appearance、journal
durability、selection publish、headless Timeline、D3/GPUの各停止線は解除しない。
