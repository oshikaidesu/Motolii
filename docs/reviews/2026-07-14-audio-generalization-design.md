...
> 2026-07-23歴史監査: cutoff全6版を[Unit 5B回収](2026-07-23-historical-audio-generalization-lineage-recovery.md)で処分した。本文のcomponent／mix意味は維持する一方、§6.2のrenderer遅延時Transport varispeedは後続D5により撤回済み、旧Slint UI記述は現行React/native surface所有へ置換する。AG-2はmixer／producerまで成立したが製品`PlaybackSession`接続が未到達のためGAP-28で追跡する。
...
- rendererが実時間未満: audio device clockを主のまま保ち、videoは古い要求／frameを捨てて最新時刻へ追いつく。品質縮退は許可されたpreview表示だけに適用する
- 再生ヘッド所有者は常に1つ

Clip retimeの明示varispeedとdevice sample-rateの固定比変換は許すが、重いpreviewを理由にTransport速度やpitchを変えない。
...
Clipは既定では1行。必要時だけaudio部分を展開し、waveform、mute、gain、「音声を分離」を表示する。audio-only Clipも同じtimeline itemとして扱う。製品surfaceは現行の[UI runtime責任境界](../ui-runtime-architecture.md)に従い、行同期のclip／waveform／mute等はnative Timeline、import dialogやform型gain編集はReact chromeへ投影する。旧Slint／egui実装を製品正本へ戻さない。
...
| AG-2 | **core完了**(#157) / 製品接続はGAP-28 | per-stream PCM cache、canonical変換、deterministic mixer、`MixProducer`は成立。製品`PlaybackSession`の単一PCM入力→`AudioProgram`置換は未実装 | core: 2 source mix、seek、callback非blocking、chunk size不変。製品: mixed source再生、10分A/V drift、seekをGAP-28で審判 |
| AG-3 | **domain完了／製品UI未実装**(Part of #158 / PR#164) | Video+Audio/Video Only source構築、waveform peaks、mute/gain command、音声分離macro(別lane必須)は成立。React import/form＋native Timeline waveform／row操作はM3 follow-up | domain: 分離Undo・保存再読込・同一lane拒否・ordinal一意。UI: 同一projection／intent、1 gesture=1 Undo、Document二重所有なし |
