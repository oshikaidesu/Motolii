...
### Create で生まれた層が Stage に描かれない(Bezier 固有ではない)

- Bezier を足したら絵が出ないので追ったが、**Rectangle も出ない**。つまり Bezier の問題ではなく
  **Create 全体**。S16(Create出産口)は 2026-08-29 に「通った」とされていた縫い目なので**回帰の疑い**
- **層は生まれている**: タイムライン最上行に現れ、帯もプレイヘッドから始まる(時刻の問題ではない)
- **データもラスタライザも無罪**: 診断レーンが `browser.rs` と同じ `Shape` を
  `rasterize_shapes`(本番の入口)へ渡して **2488画素が出る**ことを実測。
  `shape::` のテスト4件も全部緑(`ellipse_with_stroke_only_produces_visible_pixels` を含む)
- **古いビルドではない**: 起動時刻とバイナリの mtime を突き合わせて確認済み
- 残る容疑は `stage_widget.rs` の生きた GPU 提出経路(`render_frame_into`)、
  または `ShapeCacheKey` のキャッシュ衝突。どちらも診断レーンの家の外だった

**監督の反省**: この夜、原因を2回推測して2回とも外した(「座標が板の外」「古いビルド」)。
どちらもテストとタイムスタンプが答えを持っていたのに、先に触った。
[[surprise-triggers-rederivation]] の「同種の行動の2回目は補正=止まれ」がそのまま当たる。
