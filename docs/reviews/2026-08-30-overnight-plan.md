...
5.5 **Stage に見えている物をクリックで選ぶ**(利用者要望 2026-08-30
   「stageに出ているものをそのまま掴めればいいですね」)。
   いまは Timeline の行/帯でしか選べず、Stage のクリックは**選択済みの層のギズモにしか効かない**。
   click=選択 は Q1(一貫した操作文法)の土台。手前(order の大きい方)が勝つ
...
7. **無言の実装(Timeline と Stage を直観的にする)** — 利用者
   「タイムラインとstageを直観的にしないといけないぶん**無言の実装**が多いです」。
   コードで数えた結果、**在るべきなのに無い**のは:

   | | 無いもの | 重さ |
   |---|---|---|
   | Timeline | **端ドラッグ=trim**(`trim`/`resize`/端の当たり判定がゼロ) | **重い** — 帯の長さを変えられない=「並べる」の一部が欠けている |
   | Timeline | **スナップ**(playhead / 他の帯へ吸着) | 中 |
   | 両方 | **カーソルの形が変わらない**(`styles.css` に `cursor:` がゼロ) | **重い** — 触れるか掴めるかが指で分からない。Q0の手前 |
   | Stage | **zoom / pan が無い**。ヘッダの `Fit`/`100%` は死に text | 中(Q0違反でもある) |
   | 両方 | **右クリックメニューが無い** | 低(Tier 0 だが献立は他と重複) |

   在るもの: 縦横スクロール、Timeline の zoom、帯クリック=選択、帯ドラッグ=移動、
   Stage のクリック選択・移動・拡縮・回転
