...
ステータス: **M2基盤再締結解除宣言**（草稿）。**本宣言PRがmainに到達した時点で正本**となり、M2基盤再締結のA〜C退出条件を充足したことを記録する。解除のコード証跡前提はPR [#217](https://github.com/oshikaidesu/Motolii/pull/217) / main `fa6850a3981c319973cf120e64976e6f8d79b969` / [PR CI](https://github.com/oshikaidesu/Motolii/actions/runs/29646476618) / [push CI](https://github.com/oshikaidesu/Motolii/actions/runs/29646451595) である。**本宣言PRのmerge前**はM3製品実装を停止する。merge後も製品実装の着手許可は自動解禁されず、別のM3入場PRのみがU0/U1依存の再翻訳と実装許可を行える。
...
- **本宣言PRがmergeされるまで**、M3製品実装（Documentを読むUI、domain intent、preview、入力、timeline、plugin panel等）は停止のままである
- **merge後**も、M3製品実装の着手許可は自動解禁されない
...
7. **本宣言PR**で本書A〜Cの完了証跡表を充填し、ステータスを再締結解除宣言へ変更する（**本PR**。main到達で正本化）
