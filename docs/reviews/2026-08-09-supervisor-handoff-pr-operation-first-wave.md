...
2. **殴った跡が残ること** — 後から誰も思い出せないため

2について、利用者自身の言（2026-08-09）:

> 私自身があまり覚えていなくて、となると常に**PRで足跡をつけておくべき**ですね、
> **mainの統合がメインではなく**

**PRの第一目的は統合ではなく記録である。** ここを取り違えると
「mergeできる見込みが立つまでPRにしない」という運用になり、
2026-08-07/08の未統合branch 10本と同じ状態を再生産する。
mergeされるかどうかに関わらず、作業はPRとして可視物にする。
判断の理由はPR本文に書く — 後から取り出せる場所はそこしかない。
...
> 普通の開発は確認しながら行うので抜けは起きないはずが、LLMの場合、
> 完璧なコードを最初から出力できてしまう。それはいいことですが、同時に
> **履歴が起きないという点は責任が分散される元**と存じます。

人間の開発は「書く → 動かす → 落ちる → 直す」を繰り返す。
**その反復そのものが検証の履歴になる。**
動くようにする過程が、動く証拠を副産物として残す。

LLMは最初から完成した見た目のコードを出す。良いことだが、
**苦労が無いので残滓も無い。**
...
人間が書いていれば、一度は走らせて `No tests found` を見てconfigを直していた。
**書いた者が怠けたのではなく、動かす必要が生じなかった。**

そして2026-08-09のR0受入は証拠へ「Jest: 2 passed」と書いた。**嘘ではない。**
ただしその2件は `__tests__/App.test.tsx` であり、Inspectorのtestは含まれていない。
`testMatch` を直すと 2 passed → **22 passed**、眠っていた18件は修正なしで全件PASSした。
壊れてはおらず、呼ばれていなかっただけである。

**誰も嘘をつかず、誰も確認していない状態が成立する。** これが責任の分散である。

#### 対策 — 失われた履歴を人工的に作る

- **非LLM oracle**: self-reportを証拠にしない。機械が落ちるかどうかだけを見る
- **PR**: 足跡を強制的に残す（本節冒頭）
- **仮コード器具**: 呼び出し側を先に書き `???` を露出させる。
  **手で苦労を作る装置**である
- **orderの必須条項**: 「走らせていないlaneを通ったと報告しない」
- **存在するのに走らないtestを疑う**: test fileの存在をcoverageの証拠にしない

#### 今日の実測

3件発注し、**いずれも「できなかったこと」を正直に返した。虚偽のsuccessは0。**
一方で **oracleを自力で回せた発注も0**である
（sandboxがskiaのbinary downloadを塞ぐため、Rust laneが原理的に完走しない）。
検証はsupervisorが回し直した。

**LLMが自分の成果を自分で検証できる保証は無い**という前提で発注設計する。
実装と検証の担当を分け、検証はorderの外に置く。
