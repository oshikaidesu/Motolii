...
Before D1j, CAM-G0 adds a fixed existing-render 2D identity fixture at `crates/motolii-render/tests/cam_g0_planar_identity.rs` and classifies it `semantic` in `crates/motolii-testkit/golden_policy/classification.tsv`. It is generated and reviewed while the old `ViewportTransform` path is still authoritative; later camera PRs may not alter its expected bytes.
...
|---|---|---|---|
| CAM-G0 | pre-camera protected identity fixture and semantic classification | this decision, current render path | fixed matching-aspect 2D fixture records current output bytes; golden-policy rejection proves later expectation edits fail |
