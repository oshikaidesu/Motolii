...
| ORACLE-GUARD | CU-0A08SSCI-P1 | M3 prerequisite / Browser post-promotion provenance guard authority reconciliation | `DO` | — | `CU-0A08SSCI-P`は[CU-0A08SSCI-P改訂](reviews/2026-07-29-cu-0a08ssci-p-browser-post-promotion-provenance-chain-authority-amendment.md)で`DONE` | 改訂済み §H-1 Guard 1（`PC-1`〜`PC-9` / `R-1`〜`R-8` / `K-1`〜`K-3`）と `browser-ownership.test.mjs` 実装の未統一を解消する唯一の責任者。解消範囲は本粒の発注で決める。React source byte変更・`source-provenance.json` 実データ変更・(I)/(T) の採番は含めない |
