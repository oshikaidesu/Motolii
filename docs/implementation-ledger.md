...
| PRODUCT-ASSET | CU-0A08SSCI | M3 / VS-1 / B / Browser Place internal source seam | `DO` | — | `CU-0A08SSCSD`／`CU-0A08SSCD`／`CU-0A08SSC`は[発注依存証跡](#発注依存証跡)で`DONE` | decode済みidentityをCandidateCreateBrowserが受け、VS-1 Rectangleだけを既存elementPropsから配る。raw input/decode、Host transport、D2、drop終端は非目標 |
