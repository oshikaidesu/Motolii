...
| M3 | **VS-1 Rectangle配置とUndo** | React chrome + native Stage/Timeline + headless interaction、1 top-level wgpu Surface + 2 native viewport + opaque child WebView islandsを正本化。R4CまでのInspector product ownership、CU-0A08IP、`CU-G09`／`CU-G09O`／docs-only `CU-G09R`は完了した。`CU-0A08BP`は`DONE`（product-owned・非export の Browser catalog decoder、fixture/testのみ）。selection publish envelopeを決定したdocs-only `CU-104`も`DONE`で、[primary selection implementation split決定](reviews/2026-07-27-u2h-1-primary-selection-implementation-split-decision.md)を行ったdocs-only `U2h-1S`も`DONE`。U2h-1I事前審査で`projection_generation: u64`枯渇時のno-rewind意味が未決と判明したため、次のPRODUCT-ASSET粒はdocs-only `CU-104E`、`U2h-1I`は同決定待ち。`CU-0A08BT`は`WAIT`継続。Motolii Studio Previewは未実装。visible summary chromeは未決で実装しない。G0-6HはU0e-3を止める並行人間審判、G0-9DはDistribution Ready用hardware gate。eguiへ新規製品面を実装せず、plugin UI公開契約はG0-3 / GAP-13まで停止する |
...
| PRODUCT-ASSET | CU-104E | M3 / VS-1 / SPEC / projection generation exhaustion | `DO` | — | `CU-104`／`U2h-1S`は[発注依存証跡](#発注依存証跡)で`DONE`。U2h-1I order事前審査で`u64::MAX`時の`+1`とno-rewindを同時に満たす規則が未決と確認 | 枯渇actionのtyped pre-mutation refusal、queue消費、自動retry禁止、CU-109 poison非先取りをdocs-onlyで決定し、`U2h-1I`を`DO`へ戻す。Rust/test/public API/Document/serde/journal/plugin変更はしない |
| PRODUCT-ASSET | U2h-1I | M3 / VS-1 / B / primary selection publish envelope | `WAIT` | — | `U2h-1S`は[発注依存証跡](#発注依存証跡)で`DONE`。意味正本は[U2h-1 split決定](reviews/2026-07-27-u2h-1-primary-selection-implementation-split-decision.md)と[CU-104](reviews/2026-07-27-cu-104-selection-publish-envelope-decision.md)。現在の入場証跡`U2c-1`/`U2c-4`は発注依存証跡で`DONE`。`CU-104E`決定待ち | 既存private Apply/Undo/Redo publication経路へ`primary`/`projection_generation`を追加。P1/P2/P3。`document_edit_runtime.rs:108`と`find_envelope`再利用。`U2h-1P`は後続 |
...
したがって現在の短い運用判断は、**CU-0A03 / R0からCU-0A07C / R4Cまでは完了済み。CU-0A08IP、`U3a-1S`/`U3a-1I`、`CU-G03D`/`CU-G03R`、親`CU-G03`、`CU-101`、`CU-102`、`CU-G09`、`CU-G09O`、`CU-G09R`、`CU-0A08BP`、docs-only `CU-104`、docs-only `U2h-1S`は`DONE`。CU-104はselection publish envelopeのowner・visibility・generation・reconcile時点だけを決定し、`U2h-1S`は[primary selection implementation split決定](reviews/2026-07-27-u2h-1-primary-selection-implementation-split-decision.md)で`U2h-1I`/`U2h-1P`へ分割した。U2h-1I事前審査で`projection_generation: u64`枯渇時の`+1`とno-rewindが未両立と判明したため、PRODUCT-ASSET laneの次粒はdocs-only `CU-104E`、`U2h-1I`は`WAIT`。`CU-104E`は枯渇actionのtyped pre-mutation refusal、queue消費、自動retry禁止、CU-109 poison非先取りだけを決定し、完了後に`U2h-1I`を次実装粒へ戻す。`CU-0A08BP`はproduct-owned・非export の Browser catalog decoder、fixture/testのみ。`CU-0A08BT`は`WAIT`（CU-0A08BP + `U4a-2` Direct製品入口待ち）、`CU-0A08IT`は`WAIT`（`U4a-2` Direct製品入口待ち）。`U2c-2`は`WAIT`（U4a-2 Direct + U4c Advanced）。G0-6HはU0e-3だけを止める。CU-109 runtime配線、CU-110 Place、CU-111 prepared Undo/Redo、CU-106 consumer接続をCU-104E/U2h-1Iへ束ねない。Host transport、typed intent、JSX binding、drag payload、`S`行の意味決定、Rust/schema/plugin変更をCU-104E/U2h-1I/CU-0A08BTへ混ぜるならSTOPする。Motolii Studio Previewは未実装。**G0-6Hは同時に進められる人間審判だが、未完了でもR0〜R4やPreview骨格を止めず、U0e-3だけを止める。G0-9DはDistribution Readyまで`WAIT / HARDWARE`。`U2c-2`はVS-2候補かつ実製品入口待ちである。D1n、D5等の独立follow-upをVS-1の再停止理由へしない。
