...
| VISUAL-RESPONSE | G0-6H-R | M3 evidence / VS-1 / reference authority reconciliation | `DO` | — | `G0-6H-R0`は[発注依存証跡](#発注依存証跡)で`DONE` | 二つの固定commitのauthorityを分類し、route裁定・variant供給・既存handoff維持の次工程だけをhandoff。route/画像/token変更0 |
