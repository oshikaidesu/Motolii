...
| VISUAL-RESPONSE | G0-6H-AG0 | M3 evidence / VS-1 / Starter Media generator closure inventory | `DO` | — | `G0-6H-AF`は[発注依存証跡](#発注依存証跡)で`DONE` | 既存repo toolingを棚卸しし、`REUSE / ADOPT / WRAP / EXTERNAL / BUILD / REJECT`と`RETIREMENT`をdocsで裁定する。media byte/実装変更0 |
| VISUAL-RESPONSE | G0-6H-V0 | M3 evidence / VS-1 / current-route variant evidence contract | `WAIT` | — | `G0-6H-AG0`のgenerator / output closure裁定待ち | 5状態semantic mapping、固定capture環境、normal＋派生variant、immutable manifest/read-only check、記録された人間sessionの要求だけをdocsで閉じる。画像/script/fixture/token/threshold/G0-6H/U0e-3状態変更0 |
