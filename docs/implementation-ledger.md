...
| ORACLE-GUARD | CU-0A08SSCI-P | M3 prerequisite / Browser post-promotion provenance multi-entry oracle | `DO` | — | `CU-0A08SSCI`は[前提順序裁定](reviews/2026-07-29-cu-0a08ssci-browser-place-source-seam-prerequisite-order-decision.md)で`WAIT`。前提(P)(I)(T)未決。P-first採択 | post-promotion変更の多entry受理と改竄拒否をownership guardへ閉じ、(I)(T)が`DiscoveryBrowserCandidate.jsx`をbyte変更できる前提を先に成立させる。既存1 entry literalの無根拠書換え・React byte変更は含めない |
