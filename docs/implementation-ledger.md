...
| VISUAL-RESPONSE | G0-6H-V1S | M3 evidence / VS-1 / current-route capture boundary decision | `DO` | — | `G0-6H-V0`は[発注依存証跡](#発注依存証跡)で`DONE`。V1事前照合で5状態到達手段とscreen 1 fixture projection境界が未決と判明 | route/queryを増やさない同一route操作、development専用typed fixture projection、Starter Media表示意味、capture環境記録責任をdocs-onlyで裁定する。画像/script/fixture/code/ledger状態変更0 |
| VISUAL-RESPONSE | G0-6H-V1 | M3 evidence / VS-1 / current-route evidence generation | `WAIT` | — | `G0-6H-V1S`のcapture/fixture境界裁定待ち | `#plugin-browser-candidate`から5状態をcaptureし、各normal＋派生5variant、immutable generation、SHA-256 manifest、read-only照合を作る。human session、token採択、U0e-3解禁は含めない |
