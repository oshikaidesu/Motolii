...
| PRODUCT-ASSET | U2h-1S | M3 / VS-1 / SPEC / primary selection implementation split | `DO` | — | `CU-104`／`U2c-1`／`U2c-4`は[発注依存証跡](#発注依存証跡)で`DONE`。次のPRODUCT-ASSET粒として、既存Apply/Undo/Redo publish経路とselection-only入力面を別実装粒へ分けるdocs-only決定だけを行う | U2h-1Iを次実装粒へ選定し、selection-only actionはU2h-1P、Place receiptはCU-110へ分離する。Rust/JS/test/fixture、CU-109/110/111/106、公開API・Document・serde・journal・Undo・plugin契約は変更しない |
...
したがって現在の短い運用判断は、**CU-0A03 / R0からCU-0A07C / R4Cまでは完了済み。CU-0A08IP、`U3a-1S`/`U3a-1I`、`CU-G03D`/`CU-G03R`、親`CU-G03`、`CU-101`、`CU-102`、`CU-G09`、`CU-G09O`、`CU-G09R`、`CU-0A08BP`、docs-only `CU-104`は`DONE`。CU-104はselection publish envelopeのowner・visibility・generation・reconcile時点だけを決定し、次のPRODUCT-ASSET粒はdocs-only `U2h-1S`。`U2h-1S`は既存Apply/Undo/Redo経路の`U2h-1I`とselection-only入力面の`U2h-1P`を分離して次実装粒を固定する。`CU-0A08BP`はproduct-owned・非export の Browser catalog decoder、fixture/testのみ。`CU-0A08BT`は`WAIT`（CU-0A08BP + `U4a-2` Direct製品入口待ち）、`CU-0A08IT`は`WAIT`（`U4a-2` Direct製品入口待ち）。`U2c-2`は`WAIT`（U4a-2 Direct + U4c Advanced）。G0-6HはU0e-3だけを止める。CU-109 runtime配線、CU-110 Place、CU-111 prepared Undo/Redo、CU-106 consumer接続をU2h-1Sへ束ねない。Host transport、typed intent、JSX binding、drag payload、`S`行の意味決定、Rust/schema/plugin変更をU2h-1S/CU-0A08BTへ混ぜるならSTOPする。Motolii Studio Previewは未実装。**G0-6Hは同時に進められる人間審判だが、未完了でもR0〜R4やPreview骨格を止めず、U0e-3だけを止める。G0-9DはDistribution Readyまで`WAIT / HARDWARE`。`U2c-2`はVS-2候補かつ実製品入口待ちである。D1n、D5等の独立follow-upをVS-1の再停止理由へしない。
