...
| ORACLE-GUARD | CU-0A08IR | M3 prerequisite / Inspector authority hash reclosure | `DO` | — | `b0c0f916`でInspector分割決定の進行状態とDirect/U4c依存整理が正当に更新された一方、inventory guardの固定hashが旧blobのままであることを再現。CU-0A08IPの意味・実装は変更しない。pre-edit testで独立T7誤検知も判明したためCU-0A08IQ待ち | CU-0A08IQ完了後、authority本文を変更せずguardの固定hashだけを現行blobへ再締結して専用testと全reference guardを緑へ戻す。CU-0A08BPとは別ticket・別commit |
