...
| ORACLE-GUARD | CU-0A08BR | M3 prerequisite / Browser post-completion lane oracle precision | `DO` | — | Browser decoderは[発注依存証跡](#発注依存証跡)で完了済み。次の正当な製品粒選定を隔離差分で追加すると、Browser decoder guardの完了後oracleが対象IDを限定せず全後続`DO`を拒否し、287中286 passを再現 | 完了済みdecoder粒のstale状態拒否を維持しつつ、別IDの正当な後続`DO`をsynthetic受理するよう専用guardだけを精密化する。製品実装、仕様本文、台帳の次粒選定、期待値・threshold変更は含めない |
