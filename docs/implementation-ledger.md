...
| M3 | **VS-1 Rectangle配置とUndo** | Browser typed intent→offline native Host WebView→exact codec/bounded inbox→Host private platform capture→native Stage latest-epoch hit-testと、1 top-level Surface上のStage/Timeline表示まで`CU-0B04N`で成立。token後続は`WAIT`。次は横展開せず`CU-0B04R`でopaque child WebViewのfocus/geometry epochを閉じる。D2/Undoまで未貫通のためMotolii Studio Preview完成とは扱わない。G0-9DはDistribution Ready用hardware gate。eguiへ新規製品面を実装せず、plugin UI公開契約はG0-3 / GAP-13まで停止する |
...
| PRODUCT-ASSET | CU-0B04R | M3 / VS-1 / B / opaque child WebView focus and geometry epoch | `DO` | [CU-0B04N実装決定](reviews/2026-07-29-cu-0b04n-native-stage-surface-layout-implementation-decision.md) | CU-0B03 + CU-0B04N。Browser一島のopaque child mount / geometryは成立 | focus/geometry epochとHost snapshot再投影入口を閉じる。DOM/px identity、React semantic state、transparent overlay 0 |
...
現在の短い運用判断は、**Browser typed intent→offline native Host WebView→bounded inbox→Host private platform capture candidate→native Stage latest-layout-epoch hit-testと、1 top-level Surface上のStage/Timeline表示まで実装済み。次はCU-0B04Rでopaque child WebViewのfocus/geometry epochを閉じ、CU-0B05へ進む。token後続は`WAIT`。**
