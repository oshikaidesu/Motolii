...
| PRODUCT-ASSET | CU-0A08SSCS | M3 / VS-1 / SPEC / Browser Place source seam implementation scope selection | `DO` | — | `CU-0A08SSCD`／`CU-0A08SSC`／`CU-0A08SSD`は[発注依存証跡](#発注依存証跡)で`DONE` | Browser catalog入力境界を発明せず、最小closed implementation orderへ落とす範囲を一問だけ選定する |
