...
| M3 | **VS-1 Rectangle配置とUndo** | React chrome + native Stage/Timeline + headless interaction、1 top-level wgpu Surface + 2 native viewport + opaque child WebView islandsを正本化。R4CまでのInspector product ownership、CU-0A08IP、`CU-G09`／`CU-G09O`／docs-only `CU-G09R`は完了した。`CU-0A08BP`は`DONE`（product-owned・非export の Browser catalog decoder、fixture/testのみ）。selection publish envelopeを決定したdocs-only `CU-104`も`DONE`で、[primary selection implementation split決定](reviews/2026-07-27-u2h-1-primary-selection-implementation-split-decision.md)を行ったdocs-only `U2h-1S`も`DONE`。PRODUCT-ASSET laneの次実装粒は`U2h-1I`。`CU-0A08BT`は`WAIT`継続。Motolii Studio Previewは未実装。visible summary chromeは未決で実装しない。G0-6HはU0e-3を止める並行人間審判、G0-9DはDistribution Ready用hardware gate。eguiへ新規製品面を実装せず、plugin UI公開契約はG0-3 / GAP-13まで停止する |
...
したがって現在の短い運用判断は、**CU-0A03 / R0からCU-0A07C / R4Cまでは完了済み。CU-0A08IP、`U3a-1S`/`U3a-1I`、`CU-G03D`/`CU-G03R`、親`CU-G03`、`CU-101`、`CU-102`、`CU-G09`、`CU-G09O`、`CU-G09R`、`CU-0A08BP`、docs-only `CU-104`、docs-only `U2h-1S`は`DONE`。CU-104はselection publish envelopeのowner・visibility・generation・reconcile時点だけを決定し、`U2h-1S`は[primary selection implementation split決定](reviews/2026-07-27-u2h-1-primary-selection-implementation-split-decision.md)で`U2h-1I`/`U2h-1P`へ分割した。PRODUCT-ASSET laneの次実装粒は`U2h-1I`。`CU-0A08BP`はproduct-owned・非export の Browser catalog decoder、fixture/testのみ。`CU-0A08BT`は`WAIT`（CU-0A08BP + `U4a-2` Direct製品入口待ち）、`CU-0A08IT`は`WAIT`（`U4a-2` Direct製品入口待ち）。`U2c-2`は`WAIT`（U4a-2 Direct + U4c Advanced）。G0-6HはU0e-3だけを止める。CU-109 runtime配線、CU-110 Place、CU-111 prepared Undo/Redo、CU-106 consumer接続をU2h-1Iへ束ねない。Host transport、typed intent、JSX binding、drag payload、`S`行の意味決定、Rust/schema/plugin変更をU2h-1I/CU-0A08BTへ混ぜるならSTOPする。Motolii Studio Previewは未実装。**G0-6Hは同時に進められる人間審判だが、未完了でもR0〜R4やPreview骨格を止めず、U0e-3だけを止める。G0-9DはDistribution Readyまで`WAIT / HARDWARE`。`U2c-2`はVS-2候補かつ実製品入口待ちである。D1n、D5等の独立follow-upをVS-1の再停止理由へしない。
