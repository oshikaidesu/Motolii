...
| M3 | **VS-1 Rectangle配置とUndo / CU-G09R Browser rejection precedence** | React chrome + native Stage/Timeline + headless interaction、1 top-level wgpu Surface + 2 native viewport + opaque child WebView islandsを正本化。R4CまでのInspector product ownership、CU-0A08IP、docs-only CU-G09Oは完了した。CU-0A08BP v3事前審査で拒否familyの重複・未割当と、CU-G09O I2が閉じたitem/catalog以外の空ID/ref境界不足を検出したため、docs-only CU-G09Rを次粒とし、実装は追補完了まで待つ。Motolii Studio Previewは未実装。visible summary chromeは未決で実装しない。G0-6HはU0e-3を止める並行人間審判、G0-9DはDistribution Ready用hardware gate。eguiへ新規製品面を実装せず、plugin UI公開契約はG0-3 / GAP-13まで停止する |
...
| PRODUCT-ASSET | CU-G09R | M3 / VS-1 / SPEC / Browser rejection precedence | `DO` | — | `CU-G09`／`CU-G09O`は[発注依存証跡](#発注依存証跡)で`DONE`。CU-0A08BP v3事前審査でB1/B9/B14、B8/B14、B3/B8、B11/B8の重複と未割当を確認 | docs decisionとcurrent mirrorのみ。重複するmalformed入力の単一family帰属、残余入力のfamily帰属、CU-G09O I2外のID/ref空文字境界を決定し、code/fixture/public API/`S`意味変更0 |
| PRODUCT-ASSET | CU-0A08BP | M3 / VS-1 / B / Browser catalog decoder | `WAIT` | — | `CU-0A04`／`CU-G09`／`CU-G09O` は発注依存証跡で `DONE`。CU-G09R rejection precedence待ち | product-owned・非export pure decoder、fixture/testのみ。typed intent、Host transport、JSX binding、drag payload、`S`行はCU-0A08BT以降 |
...
| CU-G09R | `DO` | CU-0A08BP v3事前審査で拒否familyの重複・未割当と、CU-G09O I2が閉じたitem_id/catalog scope_ref以外の空ID/ref境界不足を確認。docs-only追補を先行しCU-0A08BPはWAITへ戻す |
...
したがって現在の短い運用判断は、**CU-0A03 / R0からCU-0A07C / R4Cまでは完了済み。CU-0A08IP、`U3a-1S`/`U3a-1I`、`CU-G03D`/`CU-G03R`、親`CU-G03`、`CU-101`、`CU-102`、`CU-G09`、`CU-G09O`は`DONE`。CU-0A08BP v3事前審査で拒否familyの重複・未割当を検出したため、次のPRODUCT-ASSET粒はdocs-only `CU-G09R`を`DO`とし、`CU-0A08BP`はCU-G09R待ちへ戻す。`CU-0A08BT`は`WAIT`（CU-0A08BP + `U4a-2` Direct製品入口待ち）、`CU-0A08IT`は`WAIT`（`U4a-2` Direct製品入口待ち）。`U2c-2`は`WAIT`（U4a-2 Direct + U4c Advanced）。G0-6HはU0e-3だけを止める。CU-109 runtime配線とCU-104は未着手のまま束ねない。Host transport、typed intent、JSX binding、drag payload、`S`行の意味決定、Rust/schema/plugin変更をCU-G09R/CU-0A08BPへ混ぜるならSTOPする。Motolii Studio Previewは未実装。**G0-6Hは同時に進められる人間審判だが、未完了でもR0〜R4やPreview骨格を止めず、U0e-3だけを止める。G0-9DはDistribution Readyまで`WAIT / HARDWARE`。`U2c-2`はVS-2候補かつ実製品入口待ちである。D1n、D5等の独立follow-upをVS-1の再停止理由へしない。
