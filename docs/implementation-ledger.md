...
| VISUAL-RESPONSE | G0-6H-V1ET | M3 evidence / VS-1 / mock empty projection and ready oracle | `DO` | — | `G0-6H-V1EB`は[発注依存証跡](#発注依存証跡)で`DONE` | Browser入力seamとprovenance/guard再締結、Vite mode carrier、Browser R-9実描画、Stage/Inspector/Timeline空投影、ready oracleをroute-level Playwrightで閉じる |
