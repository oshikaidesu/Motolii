...
| PRODUCT-ASSET | CU-0A08SSCI-T0 | M3 / VS-1 / SPEC / Browser private component verification harness grain numbering | `DO` | — | `CU-0A08SSCI-I`は[発注依存証跡](#発注依存証跡)で`DONE` | 未採番前提(T)を採番し、次docs-only裁定の唯一の問いを固定。code・型・harness形・公開境界変更0 |
