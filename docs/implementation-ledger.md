...
| M3 | **VS-1 Rectangle配置とUndo** | Browser typed intentと`CU-0B03H` offline Host bundle/closed Web codecまで成立。token後続は`WAIT`。次の唯一のPRODUCT-ASSET `DO`はexact codecをnative bounded inboxへ接続する`CU-0B03`。`CU-107PV`は実製品Host再投影`CU-0B05`待ち。Motolii Studio Previewは未実装。G0-9DはDistribution Ready用hardware gate。eguiへ新規製品面を実装せず、plugin UI公開契約はG0-3 / GAP-13まで停止する |
...
| PRODUCT-ASSET | CU-0B03 | M3 / VS-1 / H1b native Host codec/inbox | `DO` | [CU-0B03H実装決定](reviews/2026-07-29-cu-0b03h-browser-host-contract-offline-mount-decision.md) | CU-0B03H | callbackはdecode/session gate/bounded enqueueだけ。Document/D2 reentrant mutation 0 |
...
| CU-0B03H | `DONE` | [CU-0B03H実装決定](reviews/2026-07-29-cu-0b03h-browser-host-contract-offline-mount-decision.md)でBrowser一面のexact codec、CSP offline bundle、product mount、SHA-256 asset closureを完了。次PRODUCT-ASSET `DO`は`CU-0B03` |
...
現在の短い運用判断は、**CU-0B02T、CU-0A08BTI、CU-0B03Hは`DONE`。token後続は`WAIT`。次の唯一のPRODUCT-ASSET `DO`はnative Host codec/inbox `CU-0B03`で、`Browser typed intent → Host → Place → Stage/Timeline/Inspector → Undo`を貫通させる。**
