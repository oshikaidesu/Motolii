...
| M3 | **VS-1 Rectangle配置とUndo / R4系完了・CU-0A08I再判定** | React chrome + native Stage/Timeline + headless interaction、1 top-level wgpu Surface + 2 native viewport + opaque child WebView islandsを正本化。R4 readinessでInspectorをR4A oracle→R4B mock React化→R4C ownershipへ分割し、R4Aは[#353](https://github.com/oshikaidesu/Motolii/pull/353)で未変更sourceの5状態・主要操作oracleを固定した。R4Bは[#355](https://github.com/oshikaidesu/Motolii/pull/355)で固定mock内の同形React化と一方向legacy adapterを完了した。R4Cは[#357](https://github.com/oshikaidesu/Motolii/pull/357)でR4B InspectorのJSX/CSSをbyte同一でproduct ownerへ移し、mockを単一package importのconsumerへ反転した。次は`CU-0A08I`を`READY-RECHECK`として、既決fieldだけのread-only Host projection / typed intent境界を再判定する。Motolii Studio Previewは未実装。visible summary chromeは未決で実装しない。G0-6HはU0e-3を止める並行人間審判、G0-9DはDistribution Ready用hardware gate。eguiへ新規製品面を実装せず、plugin UI公開契約はG0-3 / GAP-13まで停止する |
...
→ G0-9L DONE → R0 source inventory DONE → R1 Browser ownership DONE → R2A/R2B Easing DONE → R3A/R3B KEYS/LAYERS DONE → R4S/R4A/R4B/R4C DONE → CU-0A08I READY-RECHECK → rolling VS-1 enabling order
...
|---|---|---|---|---|---|---|
| PRODUCT-ASSET | CU-0A08I | M3 / VS-1 / B / CU-0A08I | `READY-RECHECK` | — | [#357](https://github.com/oshikaidesu/Motolii/pull/357)でR4C byte同一product ownershipとmock consumer反転が完了した | 粒表の既存文言に沿って、既決fieldだけのread-only Host projection / typed intent境界を再判定してからclosed orderを作る。未決field補完、Document clone/history、双方向store、実編集接続はSTOP |
...
したがって現在の短い運用判断は、**CU-0A03 / R0からCU-0A07C / R4Cまでは完了済み。Browser、Easing trigger、KEYS/LAYERSのproduct単一ownerとmock consumer反転が成立し、Inspectorも[#357](https://github.com/oshikaidesu/Motolii/pull/357)でbyte同一product ownershipとmock consumer反転を達成した。現在粒は`CU-0A08I`の`READY-RECHECK`で、既決fieldだけのread-only Host projection / typed intent境界を再判定してからclosed orderを作る。未決field補完、Document clone/history、双方向store、実編集接続が必要ならSTOPする。Motolii Studio Previewは未実装。**G0-6Hは同時に進められる人間審判だが、未完了でもR0〜R4やPreview骨格を止めず、U0e-3だけを止める。G0-9DはDistribution Readyまで`WAIT / HARDWARE`。`U2c-2`はVS-2候補かつ実製品入口待ちである。D1n、D5等の独立follow-upをVS-1の再停止理由へしない。
