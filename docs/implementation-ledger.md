...
| PRODUCT-ASSET | CU-0A08RMD | M3 / VS-1 / SPEC / Browser typed-intent dependency adjudication | `DO` | — | `CU-0A08RM0`／`CU-0A08RS`は[発注依存証跡](#発注依存証跡)で`DONE` | [CU-0A08RM0選定 §3](reviews/2026-07-29-cu-0a08rm0-browser-typed-intent-dependency-adjudication-scope-selection.md#3-cu-0a08rmd-が閉じる唯一の問い)の一問だけをdocs-onlyで閉じる。答え以外（BT/ITの`WAIT`・依存セル、API、event shape、code変更）は0 |
