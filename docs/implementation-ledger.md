...
| VISUAL-RESPONSE | G0-6H-V1G-O | M3 evidence / VS-1 / current-route immutable publication | `DO` | — | `G0-6H-V1G-O-H`は[発注依存証跡](#発注依存証跡)で`DONE` | 別root manifestと30 PNGのimmutable publication/read-only照合を閉じる |
