...
| PRODUCT-ASSET | CU-107D | M3 / VS-1 / SPEC / CU-107 split concretization scope decision | `DO` | — | `CU-109`／`CU-G04SC`／`CU-110S`／`CU-110D`／`CU-107S`は[発注依存証跡](#発注依存証跡)で`DONE` | 閉集合と個数を先に閉じるか、`CU-110`が必要とする責任範囲の限定を先に閉じるかを一問だけ裁定する。子粒名・event shape・API・W0表書換えは含めない |
