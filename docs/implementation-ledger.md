...
| PRODUCT-ASSET | CU-0A08BDD | M3 / VS-1 / SPEC / Browser typed-intent dependency direction adjudication | `DO` | — | `CU-0A08BD0`／`CU-0A08RMD`／`CU-0A08RS`は[発注依存証跡](#発注依存証跡)で`DONE` | [CU-0A08BD0選定 §3](reviews/2026-07-29-cu-0a08bd0-browser-typed-intent-dependency-direction-scope-selection.md#3-cu-0a08bdd-が閉じる唯一の問い)の一問だけをdocs-onlyで閉じる。答え以外（BT/ITの`WAIT`・依存セル、`CU-110`前提、API、event shape、code変更）は0 |
