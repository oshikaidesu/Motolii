...
| RENDER-CONTRIBUTION-CONTRACT | P2D-RCD1A | M5 / P2D / public seam API shape | `WAIT` | — | P3 Observation decision、RCD1／RCT1／RCR1 | CompCamera／private RCS1非昇格 |
| RENDER-CONTRIBUTION-CONTRACT | P2D-RCD1I | M5 / P2D / public typed seam implementation | `WAIT` | — | P2D-RCD1A | schema／package／First Vism分岐なし |
...
| RENDER-CONTRIBUTION-SCHEMA | P2D-RCD2I | M5 / P2D / policy schema implementation | `WAIT` | — | P2D-RCD2、現行Document version 5前置確認 | 発注前準備は可能。v6 migration／command／semantic oracleを一契約で実装 |
...
| RENDER-CONTRIBUTION-FIXTURE | P2D-RCF1I-BASE | M5 / P2D / F1 F5 F6 executable harness | `WAIT` | — | P2D-RCF1、P2D-RCD1I | private RCS1非昇格 |
| RENDER-CONTRIBUTION-FIXTURE | P2D-RCF1I-ALPHA | M5 / P2D / F2 F3 executable harness | `WAIT` | — | P2D-RCF1I-BASE、P2D-RCT1、P2D-RCO1 | v1はcutout pixelとsoft-alpha typed unsupported |
...
| RENDER-CONTRIBUTION-REFRACTION | P2D-RCR1 | M5 / P2D / scene-color input contract | `DONE` | — | [input contract](reviews/2026-07-29-m5-scene-color-input-contract-decision.md)。M4-K0の既決RoD／RoI意味契約だけを使用し、runtime spike mergeは非依存 | immutable pre-requester snapshot、upstream order、K0 RoI、transparent-black domain外、whole-request failure |
| RENDER-CONTRIBUTION-COPY | P2D-RCP1 | M5 / P2D / scene-color copy decision | `WAIT` | — | P2D-RCR1、P2D-RCFP1F | lifetime、同期、画面外sample、budget |
| RENDER-CONTRIBUTION-BUDGET | P2D-RCBUD1 | M5 / P2D / cache and budget integration | `WAIT` | — | P2D-RCD1、M4-K1、P2D-RCFP1F、P2D-RCP1 | Host計上とcache key完全性 |
...
| P2D-RCD1 | `DONE` | [typed seam decision](reviews/2026-07-29-m5-render-contribution-typed-seam-decision.md)が型付き要求とcontributionのseam意味、一／一／many、whole-request admission、F5継承、trust eligibility前段分離、First Vism conformance役割を固定 |
