...
| M3 | **VS-1 Rectangle配置とUndo / CU-G09O Browser decoder output contract** | React chrome + native Stage/Timeline + headless interaction、1 top-level wgpu Surface + 2 native viewport + opaque child WebView islandsを正本化。R4CまでのInspector product ownershipとCU-0A08IPは完了した。CU-0A08BP事前審査でCU-G09のdecoder出力shapeと型gate未決を検出したため、docs-only CU-G09Oを次粒とし、実装は追補完了まで待つ。Motolii Studio Previewは未実装。visible summary chromeは未決で実装しない。G0-6HはU0e-3を止める並行人間審判、G0-9DはDistribution Ready用hardware gate。eguiへ新規製品面を実装せず、plugin UI公開契約はG0-3 / GAP-13まで停止する |
...
| PRODUCT-ASSET | CU-G09O | M3 / VS-1 / SPEC / Browser decoder output contract | `DO` | — | `CU-G09`は[発注依存証跡](#発注依存証跡)で`DONE`。CU-0A08BP order事前審査で出力shape、型gate、B11/B12写像のauthority不足を確認 | docs decisionとcurrent mirrorのみ。validated snapshot deep clone、label非分岐、B13/B14を閉じる。code/fixture/public API/`S`意味変更0 |
| PRODUCT-ASSET | CU-0A08BP | M3 / VS-1 / B / Browser catalog decoder | `WAIT` | — | `CU-0A04`と`CU-G09`は[発注依存証跡](#発注依存証跡)で`DONE`。CU-G09O decoder output contract待ち | product-owned・非export pure decoder、fixture/testのみ。typed intent、Host transport、JSX binding、drag payload、`S`行はCU-0A08BT以降 |
...
| CU-G09O | `DO` | CU-0A08BP事前審査でCU-G09の受理入力契約だけではdecoder出力shapeとmalformed型のrule-ID写像が閉じないと確認。docs-only追補を先行し、CU-0A08BPはWAITへ戻す |
...
したがって現在の短い運用判断は、**CU-0A03 / R0からCU-0A07C / R4Cまでは完了済み。CU-0A08IP、`U3a-1S`/`U3a-1I`、`CU-G03D`/`CU-G03R`、親`CU-G03`、`CU-101`、`CU-102`、`CU-G09`は`DONE`。`CU-0A08BP`事前審査でdecoder出力shapeと型gateのauthority不足を検出したため、次のPRODUCT-ASSET粒はdocs-only `CU-G09O`を`DO`とし、`CU-0A08BP`はCU-G09O待ちへ戻す。`CU-0A08BT`は`WAIT`（CU-0A08BP + `U4a-2` Direct製品入口待ち）、`CU-0A08IT`は`WAIT`（`U4a-2` Direct製品入口待ち）。`U2c-2`は`WAIT`（U4a-2 Direct + U4c Advanced）。G0-6HはU0e-3だけを止める。CU-109 runtime配線とCU-104は未着手のまま束ねない。Host transport、typed intent、JSX binding、drag payload、`S`行の意味決定、Rust/schema/plugin変更をCU-G09O/CU-0A08BPへ混ぜるならSTOPする。Motolii Studio Previewは未実装。**G0-6Hは同時に進められる人間審判だが、未完了でもR0〜R4やPreview骨格を止めず、U0e-3だけを止める。G0-9DはDistribution Readyまで`WAIT / HARDWARE`。`U2c-2`はVS-2候補かつ実製品入口待ちである。D1n、D5等の独立follow-upをVS-1の再停止理由へしない。
