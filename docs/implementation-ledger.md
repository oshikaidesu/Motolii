...
現在の短い運用判断は、**CU-0B02T、CU-0A08BTI、CU-0B03H、CU-0B03は`DONE`。Browser typed intent→offline native Host WebView→bounded inboxまで実在する。token後続は`WAIT`。次は`CU-0B04N/R`と`CU-0B05`の最小製品経路だけを再締結し、Place→Stage/Timeline/Inspector→Undoへ進む。**
