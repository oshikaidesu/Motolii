...
| VISUAL-RESPONSE | G0-6H-V1P | M3 evidence / VS-1 / current-route capture prerequisite decision | `DO` | [G0-6H-V1P選定](reviews/2026-07-28-g0-6h-v1p-capture-prerequisite-selection.md) | `G0-6H-V1S`は[発注依存証跡](#発注依存証跡)で`DONE`。V1 order draftのOpus 5 `ORDER: STOP`と現行code事実を再照合 | screen1 mock-owned typed projection seam、screen2〜5操作/oracle、font fixture観測境界の三問だけをdocsで閉じる |
| VISUAL-RESPONSE | G0-6H-V1 | M3 evidence / VS-1 / current-route evidence generation | `WAIT` | — | `G0-6H-V1P`完了待ち | `#plugin-browser-candidate`から5状態をcaptureし、各normal＋派生5variant、immutable generation、SHA-256 manifest、read-only照合を作る。human session、token採択、U0e-3解禁は含めない |
