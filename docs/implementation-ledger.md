...
| PRODUCT-ASSET | CU-206 | M3 / VS-2 / PRODUCT / ordinary native Timeline fixed viewport and two-axis pan | `DO` | — | `U3a-2Q-V`は[発注依存証跡](#発注依存証跡)で`DONE` | 既存native renderer / projection / AppKit monitorだけを接続。zoom、scrollbar、永続形式、React Timeline、公開APIは追加しない |
