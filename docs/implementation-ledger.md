...
| M5 | **identity meaning decision可 / Render Contribution三証拠leaf DO / fixture WAIT** | P0I自身が所有するcontinuity／transform／nested identity／寿命／cache入力境界／PRNG処分をdocsで先に閉じ、TextCluster内部写像とPrototype ownerは明示留保する。P2D周辺は`P2D-RC0`共通基盤と`P2D-RCE0`固定capsule／Rerun裁定をread-only入力にし、旧RCA/B/Cを再発注しない。新RCA2/B2/C2を非交差allowlistで並列化し、統合`P2D-RCI`、公開API、Document、plugin契約、実装はWAIT。P6のfontique／harfrust／Vello text stackは未実装で、K6とpremul adapterを重複実装しない |
...
| RENDER-CONTRIBUTION-AUTHORITY | P2D-RCA2 | M5 / P2D / boundary comparison | `DO` | — | P2D-RC0、P2D-RCE0。Motolii authority／code factだけ、専用v2文書だけ | 完了後P2D-RCIへ |
| RENDER-CONTRIBUTION-RERUN | P2D-RCB2 | M5 / P2D / adjudicated evidence transcription | `DO` | — | P2D-RC0、P2D-RCE0。Rerun三capsule／裁定、専用v2文書だけ、6ラベル順固定 | 完了後P2D-RCIへ |
| RENDER-CONTRIBUTION-ENGINE | P2D-RCC2 | M5 / P2D / fixture evidence comparison | `DO` | — | P2D-RC0、P2D-RCE0。engine三capsuleだけ、network禁止、専用v2文書だけ | 完了後P2D-RCIへ |
