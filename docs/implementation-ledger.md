...
| M3 | **VS-1 Rectangle配置とUndo / R4A Inspector oracle** | React chrome + native Stage/Timeline + headless interaction、1 top-level wgpu Surface + 2 native viewport + opaque child WebView islandsを正本化。U0e-2R/U0e-2、GR-D1〜R3、固定MacのG0-9L、R0〜R3を完了済み。R4 readinessでInspectorの独立React source不在とskeleton非採用を確認し、R4SでR4A oracle→R4B mock React化→R4C ownershipへ分割した。現在は`CU-0A07A / R4A`で未変更legacy sourceへ構造・style・ARIA・操作oracleを固定する。Motolii Studio Previewは未実装。visible summary chromeは未決で実装しない。G0-6HはU0e-3を止める並行人間審判、G0-9DはDistribution Ready用hardware gate。eguiへ新規製品面を実装せず、plugin UI公開契約はG0-3 / GAP-13まで停止する |
...
→ G0-9L DONE → R0 source inventory DONE → R1 Browser ownership DONE → R2A/R2B Easing DONE → R3A/R3B KEYS/LAYERS DONE → R4S DONE → R4A DO → R4B/R4C WAIT → rolling VS-1 enabling order
...
|---|---|---|---|---|---|---|
| PRODUCT-ASSET | CU-0A07A | M3 / VS-1 / A / R4A | `DO` | — | CU-0A07SでInspector独立React source不在、skeleton非採用、R4A/B/C分割を固定 | 未変更legacy sourceへ5状態の構造・style・ARIAと主要操作oracleを追加する。archived HTML、threshold、golden、製品source変更はSTOP |
...
したがって現在の短い運用判断は、**CU-0A03 / R0からCU-0A06B / R3BとCU-0A07S / R4Sは完了済み。Browser、Easing trigger、KEYS/LAYERSのproduct単一ownerとmock consumer反転が成立した。Inspectorは独立React source不在のためR4A/B/Cへ分割し、現在粒は`CU-0A07A / R4A`の`DO`で、未変更legacy sourceへ構造・style・ARIA・操作oracleを固定する。archived HTML、threshold、golden、製品source変更が必要ならSTOPする。Motolii Studio Previewは未実装。**G0-6Hは同時に進められる人間審判だが、未完了でもR0〜R4やPreview骨格を止めず、U0e-3だけを止める。G0-9DはDistribution Readyまで`WAIT / HARDWARE`。`U2c-2`はVS-2候補かつ実製品入口待ちである。D1n、D5等の独立follow-upをVS-1の再停止理由へしない。
