...
| M4 | **K0契約凍結済み(test-only) / 既知実装調査・採択地図がNEXT** | K0はRoD/RoIの契約意味をtest-only spikeで凍結済み、K1〜K8は未実装。次の開発成果は[横断開発モデル](known-implementation-adoption-model.md)に従うM4既知実装調査と採択地図であり、既存K0〜K8を実装順として自動採用しない。cache、resource accounting、invalidation、階層退避、proxy、scheduler、bake、全曲Draft等を機構classで裁定してから製品runtimeを発注する。現行`PipelineCache`／dynamic target pool／wgpu budget thresholdをResourceLedger、copy-out、disk store完成と数えない |
| M5 | **`M5-A0T` / `M5-A1` / `M5-A2` / `M5-C0`意味決定＋private fixture / `M5-R0` / `M5-T0` / `M5-P0` / `M5-I0` / `M5-D0` DONE / `M5-A0S` DONE（docs-only）** | [M5-A1 receipt](reviews/evidence/m5-known-implementation/M5-A1/README.md)でGLB import leaf、[M5-A2 receipt](reviews/evidence/m5-known-implementation/M5-A2/README.md)でOBJ lowering／欠落診断、[M5-R0 receipt](reviews/evidence/m5-known-implementation/M5-R0/README.md)でwgpu offscreen route、[M5-T0 receipt](reviews/evidence/m5-known-implementation/M5-T0/README.md)でFontique＋HarfRust KEEPとParley private probe、[M5-P0 receipt](reviews/evidence/m5-known-implementation/M5-P0/README.md)でRoI／linear／quality algorithm contract、[M5-I0 receipt](reviews/evidence/m5-known-implementation/M5-I0/README.md)でobvhs／owned flat semantic一致とstale拒否、[M5-D0 receipt](reviews/evidence/m5-known-implementation/M5-D0/README.md)でstable mixer／PCG32 identity fixture、[M5-C0 receipt](reviews/evidence/m5-known-implementation/M5-C0/README.md)でprojective Observation／typed refusal／provider換装fixtureを確認した。[M5-A0S決定回収](reviews/2026-08-02-m5-a0s-decision-recovery.md)で後発7 blobを縮小採用／観察／棄却へ処分し、[M5-C0意味決定](reviews/2026-08-02-m5-c0-observation-contract-decision.md)で初期projective Observationの意味を閉じた。[M5-C0 schema preflight](reviews/2026-08-02-m5-c0-schema-preflight.md)で公開型／wire／version／provider identityの未決を明示し、コード0で停止線を確定した。schema／runtime、製品text API／material／renderer／post／picking／Duplicator、PBR conformance、GPU pass／readback、3 OSは未実装。次はprovider identity／Document migrationのdecisionを閉じてから`C0-Schema`を再分類し、M4 K1a resource gateをruntime前に維持する |
...
| CORE | CU-201T-C | M3 / VS-2 / Clip trim command and Writer prepare | `DO` | [CU-201T-S契約](reviews/2026-08-01-cu-201t-s-clip-trim-timemap-contract-decision.md) | CU-201T-S + CU-201M-C `DONE` | journal replay、1 Undo、失敗時Document不変 |
...
| 6 | K1a | M4 | `WAIT / ADOPTION-MAP` | K0凍結(test-only)。M4既知実装調査と採択地図でresource accounting／hard budgetの供給route、接続target、oracle、cutoverを裁定するまで起票しない | 採択後の薄い接続だけを再度起票判定。独自ResourceLedgerを前提にしない |
