...
| M3 | **VS-1 Rectangle配置とUndo** | React chrome + native Stage/Timeline + headless interaction、1 top-level wgpu Surface + 2 native viewport + opaque child WebView islandsを正本化。R4CまでのInspector product ownership、CU-0A08IP、`CU-G09`／`CU-G09O`／docs-only `CU-G09R`は完了した。`CU-0A08BP`、docs-only `CU-104`/`U2h-1S`/`CU-104E`、実装粒`U2h-1I`、docs-only `U2h-1PR`/`CU-105R`/`CU-106S`/`U3a-2S0`、docs-only `U3a-2S`、docs-only `U3a-2R`、docs-only `U3a-2Z`は`DONE`。U2h-1P単独producer粒は停止し、CU-105とCU-106を責任分割した。`U2h-1P`/`CU-106P`/`CU-106F`は実consumer surfaceまで`WAIT`、次PRODUCT-ASSET判断は`U3a-2A`の`DO`。`CU-0A08BT`は`WAIT`継続。Motolii Studio Previewは未実装。visible summary chromeは未決で実装しない。G0-6HはU0e-3を止める並行人間審判、G0-9DはDistribution Ready用hardware gate。eguiへ新規製品面を実装せず、plugin UI公開契約はG0-3 / GAP-13まで停止する |
...
| PRODUCT-ASSET | U3a-2A | M3 / VS-1 / SPEC / windowed Timeline renderer adoption decision | `DO` | — | `U3a-2Z`は[発注依存証跡](#発注依存証跡)で`DONE`。着手時に[U3a-2R](reviews/2026-07-27-u3a-2r-renderer-adoption-scope-decision.md) §7 の4条件を再照合 | `direct_vello` / `egui_vello`からrenderer採択判断をdocsで閉じる。起票前にFable 5 read-only助言。egui削除・閾値・Rust/UI変更0 |
...
したがって現在の短い運用判断は、**CU-0A03 / R0からCU-0A07C / R4Cまでは完了済み。CU-0A08IP、`U3a-1S`/`U3a-1I`、`CU-G03D`/`CU-G03R`、親`CU-G03`、`CU-101`、`CU-102`、`CU-G09`/`CU-G09O`/`CU-G09R`、`CU-0A08BP`、docs-only `CU-104`/`U2h-1S`/`CU-104E`、`U2h-1I`、docs-only `U2h-1PR`/`CU-105R`/`CU-106S`/`U3a-2S0`、docs-only `U3a-2S`、docs-only `U3a-2R`、docs-only `U3a-2Z`は`DONE`。`CU-105`/`CU-106`親は`SPLIT`し、`U2h-1P`/`CU-106P`/`CU-106F`は実consumer surfaceまで`WAIT`。次PRODUCT-ASSET判断は`U3a-2A`の`DO`で、renderer採択判断docs粒のみ（Fable 5 read-only助言後。semantic zoom段階内容・playhead/range owner・製品surface実装は束ねない）。CU-109 runtime配線、CU-110 Place、CU-111 prepared Undo/Redoを混ぜない。`CU-0A08BT`/`CU-0A08IT`/`U2c-2`は既存依存待ち。Host transport、typed intent、JSX binding、drag payload、`S`行、Rust/schema/plugin変更をU3a-2Aへ混ぜるならSTOPする。Motolii Studio Previewは未実装。**G0-6HはU0e-3だけを止め、G0-9DはDistribution Readyまで`WAIT / HARDWARE`。D1n、D5等の独立follow-upをVS-1の再停止理由へしない。
