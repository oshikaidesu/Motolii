...
| VISUAL-RESPONSE | G0-6H-V1G-C | M3 evidence / VS-1 / current-route offline capture | `WAIT` | — | `G0-6H-V1G-C-P`のcapture環境authority再締結待ち | 2 mode、5 normal、派生variant、provenance、offline captureを閉じる |
| VISUAL-RESPONSE | G0-6H-V1G-C-P | M3 evidence / VS-1 / current-route capture environment authority correction | `DO` | — | `G0-6H-V1G-I`は[発注依存証跡](#発注依存証跡)で`DONE` | locale/timezone、pinned browser version/revision、font fixture/computed familyの既存authority衝突だけをdocs-onlyで再締結する |
