...
| PRODUCT-ASSET | CU-107S | M3 / VS-1 / SPEC / CU-107 split concretization selection | `DO` | — | `CU-109`／`CU-G04SC`／`CU-110S`／`CU-110D`は[発注依存証跡](#発注依存証跡)で`DONE` | `CU-107`分割の具体化範囲だけを選定する。答え・`CU-107`/`CU-110`/`CU-111`実装・`WAIT`解除は含めない |
