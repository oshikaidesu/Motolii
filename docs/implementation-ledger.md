...
| 2 | CU-0G03H | M3 core | `DO` | — | CU-G01/CU-0G02完了。既存G0-9 surface hostへbounded AX proxy、native↔Web focus移譲、実IME用editable pathとshortcut非漏出観測をdevelopment専用で揃える。製品window・人間合格・L3復旧は含めない | CU-0G03を`DO`へ上げる |
| 3 | CU-0G03 | M3 human | `WAIT` | — | CU-0G03H後、G0-9L local acceptance harnessの実windowでmacOS IME/VoiceOver/focus/keyboard受入を行い、失敗を既知制約へ格下げしない | 受入証跡保存後、CU-0G04を`DO`へ上げる |
