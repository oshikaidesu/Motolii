...
| M3 | **VS-1 Rectangle配置とUndo** | React chrome + native Stage/Timeline + headless interaction、1 top-level wgpu Surface + 2 native viewport + opaque child WebView islandsが正本。React ownership R0〜R6 Browser、Browser/Inspector read-only projection、CU-109 edit durabilityまで成立。G0-6H/CU-0B01はUI作者の人間`ACCEPT`で`DONE`。`CU-0B02T` token authority後、token後続を`WAIT`へ戻し、`CU-0A08BTI`でRectangle Browser Place typed intentを完了。次の唯一のPRODUCT-ASSET `DO`はHost接続契約`CU-0B03H`。独立ORACLE-GUARD `G0-6H-V1G-RP`はPlace順を停止しない。`CU-107PV`は実製品Host再投影`CU-0B05`待ち。Motolii Studio Previewは未実装。G0-9DはDistribution Ready用hardware gate。eguiへ新規製品面を実装せず、plugin UI公開契約はG0-3 / GAP-13まで停止する |
...
| PRODUCT-ASSET | CU-0B03H | M3 / VS-1 / SPEC / H1b Host connection contract | `DO` | [CU-0A08BTI実装決定](reviews/2026-07-29-cu-0a08bti-browser-place-typed-intent-implementation-decision.md) | CU-0A09B + CU-0A08BTI | built-in Browser 1面のoffline bundle / closed Host callback / mountだけを固定 |
...
現在の短い運用判断は、**G0-6H / CU-0B01、CU-0B02T、CU-0A08BTIは`DONE`。token後続R/N/C/Iは`WAIT`。次の唯一のPRODUCT-ASSET `DO`はHost接続契約`CU-0B03H`で、`Browser typed intent → Host → Place → Stage/Timeline/Inspector → Undo`を貫通させる。G0-9DはDistribution Readyまで`WAIT / HARDWARE`。**
