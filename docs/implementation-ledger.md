...
| M3 | **段階発注可 / Uシリーズ直列選択中** | U0a、U0b-1、U0b-2、U0c-1、U0c-2完了。次はU0d-1を単独実行 |
...
Selected U series:
U0a DONE → U0b-1 DONE → U0b-2 DONE → U0c-1 DONE → U0c-2 DONE → U0d-1 → U0d-2 → U0d-3
...
|---|---|---|---|---|---|---|
| 1 | U0d-1 | M3 | `DO` | — | U0c-2完了。builtin baseとuser deltaのin-memory modelだけを作り、codec・保存場所へ広げない | U0d-2をIssue化 |
...
| 3 | U0d-2 | M3 | `WAIT` | U0d-1 merge | version付きJSON codec、migration、原本保全 |
...
したがって現在の短い運用判断は、**M2基盤再締結とD3e、D1m、CAM-G0、D1j、D1k-S、D1k、D3f、M3 U0a、U0b-1、U0b-2、U0c-1、U0c-2は完了済み**。初回Uシリーズは並走させず、次は`U0d-1`だけを進める。Rerunのcommit固定source監査と資産分類は可能だが、現在のUシリーズ実装と並走させない。D5は骨格を完了扱いせず、本番preview／GPU計測／実機E2Eを後続へ残す。
