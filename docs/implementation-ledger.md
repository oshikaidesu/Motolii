...
| M4 | **K0契約凍結済み(test-only) / 初版採択地図済み / Wave A probeがNEXT** | K0はRoD/RoIの契約意味をtest-only spikeで凍結済み、K1〜K8は未実装。[調査](reviews/2026-08-02-m4-known-implementation-survey.md)と[13親の採択地図](m4-known-implementation-adoption-map.md)は作成済み。[disk再検索](reviews/2026-08-02-m4-disk-artifact-store-resurvey.md)でglobal CASを過剰仕様として外し、verified recipe artifact storeへREDUCEした。次はRAM、filesystem artifact、区間、priority、watermark、SVG、copy-outの独立`ADOPTION_PROBE`であり、採択依存と薄い製品接続は別粒にする。現行`PipelineCache`／dynamic target pool／wgpu budget thresholdをResourceLedger、copy-out、disk store完成と数えない |
