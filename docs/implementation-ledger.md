...
| PRODUCT-ASSET | CU-206 | M3 / VS-2 / PRODUCT / ordinary native Timeline fixed viewport and two-axis pan parent | `SPLIT` | [U3a-2Q-V ordinary Timeline viewport決定](reviews/2026-08-02-u3a-2q-v-ordinary-timeline-viewport-decision.md) | `U3a-2Q-V` | `CU-206I`はlint抑制/derived targetでCANCELLED・採用0。`CU-206C`へREMAPしreceipt前に親DONEにしない |
| PRODUCT-ASSET | CU-206C | M3 / VS-2 / PRODUCT / ordinary Timeline Project-session viewport connection | `DO` | — | `U3a-2Q-V`は[発注依存証跡](#発注依存証跡)で`DONE` | approved raw adapter→private session viewport→projection/hit/move/trim/snap→renderer。通常製品window E2Eを所有 |
