...
| M3 | **VS-1 Rectangle配置とUndo** | Browser typed intent→offline native Host WebView→exact codec/bounded inboxまで`CU-0B03`で成立。実MacでHTML5 drag後のreleaseがegui Stageへ届かない停止点を再現し、`CU-0B04S`でReact=start identity、Host private platform capture、native Stage=latest-epoch hit-test/canonical変換へ再締結した。token後続は`WAIT`。次は横展開せず`CU-0B04P`を実装する。Motolii Studio Previewは未実装。G0-9DはDistribution Ready用hardware gate。eguiへ新規製品面を実装せず、plugin UI公開契約はG0-3 / GAP-13まで停止する |
...
| PRODUCT-ASSET | CU-0B04P | M3 / VS-1 / B / Host private platform capture seam | `DO` | [CU-0B04S再締結](reviews/2026-07-29-cu-0b04s-browser-native-place-terminal-ownership-reclosure-decision.md) | CU-0B04S | macOS platform adapter + toolkit非依存state machine。Transient pointer/release/cancel候補のみ。Document/D2/Undo/Stage admission 0 |
...
現在の短い運用判断は、**Browser typed intent→offline native Host WebView→bounded inboxと、private Place core→same LayerId publish→Vector Rect canonical GPU renderは個別に実装済み。ただし実MacでWebView跨ぎreleaseがegui Stageへ届かず、両者は未貫通。CU-0B04Sで終端所有を再締結し、次はCU-0B04P Host private platform capture seamを実装する。token後続は`WAIT`。**
