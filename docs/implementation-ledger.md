...
| M3 | **VS-1 Rectangle配置とUndo `DONE` / VS-2 U4a-1 `DONE` / CU-0B02R `DONE` / CU-0B02C-P/V `DONE` / CU-203 `DONE` / CU-204S/A `DONE` / CU-205S/B1G/B1I/B2/B/T/P/W/E `DONE`** | `CU-108`で通常製品routeのBrowser Rectangle→D2/journal→Stage/Timeline/Inspector同一snapshot→Undo/Redoを実Mac完走した。`U4a-1` / `CU-202`はcommit `eb4e6658`でparameter control modelを閉じた。`CU-0B02R`はcommit `782ffa3b`でaccepted routeのproduct token consumerを閉じた。`CU-0B02C-P`は固定primitive 2 path / 7 exportをproduct ownerへ直接移管し、`CU-0B02C-V`は第二consumer比較後もprivate carry全件維持、公開token追加0で閉じた。`CU-203M / P`でfeedback source確立からproduct直接移管、9-state matrix、copy 0を閉じた。`CU-204A`で5 reason × 4 densityのprivate純粋投影を閉じた。`CU-205S`でfirst-party Opacity正常系をB1G/B1I/B2/T/P/W/Eへ分割し、commit `a29ed709`でT、commit `c0c1c741`でP、rolling `ca7f105a`〜`18517062`でW、`581876e0` / `7901e51e`を含む通常Mac製品window E2EでEを閉じた。`CU-204P`は実在通常source待ち、親`CU-205` / `U4a-2`は`SPLIT / WAIT`。Motolii Studio Previewは用語正本上未実装、G0-9DはDistribution Ready用hardware gate、plugin UI公開契約はG0-3 / GAP-13まで停止する |
...
| PRODUCT-ASSET | U3a-2Q-V | M3 / VS-1 / SPEC / ordinary Timeline visible range owner and fixed viewport | `DONE` | [U3a-2Q-V ordinary Timeline viewport決定](reviews/2026-08-02-u3a-2q-v-ordinary-timeline-viewport-decision.md) | native Timeline、production pointer、selection、move / trim / snap consumer成立 | visible rangeをProject sessionへ採択。34 logical px row、80 logical px/s、2軸scroll、Document/Undo/保存不変を固定 |
| PRODUCT-ASSET | CU-206 | M3 / VS-2 / PRODUCT / ordinary native Timeline fixed viewport and two-axis pan parent | `SPLIT` | [U3a-2Q-V ordinary Timeline viewport決定](reviews/2026-08-02-u3a-2q-v-ordinary-timeline-viewport-decision.md) | `U3a-2Q-V` | `CU-206I → CU-206R → CU-206C`へ分割。全子receipt前に親DONEにしない |
| PRODUCT-ASSET | CU-206I | M3 / VS-2 / PRODUCT / bounded native Timeline two-axis scroll input carrier | `DO` | — | `U3a-2Q-V`は[発注依存証跡](#発注依存証跡)で`DONE` | 既存AppKit local monitor / Browser Host private pollだけ。Document、renderer、winit lifecycle adapter、公開input API変更0 |
| PRODUCT-ASSET | CU-206R | M3 / VS-2 / PRODUCT / fixed-row fixed-time native Timeline renderer viewport | `WAIT` | — | `CU-206I` | 34 logical px row、80 logical px/s、ruler/bar/key/playhead同一viewport。Product接続はCU-206C |
| PRODUCT-ASSET | CU-206C | M3 / VS-2 / PRODUCT / ordinary Timeline Project-session viewport connection | `WAIT` | — | `CU-206I` + `CU-206R` | private session state→projection/hit/move/trim/snap→renderer。通常製品window E2Eを所有 |
...
| PRODUCT-ASSET | CU-0A08IT | M3 / VS-1 / B / Direct Inspector connection | `SPLIT` | [CU-0A08ITP実装決定](reviews/2026-07-29-cu-0a08itp-inspector-read-projection-jsx-connection-implementation-decision.md) | CU-0A08ITP → CU-0A08ITI | read-only projection/JSX connectionとtyped intent/Host接続を分離。ITPはU4a-2非依存、ITIは既存Direct製品入口待ち |
...
| PRODUCT-ASSET | CU-0A08ITI | M3 / VS-1 / B / Inspector typed intent and Host connection | `WAIT` | — | CU-0A08ITP + U4a-2 Direct製品入口未成立 | Host transport・typed intentだけを所有。Advanced入口のU4cはU2c-2依存 |
...
| CORE | CU-201T-C | M3 / VS-2 / Clip trim command and Writer prepare | `DONE` | [CU-201T-S契約](reviews/2026-08-01-cu-201t-s-clip-trim-timemap-contract-decision.md) | commit `a860e10e`、doc/UI tests、clippy | journal replay、1 Undo、失敗時Document不変 |
| PRODUCT-ASSET | CU-201N-S | M3 / VS-2 / SPEC / snap target priority and unit | `DONE` | [CU-201N-S契約](reviews/2026-08-01-cu-201n-s-timeline-snap-contract-decision.md) | frame grid + other Clip edge、8 logical px、deterministic tie、no-snap/stale/cancel | provider不在playhead、U7 beat/marker、汎用snap機構0 |
| PRODUCT | CU-201P | M3 / VS-2 / native Timeline interval gesture | `DONE` | [CU-201P実装決定](reviews/2026-08-01-cu-201p-native-timeline-interval-gesture-implementation-decision.md) | native press/move/release、transient preview、frame/別Clip edge snap、shared writer、通常製品実操作 | drag write 0、成功release 1 Undo、Cancel/stale/outside 0 |
| ORACLE-GUARD | CU-201R | M3 / VS-2 / random move trim sequence | `DONE` | [CU-201R系列oracle](reviews/2026-08-01-cu-201r-timeline-interval-sequence-oracle-decision.md) | fixed-seed 192-step × 3 Clip、全Undo、reopen、cancel | 重複0、move平行移動、初期bytes一致、Cancel queue 0 |
| E2E | CU-201E | M3 / VS-2 / normal product move trim reopen | `DONE` | [CU-201E E2E](reviews/2026-08-01-cu-201e-timeline-interval-normal-product-e2e-receipt.md) | current binary + normal product window + fixed project/WAL hashes | LayerId 0不変、保存interval再表示、UI drag state非永続 |
| CORE | U4b-0 | M3 / VS-2 / Add Position Key prerequisite | `SPLIT` | [U4b-0 closed contract](reviews/2026-08-02-u4b0-add-position-key-closed-contract-decision.md) | S/C DONE → product connection | raw allocator / SetProperty、solver複製、Auto Key 0 |
| PRODUCT-ASSET | U4b-0S | M3 / VS-2 / SPEC / Add Position Key closed contract | `DONE` | [U4b-0 closed contract](reviews/2026-08-02-u4b0-add-position-key-closed-contract-decision.md) | command payload、prepare result、Bezier split、区間外、version、拒否順 | code変更0。次はCORE U4b-0C |
| CORE | U4b-0C | M3 / VS-2 / Add Position Key command and prepare | `DONE` | [U4b-0 closed contract](reviews/2026-08-02-u4b0-add-position-key-closed-contract-decision.md) | `motolii-eval::Interp::split_at_progress` + `Command::{AddPositionKey, UndoAddPositionKey}` + `DocumentWriter::prepare_add_position_key` | one ID、one Undo/Redo、same-time no-op、curve同値、stale atomic拒否、JournalEdit v2 roundtrip |
| PRODUCT | U4b-0P | M3 / VS-2 / Add Position Key normal product connection | `DONE` | [U4b-0P接続・E2E](reviews/2026-08-02-u4b0p-product-add-position-key-connection-decision.md) | existing Inspector Position mark appearance → layer/generation identity → Host current playhead → existing AddPositionKey D2。通常製品windowで0秒/5秒の2 key、`◆ 2`、Easing popup入口を確認 | Position行だけ、copy / mock state / React evaluator / playhead owner / Auto Key / new API 0 |
| PRODUCT-ASSET | U4b-1S | M3 / VS-2 / SPEC / outgoing Interp durable meaning | `DONE` | [U4b-1 outgoing Interp契約](reviews/2026-08-02-u4b1-outgoing-interp-command-contract-decision.md) | layer + adjacent left/right key identity、old/new Interp、same-value、拒否順、active interval projection | code変更0。次はCORE U4b-1C |
| CORE | U4b-1C | M3 / VS-2 / outgoing Interp command and active interval | `DONE` | [U4b-1 outgoing Interp契約](reviews/2026-08-02-u4b1-outgoing-interp-command-contract-decision.md) | `Command::SetPositionKeyInterp` + Writer prepare + `project_position_interval` | 1 command/Undo/Redo、key payload不変、stale successor/old atomic拒否、JournalEdit v2 replay |
| PRODUCT | U4b-1P | M3 / VS-2 / normal product Easing connection | `WAIT-HUMAN` | [U4b-1製品Easing接続](reviews/2026-08-02-u4b1-product-easing-connection-implementation-decision.md) | existing React trigger → Host exact interval/layout → native G0-9 port → existing D2 release 1 commit | 自動検証DONE。2-key実projectの通常Mac製品windowでpopup、Undo/Redo、Save/reopenを人間受入後にDONE |
| PRODUCT | CU-210P | M3 / VS-2 / paused playhead product connection | `DONE` | [CU-210P接続・E2E](reviews/2026-08-02-cu-210p-paused-playhead-product-connection-decision.md) | ruler click → Project-session playhead → native / Stage / render同時刻。通常製品windowで0→5秒、playhead中央、Stage更新 | playback、audio clock、GPU timestamp、drag scrub、永続化0。Placeは既存zero oracle維持 |
...
| AUTHORING-SDK | SDK-S0I | Vism / language-neutral Path2D semantic fixture implementation | `DO` | — | SDK-S0S独立review `ACCEPT` P0=0/P1=0 | positive 4、negative 7、contract projection、native oracle同値をtest-onlyで閉じる |
...
| CU-0A08IT | `SPLIT` | [CU-0A08ITP実装決定](reviews/2026-07-29-cu-0a08itp-inspector-read-projection-jsx-connection-implementation-decision.md)でread-only projection/JSX接続とtyped intent/Host接続を分離 |
| CU-0A08ITP | `DONE` | decoded target既決3 fieldを既存installed identity JSXへ接続し、専用provenance正負guardを完了 |
| CU-0A08ITI | `WAIT` | U4a-2 Direct製品入口待ち。Host transportとtyped intentだけを所有 |
...
| U3a-2Q-V | `DONE` | [U3a-2Q-V ordinary Timeline viewport決定](reviews/2026-08-02-u3a-2q-v-ordinary-timeline-viewport-decision.md)で実在native consumerへvisible range ownerを`Project session`として採択し、固定行高・固定時間scale・2軸scroll・Document不変を閉じた |
