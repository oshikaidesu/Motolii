...
| ORACLE-GUARD | CU-0A08IR | M3 prerequisite / Inspector authority hash reclosure | `DO` | — | `b0c0f916`でInspector分割決定の進行状態とDirect/U4c依存整理が正当に更新された一方、inventory guardの固定hashが旧blobのままであることを再現。CU-0A08IPの意味・実装は変更しない | authority本文を変更せず、guardの固定hashだけを現行blobへ再締結して専用testと全reference guardを緑へ戻す。CU-0A08BPとは別ticket・別commit |
