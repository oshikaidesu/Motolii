...
| PRODUCT-ASSET | CU-110PIR | M3 / VS-1 / PRODUCT / Inspector safe read-only target branch | `DO` | — | `CU-110PI` | 同じInspectorCandidate内、target 3 fieldだけ。mock state / S値 / editing callback /別copy 0 |
| PRODUCT-ASSET | CU-110PIH | M3 / VS-1 / PRODUCT / Inspector Host island projection | `WAIT` | — | `CU-110PIR` | 予約済みright rect、offline second WebView、current Document + primary。public wire / intent / second owner 0 |
