...
| M3 | **VS-1 Rectangle配置とUndo** | React chrome + native Stage/Timeline + headless interaction、1 top-level wgpu Surface + 2 native viewport + opaque child WebView islandsが正本。React ownership R0〜R6 Browser、Browser/Inspector read-only projection、CU-109 edit durabilityまで成立。G0-6H/CU-0B01はUI作者の人間`ACCEPT`で`DONE`。`CU-0B02S`で親U0e-3を分割し、`CU-0B02T`でproduct-owned Dark DTCGとgenerator v2 Rust/CSS/manifestを完了。token後続は`WAIT`へ戻し、次の唯一のPRODUCT-ASSET `DO`はPlace配送連鎖の先頭`CU-107PV`。独立ORACLE-GUARD `G0-6H-V1G-RP`はcurrent-route provenance再publication debtを所有するがPlace順を停止しない。`CU-0A08BTI`/`CU-0A08ITI`/`U3a-2Q-V`は既存依存待ち。Motolii Studio Previewは未実装。G0-9DはDistribution Ready用hardware gate。eguiへ新規製品面を実装せず、plugin UI公開契約はG0-3 / GAP-13まで停止する |
...
| PRODUCT-ASSET | CU-107PV | M3 / VS-1 / CORE / nonterminal Place preview delivery | `DO` | [CU-107N決定](reviews/2026-07-28-cu-107n-cu-107-narrow-prerequisite-closed-set.md) | 既決順`PV→TC→AD→TD`の先頭。一active dragのnonterminal previewだけを成立 | terminal生成、test/dummy到達性、Document/journalへのtransport ID保存は禁止 |
...
| CU-0B02T | `DONE` | [CU-0B02T実装決定](reviews/2026-07-29-cu-0b02t-product-token-authority-implementation-decision.md)でDark DTCG単一正本、profile分離generator v2、Rust/CSS/manifest、supplier/手編集負例を閉じた。token後続を`WAIT`へ戻し、次PRODUCT-ASSET `DO`をPlace先頭`CU-107PV`へ送った |
...
現在の短い運用判断は、**G0-6H / CU-0B01は人間`ACCEPT`、`CU-0B02T`はDark token authority実装で`DONE`。親`CU-0B02` / `U0e-3`は`SPLIT`、token後続R/N/C/Iは`WAIT`。次の唯一のPRODUCT-ASSET `DO`はPlace配送先頭`CU-107PV`で、`PV→TC→AD→TD→CU-110→CU-0A08BTI`を貫通させる。G0-9DはDistribution Readyまで`WAIT / HARDWARE`。**
