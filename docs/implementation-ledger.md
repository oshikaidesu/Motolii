...
| M3 | **VS-1 Rectangle配置とUndo** | Browser typed intent→offline native Host WebView→exact codec/bounded inboxまで`CU-0B03`で成立。実MacでHTML5 drag後のreleaseがegui Stageへ届かない停止点を再現し、`CU-0B04S`で終端所有を再締結、`CU-0B04P`でHost private platform capture seamを実装した。token後続は`WAIT`。次は横展開せず`CU-0B04N`でnative Stage viewportとlatest-epoch hit-testを接続する。Motolii Studio Previewは未実装。G0-9DはDistribution Ready用hardware gate。eguiへ新規製品面を実装せず、plugin UI公開契約はG0-3 / GAP-13まで停止する |
...
| PRODUCT-ASSET | CU-0B04N | M3 / VS-1 / B / native Stage viewport and latest-epoch hit-test | `DO` | [CU-0B04S再締結](reviews/2026-07-29-cu-0b04s-browser-native-place-terminal-ownership-reclosure-decision.md) | CU-0B04P + native renderer adoption。CU-0B02N visual token consumerはgeometry/inputの入場条件にしない | 1 top-level Surface内Stage viewport、layout epoch、Host candidate hit-test/canonical変換。D2/Undo、別Surface、egui product terminal 0 |
...
現在の短い運用判断は、**Browser typed intent→offline native Host WebView→bounded inbox→Host private platform capture candidateと、private Place core→same LayerId publish→Vector Rect canonical GPU renderは個別に実装済み。次はCU-0B04Nでnative Stage viewportとlatest-layout-epoch hit-test/canonical変換を接続する。token後続は`WAIT`。**
