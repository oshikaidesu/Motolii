...
| VISUAL-RESPONSE | G0-6H-V1ETB-Q | M3 evidence / VS-1 / Browser route oracle allowlist correction | `DO` | — | V1ETA A-4とV1ETB-P最終8点をCodex再照合 | 既存current-route PlaywrightをV1ETB実装allowlistへ追加するdocs-only補正 |
| VISUAL-RESPONSE | G0-6H-V1ETB | M3 evidence / VS-1 / Browser projected Media face | `WAIT` | — | `G0-6H-V1ETB-Q`完了待ち | Browser seam、R-9、post-promotion provenance/guardをrouteで閉じる |
