...
| VISUAL-RESPONSE | G0-6H-V1G-O | M3 evidence / VS-1 / current-route immutable publication | `WAIT` | — | `G0-6H-V1G-O-H`のcommand authority hash再締結待ち | 別root manifestと30 PNGのimmutable publication/read-only照合を閉じる |
| VISUAL-RESPONSE | G0-6H-V1G-O-H | M3 evidence / VS-1 / current-route command authority hash correction | `DO` | — | `G0-6H-V1G-C`は[発注依存証跡](#発注依存証跡)で`DONE` | 決定済み2 commandのpackage wiringと3既存authority hash guardだけを同時に再締結する |
