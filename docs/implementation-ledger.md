...
| PRODUCT-ASSET | CU-110PS | M3 / VS-1 / PRODUCT / native Stage published snapshot projection | `DO` | — | `CU-110P`、`CU-110` | 既存latest-only render worker→同じVRAM display slot。CPU readback、別renderer、Timeline/Inspector/Undo 0 |
| PRODUCT-ASSET | CU-110PT | M3 / VS-1 / PRODUCT / native Timeline published snapshot projection | `WAIT` | — | `CU-110PS` + U3a-1I/U3a-2A | 既存headless projectionのnon-test callerとnative bar描画。selection入力、React Timeline 0 |
