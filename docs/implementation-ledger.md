...
| PRODUCT-ASSET | CU-106S | M3 / VS-1 / SPEC / selection consumer split recheck | `READY-RECHECK` | — | `CU-104`／`U2h-1I`／`U2h-1PR`／`CU-105R`は`DONE` | primary/focusを分離し、U2h-1P producerと同じ差分で成立する最小production callerの有無を確認。lint抑制、dummy caller、公開intent追加は禁止 |
...
したがって現在の短い運用判断は、**CU-0A03 / R0からCU-0A07C / R4Cまでは完了済み。CU-0A08IP、`U3a-1S`/`U3a-1I`、`CU-G03D`/`CU-G03R`、親`CU-G03`、`CU-101`、`CU-102`、`CU-G09`/`CU-G09O`/`CU-G09R`、`CU-0A08BP`、docs-only `CU-104`/`U2h-1S`/`CU-104E`、`U2h-1I`、docs-only `U2h-1PR`、docs-only `CU-105R`は`DONE`。`CU-105`親はU3a-1I成立済み責任、既存capacity evidence、U3a-2 semantic zoom、CU-106-familyへ`SPLIT`した。`U2h-1P`はP5受入IDとしてCU-106 primary-selection sliceまで`WAIT`。次PRODUCT-ASSET判断は`CU-106S`の`READY-RECHECK`で、primary/focusと最小production callerを再確認する。CU-109 runtime配線、CU-110 Place、CU-111 prepared Undo/Redoを混ぜない。`CU-0A08BT`/`CU-0A08IT`/`U2c-2`は既存依存待ち。Host transport、typed intent、JSX binding、drag payload、`S`行、Rust/schema/plugin変更をCU-105R/CU-106Sへ混ぜるならSTOPする。Motolii Studio Previewは未実装。**G0-6HはU0e-3だけを止め、G0-9DはDistribution Readyまで`WAIT / HARDWARE`。D1n、D5等の独立follow-upをVS-1の再停止理由へしない。
