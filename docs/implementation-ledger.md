...
| M3 | **VS-1 Rectangle配置とUndo** | Browser typed intent→offline native Host WebView→exact codec/bounded inbox→Host private platform capture→native Stage latest-epoch hit-test→1 Surface上のStage/Timeline→opaque Browser islandのfocus/geometry epochまで`CU-0B04R`で成立。token後続は`WAIT`。次は横展開せず`CU-0B05`でHost snapshot再投影E2Eを閉じる。D2/Undoまで未貫通のためMotolii Studio Preview完成とは扱わない。G0-9DはDistribution Ready用hardware gate。eguiへ新規製品面を実装せず、plugin UI公開契約はG0-3 / GAP-13まで停止する |
...
| PRODUCT-ASSET | CU-0B05 | M3 / VS-1 / E2E / Browser Host snapshot lifecycle reprojection | `DO` | [CU-0B05S契約決定](reviews/2026-07-29-cu-0b05s-browser-lifecycle-reprojection-contract-decision.md) | CU-0B04N + CU-0B04R + CU-0B05S | reload/crash/focus/resize後にHost projectionをnew instanceへ再投影し、old epoch拒否。surface間state同期、Document/history変更 0 |
| PRODUCT-ASSET | CU-107PV | M3 / VS-1 / CORE / nonterminal Place preview delivery | `WAIT` | [CU-107N決定](reviews/2026-07-28-cu-107n-cu-107-narrow-prerequisite-closed-set.md) | 実製品Host再投影CU-0B05待ち | terminal生成、test/dummy到達性、Document/journalへのtransport ID保存は禁止 |
