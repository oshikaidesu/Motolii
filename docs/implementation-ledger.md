...
| M3 | **UI責任境界・surface topology・G0-9段階化決定 / CU-0G05L判定可** | React chrome + native Stage/Timeline + headless interaction、1 top-level wgpu Surface + 2 native viewport + opaque child WebView islandsを正本化。U0a〜U0e-2、U1a-1/2、U1b-1/2、U2a-0/1、U2b-1、U2c-1/4、U0e-2R、GR-D1〜R3、CU-0G02〜04Bは完了済み。[G0-9段階化決定](reviews/2026-07-23-m3-g0-9-staged-platform-gates.md)でfixed-Mac prerequisite evidence `G0-9L`とdistribution `G0-9D`を分ける。CU-0G04Bは既存surface-hostの一続きの実window実行でminimize/restore、fullscreen往復、1×1一時不可用、pointer capture/cancel/focus lossを既存L3項目と同じraw manifestへ閉じ、Grok R2 P0/P1=0でACCEPTした。追補は`FROZEN / DELETE-LATER`で製品へimportしない。G0-9LはCU-0G05Lの反対側reviewと限定確定までPASSにせず、W0b、H1b、Motolii Studio Preview、window結合を停止する。G0-6Hも独立した人間審判としてU0e-3とW0bの製品前提を停止する。native layout/hit-test/gesture kernelはtoolkit/renderer非依存で進められる。plugin UI公開契約は分離したG0-3 / GAP-13まで停止 |
...
| 7 | CU-0G05L | M3 spec | `DO` | — | CU-0G02〜04Bのraw evidenceを固定Mac manifestへ束ね、反対側review P0/P1=0後だけG0-9Lを限定確定。製品粒・Windows・追加hardwareは解禁しない | 合格後もW0b/H1b/製品windowを自動解禁しない |
