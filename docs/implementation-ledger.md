...
| VISUAL-RESPONSE | G0-6H-V1EB | M3 evidence / VS-1 / Browser development projection decoder | `DO` | — | `G0-6H-V1R` R-11。既存read-model decoder先例とpure testで閉集合・拒否を判定 | product Browserの非公開development projection decoderだけを閉じる。component byte、provenance/guard、R-9実描画はV1ETへ |
| VISUAL-RESPONSE | G0-6H-V1ET | M3 evidence / VS-1 / mock empty projection and ready oracle | `WAIT` | — | `G0-6H-V1EB`完了待ち | Browser入力seamとprovenance/guard再締結、Vite mode carrier、Browser R-9実描画、Stage/Inspector/Timeline空投影、ready oracleをroute-level Playwrightで閉じる |
| VISUAL-RESPONSE | G0-6H-V1G | M3 evidence / VS-1 / current-route immutable generation | `WAIT` | — | `G0-6H-V1ET`完了待ち | 5 normal / 30 PNG、別root manifest、offline capture、read-only照合を閉じる |
