...
| M3 | **VS-1 Rectangle配置とUndo** | React chrome + native Stage/Timeline + headless interaction、1 top-level wgpu Surface + 2 native viewport + opaque child WebView islandsを正本化。R4CまでのInspector product ownership、CU-0A08IP、`CU-G09`／`CU-G09O`／docs-only `CU-G09R`は完了した。`CU-0A08BP`は`DONE`（product-owned・非export の Browser catalog decoder、fixture/testのみ）。selection publish envelopeを決定したdocs-only `CU-104`、docs-only `U2h-1S`、docs-only `CU-104E`、実装粒`U2h-1I`は`DONE`。U2h-1P事前審査でproduction caller不在とlint抑制必須を検出したため、docs-only `U2h-1PR`で単独producer粒を停止した。`U2h-1P`はCU-105再確認後のCU-106 primary sliceまで`WAIT`、次PRODUCT-ASSET判断は`CU-105`の`READY-RECHECK`。`CU-0A08BT`は`WAIT`継続。Motolii Studio Previewは未実装。visible summary chromeは未決で実装しない。G0-6HはU0e-3を止める並行人間審判、G0-9DはDistribution Ready用hardware gate。eguiへ新規製品面を実装せず、plugin UI公開契約はG0-3 / GAP-13まで停止する |
...
| PRODUCT-ASSET | U2h-1P | M3 / VS-1 / B / selection-only primary input acceptance | `WAIT` | — | `U2h-1I` `DONE` + `CU-105`再確認 + CU-106 primary-selection slice | P5受入IDとしてCU-106の最初のprimary sliceへ統合。producerと実在する最小callerを同じ差分で成立させ、lint抑制・dummy caller・公開intent追加を行わない |
| PRODUCT-ASSET | CU-105 | M3 / VS-1 / B / dense Timeline projection recheck | `READY-RECHECK` | — | `U3a-1I`は[発注依存証跡](#発注依存証跡)で`DONE`。既存CU-105行と現行`timeline_projection`の責任重複を再確認する | `PASS / REDUCE / STOP`を選び、1000 clip/100k key、zoom境界、selection/playhead/range不変の未到達だけを次粒へ閉じる。重複projection/hit-test/fixtureは禁止 |
...
したがって現在の短い運用判断は、**CU-0A03 / R0からCU-0A07C / R4Cまでは完了済み。CU-0A08IP、`U3a-1S`/`U3a-1I`、`CU-G03D`/`CU-G03R`、親`CU-G03`、`CU-101`、`CU-102`、`CU-G09`、`CU-G09O`、`CU-G09R`、`CU-0A08BP`、docs-only `CU-104`、docs-only `U2h-1S`、docs-only `CU-104E`、`U2h-1I`、docs-only `U2h-1PR`は`DONE`。U2h-1P事前審査でproduction caller不在とlint抑制必須を検出したため、[selection入力到達性決定](reviews/2026-07-27-u2h-1p-selection-input-reachability-decision.md)で単独producer粒を停止した。`U2h-1P`はP5受入IDとしてCU-105再確認後のCU-106 primary-selection sliceまで`WAIT`。次PRODUCT-ASSET判断は`CU-105`の`READY-RECHECK`であり、完了済みU3a-1Iとの差分を`PASS / REDUCE / STOP`で処分する。CU-109 runtime配線、CU-110 Place、CU-111 prepared Undo/Redoを混ぜない。`CU-0A08BP`はproduct-owned・非export の Browser catalog decoder、fixture/testのみ。`CU-0A08BT`は`WAIT`（CU-0A08BP + `U4a-2` Direct製品入口待ち）、`CU-0A08IT`は`WAIT`（`U4a-2` Direct製品入口待ち）。`U2c-2`は`WAIT`（U4a-2 Direct + U4c Advanced）。Host transport、typed intent、JSX binding、drag payload、`S`行の意味決定、Rust/schema/plugin変更をU2h-1PR/CU-105再確認へ混ぜるならSTOPする。Motolii Studio Previewは未実装。**G0-6Hは同時に進められる人間審判だが、未完了でもR0〜R4やPreview骨格を止めず、U0e-3だけを止める。G0-9DはDistribution Readyまで`WAIT / HARDWARE`。`U2c-2`はVS-2候補かつ実製品入口待ちである。D1n、D5等の独立follow-upをVS-1の再停止理由へしない。
