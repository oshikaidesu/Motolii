...
| M5 | **identity meaning decision可 / Render Contribution共通基盤DONE・Godot DONE・fragment配置DO / fixture WAIT** | P0I自身が所有するcontinuity／transform／nested identity／寿命／cache入力境界／PRNG処分をdocsで先に閉じ、TextCluster内部写像とPrototype ownerは明示留保する。P2D周辺は`P2D-RC0`、`P2D-RCE0`、`GR-D4`を維持する。RCC3-GODOTはACCEPT、RCA3は検収中。RCB3/BevyはREJECT、UnrealはDESIGN_STOPとし、旧差分を使わず固定fragment配置RCB4/RCC4-BEVY/UNREALへ縮小。横断`RCC5`、統合`P2D-RCI`、公開API、Document、plugin契約、実装はWAIT。P6のfontique／harfrust／Vello text stackは未実装で、K6とpremul adapterを重複実装しない |
...
| RENDER-CONTRIBUTION-RERUN | P2D-RCB4 | M5 / P2D / fixed Rerun fragment map | `DO` | — | P2D-RC0/RCE0、GR-D4、固定fragment | 配置だけ |
| RENDER-CONTRIBUTION-ENGINE | P2D-RCC4-BEVY | M5 / P2D / Bevy fragment map | `DO` | — | P2D-RCE0、GR-D4、固定fragment | 配置だけ |
| RENDER-CONTRIBUTION-ENGINE | P2D-RCC4-UNREAL | M5 / P2D / Unreal fragment map | `DO` | — | P2D-RCE0、GR-D4、固定fragment | 配置だけ |
| RENDER-CONTRIBUTION-ENGINE | P2D-RCC5 | M5 / P2D / fixture comparison | `WAIT` | — | P2D-RCC3-GODOT、P2D-RCC4-BEVY/UNREAL | provider三件後に登録 |
