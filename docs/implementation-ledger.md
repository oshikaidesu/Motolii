...
| M3 | **VS-1 Rectangle配置とUndo / R4 Inspector再確認** | React chrome + native Stage/Timeline + headless interaction、1 top-level wgpu Surface + 2 native viewport + opaque child WebView islandsを正本化。U0e-2R/U0e-2、GR-D1〜R3、固定MacのG0-9L、R0〜R3を完了済み。R3Bは[#350](https://github.com/oshikaidesu/Motolii/pull/350)でKEYS/LAYERS JSX/CSSをbyte同一で`ui/motolii-web`へ移し、mockをconsumerへ反転、Timeline state ownerと二重copy 0を閉じた。次は`CU-0A07 / R4 Inspector`を`READY-RECHECK`として、legacy parity oracleと正しい同形React sourceの有無を再確認する。Motolii Studio Previewは未実装。visible summary chromeは未決で実装しない。G0-6HはU0e-3を止める並行人間審判、G0-9DはDistribution Ready用hardware gate。eguiへ新規製品面を実装せず、plugin UI公開契約はG0-3 / GAP-13まで停止する |
...
→ G0-9L DONE → R0 source inventory DONE → R1 Browser ownership DONE → R2A/R2B Easing DONE → R3A/R3B KEYS/LAYERS DONE → R4 Inspector READY-RECHECK → rolling VS-1 enabling order
...
|---|---|---|---|---|---|---|
| PRODUCT-ASSET | CU-0A07 | M3 / VS-1 / A / R4 | `READY-RECHECK` | — | [#350](https://github.com/oshikaidesu/Motolii/pull/350)でCU-0A06Bを完了。KEYS/LAYERS product単独owner、mock consumer反転、Timeline state owner、Grok ACCEPTを固定 | Inspectorのlegacy parity oracle、独立React source、CSS/model/test closureを再確認する。正しい同形source不在ならmock内React化とownershipを分割し、skeleton代用やlegacy runtime importはSTOP |
...
したがって現在の短い運用判断は、**CU-0A03 / R0からCU-0A06B / R3Bまでは完了済み。Browser、Easing trigger、KEYS/LAYERSのproduct単一ownerとmock consumer反転が成立した。現在粒は`CU-0A07 / R4 Inspector`の`READY-RECHECK`で、legacy parity oracleと正しい同形React sourceの有無、CSS/model/test closure、state ownerを再確認する。skeleton代用、legacy runtime import、未決fieldの意味発明が必要ならSTOPする。Motolii Studio Previewは未実装。**G0-6Hは同時に進められる人間審判だが、未完了でもR0〜R4やPreview骨格を止めず、U0e-3だけを止める。G0-9DはDistribution Readyまで`WAIT / HARDWARE`。`U2c-2`はVS-2候補かつ実製品入口待ちである。D1n、D5等の独立follow-upをVS-1の再停止理由へしない。
