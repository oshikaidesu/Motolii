...
| M5 | **identity meaning decision可 / 既知実装調査・採択地図がruntime前提** | P0I等の意味decisionとtest-only fixtureは継続できる。製品runtimeは[横断開発モデル](known-implementation-adoption-model.md)に従い、scene/object、camera observation、spatial renderer、glTF、depth、text、Vello、post、picking/gizmo/bounds等の既知実装調査とM5採択地図を閉じてから発注する。P0I〜P7を独自3D engine／scene framework／text stackの実装列にせず、P6はK6とpremul adapterを重複実装しない |
