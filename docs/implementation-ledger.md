...
| M3 | **VS-1 Rectangle配置とUndo** | React chrome + native Stage/Timeline + headless interaction、1 top-level wgpu Surface + 2 native viewport + opaque child WebView islandsを正本化。R4CまでのInspector product ownership、CU-0A08IP、`CU-G09`／`CU-G09O`／docs-only `CU-G09R`は完了した。`CU-0A08BP`は`DONE`（product-owned・非export の Browser catalog decoder、fixture/testのみ）。selection publish envelopeを決定したdocs-only `CU-104`も`DONE`で、[primary selection implementation split決定](reviews/2026-07-27-u2h-1-primary-selection-implementation-split-decision.md)を行ったdocs-only `U2h-1S`も`DONE`。U2h-1I事前審査で`projection_generation: u64`枯渇時のno-rewind意味が未決と判明したためdocs-only `CU-104E`を`DONE`とし（[CU-104E projection generation枯渇境界決定](reviews/2026-07-27-cu-104e-projection-generation-exhaustion-decision.md)）、`U2h-1I`は`DONE`。次PRODUCT-ASSET実装粒はselection-only入力面の`U2h-1P`（着手可）。`CU-0A08BT`は`WAIT`継続。Motolii Studio Previewは未実装。visible summary chromeは未決で実装しない。G0-6HはU0e-3を止める並行人間審判、G0-9DはDistribution Ready用hardware gate。eguiへ新規製品面を実装せず、plugin UI公開契約はG0-3 / GAP-13まで停止する |
...
| PRODUCT-ASSET | U2h-1P | M3 / VS-1 / B / selection-only primary input | `DO` | — | `U2h-1I`は[発注依存証跡](#発注依存証跡)で`DONE`。[U2h-1 primary selection implementation split決定](reviews/2026-07-27-u2h-1-primary-selection-implementation-split-decision.md) §5と[CU-104](reviews/2026-07-27-cu-104-selection-publish-envelope-decision.md) §7 P5を正とする | private `ReplacePrimary`／`ClearPrimary` action/kind、valid replaceの単一envelope、unknown／table-only IDのtyped rejectだけを閉じる。Place receipt、CU-109/110/111/106、公開API、Document、serde、journal、Undo/history、plugin契約は変更しない |
...
したがって現在の短い運用判断は、**CU-0A03 / R0からCU-0A07C / R4Cまでは完了済み。CU-0A08IP、`U3a-1S`/`U3a-1I`、`CU-G03D`/`CU-G03R`、親`CU-G03`、`CU-101`、`CU-102`、`CU-G09`、`CU-G09O`、`CU-G09R`、`CU-0A08BP`、docs-only `CU-104`、docs-only `U2h-1S`は`DONE`。CU-104はselection publish envelopeのowner・visibility・generation・reconcile時点だけを決定し、`U2h-1S`は[primary selection implementation split決定](reviews/2026-07-27-u2h-1-primary-selection-implementation-split-decision.md)で`U2h-1I`/`U2h-1P`へ分割した。U2h-1I事前審査で`projection_generation: u64`枯渇時の`+1`とno-rewindが未両立と判明したためdocs-only `CU-104E`を`DONE`とし（[CU-104E projection generation枯渇境界決定](reviews/2026-07-27-cu-104e-projection-generation-exhaustion-decision.md)）、`U2h-1I`は`DONE`。次PRODUCT-ASSET実装粒はselection-only入力面の`U2h-1P`（着手可）。`CU-104E`は枯渇actionのtyped pre-mutation refusal、queue消費、自動retry禁止、CU-109 poison非先取りだけを決定した。`CU-0A08BP`はproduct-owned・非export の Browser catalog decoder、fixture/testのみ。`CU-0A08BT`は`WAIT`（CU-0A08BP + `U4a-2` Direct製品入口待ち）、`CU-0A08IT`は`WAIT`（`U4a-2` Direct製品入口待ち）。`U2c-2`は`WAIT`（U4a-2 Direct + U4c Advanced）。G0-6HはU0e-3だけを止める。CU-109 runtime配線、CU-110 Place、CU-111 prepared Undo/Redo、CU-106 consumer接続をCU-104E/U2h-1I/U2h-1Pへ束ねない。Host transport、typed intent、JSX binding、drag payload、`S`行の意味決定、Rust/schema/plugin変更をCU-104E/U2h-1I/U2h-1P/CU-0A08BTへ混ぜるならSTOPする。Motolii Studio Previewは未実装。**G0-6Hは同時に進められる人間審判だが、未完了でもR0〜R4やPreview骨格を止めず、U0e-3だけを止める。G0-9DはDistribution Readyまで`WAIT / HARDWARE`。`U2c-2`はVS-2候補かつ実製品入口待ちである。D1n、D5等の独立follow-upをVS-1の再停止理由へしない。
