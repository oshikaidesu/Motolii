...
| PRODUCT-ASSET | CU-0A08SS0 | M3 / VS-1 / SPEC / Browser Place source seam implementation-boundary scope selection | `DO` | — | `CU-0A08BDD`／`CU-0A08BD0`／`CU-0A08RMD`は[発注依存証跡](#発注依存証跡)で`DONE` | Browser source seamの最小実装境界を特定する一問だけを選定する。型名、event、payload、API、module path、code変更は0 |
