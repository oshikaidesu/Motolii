...
| M5 | **identity meaning decision可 / Render Contribution共通基盤DONE・RCA3/RCB3/RCC3登録 / fixture WAIT** | P0I自身が所有するcontinuity／transform／nested identity／寿命／cache入力境界／PRNG処分をdocsで先に閉じ、TextCluster内部写像とPrototype ownerは明示留保する。P2D周辺は`P2D-RC0`共通基盤と`P2D-RCE0`固定capsule／Rerun裁定、`GR-D4`再開可能loopを維持する。旧RCA/B/CとRCA2/B2/C2の差分は不採用。固定templateのRCA3/RCB3/RCC3三providerだけ登録済み。横断`RCC4`、統合`P2D-RCI`、公開API、Document、plugin契約、実装はWAIT。P6のfontique／harfrust／Vello text stackは未実装で、K6とpremul adapterを重複実装しない |
...
| RENDER-CONTRIBUTION-AUTHORITY | P2D-RCA3 | M5 / P2D / fixed boundary comparison | `DO` | — | P2D-RC0/RCE0、GR-D4、固定6行template | ACCEPT後も公開契約を決めずRCI入力へ |
| RENDER-CONTRIBUTION-RERUN | P2D-RCB3 | M5 / P2D / fixed Rerun transcription | `DO` | — | P2D-RC0/RCE0、GR-D4、固定A1〜A6×5 field | fixture対応は別grainへ |
| RENDER-CONTRIBUTION-ENGINE | P2D-RCC3-BEVY | M5 / P2D / Bevy transcription | `DO` | — | P2D-RCE0、GR-D4、Bevy capsule | 三provider完了後RCC4 |
| RENDER-CONTRIBUTION-ENGINE | P2D-RCC3-GODOT | M5 / P2D / Godot transcription | `DO` | — | P2D-RCE0、GR-D4、Godot capsule | 三provider完了後RCC4 |
| RENDER-CONTRIBUTION-ENGINE | P2D-RCC3-UNREAL | M5 / P2D / Unreal transcription | `DO` | — | P2D-RCE0、GR-D4、Unreal capsule | 三provider完了後RCC4 |
| RENDER-CONTRIBUTION-ENGINE | P2D-RCC4 | M5 / P2D / fixture comparison | `WAIT` | — | P2D-RCC3-BEVY/GODOT/UNREAL | 三provider ACCEPT後に登録 |
