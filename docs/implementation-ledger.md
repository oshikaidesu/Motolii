...
| VISUAL-RESPONSE | G0-6H-V1 | M3 evidence / VS-1 / current-route evidence generation | `DO` | — | `G0-6H-V0`は[発注依存証跡](#発注依存証跡)で`DONE` | `#plugin-browser-candidate`から5状態をcaptureし、各normal＋派生5variant、immutable generation、SHA-256 manifest、read-only照合を作る。human session、token採択、U0e-3解禁は含めない |
