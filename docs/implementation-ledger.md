...
| PRODUCT-ASSET | CU-0A08SSCI-I0 | M3 / VS-1 / SPEC / Browser scoped identity input seam scope selection | `DO` | [CU-0A08SSCI-I0選定](reviews/2026-07-29-cu-0a08ssci-i0-browser-scoped-identity-input-seam-scope-selection.md) | `CU-0A08SSCI-P1`は[発注依存証跡](#発注依存証跡)で`DONE` | 未採番前提(I)を採番し、次docs-only裁定が閉じる唯一の問いだけを選定。code・型・props名・公開境界変更0 |
