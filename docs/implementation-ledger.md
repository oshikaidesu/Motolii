...
| PRODUCT-ASSET | CU-108RD | M3 / VS-1 / PRODUCT / exclusive drop release routing repair | `DO` | — | `CU-108RDS`は[発注依存証跡](#発注依存証跡)で`DONE` | exact AppKit upを内部sequence付き単一entryへ入れ、Placeまたは通常clickの一方だけがclaimする。R-DROP外UI、Document、journal、Undo、公開API変更0 |
