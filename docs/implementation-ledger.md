...
| M5 | **暫定採択地図あり / decision recoveryがruntime前提** | [M5暫定採択地図](m5-known-implementation-adoption-map.md)で10機構classと11子を分離した。最初はdocs-only `M5-A0`で未統合`416aa2c2`／歴史`33e957df`を現行mainへ処分する。並列調査はA1/R0/T0/P0まで、製品runtimeはdecision recovery、Observation、resource gate後。P0I〜P7を独自3D engine／scene framework／text stackの実装列にせず、P6は現行Fontique／HarfRust／Velloとpremul adapterを重複実装しない |
