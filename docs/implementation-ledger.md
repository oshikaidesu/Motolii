...
| PRODUCT-ASSET | CU-104 | M3 / VS-1 / SPEC / selection publish envelope decision | `DO` | — | `CU-G02`／`CU-G03`は[発注依存証跡](#発注依存証跡)で`DONE`。既存U2h-1再採択を越えず、owner・visibility・`projection_generation`更新条件・Apply/Undo/Redo後publish前reconcile時点だけをdocsで閉じる | U2h-1実装は別粒。CU-109/CU-111/CU-110、consumer接続、additive/range/marquee/AX、Document/journal/Undoへのselection保存は含めない |
...
| ORACLE-GUARD | CU-104R | M3 prerequisite / CU-104 completion mirror closure | `DO` | — | 隔離CU-104 decision差分でCU-104を`DONE`へ正当更新した後、Grok検収が`docs/reviews/2026-07-24-m3-vertical-slice-execution-decision.md` Browser行と`docs/README.md`の`CU-104` `DO`残存、および`docs/reviews/2026-07-27-cu-104-selection-publish-envelope-decision.md` SN6のpost-publish follow-up reconcile禁止欠落を検出 | 上記3文書だけを最小修復し、次PRODUCT-ASSET粒は未選定のまま、Browser専用test 114→115、`npm run test:reference-guard`、docs整合を閉じる。guard期待値・CU-104意味・code/fixture・次粒選定を変更しない |
...
したがって現在の短い運用判断は、**CU-0A03 / R0からCU-0A07C / R4Cまでは完了済み。CU-0A08IP、`U3a-1S`/`U3a-1I`、`CU-G03D`/`CU-G03R`、親`CU-G03`、`CU-101`、`CU-102`、`CU-G09`、`CU-G09O`、`CU-G09R`、`CU-0A08BP`は`DONE`。`CU-0A08BP`は`DONE`（product-owned・非export の Browser catalog decoder、fixture/testのみ）。次のPRODUCT-ASSET粒はdocs-only `CU-104`（`DO`）で、selection publish envelopeのowner・visibility・generation・reconcile時点だけを決定する。`CU-0A08BT`は`WAIT`（CU-0A08BP + `U4a-2` Direct製品入口待ち）、`CU-0A08IT`は`WAIT`（`U4a-2` Direct製品入口待ち）。`U2c-2`は`WAIT`（U4a-2 Direct + U4c Advanced）。G0-6HはU0e-3だけを止める。CU-109 runtime配線とU2h-1実装はCU-104へ束ねない。Host transport、typed intent、JSX binding、drag payload、`S`行の意味決定、Rust/schema/plugin変更をCU-104/CU-0A08BTへ混ぜるならSTOPする。Motolii Studio Previewは未実装。**G0-6Hは同時に進められる人間審判だが、未完了でもR0〜R4やPreview骨格を止めず、U0e-3だけを止める。G0-9DはDistribution Readyまで`WAIT / HARDWARE`。`U2c-2`はVS-2候補かつ実製品入口待ちである。D1n、D5等の独立follow-upをVS-1の再停止理由へしない。
