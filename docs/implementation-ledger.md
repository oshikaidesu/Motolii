...
| VISUAL-RESPONSE | G0-6H-S | M3 evidence / VS-1 / human-judgment input route adjudication | `DO` | — | `G0-6H-R`は[発注依存証跡](#発注依存証跡)で`DONE` | `#reference/*`と`#plugin-browser-candidate`のどちらをG0-6H人間審判入力へ固定するかだけをdocsで裁定。画像生成・variant供給・人間checklist・token・G0-6H/U0e-3状態変更0 |
