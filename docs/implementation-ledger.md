...
| M3 | **VS-1 Rectangle配置とUndo / CU-0A08BP Browser catalog decoder** | React chrome + native Stage/Timeline + headless interaction、1 top-level wgpu Surface + 2 native viewport + opaque child WebView islandsを正本化。R4CまでのInspector product ownershipとCU-0A08IP、docs-only `CU-G09O`（[Browser decoder output契約決定](reviews/2026-07-26-cu-g09o-browser-decoder-output-contract-decision.md)）は完了した。次PRODUCT-ASSET粒は`CU-0A08BP`（`DO`）。`CU-0A08BT`は`WAIT`（CU-0A08BP + U4a-2）。Motolii Studio Previewは未実装。visible summary chromeは未決で実装しない。G0-6HはU0e-3を止める並行人間審判、G0-9DはDistribution Ready用hardware gate。eguiへ新規製品面を実装せず、plugin UI公開契約はG0-3 / GAP-13まで停止する |
...
| PRODUCT-ASSET | CU-0A08BP | M3 / VS-1 / B / Browser catalog decoder | `DO` | — | `CU-0A04`／`CU-G09`／`CU-G09O` は発注依存証跡で `DONE` | product-owned・非export pure decoder、fixture/testのみ。typed intent、Host transport、JSX binding、drag payload、`S`行はCU-0A08BT以降 |
