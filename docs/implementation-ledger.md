...
| VISUAL-RESPONSE | G0-6H-AG | M3 evidence / VS-1 / Starter Media fixed evidence capsule | `DO` | — | `G0-6H-AG0`は[発注依存証跡](#発注依存証跡)で`DONE` | 固定Starter Media出力、raw provenance、read-only integrity checkを一つの証拠カプセルに閉じる。route/React/Document/public境界変更0 |
| VISUAL-RESPONSE | G0-6H-V0 | M3 evidence / VS-1 / current-route variant evidence contract | `WAIT` | — | `G0-6H-AG`の固定evidence capsule待ち | 5状態semantic mapping、固定capture環境、normal＋派生variant、immutable manifest/read-only check、記録された人間sessionの要求だけをdocsで閉じる。画像/script/fixture/token/threshold/G0-6H/U0e-3状態変更0 |
