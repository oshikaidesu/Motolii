...
| M5 | **`M5-A0T` / `M5-A1` / `M5-R0` DONE / T0/P0/I0/D0検証NEXT / `M5-A0S` WAIT** | [M5-A1 receipt](reviews/evidence/m5-known-implementation/M5-A1/README.md)でimport leaf、[M5-R0 receipt](reviews/evidence/m5-known-implementation/M5-R0/README.md)でwgpu offscreen routeをKEEPした。R0は4 material caseのGPU readbackとCPU reference一致、adapter／limit refusalを確認した。製品material／renderer、PBR conformance、3 OSは未実装。次はT0/P0/I0/D0を一粒一commitで検証し、runtime前に`M5-A0S`、Observation、M4 resource gateを閉じる |
