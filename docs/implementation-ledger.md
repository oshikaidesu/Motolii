...
| M5 | **`M5-A0T` / `M5-A1` / `M5-R0` / `M5-T0` DONE / P0/I0/D0検証NEXT / `M5-A0S` WAIT** | [M5-A1 receipt](reviews/evidence/m5-known-implementation/M5-A1/README.md)でimport leaf、[M5-R0 receipt](reviews/evidence/m5-known-implementation/M5-R0/README.md)でwgpu offscreen route、[M5-T0 receipt](reviews/evidence/m5-known-implementation/M5-T0/README.md)でFontique＋HarfRust KEEPとParley private probeを確認した。T0はCJK／Latin／emoji／RTL、fallback、cluster、missing glyphを通し、variable axisなしをREDUCEした。製品text API／material／renderer、PBR conformance、3 OSは未実装。次はP0/I0/D0を一粒一commitで検証し、runtime前に`M5-A0S`、Observation、M4 resource gateを閉じる |
