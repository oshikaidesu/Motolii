...
| M5 | **技術採択地図 `M5-A0T` DONE / 子検証NEXT / 意味回収 `M5-A0S` WAIT** | [M5採択・検証地図](m5-known-implementation-adoption-map.md)で10機構classと12子を確定した。`codex/m5-known-implementation-plan-20260802`上でA1/R0/T0/P0/I0/D0を一粒一commitで検証し、private leafと外部oracleの適合を閉じる。製品runtime接続前にdocs-only `M5-A0S`で未統合`416aa2c2`／歴史`33e957df`を現行mainへ処分し、Observation／M4 resource gateを閉じる。P0I〜P7を独自3D engine／scene framework／text stackの実装列にせず、検証成功を製品完成と数えない |
