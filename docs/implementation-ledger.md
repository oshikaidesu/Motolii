...
| PRODUCT-ASSET | CU-110PT | M3 / VS-1 / PRODUCT / native Timeline published snapshot projection | `DO` | — | `CU-110PT0` | 既存headless projectionのnon-test callerとnative bar描画。selection入力、React Timeline 0 |
| PRODUCT-ASSET | CU-110PI | M3 / VS-1 / PRODUCT / Inspector published snapshot identity projection | `WAIT` | — | `CU-110PT` + CU-0A08IP/CU-0A08ITP | product-owned Inspectorへ同じprimary identityをread-only投影。mock state、S値、typed editing intent 0 |
