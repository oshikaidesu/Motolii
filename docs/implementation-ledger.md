...
| VISUAL-RESPONSE | G0-6H-M | M3 evidence / VS-1 / current-route semantic gap mapping | `DO` | — | `G0-6H-M0`は[発注依存証跡](#発注依存証跡)で`DONE` | `対応 / partial / 対応なし / 未確認`表と人間裁定一点を閉じる。adapter/fixture/route/spec意味変更0 |
| VISUAL-RESPONSE | G0-6H-V0 | M3 evidence / VS-1 / current-route variant evidence contract | `WAIT` | — | `G0-6H-M`とscenario意味の人間裁定待ち | 5状態semantic mapping、固定capture環境、normal＋派生variant、immutable manifest/read-only check、記録された人間sessionの要求だけをdocsで閉じる。画像/script/fixture/token/threshold/G0-6H/U0e-3状態変更0 |
