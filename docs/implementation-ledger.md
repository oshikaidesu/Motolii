...
| M3 | **段階発注可** | U0aのSlint→egui移行とtoolkit隔離gate完了。U0b-1、U0e-1、U1a-1から個別依存どおり進める |
...
[M2基盤再締結ゲート](reviews/2026-07-15-m2-foundation-reclosure-gate.md)解除後のM3入場再翻訳は完了済み。以後は親IDを一括実装せず、枝番の個別依存とSTOP条件を優先する。

## 主クリティカルパス

```text
Shared Effect:
D1l DONE → D3e DONE → U2g → K2

M3 shell / preview:
U0a入場移行 DONE → U1a-1 → U1b-1 → U1b-2 → U1f #169
                              └→ U2b-1 → U2c-1〜5 → U2f #168

Unified Camera:
CAM-G0 DONE → D1j DONE → D1k-S DONE → D1k DONE → D3f DONE → U1f #169 → U2d
...
| 2 | U0e-1 | M3 | `DONE` | — | fixture限定typed DTCG形、決定的adapter生成、check/write、手編集拒否、原子的temp cleanupを自動審判。製品token値なし | U0e-2をIssue化 |
...
|---|---|---|---|---|---|
| 1 | U1b-1 | M3 | `WAIT` | U1a-1 merge | render worker/latest mailbox。古い結果E2EはU1b-2 |
| 2 | U0b-2 | M3 | `WAIT` | U0b-1 merge | toolkit非依存domain intent。U0c-1/U2a-1の入口 |
| 3 | U0e-2 | M3 | `WAIT` | U0e-1 merge | 5 reference screen fixture。具体値はG0-6H待ち |
...
| U1f | `BLOCKED` | [#169](https://github.com/oshikaidesu/Motolii/issues/169) | U1b、U0e（D1k/D3fは完了済み） | K0は依存ではない。保守的Draftで成立させる |
...
U0aのegui入場移行と最新mainへの再翻訳は完了した。次表の直前条件を満たす枝番だけを実装する。

| 目的 | 必要な直前条件 |
|---|---|
| UI shellを始める | U0a完了 |
| 静止previewを出す | U0a + D3（完了済み） |
...
したがって現在の短い運用判断は、**M3へ入場済みで、U0b-1、U0e-1、U1a-1を別境界として進める**。D5は骨格を完了扱いせず、本番preview／GPU計測／実機E2Eを後続へ残す。
