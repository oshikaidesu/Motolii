...
| M5 | **identity decision可 / 3D import-render SPEC active / fixture WAIT** | P0I decision laneと独立に、`M5-3D-R0`でP1/P2のOSS parser、private二段派生、core PBR＋fixed neutral environment、同一renderer責任をdocs化中。branch到達はmain統合や実装ではない。P1/P2はcrate evidence、fixture、P3/M4依存前に起動しない。P6のfontique／harfrust／Vello text stackは未実装で、K6とpremul adapterを重複実装しない |
...
| M5-3D | M5-3D-R0 | M5 / P1-P2 import-render boundary decision | `ACTIVE / SPEC` | — | [M5 3D import／render境界](reviews/2026-08-01-m5-3d-import-rendering-boundary-decision.md)を本branchで作成。Fable 5 read-only助言をCodexが現行specと一次資料へ再照合 | docs check、独立最終review、commit後もmain統合まではDONEにしない |
| M5-3D | P1 | M5 / glTF-GLB-OBJ import | `WAIT` | — | `M5-3D-R0` main統合、`gltf`／`tobj`／image crateのversion・license・maintenance・3 OS・malformed/budget evidence、exact fixture | faithful private asset＋typed diagnostics。Document／serde／GPU実装なし |
| M5-3D | P2 | M5 / material renderer | `WAIT` | — | P1、fixed neutral environment evidence、P3 private Observation前提、M4 resource ownerと既決P2D境界 | 同じscene/material/shader systemでLayer Order contribution。shared-depthはP2Dへ分離 |
| M5-3D | M5-3D-RS1 | M5 / renderling comparison spike | `WAIT` | — | `M5-3D-R0` main統合後にexact crate/version/commit、隔離worktree、Motolii fixture、workspace lockfile不変を閉じる | comparison receiptだけ。dependency採択／public API／Document／通常build変更なし |
...
現在の短い運用判断は、**Browser typed intent→offline native Host WebView→bounded inbox→Host private platform capture candidate→native Stage latest-layout-epoch hit-test→1 top-level Surface上のStage/Timeline→opaque Browser islandのfocus/geometry epochまで実装済み。次はCU-0B05でHost snapshot lifecycle再投影を閉じる。token後続は`WAIT`。**
