...
| M3 | **段階発注可 / Uシリーズ直列選択中** | U0a、U0b-1、U0b-2、U0c-1、U0c-2、U0d-1、U0d-2、U0d-3完了。次はU2a-1を単独実行 |
...
|---|---|---|---|---|---|---|
| 1 | U2a-1 | M3 | `DO` | — | U0d-3とD2完了。既存D2 macro/merge境界へdomain intentを変換するadapterだけを行う | U1a-1を単独実行 |
...
したがって現在の短い運用判断は、**M2基盤再締結とD3e、D1m、CAM-G0、D1j、D1k-S、D1k、D3f、M3 U0a、U0b-1、U0b-2、U0c-1、U0c-2、U0d-1、U0d-2、U0d-3は完了済み**。初回Uシリーズは並走させず、U0d-3のmain merge後に`U2a-1`だけを進める。Rerunのcommit固定source監査と資産分類は可能だが、現在のUシリーズ実装と並走させない。D5は骨格を完了扱いせず、本番preview／GPU計測／実機E2Eを後続へ残す。
