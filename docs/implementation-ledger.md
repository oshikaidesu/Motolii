...
| PRODUCT-ASSET | CU-0A08SSD | M3 / VS-1 / SPEC / Browser Place source seam implementation-boundary adjudication | `DO` | — | `CU-0A08SS0`／`CU-0A08BDD`／`CU-0A08BD0`は[発注依存証跡](#発注依存証跡)で`DONE` | [CU-0A08SS0選定 §3](reviews/2026-07-29-cu-0a08ss0-browser-place-source-seam-implementation-boundary-scope-selection.md#3-cu-0a08ssd-が閉じる唯一の問い)の一問だけをdocs-onlyで閉じる |
