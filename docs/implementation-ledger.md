...
| M3 | **VS-1 Rectangle配置とUndo** | Browser typed intent→offline native Host WebView→exact codec/bounded inboxまで`CU-0B03`で成立。token後続は`WAIT`。次は横展開せず`CU-0B04N/R`と`CU-0B05`の最小製品経路を再締結してPlace配送へ進む。`CU-107PV`は実製品Host再投影`CU-0B05`待ち。Motolii Studio Previewは未実装。G0-9DはDistribution Ready用hardware gate。eguiへ新規製品面を実装せず、plugin UI公開契約はG0-3 / GAP-13まで停止する |
...
現在の短い運用判断は、**Browser typed intent→offline native Host WebView→bounded inbox→native Stage final release→private Place D2→same LayerId publish→Vector Rect canonical GPU renderまで実装済みで、CU-103は`DONE`。token後続は`WAIT`。次は同じsnapshot/LayerIdのTimeline/Inspector製品projection、実機drop、CU-111 Undo/Redoだけを直列に閉じる。**
