...
Plugin Resultsで候補を選択した後は、同じBrowser内のEffect Detailへ短い説明、用途、適用方法をまとめる。説明を全cardへ反復せず、選択変更へ追従する一箇所だけに置く。詳細なcapability、provider、package identityはDeveloper infoへ分離し、短い説明を技術情報で埋めない。
