# プラグイン生態系(発見・導入・再現) — 設計

作成日: 2026-07-12  
状態: **設計確定寄り**(2026-07-12 本番寄せ)。**発見・導入の最小地図は新規獲得のため早期**(backlog GAP-13)。動的ロード前提のフル Sync／商用 pack はヒーロー後(V2-1 / V2-8 の重い層)。[concept.md](concept.md)「プラグインファーストの範囲」「拡張の入手」と整合  
関連: F-9 / G-2 / G-3・エコシステム死、[plugin-authoring.md](plugin-authoring.md)、backlog GAP-13 / V2-1 / V2-8
...
> **AviUtl2 カタログからの取り込み:** [aviutl2-catalog](https://github.com/Neosku/aviutl2-catalog) の **「ホストを持たず GitHub／外部を索引するクライアント」** という姿勢は採る(新規ユーザーが既にその UX を既存資産として知っている)。採らないのは **中央正本 `index` の恒久運用・popularity/trend/telemetry**(同リポジトリ周辺のデータ集中は反面教師)。正本は作者の GitHub／外部店。アプリは購読・索引・導入だけ行う。
...
### 1.5 音楽ソフト / DAW 圏から学ぶこと

映像(AE/AviUtl)より **技術と流通が閉じにくい**。方向性が多いのはその帰結でもある。

**なぜ閉じにくいか(映像との差):**

| | 音楽(DAW) | 映像(AE等) |
|---|---|---|
| プラグイン形式 | **VST3 / AU / CLAP** 等、ホスト横断が標準 | ホスト固有(.aex 等)が強い |
| 置き場 | OS 共通の Plug-Ins フォルダ(それでも地獄はある) | アプリ専用フォルダ手置きが多い |
| 隔離 | 音声バッファが小さい → Bitwig 式プロセス外が現実的(concept 既存) | ピクセルは重い → 同一プロセス寄り |
| 発見の外部 | **[KVR Audio](https://www.kvraudio.com/)** が長年の横断ディレクトリ | 同等の中立ディレクトリが弱い |
| オープン索引 | **ReaPack**(REAPER)が成立 | AviUtl2 カタログは中央寄り |

方向性が多い(Ableton / Bitwig / REAPER / FL / Studio One…)のは、**プラグイン資産がホストを跨げる**から乗り換え・併用が起き、ホスト差別化が「生態系の閉じ方」ではなくワークフローになる、という構造。

**DAW 圏の既存例 → motolii への写像:**

| DAW 側 | 役割 | motolii |
|---|---|---|
| **ReaPack** | 分散索引・Sync・Browse | tap / 探す / 同期(既に主参考) |
| **KVR** | **DAWの外**にある地図+レビュー/人気 | 人気の熱を外部に託す前例そのもの(§1.4) |
| **Ableton Pack (.alp)** / Sound Sets | ベンダーが袋で配るコンテンツ一式 | `.motoliipack` / 商用袋に近い。**ユーザー自作の「導入済み一覧」書き出しとは別物** |
| **FX Chain / Track Template / Rack** | 鎖やテンプレを渡すと必要プラグインが暗黙に付く | **kit / lock の親戚**。足りないと音が欠けて気づく(AviUtl と同型の苦痛) |
| **Bitwig / Cubase ブラウザのコレクション・お気に入り** | ユーザー棚 | §1.2 の棚 |
| **VCV Rack** | Library=地図、`.vcv` パッチ=モジュール集合の共有 | 地図と kit が分かれている良い模型。パッチを開いて足りないモジュールに気づく |
| **DAWproject**(Bitwig+PreSonus) | プロジェクト交換。プラグインはベストエフォート | 未知 id パススルー(F-9)と同族の割り切り |
| **CLAP** | オープンなプラグイン契約 | 境界設計の参考(concept 既存)。流通キットの話ではない |
| Plugin Boutique 等 | 店内セール・ランキング | 中央人気の商用形。**採らない**(ホスト+KPI) |

**DAW から持ってこられる結論:**

1. **地図の人気はホスト外に置ける** — KVR が何十年もその役。motolii アプリ内に累積DLを生やさなくてよい。
2. **「鎖・テンプレ・パッチ」が実質 kit** — 音楽勢はセット共有を別ボタンにしなくても、テンプレ配布で界隈ガラパゴスを回してきた。痛いのは **不足プラグインの手置き**。明示 kit(揃える)は、その暗黙 kit を **先に充足する**ためのボタン。
3. **Pack(.alp)はベンダー袋** — ユーザー間の「私の必須セット」とは別。両方いる(商用 pack + ユーザー kit)。
4. **形式が開いているほど ReaPack/KVR 型が育つ** — 映像側で motolii がソース第一・wgpu 契約にするのは、同じ「閉じにくさ」を意図的に作る側(F-9)。

映像ツールが kit を厚くしてこなかったのに対し、DAW は **暗黙 kit(テンプレ)と外部地図(KVR)と分散索引(ReaPack)を分裂して持っている**。motolii はそれを **kit 明示 + tap + 外部 links** に畳む。
...
## 9. 段階切り分け(早期地図 vs 重い基盤)

| 段階 | スコープ | いつ |
|---|---|---|
| **今〜M3近傍(GAP-13)** | 発見の最小地図: GitHub 索引(Releases／`motolii.toml` メタ)・探す(存在・語彙・updated)・D&D／ワンボタン導入・ローカル installed 状態。アカウントなし。人気順なし | **新規獲得のため早期**(AviUtl2 圏の既存カタログ UX に対抗)。静的リンク時代でも「地図＋手元への導入」は価値がある |
| **ヒーロー後 v2-alpha(V2-8 前半)** | 分散 tap 購読の本実装・kit/lock の書き出しインポートをコミュニティ主回路として UI 化 | ヒーロー後。設計は本ドキュメントで固定済み |
| **v2-beta(V2-8 後半)** | git Sync + verify/repair/doctor + 商用 pack メタ(店の外への誘導) | 同上 |
| **V2-1(別軸)** | 動的ロード(C ABI/`abi_stable`)。カタログと独立 — 地図は静的リンクでも先行可 | 境界実証後 |

**やらない順序の誤り:** 「動的ロードが無いから地図も後回し」— 発見 UX の欠落はユーザーを AviUtl2 資産へ流す。重い基盤(dylib・サンドボックス)だけをヒーロー後に残す。

本ドキュメントの改訂はスキーマ未決(§10)を潰す形で行う。GAP-13 着手前に §10 の「早期スライスに必要な項」だけ先に潰す。
...
| 配布は段階的(地図は早期) | GAP-13=早期地図。V2-8=フル生態系。V2-1=動的ロード(別軸) |
...
| 2026-07-12 | **本番寄せ**: 発見地図を早期(GAP-13)へ。ヒーロー後は動的ロード等の重い層のみ。AviUtl2カタログは「GitHub索引クライアント」姿勢を採り中央index/人気は反面教師と明記 |
