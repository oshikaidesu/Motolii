...
ここでいう「ユーザー」はcodeを書かない固定身分ではない。利用、調整、構成、inspection、fork、authoring、共有は同じcreatorが必要に応じて移動する連続した行為である。Hostが上記の責任を引き受ける目的は開発を閉じることではなく、多数のcreator-authorが作品全体の整合性を再実装せず、表現そのものへ集中できるようにすることにある([Creator / Developer連続体](reviews/2026-07-22-creator-developer-continuum-decision.md))。
