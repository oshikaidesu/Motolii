...
3. **2.5D/3D合成**: 動画テクスチャ平面とglTFメッシュを同一シーンで合成(合成順序はレイヤー順、Zはカメラ投影のみ)  
   ...
   - **カメラ文脈はコンポ全体で共有する**: v1では「レイヤごとにカメラを持つ」ことはしない。M2は特異点のない`CompCameraDoc::PlanarOrthographic`(center/roll radians/height)を常在cameraとして持ち、出力aspectはCompositionだけが所有する。M5のSpatial/Perspectiveはorientation補間・clip・target constraintを先に決めた**追加variant**とし、position+target+暗黙world-upをpose保存へ焼かない。3D系レイヤソースプラグイン(点群/メッシュ等)は評価済みcameraを参照して投影し、2Dテクスチャを返す。
