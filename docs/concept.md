...
- **プラグインファーストの"範囲"(2026-07-09決定)**: 「大部分をプラグインで設計できる」(AviUtlの実例)を **v1から全面採用**する。ただしこれは**"配布/マーケットの仕組み"ではなく"境界の設計"**を指す。切り分け:
  ...
  - **早い段階で必要(発見・導入の地図)**: **ホストレスな拡張カタログ**(GitHub 等をクライアントが索引・取得。中央ストア／アカウント／ライセンスサーバなし)。AviUtl2圏は[aviutl2-catalog](https://github.com/Neosku/aviutl2-catalog)等が既に既存資産化しており、無いと新規ユーザーがそちらへ流れる。設計の正本は[plugin-ecosystem.md](plugin-ecosystem.md)(GAP-13)。**中央 `index.json` 一枚の恒久運用・人気順・telemetryは採らない**(同カタログのデータ集中＋人気軸は反面教師)。採るのは「ホストを持たず GitHub／外部を正本にする」姿勢
  - **ヒーロー誕生後/v2に回す(重い配布基盤)**: 動的ロード(C ABI)・サンドボックス実行・フル tap Sync／商用 pack は、**Reddit公開のヒーロー(M1出口デモ)が生まれてから**本実装してよい。未検証の境界の上に重基盤を作る早すぎ投資(Olive/Natron型)を避ける。"動く形が正義"はレンダ境界の話であり、**発見 UX の欠落を後回しにする言い訳には使わない**
  - **配布の形(2026-07-12設計)**: 中央マーケットではなく **分散tap購読 + lock再現 + D&D導入**(ホストレス)。ランキング/telemetryなし。UIはReaPack基調。**Civitai Manager系UIは不採用**。**コミュニティ成長の主回路は使用セット(kit/lock)の書き出し→インポート**。有料は Gumroad/BOOTH 等の**店の外**。詳細は[plugin-ecosystem.md](plugin-ecosystem.md)
  - 並列実装の安全化に必要なのは配布基盤ではなく、**境界の約束の固定**(凍結ゲートの型シグネチャ + AGENTS.mdの絶対規律 + 下記paramバージョニング)。ここが揃えば複数エージェントが並列にエフェクトを足しても壊れない。
- **拡張の入手・有料の置き場(2026-07-12決定)**: 本体は**アカウントなし**。課金・認証・割れ止めはホストの責務にしない(割れ対策に製品を最適化しない)。
  - **発見の既定は無料／コミュニティ**(意図単位・タグ。有料をホーム最上段にしない)
  - **コミュニティ路線(推奨)**: ソース／パッケージを **GitHub に載せる**(有料化前の公開・反馈もこの路線)。D&D／ソース配布+ホスト側ビルド(F-9)
  - **有料**: 店の外で完結。カタログは外部URLへ誘導するだけ可。アプリは入れられたプラグインを区別せず動かす
  - 作者ごとのライセンスサーバ実装をユーザーに課すAE型は採らない
