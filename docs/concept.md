...
- **UI runtime責任境界をReact chrome + native GPU workspaceへ固定する(2026-07-21決定)**: React / WebViewはAsset Browser、Inspector、parameter/form、panel、toolbar、dialog、検索、設定、Host/community共通kitを所有し、StageとTimeline全体(ruler/track header/lane/clip/key/playhead/graph/selectionを含む)はnative Rust/wgpuが所有する。Canvasやbrowser WebGPUが必要なこと自体をReact所有の理由にせず、高頻度直接操作面はnativeへ置く。native側は汎用widget toolkitを再実装せず、renderer非依存のheadless interaction部品を優先し、Motolii固有のsnap/selection/transient preview/D2 commitだけを所有する。exact surface/compositor、direct wgpu + Vello局所pass、egui撤去、plugin sandbox/公開契約はG0-9実機spikeまで未決。正本は[UI runtime責任境界](ui-runtime-architecture.md)、証拠は[再選定](reviews/2026-07-21-m3-react-webview-runtime-reconsideration.md)と[egui採用時の証拠](reviews/2026-07-18-m3-egui-selection.md)
