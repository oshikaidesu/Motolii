...
3. **単一XYZ世界・単一カメラでの2.5D/3D合成**: 動画・画像・テキスト・図形・点群・glTFを含む全オブジェクトが常に同じ正準XYZ世界とworld transformを持つ。2Dオブジェクトも既定で`z=0`平面へ置き、2D/3Dで座標世界やカメラを切り替えない。Zは常にカメラ投影・視差へ効き、切り替えるのは遮蔽ポリシーだけとする(2026-07-14決定)
   ...
   - **カメラ文脈はコンポ全体で常在・共有する**: コア(Document/Render)側に`CompCamera`を必ず1つ持ち、全レイヤーが参照する。`Output Frame`はこのカメラの投影開口であり、別の2D書き出し枠ではない。通常UIで「3Dカメラを追加する」操作は見せず、Output Frame操作を同じカメラ編集として扱う。既定はOrthographicで`z=0`平面の正準高さ1.0が出力高に一致し、Perspectiveへ替えても同じcamera/object/transformを使う。詳細は[統一カメラ設計](reviews/2026-07-14-unified-stage-camera-design.md)。
