...
- **プラグインファーストの"範囲"(2026-07-09決定)**: 「大部分をプラグインで設計できる」(AviUtlの実例)を **v1から全面採用**する。ただしこれは**"配布/マーケットの仕組み"ではなく"境界の設計"**を指す。切り分け:
  ...
  - **配布の形(2026-07-12設計草案)**: 中央マーケットではなく **分散tap購読 + lock再現 + D&D導入**(ホストレス)。ランキング/telemetryなし。UIはReaPack基調で簡潔に。**Civitai Manager系UIは不採用**(自己満足のカード祭りを発見の主役にしない)。詳細は[plugin-ecosystem.md](plugin-ecosystem.md)
