...
- **区間イージングエディタとGraph Viewを分ける(2026-07-19改訂)**: Flow/Alight Motion式の`Interval Easing Editor`は、2キーフレーム間の正規化補間をpreset・Bezier handle・Overshootで素早く編集するショートカットとして維持する。これとは別に、実時間×実値でfocus中channelの複数key・複数区間を俯瞰するdock可能な`Graph View`を採る。全体構成はApple Motion、tangent直接操作はCinema 4D、状態語彙はMaya、表示navigationはBlenderを参照し、AEの複雑さや先例の自動平滑化伝播をそのまま持ち込まない。詳細は[Graph View参照・比較決定](reviews/2026-07-19-graph-view-reference-decision.md)。
