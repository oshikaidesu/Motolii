...
**Lottieを基幹モデル、Rerun/re_rendererをビュー・描画基盤とする映像編集・コンポジットソフト。AEの良いレイヤー／キーフレームの作法、Abletonの整頓された密度、Figmaの直接性を組み合わせる。**

自分のMVを作ることが出発点であり具体的な検収の場。継続ゴールは、AE利用者や動画編集利用者が説明書を読まず普通に快適に制作でき、既存ソフトが強いてきた手間を構造から減らすこと。「自分用」を、一般的な操作や他の利用者の導線を省く理由にしない。

## 守るもの

- **ユーザーの意図が先。** 守るのは既存コードや仮仕様ではない。UIと実装の大幅な改修は可能だが、分類や階層など意味のある入口を余白扱いして消さない。
- **構造による解決。** グループは操作の束ではなく親子の容器。描画・入力・削除・複製・Undoまで同じ関係から導く。修正のたびに個別の例外を追加しない。
- **普通の上に独自性を置く。** 選択解除、複数選択、範囲選択、入力欄、ショートカット、慣性などを制作の基礎として扱う。理想を説明文で埋め合わせない。
- **一つの3D空間。** 文字、板、立体、点群は同じStageに存在する。視点と作品のカメラ／出力を区別し、暗黙に畳まない。
- **気持ちよく触れられる密度。** 読む順序、意味の境界、反復する寸法、選択と所属の色を揃える。単に小さくする・灰色にする・カードにすることを完成としない。
- **反復を維持する。** Flutterの通常UI変更はhot reload。Rust/GPU側の必要な更新は別工程。テストやビルドの回数を成果にせず、実際に制作できることを検収する。

## 意味と技術の所有

| 対象 | 役割 |
|---|---|
| Lottie | レイヤー、キー、補間、変換などの基幹モデル。単なるJSON出力形式や比較用oracleだけに縮めない |
| Document / Intent（Rust） | 作品、親子関係、値、キー、Undo、preview/commit/cancelの正本 |
| Rerun / re_renderer / wgpu | 記録・評価に使う既存部品と、同じ空間を描くビュー／GPU基盤。独立した第二の描画エンジンを作らない |
| Flutter | 画面、標準入力、操作と状態の表示。作品をDartにも複製して二重管理しない |
| macOS接続 | OS窓、ファイル操作、Rust描画結果の共有テクスチャ接続。作品の意味を持たない |
| Vism / vgpu由来の体系 | 小さな契約で表現を独立して作り、既存技術へ委ねる入口。LLMがホスト全体を理解せず作成・反復できることを重視する |

ゼロコピーの描画経路と速いUI反復を両立する。独自に持つ責任は編集の意味と表現データを中心にし、既存部品・標準機構を使える部分は使う。現在のmacOS検証を他OSや製品配布の完了と同一視しない。

## モーショングラフィックスの 3 層 — 骨格・肌・魂(2026-09-14)

モーショングラフィックスの系譜はグラフィックデザインに時間を足した物で、AE の系譜は映画の合成。AE は合成の道具でデザインをさせるので、箱・流し込み・格子が無く、`sourceRectAtTime()` の式や拡張で継ぐ手順を利用者に踏ませてきた。Web は逆に、デザインの語彙(箱・文字組み・Flex / Grid)を整えたが、時間と画素が薄い。**3 つを同時に持つ道具はまだ無い。**
...
正本は裁定273(2026-08-27)・裁定473(2026-08-30)・二層の裁定(2026-08-29)。

## 目的 — 自分の MV を1本作ること

動機は、自分が作りたい映像が既存の道具では作れないこと。それだけ。

「映像の生態系が AE か Blender かで閉じている」は外から確かめられる事実だが、
**大義であって目的ではない**。大義は後から付いてくる。

*これが切る物:* marketplace、カタログ、ガバナンス、第三者 SDK、配布規格、
他人向けの導線。全部「自分で1本できた後」。

## 持ち物 — 編集の意味と、エフェクトのデータだけ

技術は rerun の物。GPU も、デコードも、描画も、色も、**式も**借りる。
W3C Compositing の17モードは既に依存ツリーの中に在る。

*これが切る物:* 自作のブレンド式、第二の spatial engine、独自の色管理、
Lottie importer(Lottie は審判であって交換形式ではない)。

## 手つき — 土台は一般の動画編集ソフト、AE はその上の第2層

Premiere・DaVinci・Final Cut・CapCut・AviUtl の交差集合が土台。
AE 自体がガラパゴスなので、土台に据えると孤島を継承する。

*これが切る物:* AE の痛点の移植。AE 利用者の乗り換えを主目的に据えること。

## 世界の形 — 物は空間に居る

板も立体も点群も、同じ空間に立ったまま置かれる。
畳むのは書き出す時と、畳めと言われた時だけ。見る事と出す事は別。

*これが切る物:* 2D/3D の札。視点が出力に混じること。

## Vism — 配布規格ではなく、LLM が単独で書ける道路

エフェクトを大量に並列生産して自分の環境を最強にするための口。
要求は「配布できること」ではなく、**契約が小さい・ホスト内部を知らずに書ける・
単体で試せる・並行して書いても衝突しない**。

## 優先順位の根

**自分が1本作るのに要る物が先。それ以外は後。**
...
以下は Vism を**配布規格**として構想していた頃の文書。裁定273で大義の側へ降ろされた。
リンクの宛先として残す。ここに書かれた優先順位で実装しない。
...
RN、Rerun、Skiaが共存する`ui/motolii-rn/`を唯一の製品sourceとして使う。Stage／spatial runtimeは固定Rerun subsystem、Timeline／Curveはrust-skia、media／exportは既存HostとFFmpegを使う。旧R0 UI sourceはmainに残さずGit履歴だけへ置く。Rerunを製品全体の永続authorityにはせず、Motoliiを第二のspatial engineにもしない。Vismは原則Rerun custom visualizerへ載せ、Rerunの機構を外すのはnamed product oracleとの実測衝突がある時だけとする。
...
- **区間イージング（2026-07-10決定）**: Flow/Alight Motion式に、2キーフレーム間の正規化補間をpreset・Bezier handle・Overshootで編集する。実時間×実値のmulti-key Graph Viewは[React比較記録](reviews/2026-07-19-graph-view-reference-decision.md)に留まり、製品採択・M3 task化は未決である。
  ...
  - **スコープの切り分け(混同禁止)**: Graph Viewは**時間方向の値グラフエディタ**、Interval Easing Editorは1区間の正規化time remap、**空間モーションパス**は位置の2D経路である。三者を同じcurve state、座標、操作面へ統合しない。空間モーションパスはv1コアには入れず**プラグイン領域 or v1後半**として切り分ける。
