...
RN、Rerun、Skiaが共存する`ui/motolii-rn/`をcopyせずその場で製品sourceへ繰り上げる。Stage／spatial runtimeは固定Rerun subsystem、Timeline／Curveはrust-skia、media／exportは既存HostとFFmpegを使う。`ui/motolii-rn-legacy/`はread-onlyの旧shellである。Rerunを製品全体の永続authorityにはせず、Motoliiを第二のspatial engineにもしない。Vismは原則Rerun custom visualizerへ載せ、Rerunの機構を外すのはnamed product oracleとの実測衝突がある時だけとする。
