...
この再利用可能な表現単位を**映像表現モジュール（Visual Module）**と呼ぶ。定義は「時間・入力・型付きparameterから、映像または制御値を生成する表現単位」。音楽制作におけるVST pluginと役割は似ているが、音声用VST pluginを読み込む機能、VST SDK/APIの実装、DAW化を目標にするという意味ではない。VSTから参照するのは、Hostと表現実装を分離し、時間・parameter・automation・保存互換を共通言語にすることで、小さな作者と制作資産の生態系を成立させた構造である。AEのプラグインAPIやproject構造を再現することも目的にしない。
...
この北極星は、現在の完成条件を膨らませる口実ではなく、**迷った時に境界を削るための判断基準**である。v1はまず一人の制作者が3〜5分のMVを最後まで完成できることを優先し、Visual Moduleの動的配布marketplace、第三者SDK、独自plugin UI、音声用VST pluginの読み込み等を完成条件へ追加しない。一方、短期実装の都合で将来の表現単位をHost内部へ焼き込み、公開境界を閉ざすこともしない。
