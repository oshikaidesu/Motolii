...
Motoliiは、持ち運べる映像表現Vismを演奏・編集する最初のHostとして、**クリエイターの感覚的な意図を、軽量で再現可能な画へ翻訳する**。「知覚表現コンパイラ」はこのHostの働きを説明する内部比喩であり、上記の一文定義、Vism、Kit、Project、plugin capabilityを置換する新しい製品種別や実装層ではない。
...
- **Host UIとコミュニティUIは同じkitを使う(2026-07-21決定、公開方式は比較中)**: Hostだけが完全なcomponentを使い、第三者を縮小した別toolkitへ閉じ込める二層構造を長期の正規形にしない。`NodeDesc`自動生成panelは全保存paramを操作できる必須fallbackとして維持する。自由UIのruntime、sandbox、権限、互換、配布はG0-9完了まで公開契約にしない。根拠は[React / WebView再選定](reviews/2026-07-21-m3-react-webview-runtime-reconsideration.md)と[歴史的plugin UI境界](reviews/2026-07-12-plugin-ui-v1-boundary.md)
- **UI runtime責任境界をReact chrome + native GPU workspaceへ固定する(2026-07-21決定)**: React / WebViewはAsset Browser、Inspector、parameter/form、panel、toolbar、dialog、検索、設定、Host/community共通kitを所有し、StageとTimeline全体(ruler/track header/lane/clip/key/playhead/graph/selectionを含む)はnative Rust/wgpuが所有する。Canvasやbrowser WebGPUが必要なこと自体をReact所有の理由にせず、高頻度直接操作面はnativeへ置く。native側は汎用widget toolkitを再実装せず、renderer非依存のheadless interaction部品を優先し、Motolii固有のsnap/selection/transient preview/D2 commitだけを所有する。通常windowのtopologyは1 top-level wgpu Surface内のStage/Timeline viewport + opaque child WebView islandsへ固定した。direct wgpu + Vello局所passの製品採否、platform受入、egui撤去、plugin sandbox/公開契約はG0-9実機spikeまで未決。正本は[UI runtime責任境界](ui-runtime-architecture.md)と[surface topology](reviews/2026-07-21-ui-surface-topology-decision.md)、証拠は[再選定](reviews/2026-07-21-m3-react-webview-runtime-reconsideration.md)と[egui採用時の証拠](reviews/2026-07-18-m3-egui-selection.md)
