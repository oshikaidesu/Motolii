...
- **問題外の責任を増やさない**: スクラッチ実装の初期差分が最短でも、OS差、更新追従、権限、試験、配布、移行まで所有すれば総経路は重い。Motolii固有の意味と性能境界だけを直接所有し、一般問題は既存実装、薄いadapter、外部runnerへ委ねられない理由を確認してから自作する。依存が持ち込むversion、license、供給網、撤去責任も同じ表で数える。運用正本は[依存優先・責任最小化ゲート](reviews/2026-07-24-dependency-first-responsibility-gate.md)とする。
