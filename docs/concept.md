...
- **UI runtimeはG0-9再選定中**(2026-07-21): eguiの既存wgpu device/native texture共有、Apple M4 / Metal、IME、lifecycleの証拠とU0a骨格は保持する。React / WebView、Reactモック資産、hot reload、局所Canvas/browser WebGPU、native wgpu Stage合成を同じfixtureで比較し、CPU pixel bridgeは候補共通で拒否する。toolkit依存を`motolii-ui`へ隔離し、UI型・px・eventをdomainへ出さない境界は不変。詳細は[再選定](reviews/2026-07-21-m3-react-webview-runtime-reconsideration.md)、[egui採用時の証拠](reviews/2026-07-18-m3-egui-selection.md)、M3仕様
