...
| Inspector / Effect | 選択対象の意味とparameter編集 | query／selection結果を表示入力にできる。`re_component_ui`はproperty UIの先例 | runtime `WRAP` + UI `PATTERN ONLY` | primary selection、型付きparameter、form buffer、D2 command、diagnostic | Echo BloomとLottie Path Operations（既存`PathOp` 8種）のRN local selection state。PathOpのDocument適用／D2とStage再評価は未接続 |
