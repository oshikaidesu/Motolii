...
| keyframe easing | Flow / Alight Motionの区間選択→Interval Easing Editor | 区間editorをmulti-key Graph Viewと呼ぶこと |
...
- 密度と列幅は英語基準で設計し、言語ごとのhard-coded layoutへ分岐しない。追加language packは同一fixtureで表示検査し、全テキストはelide+tooltipを既定とする。翻訳の伸長でレイアウトが壊れないことはG0-6の疑似長文locale(pseudo-locale)fixtureで機械検査する
