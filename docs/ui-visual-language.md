...
- 組み込みテーマとして `Motolii Light` と `Motolii Dark` を同格で提供し、初回起動の既定値は `Motolii Light` とする
...
- clean profileの初回起動はlight、設定でdarkを選んだprofileの再起動はdark、theme fileが壊れたprofileは診断付きでlightへfallbackすることを統合テストする
