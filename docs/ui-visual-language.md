...
| egui実装 | Rerunの`re_ui`、dense list/property row、dark/light theme、panel、selection、notification、component catalogとsnapshot試験 | Rerunの具体色・font・icon・専門家向け語彙・画面階層をそのままMotoliiへ複製すること |
| multi-key Graph View | Apple Motionのparameter list+graph構成、Cinema 4Dのtangent直接操作、Mayaのtangent状態語彙、Blenderの表示navigation | AEの複雑な全機能、自動平滑化による離れた区間への暗黙伝播 |

OpenCut等は操作と配置、Abletonは制作ソフトの操作トポロジーと視覚言語、Appleは抑制された視覚階層、Rerunはそれらをegui製品componentへ落とす実装先例である。Abletonの参照範囲は[UI操作言語](ui-interaction-language.md#21-主参照はプロ用ソフトの操作トポロジー)に限定し、「Ableton風」を理由にArrangement Viewの画面構成やDAWの概念を持ち込まない。Rerunも同様に、[学習・転移計画](reviews/2026-07-20-rerun-learning-transfer-plan.md)のRR-1/RR-8を通してcomponent責任と試験方法を学び、Rerun風の画面を目標にしない。
...
- Media Browserの視覚探索は文字量を増やさず、`thumbnail-only / thumbnail+name / list`を選択可能にする。Media thumbnailは全modeで16:9の枠へ`contain`表示し、見栄えのための中央crop、自動トリミング、縦横比変更を禁止する。余白はneutralなletterboxとし、thumbnail-onlyで必要な名前・関係状態はaccessible name、tooltip、focus、短いoverlayへ退避して、文字サイズを下げて詰め込まない。folderだけは画像内容から識別できないため、16:9枠内の下端overlayで名前を常時表示し、通常Media名の省略規則を適用しない
...
- gridの列数や均等な余白を維持するために可読文字を縮小しない。文字・hit targetの可読scaleを先に守り、幅が足りなければ列数を減らす、補助情報を折り畳む、scroll/reflowする。余白充填や固定列数は合格条件にしない
...
制作中に文字を読み直さず固定領域へ戻れるよう、Media、Inbox、Plugins、Stage、Inspector、Timelineへ小面積の位置色を割り当ててよい。Media内のProjectと登録folderは別panel色へ分裂させず、source icon、名称、選択outline、`IN PROJECT`等の関係表示で区別する。Abletonの色による再発見から借りるのは、色を作品装飾にせず反復認知へ使う点である。
...
Ableton Live 12がメインwindowと第2windowの表示倍率を調整できる先例を持つように、MotoliiもOSのDPI scaleとは別に、利用者がUI全体の表示倍率を変更できることを機構要件とする。

- UI scaleはUser settingsであり、Document、journal、Undo、plugin parameter、render設定、Final出力へ入れない
- font、icon、control、hit target、spacing、罫線、panel chromeを同じlogical scaleから導出する。文字だけを拡大してcontrolからはみ出す実装や、個別componentごとの倍率分岐を作らない
- UI scaleとTimeline/Stageの内容zoomを別操作・別状態にする。UI scaleを変えてもproject時刻、key位置、Stage View、Output Frame、Document serialize、Final pixelは変わらない
- 拡大時は固定px座標で切り落とさず、既存のpanel resize、elide+tooltip、scroll、優先度に基づくtoolbar reflowで同じ機能へ到達できること。縮小時もhit targetとfocus indicatorの識別性を失わない
- 別window/別monitorでもscaleを正しく再解決する。windowごとの独立倍率を保存するか、global倍率へ追従するかはU1eの実機spikeで決め、モックやegui実装で恒久形式を先取りしない
- 許容範囲、刻み、既定値、shortcut、OS scaleとの合成規則はG0-6の100/125/150/200%比較とU1eの実機結果で固定する

### 高コントラスト表示

Ableton Live 12のHigh Contrast optionを先例に、MotoliiはDark/Lightの名称とは独立した高コントラスト表示をUser settingsとして提供する。これは任意の彩度調整ではなく、semantic token解決時に境界・文字・icon・focus・状態差を強めるaccessibility modeである。

- Dark/Lightの両方から同じcomponent stateと意味roleを保った高コントラストvariantを決定的に生成する。component側へ`if high_contrast`の場当たり色分岐を置かない
- 通常modeで定めた最低contrastを緩めず、panel境界、disabledとの差、focus、selection、warning/errorを同一fixtureで再測定する
- 高コントラストでも色を唯一の手掛かりにしない。icon、outline、線種、pattern、位置の併用規律を維持する
- mode変更はDocument、Undo、preview評価、Final pixelを変えず、再起動後にUser settingsから復元する
- custom themeが高コントラストvariantを解決できない、または検査に失敗した場合は診断を出し、安全な組み込み高コントラストthemeへfallbackする
...
- UI fontはUser settingsとして`Default / System / Custom`を選択可能にし、Document、project、Undo、render、plugin契約へ保存しない
- 本文UI font、tabular数字を保証するtechnical font、CJK fallback、icon assetを別roleにする。1つのcustom fontへtimecode、全言語、iconまで背負わせない
- system/custom fontはglyph coverage、baseline、主要記号、数字幅を読み込み時に検査する。不足glyphは既定fallback chainで補い、読み込み不能時は診断付きで組み込みfontへ戻す
- font変更時もcomponentの意味、focus順、hit target、UI scale、Document/Final結果を変えない。英語・疑似長文・CJK sampleを同じreference fixtureへ流し、文字切れとcontrol重なりを検査する
- custom font fileのpathと選択は端末ローカルのUser settingsに限定し、project共有やplugin packageへ暗黙同梱しない

### 多言語と文字幅

- UIのsource locale、初期表示、fallback、reference fixtureは英語(`en-US`)を正本とする。最初の製品審判は英語だけで行い、日本語は最初の追加language packとして別patchで扱う
- v1の言語機構はLTRに絞る。日本語、簡体中文、韓国語、独、仏、西等は翻訳catalog(.po)の追加だけで増やせることを要件とし、UI code、Document schema、`CommandId`、plugin IDの変更を要求する設計を不合格とする
- language選択はUser settingsであり、Document、project、Undoへ保存しない。catalog欠落時と未翻訳messageは英語へfallbackし、表示文言をdomain identityやcommand routingに使わない
...
- UI表示言語と作品内テキスト入力を分離する。英語UIだけを出荷する段階でも日本語IME受け入れU1dは維持し、利用者の作品へ日本語を入力できることをUI翻訳patch待ちにしない
...
- UI scaleの100/125/150/200% fixtureで、同じ操作が同じdomain intent/commandを発行し、Document serialize、Stage View、Output Frame、Final pixelが不変であることを検査する
- Dark/Lightそれぞれの通常/高コントラストreference imageとcomponent state matrixを生成し、mode変更のUser settings復元、custom theme失敗時の安全なfallbackを検査する
...
- 英語source catalogのmessage key欠落、UI code内の未登録user-facing文字列、未翻訳時の英語fallbackを検査する
- Default/System/Custom font fixtureで英語・主要記号・tabular数字・CJK fallbackを検査し、欠落または読込不能custom fontから組み込みfontへ安全に戻ることを確認する
...
- UI scale 100/125/150/200%で主要領域と頻用操作へ到達でき、文字切れ、control重なり、過小hit targetがないか
- Dark/Lightの高コントラスト表示でpanel境界、focus、selection、disabled、warning/errorを識別でき、通常modeより情報階層が崩れていないか
...
- **実装先例**: Rerunはegui 0.35でdesign token、dense component、dark/light、GPU viewport、component snapshotを一つの製品へ統合している。これはMotoliiの具体token値を決める根拠ではなく、eguiで視覚基準を実装・検査できることの先例
...
- [Ableton Live 12 Manual — First Steps / Display & Input / Theme & Colors](https://www.ableton.com/en/manual/first-steps/)
- [Ableton Live 12 Manual — Accessibility Options](https://www.ableton.com/en/live-manual/12/accessibility-options-in-live/)
