...
| INF-8 | **DX: WGSLホットリロード(開発ビルド限定)+高速起動/React UI反復計測** — AE型「再起動地獄」の予防([dev-experience.md](dev-experience.md)、[作者経路](reviews/2026-07-27-wgsl-hot-reload-author-journey.md))。**非ブロッキング評価**: 不合格でも基盤採否には影響しない | プラグイン作者(人間・LLM目視局面)の反復速度は採用の入口。パイプラインはホスト所有なのでホスト単独で差し替え可能 | (a) dev WGSL保存→正確なsource snapshot→最新候補だけを既存render serial pointでactivate→通常request経由で新frame表示。error時はlast-good artifactでPreview継続し`source != active`を表示、未反映sourceのExportは型付き拒否。burst、A→B→A、修正回復、`Rebuild required`、stale候補、device lost分離をfixture化しevent→displayを計測 (b) 起動→INF-6復元を計測 (c) React component変更→HMRまたはproduction bundle再読込→同じHost snapshot/session再投影→reference screen表示の所要時間とerror復旧を記録。eguiは既存baselineの比較値だけにする | P2 | 未着手(aはM3 render workerの製品接続後。未実装M4 frame cacheを前提にしない。bはINF-6後、cはM3 product package後) | dev-experience, INF-6, F-10, M4, M3 |
