...
| V2-8 | **プラグイン生態系(発見・導入・再現)**: 分散tap / lock·kit共有 / D&D import / `verify`·`repair`·`doctor`。中央ランキングなし。UIはReaPack基調(**Civitai Manager UI不採用**)。コミュニティ主回路=使用セットの書き出し→インポート | F-9, G-3, [plugin-ecosystem.md](plugin-ecosystem.md) |
