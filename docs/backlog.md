...
| GAP-3 | **メディア再リンク/オフライン素材**(相対/絶対パス、素材移動、欠落時UI) | NLEの基礎。内容同一性formatは[直列核4契約](reviews/2026-08-08-serial-core-known-contracts-decision.md)で`SourceFingerprintV1`（source exact bytesのSHA-256+size）へ決定したが、現行`resolve_asset_path`は候補の実在だけを見て指紋を照合せず、同名誤採択、一括再リンク、offline状態、永続D2 locator更新が未成立 | strict fingerprint codec／Asset admit-removeを先に実装し、次にpath候補探索、full fingerprint照合、ユーザー確認、同一`AssetId`のlocator更新を別責任で接続する。異なるbytesはsilent relinkせず将来の明示Replace Sourceへ送る。legacy head/tailや短いraw hashをauthorityへ昇格しない | P2 | M2(Asset), M4-K4, [直列核4契約](reviews/2026-08-08-serial-core-known-contracts-decision.md), [Unit 5F回収](reviews/2026-07-23-historical-media-portability-gpu-resurvey-plan-recovery.md) |
