...
| GAP-13 | **再評価中**: `NodeDesc`自動panel fallbackは維持。Host/コミュニティ同一UI kitを長期原則とし、自由UIのruntime、sandbox、権限、互換、配布をG0-9で比較 | 別kitは参加窓口を狭め、無制限自由UIはtheme/input/a11y/resource/securityを分岐させる | [React / WebView再選定](reviews/2026-07-21-m3-react-webview-runtime-reconsideration.md)の同一fixtureとSTOPを通す。比較前に公開API・永続形式を作らない | **P1** | G0-9; plugin-ui-model, M3, F-8, GAP-2, F-1 |
...
| V2-8 | **プラグイン専用UI語彙の解凍判断**: Host所有の宣言レイアウト / gizmo / curve / visualizationと、Host同一kitのcommunity panelをG0-9結果から再分類する。`NodeDesc`標準panelで全保存paramを操作できる条件を維持し、runtime・sandbox・権限・互換・配布を分けて決める | [UI runtime再選定](reviews/2026-07-21-m3-react-webview-runtime-reconsideration.md), [歴史的plugin-ui境界](reviews/2026-07-12-plugin-ui-v1-boundary.md), M3 |
