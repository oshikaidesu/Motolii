...
| GAP-6 | **決定済み**: 入力/全ショートカット再割当&アクセシビリティ(SlintのAccessKit、IME前提の入力設計) | 実装待ち。意味論を実装者判断へ戻すとIME、keymap、状態寿命が分岐し、一部操作だけhard-codeされる | [M3着手前決定§2](reviews/2026-07-16-m3-preflight-decisions.md#2-g0-2-inputとui状態の意味)どおりU0b〜U0dを実装。全bindingを追加/置換/無効化できるversion付きJSON fallbackとraw key分岐拒否を審判し、OS別IMEはU1dで実機審判 | P2 | M3 G0-2完了/U0b〜U0d |
...
| GAP-13 | **決定済み(縮小採用)**: plugin UIはv1 Host自動生成panelのみ。宣言語彙は型ごとに解凍し、自由`.slint`/wgpu UIは延期 | 自由UIを公開契約へ戻すと共通component、theme、input、a11yがpluginごとに分岐する | [M3着手前決定§3](reviews/2026-07-16-m3-preflight-decisions.md#3-g0-3-plugin-uiは表現を開き操作文法を閉じる)と改訂済みM3§拡張方式を維持。新語彙は互換・保存・Undo・a11yを型ごとに解凍 | **P1** | G0-3完了; plugin-ui-model, M3, F-8, GAP-2, F-1 |
| GAP-14 | **Shared Effect lifecycle決定** — 参照中Definitionの削除、Useのunlink/copy-local、未参照Definitionの保持/GC | Resolve Shared NodeとNuke Cloneは共有recipe+複数Useを支持するが、原本/clone削除の所有意味は同一でない。ここをUI都合で埋めるとD1l migration後のproject意味を壊す | reject/cascade/materializeの反例表、各操作の1 Undo、未知plugin保持、削除→Undo→再保存roundtripを決定。D1lは判断までschema実装禁止 | **P0** | [#166](https://github.com/oshikaidesu/Motolii/issues/166), [先例境界監査](reviews/2026-07-15-prior-art-complaint-boundary-audit.md), M2-D1l, GR-PV |
...
| ID | タイトル | 完了条件(方向) | 関連 |
|---|---|---|---|
| SCR-1 | runtime非依存Editor Generator hook: 型付きD2 command batchをpreflightし1 macroでcommit | 成功=Undo 1回、失敗/cancel/stale snapshot=Document・履歴不変、`&mut Document`/Slint/JS型を公開境界へ出さない | M3-U9a, D2, GR-UI |
...
| V2-8 | live JS layer/毎frame expression(JS runtimeが評価経路へ常駐) | SCR-1〜4とは別境界、PP-Gate |
