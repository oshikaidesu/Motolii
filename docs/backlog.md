...
| GAP-14 | **書き出しRGB→YUVのGPU化**(`encode.rs`のswscale色変換を`ColorParams`逆変換+packシェーダへ置換)。ユーザー方針(2026-07-14: 書き出しはGPU資産を使う)+先例=OBS libobs。実装スケッチは[2026-07-14調査メモ](reviews/2026-07-14-color-conversion-prior-art.md)§3(**反対側レビュー未実施**) | F-5ガード「色変換をmotolii-gpu 1箇所に閉じる」の唯一の例外がswscaleに残り、係数・siting・丸めが資産管理外。エンコーダへのRGB直渡し構造はhwエンコーダ対応時の色事故経路(NVENC内部変換) | GPU逆変換 vs CPU参照/swscale外部正解/往復ゴールデンの3テスト緑+`-vf scale`の色変換削除(色タグ4種は維持)。lavapipeで決定的 | P2 | F-5, B-3, B-4, GAP-5, INF-3 |
