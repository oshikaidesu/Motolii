...
| GAP-13 | **ホストレス拡張カタログ(発見・導入の最小地図)**。中央ストアなし。GitHub 索引クライアント | AviUtl2圏は[aviutl2-catalog](https://github.com/Neosku/aviutl2-catalog)等が既存資産。無いと新規ユーザーが流れる。中央index/人気順は採らない | 探す(存在・語彙・updated)+D&D/ワンボタン導入+ローカル installed。アカウントなし。設計=[plugin-ecosystem.md](plugin-ecosystem.md)§9 | **P1** | concept、F-8、F-9、M3、V2-8 |
...
| ID | タイトル | 関連 |
|---|---|---|
| V2-1 | 動的プラグインロード(C ABI / `abi_stable`)。**発見カタログ(GAP-13)とは別軸** | A-2, G-1 |
...
| V2-8 | **プラグイン生態系の重い層**: 分散tap本実装 / lock·kit UI / Sync / `verify`·`repair`·`doctor` / 商用packメタ。中央ランキングなし。UIはReaPack基調。**早期地図はGAP-13** | F-9, G-3, GAP-13, [plugin-ecosystem.md](plugin-ecosystem.md) |
