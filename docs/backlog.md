...
| GAP-3 | **メディア再リンク/オフライン素材**(相対/絶対パス、素材移動、欠落時UI) | NLEの基礎。プロジェクト移動で素材ロスト→再リンク導線が無いと実用不可。現行`Asset`は`content_hash`/`size_bytes`/`head_hash`/`tail_hash`を生文字列と任意欄で持つが、同一性formatは未締結 | パス解決規約+欠落検出+再リンクのモデル。M4 `source_id`と別形式を作らず、共有するversion付き指紋についてalgorithm、head/tail chunk長、encoding、size、任意full hash、collision時の照合を先に決める。歴史案のXXH3/N MiB/hexを未裁定defaultとして焼かない | P2 | M2(Asset), M4-K4, [Unit 4C回収](reviews/2026-07-23-historical-d1-spec-holes-lineage-recovery.md) |
