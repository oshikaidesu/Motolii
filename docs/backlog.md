...
|---|---|---|---|---|---|---|
| INF-1 | **S1 Slintスパイクを実機GUIで実走**しIME/スレッド分離/Manualデバイス共有の合否を記録 | ヘッドレスで未実走=**UI基盤(Slint)前提が未検証**。IME不合格なら基盤が覆る | `docs/spikes/s1-slint.md`に合否記録(特にIME)。M3仕様を確定。**あわせて公式live-preview(`SLINT_LIVE_PREVIEW`)の実用性を評価し記録**([dev-experience.md](dev-experience.md)) | **P0** | 未着手(要GUI・人間) | M0-S1, dev-experience |
...
| INF-8 | **DX: WGSLホットリロード(開発ビルド限定)+高速起動の計測** — AE型「再起動地獄」の予防。純関数契約+ホスト所有PipelineCacheの副産物として安く取れる([dev-experience.md](dev-experience.md)) | プラグイン作者(人間・LLM目視局面)の反復速度は採用の入口。パイプラインはホスト所有なのでホスト単独で差し替え可能 | devビルドでWGSL編集→次描画に反映(該当キャッシュ無効化込み)の統合テスト、コンパイルエラー時に直前パイプラインで描画継続。起動→INF-6復元の所要時間を計測記録 | P2 | 未着手(INF-6・M4キャッシュ後が安い) | dev-experience, INF-6, F-10, M4 |
...
| GAP-13 | **プラグインUIモデル(宣言語彙 vs 自由描画)** — ホスト標準ウィジェット語彙(XYパッド・カーブ等)/宣言的レイアウト/ビューポートギズモ宣言の4段はしご。自由描画UI(AE `PF_CUSTOM_UI`型)はv1で開けない | 「プラグインにUIをどこまで開けるか」は後戻り不能な公開契約。先に自由描画を開けるとAE圏の再演(UI不統一・ホストがテーマを進化させられない)。空間パラメータ(x/y同時操作)のUI形はM3設計の前提 | [plugin-ui-model.md](plugin-ui-model.md)の批判レビュー+AM実機確認(同§6)→ M3仕様へレベル1/3の語彙を反映。`ParamDef`UIヒントの加法的追加は解凍手続き要否を確認 | **P1** | 設計ノート済・批判レビュー待ち | plugin-ui-model, F-8, GAP-2, M3, F-1 |
