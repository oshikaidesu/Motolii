...
| INF-8 | **DX: WGSLホットリロード(開発ビルド限定)+高速起動/React UI反復計測** — AE型「再起動地獄」の予防([dev-experience.md](dev-experience.md))。**非ブロッキング評価**: 不合格でも基盤採否には影響しない | プラグイン作者(人間・LLM目視局面)の反復速度は採用の入口。パイプラインはホスト所有なのでホスト単独で差し替え可能 | (a) devビルドでWGSL編集→次描画に反映、error時は直前pipelineで継続 (b) 起動→INF-6復元を計測 (c) React component変更→HMRまたはproduction bundle再読込→同じHost snapshot/session再投影→reference screen表示の所要時間とerror復旧を記録。eguiは既存baselineの比較値だけにする | P2 | 未着手(a/bはINF-6・M4キャッシュ後、cはM3 product package後) | dev-experience, INF-6, F-10, M4, M3 |
...
| GAP-3 | **メディア再リンク/オフライン素材**(相対/絶対パス、素材移動、欠落時UI) | NLEの基礎。内容同一性formatは[直列核4契約](reviews/2026-08-08-serial-core-known-contracts-decision.md)で`SourceFingerprintV1`（provenance tag付きsource exact bytes SHA-256+size）へ決定し、source-consuming workerはfingerprintとexact bytesを結合したHost-private SourceBindingだけを使うが、現行`resolve_asset_path`は候補の実在だけを見て指紋を照合せず、同名誤採択、一括再リンク、offline状態、永続D2 locator更新が未成立 | 前提3件(strict fingerprint codec／Asset admit-remove／budgeted SourceBinding)は2026-08-09 main到達済み(`d273061d`/`49ef4833`/`a287c828`)。次にpath候補探索、full fingerprint照合、ユーザー確認、同一`AssetId`のlocator更新、offline→online時のHost-private availability／current-state request refreshを別責任で接続する。異なるbytesはsilent relinkせず将来の明示Replace Sourceへ送る。legacy head/tailやprefixなしraw hashをauthorityへ昇格しない | P2 | M2(Asset), M4-K4, [直列核4契約](reviews/2026-08-08-serial-core-known-contracts-decision.md), [Unit 4C回収](reviews/2026-07-23-historical-d1-spec-holes-lineage-recovery.md), [Unit 5F回収](reviews/2026-07-23-historical-media-portability-gpu-resurvey-plan-recovery.md) |
...
| GAP-7 | プロジェクト/素材のパッケージ化・可搬性(collect files相当) | 納品・バックアップ・別マシン移行で必要。全部内包と外部参照はsize、共同作業、欠落責任を交換するため、単一boolやfolder conventionでは閉じない | GAP-3の`SourceFingerprintV1`を再利用し、収集対象、manifest、重複、欠落、font／plugin／proxy、部分納品、再open検証を調査後に分解。schema予約とv1実装を別判断にする | P2 | M2, M4-K4, F-5, [Unit 5F回収](reviews/2026-07-23-historical-media-portability-gpu-resurvey-plan-recovery.md) |
...
| GAP-26 | **ffmpeg process／export artifact reliability** — 現行はfinish時stderr drainとerror後finishを持つが、処理中continuous drain、ffprobe成功検証、temp→atomic install、teardown timeout、起動時capability probeが未到達。decodeも`next_frame`がChild mutex保持中にstdoutをblockし、外部killが同mutexを必要とする | frame入出力中stderr floodで相互停止し得る。stalled decodeをkillできず古いseekが残る可能性がある。最終pathへ直接`-y`すると中断・失敗で既存正常成果物を失い、部分mp4が残る。exit 0だけでは期待streamを証明しない | G1→G2/G3→G4→G5を独立境界で閉じ、decode block中cancel／kill、長時間同時stderr、write/render失敗、SIGKILL、exit 0不正output、既存final byte不変、FD/zombie、古いtoolを負例化。部分mp4を公開成果物へせず、色tag／音声mux／同一render経路を維持 | **P1** | M0 S2, M1 G1〜G6, [Unit 5A回収](reviews/2026-07-23-historical-r1-export-gpu-safety-lineage-recovery.md), [Unit 5I回収](reviews/2026-07-23-historical-s2-decode-pipeline-lineage-recovery.md), motolii-media/export |
...
| GAP-32 | **現行製品Stage／実素材release受入** — M1 R9はprivate素材＋Slint／ProjectV1 spikeの歴史sign-offで、`verify-b4`はcaller可変max tolerance、CI fixtureは32×24・3 frame・overlay alpha 0 | M1完了を現行release proofへ外挿すると、visible composite、chroma、rotation、実解像度、player差が未審判のまま出荷合格になる。逆に歴史R9を撤回しても当時milestoneの意味を壊す | 現行Document＋製品Stageでpre-encode semantic oracleとcodec roundtrip固定metricを分離。user可変合否閾値を除き、licensed／local manifestへhash・codec・resolution・duration・command・tool/player/OS/GPU版を固定。visible alpha、chroma edge、motion、rotation、range端とhuman playbackを別審判で記録 | **P1** | M1 R9/T11, M3 product Stage, GAP-5/29/31, [Unit 5H回収](reviews/2026-07-23-historical-r9-real-material-export-acceptance-lineage-recovery.md) |
| GAP-33 | **workspace test fixtureのworktree-root `target/`排出廃止** — plugin/testkitのbuild scriptとtestが`CARGO_TARGET_DIR`に関係なくroot `target/`へ派生物を作るため、監督runnerは既知三entryの清掃契約を持つ | hardcodeはfail-closedだがproducer追加のたびにrunner更新が必要で、自己更新bootstrapとscope列挙量を増やす。一般cache cleanerへ育てると監査境界を壊す | producerごとにOS tempまたは明示fixture rootへ隔離し、並列test衝突、失敗時残骸、path escape、cleanupを負例化。全producer移行後にGR-D3の既知entry listを段階的にゼロへ縮小する。`.gitignore`拡張や`target/**` allowlistで隠さない | **P2** | GR-D3, plugin/testkit build/test infrastructure |
