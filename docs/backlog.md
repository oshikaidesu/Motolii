...
| GAP-8 | **シェイプ間リンク(レイヤー参照付きParamSource)** — LookAt/Follow/ParentRef。AEエクスプレッション非採用の代替 | **M横断最大ギャップ**。現行ParamSourceに別レイヤー参照が無く「向ける・追従」が式か手キーフレームに戻る | M2スキーマ+oc-eval評価+F-3順序+M3ターゲットピッカー+M4無効化伝播の一括設計 | **P1** | concept, M2-D1/D3, M3, M4-K2, F-3 |
