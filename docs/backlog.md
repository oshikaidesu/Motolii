...
| V2-8 | **プラグイン生態系(発見・導入・再現)**: 分散tap / `plugins.lock.toml` / D&D import / `verify`·`repair`·`doctor`。中央ランキングなし。UIはReaPack基調(**Civitai Manager UI不採用**) | F-9, G-3, [plugin-ecosystem.md](plugin-ecosystem.md) |
