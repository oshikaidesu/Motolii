...
Motoliiの長期の北極星は、映像表現を特定projectの手順から切り離し、**映像表現モジュール（Visual Module）**として演奏・再利用・保存・配布できるようにすることにある。このplugin境界は単なる内製effect追加口ではない。Host全体をforkせず、ひとつの表現に集中できる作者面を作る。
