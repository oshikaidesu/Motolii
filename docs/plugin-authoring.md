...
3. **隠れた可変状態を持たない** — 出力は時刻`t`と入力(テクスチャ/params/ctx)だけで決まる。`&self`にキャッシュや乱数シードを溜めない。**フレームN-1依存の物理ステッパ・積分型シミュレーションも禁止**(2D/3D問わず。[concept.md](concept.md)横断決定)。必要なら外部ベイク済みを純関数再生
