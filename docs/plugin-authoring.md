...
実装の型紙は `crates/motoly-plugin` の参照プラグイン(`reference`モジュール)。  
...
- **テスト** — Render系は`motoly-testkit`ゴールデン。ParamDriverは値列の単体テスト

## 5. 最小スケルトン(Filter)

```rust
use motoly_plugin::{FilterPlugin, NodeDesc, PluginError, PluginId, ResolvedParams, TextureRef, ValueType};
