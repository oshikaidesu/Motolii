...
VSM-A0I-1〜3でContract Catalog、Documentのprepared resolution、graph／exportのruntime必須化までは実装済みである。VSM-A1-3でOpacityを外部crate `plugins/motolii-plugin-opacity` へ移し、`motolii-plugins-firstparty` で組み立てる。別crate作者面の公開façade、依存allowlist、first-party組み立て点は[A1S](reviews/2026-07-17-vism-a1-public-crate-boundary-spec.md)を正本とする。
