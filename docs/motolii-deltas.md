...
- **1属性=1文字の属性表示を持たない**(`P`/`S`/`R`/`T`/`A`) — 代わりに**述語で絞る**
  (`U`=キーか式を持つ物 / `UU`=既定値から変わった物 / `SS`=選択中の物。公式名は
  Reveal Animating Properties / Reveal Modified Properties)。AE の1対1は変形属性を
  固定5個に凍らせているから成り立っており、**vism のエフェクトが任意の宣言済み
  パラメータを持つ Motolii では最初の1本目で文字が尽きる**。述語は属性を名指さない
  ので個数に依らない(2026-08-30 利用者裁定)
