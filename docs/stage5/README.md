...
- **色の property(2026-09-13)**: 色は property で鍵が打てる(Inspector には行を出さない。置き場は未裁定)。stop を消すと `fill.stop.<n>` の track の番号がずれる(構造の変更は data)。Browser の「Used here」は書類の brush だけを数え、鍵で変えた色は載らない。
