...
- Motion Blur のサンプルは移動方向の前後両側へ伸びる。中心合わせの方針（[motion.rs](../../motolii/crates/motolii-render/src/extensions/motion.rs) の 50 行目 の `sample_times` が `-0.5 → +0.5`）どおりで、先例の Alight Motion・AE の既定と一致する。
