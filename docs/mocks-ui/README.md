...
`scripts/reference-generation.mjs`はGR-R3のtest-evidence専用publisherである。manifestと全PNGを一意なstaging generationへ書いてfile/directoryを同期し、immutable generationへrenameした後、`CURRENT`一ファイルだけを原子的に交換する。閉じたmanifest schema、browser/source SHA、screen/variant順、PNG byte SHA、decode済みRGBAを検証し、normalと同一画素の派生画像や別screenの重複normalを拒否する。全I/O checkpointの失敗注入では、readerが検証済みの旧世代か新世代の完全な一方だけを読めることを固定する。この形式はDocument、plugin契約、製品公開APIではない。

U0e-2の固定5画面は`#reference/*`だけで開き、`reference-provenance.json`をsource/fixture/capture正本とする。`npm run generate-reference`だけが`reference-output/`を更新し、`npm run check-reference`は固定Chromiumと同梱fontで30 PNGをread-only再現検査する。目視の採否は[U0e-2 / G0-6H handoff](reference-handoff.md)へ記入し、未記入の間は製品tokenへ昇格しない。
