...
状態: **板・網・点群・線(パス・文字の輪郭)で実装済み(線は 2026-09-13)。場の座標の既定は未決(§6)。**
...
| 線(パス・文字の輪郭) | 輪郭を ≤ 4 px に刻んだ頂点(§4-1) | 実装済み(2026-09-13) |
...
場の効果が乗った形・文字は、絵(raster の板)ではなく**輪郭を ≤ 4 px の直線に刻んだ mesh**として描く
(`path_model` の `step`、[`engine/texture.rs`](../motolii/crates/motolii-render/src/engine/texture.rs) の `FIELD_STEP`)。
頂点段の場がその頂点を動かすので、辺は縁まで鋭いまま曲がり、拡大しても甘くならない。

線(stroke)には**錨**がある: 線を三角形にした頂点それぞれが「自分の中心線の点」を持ち(fork の
`Material.field_anchor`、texcoord に載せる)、場はその点で評価される。両側が同じ量だけ動くので線幅が保たれ、
千切れない(錨無しだと外側と内側が別々に動いて線が裂ける — 2026-09-13 に実測)。

絵の効果(blur 等)が続く形は、絵に焼いてから場の板(§4 の格子)で受ける。warp は素材の絵が要るので同じ。
