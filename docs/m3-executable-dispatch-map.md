...
[TRIM実装受入](reviews/2026-08-04-cu-201p-trim-implementation-acceptance.md)に裏付けられる。`CU-201R`はcommit `d0f7dfec`と[oracle受入](reviews/2026-08-04-cu-201r-random-move-trim-oracle-acceptance.md)で閉じ、`CU-201E`は通常製品E2Eまで`DONE`、`U4b-0`もcontract/code/mainまで`DONE`である。通常Add Position Keyの入口は[Inspector Position entry reclosure](reviews/2026-08-04-inspector-position-key-product-entry-reclosure.md)でPosition行へ選定し、current-playhead carrierはcommit `75ccd5e7`で成立した。[direct-promotion contract](reviews/2026-08-04-inspector-position-row-direct-promotion-contract.md)のnormal Position row/projection `CU-0A08ITIA`は`DO`で、有限`Const(Vec2)`のX/Yまたはtag-only `Keyframes`の`animated` presenceだけを投影し、keyを評価・計数しない。typed Host intent/queueの`CU-0A08ITIB`は`WAIT_TARGET`である。これは`P04-C2` Easingのactive interval/outgoing Interp欠落と別である。[P02-C3 playhead implementation acceptance](reviews/2026-08-04-native-timeline-editor-playhead-implementation-acceptance.md)によりruler producer/carrierはcommit `75ccd5e7`でcode/main `DONE`。[P04-C2 diagnostic correction acceptance](reviews/2026-08-04-p04-c2-diagnostic-correction-implementation-acceptance.md)によりdiagnostic correctionも`DONE / ACCEPTED`となり、popup terminal visual routeは`WAIT_TARGET`のままである。現在のimplementation `DO`は`CU-0A08ITIA`だけである。
...
| `P04-C2` | `SPLIT / DIAGNOSTIC-CORRECTION DONE / ACCEPTED / POPUP-TERMINAL CONTRACT_CLOSED / TERMINAL_VISUAL WAIT_TARGET` | `ACTIVE-INTERVAL`と`INTERP-COMMAND`に加え、既存diagnostic consumerへの`SetPositionKeyInterp` label correctionも[implementation acceptance](reviews/2026-08-04-p04-c2-diagnostic-correction-implementation-acceptance.md)どおりDONE / ACCEPTED。terminal-adoption amendmentはProductApp sole EventLoop/shared GpuCtxとG0-9 PATTERNを固定したが、exact product popup renderer targetは無い。partial React/IPC routeとrenderer portは採用しない。Inspector Position Add Key routeは別の`CU-0A08ITIA DO`（read-only）/ `CU-0A08ITIB WAIT_TARGET`（typed write）であり混同しない | popup visual codeは`WAIT_TARGET`; value-changing easing変更はまだ通常製品routeに到達しない。P04-C2内のnext implementation `DO`はなし |
