...
| `P04-C2` | `SPLIT / DIAGNOSTIC-CORRECTION DONE / ACCEPTED / POPUP-TERMINAL CONTRACT_CLOSED / TERMINAL_VISUAL WAIT_TARGET` | `ACTIVE-INTERVAL`と`INTERP-COMMAND`に加え、既存diagnostic consumerへの`SetPositionKeyInterp` label correctionも[implementation acceptance](reviews/2026-08-04-p04-c2-diagnostic-correction-implementation-acceptance.md)どおりDONE / ACCEPTED。terminal-adoption amendmentはProductApp sole EventLoop/shared GpuCtxとG0-9 PATTERNを固定したが、exact product popup renderer targetは無い。partial React/IPC routeとrenderer portは採用しない。Inspector Position Add Key routeは別の`CU-0A08ITI TARGET_MISSING`であり混同しない | popup visual codeは`WAIT_TARGET`; value-changing easing変更はまだ通常製品routeに到達しない。next implementation `DO`はなし |
