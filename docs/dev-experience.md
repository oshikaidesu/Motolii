...
- エディタUI(Slint側)のホットリロードは**自作案件ではない見込み** — Slint公式のlive-preview(§2)が既製で存在する。実用性の合否は**INF-1(S1実機スパイク)の評価項目**として確認する(devビルド限定フラグという形はレベル0と同型)。本文書の設計対象はプラグイン・シェーダの反復に限る
- プラグインが自前UIを持つ形のリロードは論点にならない — プラグインはUIを描かない([plugin-ui-model.md](plugin-ui-model.md))ため、ウィジェットは全部ホストコードでありエディタUI側の話に畳まれる
