...
ステータス: **ドラフト**(凍結ゲートで確定。**2026-07-13**: [#103決定パック](../reviews/2026-07-13-decision-pack-adoption.md)・**PathOp意味論表【決定】**・残小項目(Undo/ExportJob/Group/audio)採択。以後は原則コード依頼+受入。**2026-07-12**: D1-prelude・色契約・LayerId・時刻serde・duration半開・D1再分割・補間/変形意味論)
...
- **停止条件**: Autograph型Modifier列、汎用Element Domain、永続Constraint Graphは未決であり、本書の[PP-Gate](../interaction-simplicity-model.md#4-param-pipeline-gatepp-gate)等の独立仕様改訂前に既存`DocParam`へ焼かない。Gate前のRelative Moveは選択keyへ同差分を適用する一回のD2 macroに限定する。
