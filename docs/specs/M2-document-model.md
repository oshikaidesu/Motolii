...
| VSM-A0S | **仕様決定／A0I-1〜3完了**: D1f/D1hのfirst-party既知表をimmutable Contract Catalogへ移し、intrinsic validation／prepared contract resolution／executor availabilityを三段化する。raw load/save/recoveryは構造保持、製品open/Writerはcatalog、graph/exportは検証済みruntimeを必須とする。raw recipeはmigrationで変更しない | [A0D](../reviews/2026-07-17-vism-a0d-contract-migration-ownership-decision.md), GR-PV | `cb2c9a7`→`e4f42c6`→`057e2e9`。contract/runtime、prepared resolution、製品実行入口の順で独立コミットしworkspace全緑 |
