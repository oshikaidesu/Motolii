...
- **D2/U4b-0 follow-up（CORE実装済み・製品接続未実装）**: explicit Add Position Keyだけがfresh `KeyframeId`を導入する。帯域外`allocate_keyframe_id`+`SetProperty`は棄却し、Position専用`AddPositionKey` / `UndoAddPositionKey`に完全old/new `DocParam`、added ID、1 IDの`StableIdReservation`を持たせる。same-timeは既存IDを返すno-op、Undo後counter非巻戻し、Redo同一ID。Bezier分割は`motolii-eval`所有の同一solver APIを使い、区間外は端値clampを保存するcanonical Hold形とする。JournalEdit v2を維持し、stable-ID非対応文書は暗黙version更新せず拒否する（[closed contract](../reviews/2026-08-02-u4b0-add-position-key-closed-contract-decision.md)）。
- **D2/U4b-1 outgoing Interp（CORE実装済み・製品接続未実装）**: Position区間をLayerId + 隣接left/right KeyframeIdでaddressし、left keyのoutgoing `Interp`だけをold/new対称`SetPositionKeyInterp`で変更する。same-valueはno-op、stale old/successor/last key/invalid Interpは無変更拒否する。fresh ID、Document schema、JournalEdit v2 versionを変えない（[contract](../reviews/2026-08-02-u4b1-outgoing-interp-command-contract-decision.md)）。
