...
**【決定】ではない。** ユーザー指示により確定を保留。比較材料: [pathop-ae-cavalry-comparison](../reviews/2026-07-12-pathop-ae-cavalry-comparison.md)(豊富さ・実声・**拡張=Cavalry / 意味=Lottie(枯れ)の3層**)。確定後に本節を【決定】へ昇格し、その写しとしてD1i-2実装に入る(GR-PV-1)。
...
- コマンドの粒度(プロパティ単位 vs 操作単位)とUI側(M3)の編集状態との責務分担 — **決定先: [#103](https://github.com/oshikaidesu/Motolii/issues/103) 9項目目**(D2発注書起草前に承認。実装者に埋めさせない)
- 波形表示用のピークデータ生成をどこで持つか — **低優先化済み(2026-07-12)**: 音源はコンプ済みで情報量が薄いことが多く補助表示にすぎない。持ち場はM3/U3発注時にUI都合で決めてよい。**M2ブロッカーではない**
