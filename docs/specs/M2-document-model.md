...
ステータス: **ドラフト**(凍結ゲートで確定。**2026-07-12**: D1-prelude・色契約・LayerId・時刻serde・duration半開・D1再分割・補間/変形意味論・**PathOp意味論表(D1i-2文書側)を追加**)
...
## PathOp意味論表(D1i-2 / 【決定】2026-07-12)

予防GR-PV-1: **意味を先に焼き、実装・ゴールデンはその写し**。画素意味の変更は新variant(S16)。Lottie `ty` コードとAEパス演算子を前例にするが、単位はMotolii正準空間へ写す(Lottieの%・度をそのままスキーマに入れない)。
...
| 適用順 | `modifiers[i]` は **index 0から順**(D1i-1)。Lottie内容リストの「逆順合成」とはスタック表現が違う — Motoliiはroot stackなので順方向を正とする |
| 複数輪郭 | 入力パス集合の**各輪郭に独立適用**(Trim `Sequential`のみ連結長で扱う) |
...
| `op` | Lottie `ty` | パラメータ(型・単位・範囲) | 意味(先人整合) | 丸め/整数 | v1で焼かない席 |
|---|---|---|---|---|---|
| `pucker_bloat` | `pb` | `amount: F64` ∈ **[-1, 1]**。0=恒等。+1=頂点が重心へ(Lottie +100%)。-1=重心から距離2倍(Lottie −100%)。接線は頂点と逆向きに補間([lottie-docs](https://lottiefiles.github.io/lottie-docs/shapes/)) | AE Pucker & Bloat / Lottie Pucker Bloat | 範囲外は**validate拒否** | なし |
| `zig_zag` | `zz` | `amount: F64` ≥ 0(正準・峰谷距離=Lottie `s`)。`ridges: F64` ≥ 0(セグメントあたりの峰数=Lottie `r`) | AE Zig Zag / Lottie Zig Zag | `ridges`は評価時に`floor`して非負整数化。非有限拒否。**点種はCorner固定**(Lottie `pt=1`)。Smooth(`pt=2`)は将来variant | `point_type` |
| `offset` | `op` | `distance: F64`(正準。正=外向き、負=内向き=Lottie `a`) | AE Offset Paths / Lottie Offset Path | 接合はv1 **Miter・miter_limit=4**固定。自己交差で増えた輪郭は全て出力 | `line_join` / `miter_limit`(追加的拡張) |
| `round_corners` | `rd` | `radius: F64` ≥ 0(正準=Lottie `r`) | AE Round Corners / Lottie Rounded Corners | 幾何上、角に入らない半径は局所で縮小(クランプ)。負は拒否 | なし |
| `trim` | `tm` | `start`/`end`: F64 ∈ **[0, 1]**(Lottie 0–100%を単位区間へ)。`offset`: F64(**周回**。1.0=1周。Lottieの度/360)。`mode`: `parallel`\|`sequential`(既定 parallel) | AE Trim Paths / Lottie Trim Path。`start==end`→非描画。`start=0,end=1`(offset無視の恒等区間)→入力そのまま。[LAC式](https://lottie.github.io/lottie-spec/1.0.1/specs/shapes/)のwrapを周回単位で踏襲 | start/end範囲外は拒否。offsetは有限ならwrap | なし |
| `twist` | `tw` | `angle: F64`(**ラジアン**。スキーマの回転契約に合わせる。Lottie/AEの度はUI変換のみ) | AE Twist / Lottie Twist | 回転中心は**入力パス集合のバウンディングボックス中心**(導出。永続しない) | 永続`center: Vec2`(追加的。要なら`min_reader_version`上げ) |
| `wiggle` | (AE Wiggle Paths。Lottie 1.0コア外) | `amp: F64` ≥ 0(正準)。`freq: F64` ≥ 0(Hz=1秒あたり周期)。`seed: F64`(有限) | AE Wiggle Paths相当。時刻tと頂点indexとseedから決定論変位 | `seed`は`to_bits`でu64化しPRNG入力へ。アルゴリズム名 **`MotoliiWiggleV1`** をD1i-2実装PRで固定しゴールデン鎖。変更は`wiggle_v2` | 相関・大小サイズ分岐(AE詳細) |
| `repeater` | `rp` | `copies: F64` ≥ 1。`offset: F64`(Lottie `o`: 最初のインスタンスに掛ける変換の倍率オフセット)。**`transform: Transform2D`**(D1i-2で席追加。serde default=恒等) | AE/Lottie Repeater。コピーi(0..n-1)に `transform^(offset+i)` を適用([lottie-docs](https://lottiefiles.github.io/lottie-docs/shapes/))。`RenderCtx::instance=(i,n)`を供給(F-7) | `copies`は`floor`で整数化、`floor(copies)<1`は拒否。n上限は資源上限表(S10)に従う | コピー間opacityランプ(`so`/`eo`)はv1対象外 |

### D1i-2実装への拘束

1. 本表と食い違う「便利なデフォルト」は禁止(H-3)
2. validateは上表の拒否項目を型付きエラーにする(完了条件)
3. 意味論ゴールデンは上表の境界(恒等・空・Parallel/Sequential・Pucker ±1等)を最低限カバーする
4. `Repeater.transform`席の追加は**追加的**(default恒等)。旧JSONに無いキーはdefaultで読める — `min_reader_version`は上げない
...
| D1i-2 | **第二監査フォローアップ③b(S5)— PathOp意味論表と実装**: 仕様の「PathOp意味論表」節(2026-07-12焼成)にvalidate+幾何実装+variantゴールデンを一致させる。**Repeater.transform席追加**(default恒等)。Wiggleは`MotoliiWiggleV1`を実装PRで固定 | D1i-1 | 意味論表の全行が仕様に存在する(文書側**済**)。validateが表の拒否項目で型付きエラー。variantごとの保護された意味論ゴールデン。`Repeater`に`transform`があり旧JSONはdefault恒等で読める |
...
13. **意味を先に仕様へ書く**(GR-PV-1) — PathOp意味論表は仕様へ焼成済み。D1i-2は表への一致実装。表が無い新規演算は仕様改訂が先
