...
ステータス: **段階発注可**(コア締結宣言は**撤回** — [記録](../reviews/2026-07-14-m2-core-closure.md)。P1修復 #153/#154 完了。D7(#145/#149)は維持。**次発注可: D5(#144)**。再締結は[基盤再締結ゲート](../reviews/2026-07-15-m2-foundation-reclosure-gate.md)のA〜C全証跡後)。
...
| D1l | **READY**(Issue化可): Effect Definition/Use共有schema+inline migration。plugin/version/enabled/params/extraを持つ`EffectDefinition`をDocument台帳へ置き、各ItemEnvelopeの順序付きstackは`EffectUseId + EffectDefinitionId`参照。非隣接共有可。Groupは同じstack形でOwned合成後。Composite Set/Backdropは追加しない。Copy Local Commandは`use_id`/旧Definition ID/予約区間/採番済み完全`new_definition`を持ち、配下KeyframeIdを全再採番する。apply/redo中にIDを選ばない | D1e, D1f, D1i-2, **GAP-14**, [2026-07-15決定](../reviews/2026-07-15-relative-scope-duplicator-decision.md), GR-PV | (1)inline EffectInstance→definition1+use1 migrationでpixel/order/extra/要素数保持 (2)ID一意・参照整合・欠落拒否。新規導入ID集合はWriterの非空`[before,after)`全点と一致し、初回apply/journal replayでcounterをcommit。Redoは再commitしない。中間counter・空/穴あき/過大区間・予約外nested ID・衝突を無変更Reject。Copy Localの既存Useは予約外 (3)同definition複数use roundtrip (4)同layer内複数useはuse IDで識別 (5)rename/reorderで参照不変 (6)旧reader拒否+再migration冪等 (7)target集合/timeline隣接をdefinitionへ保存しない (8)Composite Set/Backdrop構文不能 (9)[GAP-14](../reviews/2026-07-15-shared-effect-lifecycle-decision.md)§4–§5: 参照中Delete Reject、orphan保持、Copy Local deep-copy、各操作1 Undo、未知plugin喪失なし (10)keyframed Copy Localでnested全KeyframeIdを固定順に再採番、旧参照/payload完全性、validate/save、Undo/Redo同一ID、Undo参照干渉Reject |
