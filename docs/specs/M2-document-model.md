...
ステータス: **ドラフト**(凍結ゲートで確定。**2026-07-12**: D1-prelude 節追加・色契約節追加)
...
2. **レンダ中間表現との区別**: 保存値とレンダ中間(現状premultiplied、将来はlinear)を混同しない。スキーマ値→レンダ入力の変換は**レンダ側の責務**(絶対規律「色変換の一元化」)。UI/JSONのstraightをComposite境界でpremulへ落とす既存方針([M1仕様](M1-vertical-slice.md)注)を維持
...
| ID | 内容 | 依存 | 完了条件(概要) |
|---|---|---|---|
| D1 | motolii-doc: ドキュメントモデル(プロジェクト/Asset/BPM/トラック/**トラック項目=クリップ or グループ(再帰)**/**トラック項目ごと(クリップ/グループ共通)の順序付きエフェクトスタック**/**TimeMap(恒等+定数速度)**/**変形(正準座標・アンカー・親参照予約)**/クリッピングマスク設定(有効+モード)/キーフレーム)+ serde + バージョン/マイグレーション枠組み。**version/互換の枠と所有権骨格はD1-preludeとして入場条件M2E-12で先行実装済みの前提**(D1本体はスキーマ本体のみ)。**M2E-11宣言**: ProjectV1非継承・クリップ区間は`RationalTime`・bpmは有理数。**M2E-13**: `Color`は非線形sRGB・straight・0–1 | 凍結ゲート, **[M2入場条件](../reviews/2026-07-11-M2-entry-gate.md) 全緑** | 保存→読み込み→同一レンダ結果のroundtripテスト。グループのネストがroundtripで保存される。エフェクトスタック付きグループがroundtripで保存される。TimeMap・親参照フィールドがroundtripで保存される。**未知プラグインIDを含むプロジェクトがロード失敗せず警告+パススルーで開き、再保存で未知部分が失われない**(F-9)。**拍時刻がRationalTimeに畳める**(bpm有理数) |
