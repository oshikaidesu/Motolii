...
| D1l | **完了**(#172、`cursor/d1l-effect-definition-use`): Effect Definition/Use共有schema+inline migration。plugin/version/enabled/params/extraを持つ`EffectDefinition`をDocument台帳(`Document.effect_definitions`)へ置き、各ItemEnvelopeの順序付きstackは`EffectUse{id, definition_id}`参照。非隣接共有可。Groupは同じstack形でOwned合成後。Composite Set/Backdropは追加しない。lifecycle command(`AddEffect`/`RemoveEffect`(=RemoveUse)/`SetEffectEnabled`/`EffectParam`/`DeleteEffectDefinition`/`AddEffectDefinition`/`CopyLocalEffect`/`UndoCopyLocalEffect`)・`migrate::inline_effects_to_definition_use`・`validate::validate_effect_definitions`実装済み。`cargo test -p motolii-doc`(355 test)・`cargo test --workspace`全緑 | D1e, D1f, D1i-2, **GAP-14**, [2026-07-15決定](../reviews/2026-07-15-relative-scope-duplicator-decision.md), GR-PV | (1)inline EffectInstance→definition1+use1 migrationでpixel/order/extra/要素数保持 (2)ID一意・参照整合・欠落拒否 (3)同definition複数use roundtrip (4)同layer内複数useはuse IDで識別 (5)rename/reorderで参照不変 (6)旧reader拒否+再migration冪等 (7)target集合/timeline隣接をdefinitionへ保存しない (8)Composite Set/Backdrop構文不能 (9)[GAP-14](../reviews/2026-07-15-shared-effect-lifecycle-decision.md)§4–§5: 参照中Delete Reject、orphan保持、Copy Local deep-copy、各操作1 Undo、未知plugin喪失なし — `crates/motolii-doc/tests/d1l_effect_definition.rs`で機械判定(13 test)。migration側の`next_stable_id`整合・冪等性バグを本PRで修正 |
