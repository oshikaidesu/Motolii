...
| D3e | **READY（D1l main到達、専用評価実装未着手）**: Shared Effect Use評価接続。各Use位置で個別評価、同definition params共有。Groupは子合成後1回。source非消費。既存`resolve_effect_definition`配線だけをD3e完了と数えない | D1l, D3 | (1)非隣接3 layerが同definitionを各stack位置で使いinline複製とpixel同一 (2)definition変更で全useの画が変わる (3)use並べ替えは対象layerだけ (4)Groupは子合成後1回 (5)timeline順/renameでpixel不変 (6)欠落typed error (7)preview/export同一 (8)Composite Set/Backdropへ縮退しない |
