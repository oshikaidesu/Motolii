...
着手条件: Wave1〜4の実装とP1修復(#153/#154)は main 到達済み。再締結中に着手できるM2作業は下表の依存と状態に従う。D3e、D1m、CAM-G0、D1j、D1k-Sは完了済みで、次の camera 出口は D1k 実装。D5は新規発注ではなく、landed済み骨格の統合/E2E審判待ち。Transport方針【採択】正本=[先例調査](../reviews/2026-07-14-d5-transport-prior-art.md)。
...
| D1k | **WAIT**: radian+planar orthographic runtime camera、camera-bearing render入力の実装。aspect=Composition正本、FrameDesc mismatch typed reject、Draft整数除算がaspectを壊す時は入力desc不変、resolutionを重複保持せずSlint非依存 | D1k-S | [解凍記録](../reviews/2026-07-18-d1k-runtime-camera-thaw-spec.md)どおりのAPI・render signature。center/corner/roll/NDC→Y-down px fixture、typed invalid/aspect mismatch、`1920x1080/2=960x540`・`16x9/2=16x9`、preview/export同一関数、旧position/target/FOV shim禁止、CAM-G0 oracle不変 |
