...
ステータス: **段階発注可／M2基盤再締結済み**(旧コア締結宣言は**撤回** — [記録](../reviews/2026-07-14-m2-core-closure.md)。P1修復 #153/#154と再締結A〜C証跡はmain発効済み。D7(#145/#149)は維持。**D5(#144)骨格landed・統合/E2E審判pending**で、再締結の閉集合外。PP-GateはM3前追跡のまま)。2026-07-23に歴史から再採択したD1nは独立した保存hardening follow-upであり、未実装の間はexternal change検出/cloud-safeを公約しない。M3製品実装はU0a入場完了後、各タスクの個別依存に従う。
着手条件: Wave1〜4、P1修復(#153/#154)、D3e、D1m、CAM-G0、D1j、D1k-S、D1k、D3fと再締結証跡はmain到達済み。後続M2作業は下表の依存と状態に従う。D1nは[historical foundation回収](../reviews/2026-07-23-historical-foundation-lineage-recovery.md#3-d1n-external-revision再採択)を正本とする未実装follow-up。D5は新規発注ではなく、landed済み骨格の統合/E2E審判待ち。Transport方針【採択】正本=[先例調査](../reviews/2026-07-14-d5-transport-prior-art.md)。
...
[直列核4契約](../reviews/2026-08-08-serial-core-known-contracts-decision.md)により、Assetのadvisory probe、product adapterによるfresh full-hash、D2 `AdmitAsset { asset }`、既存Assetのuse作成、unused時だけの`RemoveAsset { asset }`を別actionとする。probe receiptとadmit直前のfull fingerprintが違えばDocument／journal無変更のtyped `source-changed`とし、path読取／hashingは純粋なCommandへ入れない。`AssetId`はDocument内のstable object identityであってcontent dedup keyではなく、workerやpath／hashから導出しない。IDは`AssetTable::peek_next`と同型のtable-local採番値からedit threadがCommandへ記録し、global `Document.next_stable_id`／`StableIdReservation`へ混ぜない。

Undo／Redoは既存`LayerIdTable::peek_next`／inverse-only `restore`とD2 `AddTrackItem`↔`RemoveTrackItem`先例へ寄せる。`AssetTable::restore`はduplicateを拒否し、退役済みIDのaccepted Command復元だけを許し、counterを巻き戻さない。通常の`insert`は退役IDを拒否し続ける。新規admitはprepare時の`peek_next`をjournal直前のlive値と再照合し、staleならDocument／journal無変更で拒否する。Undo／Redo／journal replayは同じ完全なAsset値を相互inverseで扱い、Command applyはclone／validate／swapのatomic routeを使う。1Aは現行に存在しないprivateな一つのexhaustive `AssetRef` traversalを新設し、`Document::validate`のdangling検査と参照中Assetのremove判定を同じconsumerにする。Soundtrack／Clip／SVG／font等のdirect carrierと、Plugin／EffectDefinition／Vector PathOp／Composition camera等のnested `DocValue::AssetRef`を別々の部分正本にしない。`RemoveAsset`はlive tableの同一IDにある完全な値とpayloadもremove直前に照合し、relink等でlocator／fingerprintが変化したstale payloadはtyped mismatchとしてDocument／journal無変更で拒否する。`AdmitAsset`／`RemoveAsset`追加時はjournal Edit formatをv3へ上げ、cutover後の全新規Editをv3にする。v1 containerへの最初のv3 commitでは、exclusive `ProjectSession`がv2 header、既存accepted frame bytes、最初のv3 frameを一つのsibling tempへ書き、旧WALまでのreplay同値とcandidate Documentまでのreplay同値を検証してからfsync→atomic file replace→directory fsyncする。headerだけを先に置換する中間状態は作らない。新readerはcontainer v1/v2とEdit v1/v2/v3を読み、v2へだけv3をappendする。旧readerはunsupported container versionをreplay前にhard refusalし、未知Editのsnapshot fallbackからstale writable projectを返さない。初回migration+v3 commitまたは後続appendの失敗時はtransaction開始前のWAL、Document、journalを無変更にし、atomic replace後のcrashは新readerが最初のv3 Commandをreplayする。Document schema field追加ではないためDocument version／`min_reader_version`は据え置く。1A cutover後は`AssetTable::allocate`をproduct作成口から退役させ、product callsiteを0にする。

新規file-backed Assetはstrict codecがsource exact bytesから作る`motolii-source-v1:sha256:<64 lowercase hex>`と`size_bytes`が揃った時だけ`SourceFingerprintV1`としてrelink／persistent cache authorityになれる。prefixはexact source bytesをhashしたprovenance／format markerである。path／mtime／file name、`head_hash`／`tail_hash`、prefixなしの`sha256:*`を含むlegacy raw hashはauthorityにしない。既存raw fieldはread互換のため保持し、strict再hash前に文字列shapeだけで自動昇格しない。loaded projectのV1-shaped tagも最初のpersistent-cache hit／relink／source job前にSourceBindingのfull再hashで一致するまでsession verified authorityにしない。source-consuming jobはraw locatorでなく、保存済みfingerprintとexact consumed bytesを結合したHost-private `SourceBinding`だけを使う。既知のportable routeはResourceLedger下でsource全長を事前予約し、bounded copyしながらfull hashしたHost-owned immutable tempである。queued／running jobがbinding capabilityを持つ間はresident／pinnedとしてtempと予約を保持し、最後のdrop後だけevictする。予約不能はrequired bytes／budget付きtyped refusalでjob開始0とし、raw locatorへfallbackしない。stable immutable handle routeはtask固有oracleが成立するplatformだけに限る。同じfingerprintへのrelinkはrecipe／CacheEpochを変えず、offline→online時だけHost-private availability／current-state requestをrefreshする。異なるbytesへの差し替えはlocator更新ではなく将来の明示Replace Source actionとする。Asset Command、`AssetTable`追補、strict codec、全typed use count、product import／source-binding／relink adapterは未実装であり、D1a／D2完了を繰り上げない。relinkのD2 Command形とtransient refreshはGAP-3の別cutで閉じる。
...
- **所有権は単一writer+不変スナップショット(F-2)**: Documentを書き換えるのはコマンド適用の編集スレッドただ1箇所。レンダ・書き出し・解析・プロキシ生成は不変snapshotの読み手であり、M4 consumerは`(Arc<Document>, CacheEpoch, InvalidationFootprint)`のHost-private state envelopeを一回のatomic captureで取得する。Document mutationを必要とするbackground結果だけをmessageでedit writerへ返し、writerがCommandとして適用する。M4のreconstructible artifact候補はDocument mutationではなく、[M4並行契約](M4-cache-and-analysis.md)どおりworkerがcandidate receiptをmessageでHost artifact／catalog ownerへ返し、同ownerだけがatomic publishする
...
**2026-07-23コード再照合**: A4の同一lane重なり拒否は`audio_edit`の分離plannerにはあるが、`Document::validate`の全Track/Group sibling区間検査には未接続。A6は現行`Document.bpm`の定数テンポまでで、可変Tempo/Meter mapの永続席は未実装。D3やM2全体の完了表示から両者の実装完了を推論しない。処分根拠と再入場条件は[Unit 4C歴史回収](../reviews/2026-07-23-historical-d1-spec-holes-lineage-recovery.md)。

| ID | 【決定】 | 実装割当 |
|---|---|---|
| A4 | 同一Track内クリップ重なりは**validate禁止**。重なりは別Track、または将来の明示Transition | **決定済み・全体validate未実装**: D1bフォロー / D3 |
| A6 | Tempo/Meter mapを音源開始と**独立**に持つ。`beat_origin=0`、先頭拍子4/4。`Soundtrack.start_offset`と分離 | **決定済み・可変map未実装**: D1フォロー(小さな席)+M3スナップ |
...
- **D2/U2a-0追補**: 複数の決定済みcommandを1 gestureとして渡すUI境界は、[U2a-1契約](../reviews/2026-07-20-m3-u2a-1-command-adapter-contract.md)のone-shot `apply_macro`を使う。全件成功時だけ1 revision/1 Undoとし、空列・途中失敗はDocument、Undo/Redo、revision、gesture counterを変えない。既存`apply_command`の意味、Document schema、journalは変えない。
- **D2/U4b-0 follow-up（closed contract決定・code未実装）**: explicit Add Position Keyだけがfresh `KeyframeId`を導入する。帯域外`allocate_keyframe_id`+`SetProperty`はpre-edit snapshotからのjournal replayでcounterを再現しないため製品経路から棄却し、Position専用`AddPositionKey` / `UndoAddPositionKey`に完全old/new `DocParam`、added ID、1 IDの`StableIdReservation`を持たせる。same-timeは既存IDを返すno-op、Undo後counter非巻戻し、Redo同一ID。Const(Vec2)とVec2 Keyframesだけを対象とし、Bezierは既存solver+de Casteljau、same-timeはexact RationalTime、curve oracleだけ`1e-6`。journal v2／Document version／min-readerは据え置く。exact Rust型、degenerate reject、allowlistは[closed contract](../reviews/2026-08-04-u4b0-durable-position-key-closed-contract.md)に固定した。
- **D1m/D2→M3 CU-G03 durability順序（CU-G03D/CU-G03R完了、2026-08-13 live再照合追補）**: VS-1の製品編集は[CU-G03決定](../reviews/2026-07-26-cu-g03-edit-durability-ordering-decision.md)どおり、1 accepted action = 1 replay可能`Command`に限定する。live writerのcloneでcommand / Undo / Redoとplugin prepareを完了→既存`JournalEdit` v3のedit-only durable commit→commit receiptの必要時再照合→prepared writerをlive single writerへ一度swap→Transient selection reconcile→snapshot envelope 1回publishの順にする。Apply/Redoはforward、Undoはinverseを各1件記録し、reopen後の最終Document意味を一致させる。replace前のjournal errorはtyped reject後もwrite可能、replace後またはcatalog更新後の不明状態はaccepted WAL tipとrecovery Documentを照合し、未解決中だけwriteをblockする。Document read、Stage、Timeline、Inspector、transportは止めず、snapshot readで再照合を再試行する。Commit fsync後・catalog保存前に残ったcommitted Edit tailは[#369](https://github.com/oshikaidesu/Motolii/pull/369)でcatalog counterだけのmain fast pathから既存replayへ送る。UI／Vism別writer、同一Command自動再送、複数command macro耐久、journal payload/min-reader、再起動後Undo履歴、selection永続化は本決定へ含めない。
...
| VSM-A0S | **仕様決定／A0I-1〜3 + D1m保存/open所有を実装済み**: D1f/D1hのfirst-party既知表をimmutable Contract Catalogへ移し、intrinsic validation／prepared contract resolution／executor availabilityを三段化する。`load_document*`とjournal recoveryの構造保持はcatalog非依存。path書込みsave/recovery mutationはsession必須（D1m）。製品open/Writerはcatalog、graph/exportは検証済みruntimeを必須とする。製品catalog必須openはsession所有の`open_project_resolved`のみ。raw recipeはmigrationで変更しない | [A0D](../reviews/2026-07-17-vism-a0d-contract-migration-ownership-decision.md), [A0S](../reviews/2026-07-17-vism-a0s-contract-catalog-spec.md), [D1m決定](../reviews/2026-07-16-m2-project-sidecar-session-decision.md), GR-PV | A0I `cb2c9a7`→`e4f42c6`→`057e2e9`に続きD1mでsession所有façadeへ追随済み。contract/runtime、prepared resolution、製品実行入口、session capabilityの負例を維持 |
...
| D3 | **完了／Vector Rect lowering追補決定**(#110 / CU-103): ドキュメント→レンダグラフ変換(motolii-docとmotolii-renderの接続層)。**凍結ゲートで確定した単一評価モデル(F-3: ソース(TimeMap)→エフェクトスタック→変形→クリッピングマスク→グループ合成の決定的順序)をそのまま実装する**。順序の解釈をこのタスク内で発明しない。**M2E-11⑤: ExportOverlayRequest形式のジョブミラーを温存せず、Document→render層リクエストを直結**。Document≠ExportJob(書き出し設定は別構造 — 採択: snapshot/ref・出力先・範囲・fps override・解像度・codec/container・音声mux)。**評価時刻の意味論宣言(#55、監査T-9/LG-3)**: v1の評価時刻はソースPTS(タイムライン=ソースの縮退)であり、D3でtimeline_timeに再定義する — この再定義を暗黙に行わず、変換層の宣言として明示する。**D1h二段化の後段**: 実行時結線で実DataTrack出力型と期待型を照合。**#103織り込み**: A4重なり禁止前提・B④3軸表・B⑦fps役割。**CU-103追補**: 既存`VectorRecipe::StandardShape::Rect`はv1で決定的な不透明白fill・strokeなしとして既存`OverlayRect`へlowerする。centerはlocal `[0,0]`、width/heightは既存`DocParam`をtimeline timeで評価する。schema・公開API・plugin契約を増やさず、将来GAP-15でstyleを追加しても既存shapeの白fill意味を変更しない。Ellipse、SvgAsset、TextPath、Group、modifiersは対応taskまで型付きunsupportedを維持し、Rectへ黙って縮退しない | D1i-2(PathOp意味論確定後) **かつ** [#103決定](../reviews/2026-07-13-decision-pack-adoption.md) **かつ** 第二凍結点 | M1のE2Eテストがドキュメントモデル経由で通る。DataTrack出力型照合のテスト。マスク付きグループ内のエフェクト付きレイヤーの評価順ゴールデンテスト。グループ自体にエフェクトスタックを積んだ場合(子合成→グループスタック適用)の評価順ゴールデンテスト。B④3軸の挙動テスト。CU-103はRectだけがOverlayRect一件へlowerされ、非Rect Vectorはtyped reject、preview/exportが同じgraph/pixelになる |
...
| D5 | **骨格landed / 統合pending**(#144): `motolii-transport`にTransport(音声クロック常時主)・DRS骨格・`PlaybackSession`(`open_negotiated_shared`+共有`DeviceWaitLatency`)を実装。映像はフレームドロップで追従。低速時DRS(Draft 1/2→1/4、GPU timestamp query正本・非対応時無効)。レイテンシ補償=cpalデバイス待ちのみ。[先例調査【採択】](../reviews/2026-07-14-d5-transport-prior-art.md)。**未達**: 本番プレビューループ(U5)統合、レンダ中のGPU timestamp query収集、10分実機E2E審判 | D3, D4, **D4-FU** | **骨格テスト緑**: リング経路ドリフト(補償あり/なし対照)・0.5x PCMビット同一・DRS gpu正本・パンピング・グリッチ・`render_frame`統合・共有カウンタ。**完了審判pending**: 10分実機ドリフト、GPU計測配線済みDRS、U5接続 |
| D6 | **完了**(#133 / #154、VSM-A0I-2〜3入口更新): 書き出しへの楽曲mux: 元の音声ファイルをオフセット付きでffmpegに渡し映像とmux。ミキシングバウンス不要のため、コーデック互換ならストリームコピー(無劣化)を優先。D1f接続: prepared／execution diagnostic非空なら書き出し拒否(`ExportError::DegradedPlugins`)。未来版`doc.layer_source.rect`の除外迂回を#154で塞ぐ。`ExportJob`は`PluginRuntime`必須で、内部registry生成なし | D4, VSM-A0I-2〜3 | 書き出したmp4の音声が元素材とサンプル一致し、映像とズレない。contract-only runtimeの`ExecutorMissing`も出力作成前に拒否。`cargo test -p motolii-export --test d6_audio_mux` / `cargo test -p motolii-media mux` 緑 |
...
| D1i-4 | **意味契約・gate実装完了／自己保護だけGAP-25**(#107 / [2026-07-17追補](../reviews/2026-07-17-d1i4-semantic-oracle-boundary-decision.md)): **第二監査フォローアップ③d(S16執行)— ゴールデン更新禁止のCI検査**: ゴールデンを**2分類**に分ける — (a)**semantic oracle artifact**(S16。既存variantの意味を表すvariant名・入力・期待写像/数値/bytes。**更新禁止・例外なし** — regenerateマーカーでも迂回不可。変更したければ新variant+新oracle) / (b)**暫定ゴールデン**(C-1系のsRGBブレンド依存等。regenerateマーカーで更新可)。API・fixture・runtime配線を担うtest harnessは保護対象へ含めず、oracleを読んでactualと比較する。CIは(a)への変更を機械的に拒否する。台帳=`crates/motolii-testkit/golden_policy/classification.tsv`、施行=`scripts/check-golden-update-policy.sh`+`golden_update_policy`。2026-07-23監査でscript／CI route自体のCODEOWNERS自己保護不足を確認し、oracle意味を変えずGAP-25へ分離 | D1i-2, [Unit 4O回収](../reviews/2026-07-23-historical-semantic-oracle-boundary-recovery.md) | semantic oracleを書き換えるPRが**マーカー有無に関わらず**CIで落ち、harness-onlyの公開API配線変更は通る施行テスト。暫定ゴールデンはマーカー付きでのみ更新できる分類テスト。gate自己保護はGAP-25 |

## 将来境界の持越し決定（実装なし）

| ID | 状態 | 内容 | 依存 | 完了条件(概要) |
|---|---|---|---|---|
| M2-GAP-15 | **決定（実装なし）**: Param Pipeline / generic Element Domain / Constraint GraphはM2恒久面へ追加せず、現行`DocParam`/typed ID/LookAt・Follow意味を維持。persistent offset/複数source/parameter modifier→PP-Gate、cross-kind永続ID→ED-Gate、generic constraint/order/plugin→CG-Gateを先に通す。本行は別のM3入場PRを代替しない。global backlogのGAP-15（基本Shape語彙）とは別ID | [持越し境界](../reviews/2026-07-16-m2-param-element-constraint-disposition.md), [Unit 4N回収](../reviews/2026-07-23-historical-param-element-constraint-lineage-recovery.md), GR-PV | (1)既存field/variant解釈不変 (2)各発火条件前にdecision/spec PR必須 (3)M3通常property UIは現行single sourceだけ (4)将来変更は追加的またはD1e明示migration |
...
| D1l | **Document意味・評価は完了／narrow enforcementだけGAP-23/24で再開**（`a23a4ad`、`74af37e`、docs follow-up `02192c2`）: Effect Definition/Use共有schema+inline migration。Definition台帳+Use参照、v2 lifecycle Command、Writer prepare、Copy Local、journal／Undoは実装・再締結済み。製品の新規Documentは`new_current()`で現行v5、`new_v1()`はv1 legacy専用。2026-07-23監査でlegacy constructorのdeprecated属性＋25 lint suppressionをGAP-23、known Edit apply失敗も先行Snapshotへfallbackし得る境界をGAP-24として再開。作品形式やlifecycleを巻き戻さず、機構だけを修復する | D1e, D1f, D1i-2, **GAP-14**, [journal/Undo追補](../reviews/2026-07-15-d1l-journal-revert-boundary-decision.md), [constructor決定](../reviews/2026-07-16-d1l-current-document-constructor-decision.md), [Unit 4L回収](../reviews/2026-07-23-historical-d1l-constructor-lint-lineage-recovery.md), [Unit 4M回収](../reviews/2026-07-23-historical-d1l-journal-undo-lineage-recovery.md), GR-PV | (1)inline migrationでpixel/order/extra/要素数保持、旧reader拒否+再migration冪等 (2)ID一意・参照整合・欠落拒否 (3)共有/同layer複数Use roundtrip・rename/reorder参照不変 (4)target集合/timeline隣接非保存、Composite Set/Backdrop構文不能 (5)GAP-14 §4–§5全条件 (6)nested Keyframe Copy Localの固定順再採番・payload完全性・Undo干渉Reject (7)v1全Command corpusをD1e共用plannerでlossless replay、v1/v2混在 (8)Create/Link/Copy Local準備はDocument不変・raw caller採番なし (9)reservation閉集合6 variantだけcounter以外全文一致+非巻戻し、version/min不変、Redo全文一致 (10)Unlinkに偽reservationなし、全失敗時Document全文不変 (11)現行reader/writer/min=5、`new_current()` roundtrip、AST gate、v1〜v4と版偽装inline/hybridの型付き拒否は緑。ただしdoc-hidden／suppressionなしはGAP-23、先行Snapshot付きknown apply failureの非fallbackはGAP-24で閉じる |
| D1m | **完了**（`cursor/d1m-project-session`）: project-scoped sidecar identity + process間read-write session lock。`<parent>/<file-name>.motolii/`とsibling `.motolii.lock`へnative `OsString` suffixで一意化し、canonical path aliasを同一identityへ畳み、OSの非blocking exclusive lockを保持する`ProjectSession`経由だけでproject/journalを変更する。legacy親共有layoutは自動帰属せずstaging検証後のatomic installのみ。unknown legacy entry診断は[sidecar/session決定の2026-07-18追補](../reviews/2026-07-16-m2-project-sidecar-session-decision.md#legacy-sidecar-migration-diagnostic-report-2026-07-18)の`LegacySidecarMigrationReport`で実装済み。read-only fallback/lock steal/Save Asは作らない。catalog非依存`load_document*`とunknown保持は[A0S](../reviews/2026-07-17-vism-a0s-contract-catalog-spec.md)と両立 | D1c-FU, D1d, [sidecar/session決定](../reviews/2026-07-16-m2-project-sidecar-session-decision.md), [A0S](../reviews/2026-07-17-vism-a0s-contract-catalog-spec.md), GR-PV | (1)同一directoryの2 projectでsidecar全要素が非衝突、相互byte不変 (2)subprocess保持中の同一path/symlink alias openが即時`ProjectAlreadyOpen`、Windows CI+macOS証跡 (3)正常Drop/強制終了後に再取得+復旧可 (4)path mutationと低水準`open_project*`/`recover_project`はsessionまたはcrate-privateのみ。製品catalog必須openはsession所有の`open_project_resolved`のみ (5)family存在述語/状態表どおりlegacy通常open・partial・invalidをtyped reject、明示移行はsource保持・media除外・verify・atomic install・再実行冪等 (6)D1d意味/保護golden不変、path literalのみhelper化可 (7)`cargo test --workspace`全緑 (8)明示migrateの成功型が`Result<LegacySidecarMigrationReport, SessionError>`であり、[decision追補](../reviews/2026-07-16-m2-project-sidecar-session-decision.md#legacy-sidecar-migration-diagnostic-report-2026-07-18)のdisposition/untouched preflight/非永続/負例（I/O→`Err`・FS不変）に従う |
| D1n | **決定／未実装（2026-07-23歴史回収）**: `ProjectSession`にmain/journal/catalog exact bounded bytesと、対象operationが触るcatalog参照generationのtransient observed revisionを持たせる。mutation直前に閉じたread/write setを比較し、replace/appear/disappear/same-size+mtime差替えをcomponent付きtyped conflictとしてwrite 0で拒否する。journalはD1d scan既読bytesを再利用し二重readしない。成功後だけdurable resultでrevision更新。watch/mtime/provider名はhint、自動merge/overwrite/分散lock保証なし | D1m, [historical foundation回収 §3](../reviews/2026-07-23-historical-foundation-lineage-recovery.md#3-d1n-external-revision再採択), GR-PV | (1)same-size/mtime main差替えを検出しmain/sidecar不変 (2)journal middle/ignored-tail、catalog、対象generationのreplace/appear/disappearをcomponent付き拒否 (3)local連続saveはrevision更新して成功 (4)precondition mismatch/malformed/limit/I/Oはwrite 0、detect可能なafter-write peer-winsは別typed error+recovery保持 (5)watch無しで全conflict試験合格 (6)journal二重readなし、generation hashは対象operationだけ (7)D1d/D1m/atomic save意味不変、workspace全緑 |
| D3e | **完了**: Shared Effect Use評価接続。`PreparedDocumentPlugins::EffectDefinition` recipeのみを`apply_effect`が評価。各Use位置で個別評価、同definition params共有。Groupは子合成後1回。source非消費 | D1l, D3 | (1)非隣接3 layerが同definitionを各stack位置で使いinline複製とpixel同一 `p1_non_adjacent_shared_definition_matches_inline_copies` (2)definition変更で全useの画が変わる `p2_definition_param_change_updates_all_uses` (3)use並べ替えは対象layerだけ `p3_use_reorder_affects_only_target_layer_region` (4)Groupは子合成後1回 `p4_group_effect_applies_once_after_child_composite` (5)timeline順/renameでpixel不変 `p5_timeline_order_and_layer_rename_leave_pixels_unchanged` (6)欠落typed error `n1_dangling_definition_is_typed_error` (7)preview/export同一 `p7_preview_and_export_share_final_render_path` (8)Composite Set/Backdropへ縮退しない。prepared rename `p6_prepared_rename_executes_new_key_while_raw_unchanged`、degraded拒否 `n3_degraded_prepared_recipe_blocks_graph_execution` |
...
| ID | 状態 | 内容 | 依存 | 完了条件(概要) |
|---|---|---|---|---|
| CAM-G0 | **完了**: camera接続前の既存`ViewportTransform` matching-aspect 2D出力を、未分類harness `crates/motolii-render/tests/cam_g0_planar_identity.rs`とsemantic oracle `crates/motolii-render/tests/oracles/cam_g0_planar_identity.tsv`で固定。oracleのみ`classification.tsv`へsemantic登録。`golden_update_policy.rs`にoracle/harness分類・oracle変更拒否・harness変更許可の断言。固定入力メタと期待RGBA bytesをレビュー可能なtextで保持し、GPU出力を`EXACT`比較 | camera決定, GR-PV | harness+oracle追加、oracle semantic登録、`cam_g0_planar_identity`（`MOTOLII_REQUIRE_GPU=1`）緑、golden policy CAM-G0断言+`check-golden-update-policy.sh`緑、`cargo test --workspace`全緑 |
| D1j | **完了**: `LATEST/READER/WRITER==4`前置確認後、v5へinternally-tagged `Composition.camera: CompCameraDoc::PlanarOrthographic`を追加。v1〜v4へ既定cameraをD1e migrationし、版偽装をtyped reject | CAM-G0, D1l, D1e, camera決定, GR-PV | versionが4以外なら停止。migration冪等、counts/ID/extra不変、wire roundtrip/未知kind拒否、非有限/height<=0拒否、reader/writer/min=5、旧reader拒否、v1〜v4版偽装拒否 |
| D1k-S | **完了**: CQ-5解凍記録。runtime planar `CompCamera` APIと全公開render entryの必須camera signatureを実装前に凍結 | D1j, [camera決定](../reviews/2026-07-16-m2-comp-camera-decision.md) | [解凍記録](../reviews/2026-07-18-d1k-runtime-camera-thaw-spec.md)に旧/新API disposition、必須signature、方程式、`Quality::render_desc`、migration/wire非影響、golden非影響、D1k試験列挙が固定されている |
| D1k | **完了**: radian+planar orthographic runtime camera、camera-bearing render入力を実装。aspect=Composition正本、FrameDesc mismatch typed reject、Draft整数除算がaspectを壊す時は入力desc不変、resolutionを重複保持せずUI toolkit非依存 | D1k-S | [解凍記録](../reviews/2026-07-18-d1k-runtime-camera-thaw-spec.md)どおりのopaque API・必須render signature。center/corner/roll/NDC→Y-down px、極値比の有限性、typed invalid/aspect mismatch、全entry早期検証、graph camera伝播、`1920x1080/2=960x540`・`16x9/2=16x9`を自動審判。`cargo test --workspace`、plugin、purity、CAM-G0全緑。旧position/target/FOV shimなし、oracle不変 |
| D3f | **完了**: Document cameraを`EvaluationTime.timeline_time`で評価し`DocumentFrameGraph.camera`としてpreview/export/renderへ接続。`view(camera)∘world`をAffinePlaceへ単一投影 | D1k, D3 | CAM-G0 byte不変、既存transform/eval期待値不変、center/roll/height animation方程式一致、preview/export同一、depth/order不変 |

並列レーン(2026-07-16改訂、2026-07-23 D1n回収): **D1a**→(D1b/D1f と D8)→D1c→D1c-FU(#101)→D1d→**D1m→D1n**。第二監査フォローアップは**D1g→D1h→D1i-1→D1i-2(直列)**。**D2はD1i-1後、D3はD1i-2後**、D3→D7。D1eはD1i-1かつD1c-FU後。D1i-3はD3後、D1i-4はD1i-2後。D4は独立。D5が合流点。D6はD4後。旧形式JSONの変換は全てD1e担当(D1g/D1i-1は拒否のみ)。**Shared Effect**: GAP-14→D1l→D3e。**保存所有権**: D1d→D1m→D1n。**camera**: CAM-G0→D1j→D1k-S→D1k→D3f(上のCompCameraレーン)。**予防**: この直列を飛ばす並列発注はGR-PV-4違反([permanence-prevention](../reviews/2026-07-12-m2-permanence-prevention.md)、実装ガード16)。
...
| AG-2 | **core完了**(#157) / 製品接続はGAP-28: per-stream PCM cache、決定論的mixer、`MixProducer`は成立。`PlaybackSession`の`AudioProgram`接続は未実装 | AG-1 | core fixtureに加え、GAP-28でmixed source製品再生・seek・10分driftを審判 |
| AG-3 | **domain完了／製品UI未実装**(Part of #158 / PR#164): commands／分離／waveform peaksは成立。React import/form＋native Timeline waveform／row操作はM3 | AG-1, AG-2 | 設計§7のdomain拒否テスト。UIは現行surface owner、同一projection／intent、Undo境界で審判 |
...
**2026-07-12 第二監査によるフォローアップ（2026-07-23現行化）**: [第二実コード監査](../reviews/2026-07-12-code-audit-2nd-d1.md)が当時確認したS1〜S18のうち、S1〜S7・S9〜S16・S18とB④はD1c-FU、D1d〜D1i、D2、D3へ実装・試験済み。S8の`DataTrackId`正準identity案は、後続VSM-A7が既存文字列IDだけを意味fixtureへ用い、provider／consumer／Kitのidentity責任をVSM-B2まで凍結したため、旧案を実装済みとも直採用ともみなさない（GAP-21）。S17の中間OTIO写像+loss report試験は現行コードに存在せず、GAP-22へ再回収した。再照合の根拠と復活禁止事項は[Unit 4C-2回収](../reviews/2026-07-23-historical-d1-code-audit-lineage-recovery.md)を正とする。
...
11. **クラウド同期フォルダの検出と外部変更** → provider/path検出口は**D1c**、保存直前のexact disk-family preconditionは**D1n**、watch/競合表示UIはM3。watch/mtimeを保存正本にしない
12. **Undoの退行防止** → **D2**
...
- ~~Param Pipeline / generic Element Domain / Constraint GraphのM2持越し境界~~ → **【決定】M2-GAP-15**(2026-07-16): 現行意味不変で延期し、各発火条件前にPP/ED/CG-Gateを必須化。global backlog GAP-15とは別ID
...
- ~~クラウド同期/非協調editorによる外部差替えの保存前審判~~ → **【決定】D1n**(2026-07-23歴史回収): D1m後のtransient exact-byte revision precondition。watch/mtimeを権威にせず、検出競合はwrite 0のtyped reject。実装は未着手
