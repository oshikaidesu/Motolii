...
正本: [2026-07-15-relative-scope-duplicator-decision.md](../reviews/2026-07-15-relative-scope-duplicator-decision.md)、lifecycle=[GAP-14決定](../reviews/2026-07-15-shared-effect-lifecycle-decision.md)。**Definition/Use分離は決定済。D1l schema実装はGAP-14決定後に解禁**(本PRでlifecycle凍結)。
...
| D1l | **READY**(Issue化可): Effect Definition/Use共有schema+inline migration。plugin/version/enabled/params/extraを持つ`EffectDefinition`をDocument台帳へ置き、各ItemEnvelopeの順序付きstackは`EffectUseId + EffectDefinitionId`参照。非隣接共有可。Groupは同じstack形でOwned合成後。Composite Set/Backdropは追加しない | D1e, D1f, D1i-2, **GAP-14**, [2026-07-15決定](../reviews/2026-07-15-relative-scope-duplicator-decision.md), GR-PV | (1)inline EffectInstance→definition1+use1 migrationでpixel/order/extra/要素数保持 (2)ID一意・参照整合・欠落拒否 (3)同definition複数use roundtrip (4)同layer内複数useはuse IDで識別 (5)rename/reorderで参照不変 (6)旧reader拒否+再migration冪等 (7)target集合/timeline隣接をdefinitionへ保存しない (8)Composite Set/Backdrop構文不能 (9)[GAP-14](../reviews/2026-07-15-shared-effect-lifecycle-decision.md)§4–§5: 参照中Delete Reject、orphan保持、Copy Local deep-copy、各操作1 Undo、未知plugin喪失なし |
...
並列レーン(2026-07-13改訂): **D1a**→(D1b/D1f と D8)→D1c→D1c-FU(#101)→D1d。第二監査フォローアップは**D1g→D1h→D1i-1→D1i-2(直列)**。**D2はD1i-1後、D3はD1i-2後**、D3→D7。D1eはD1i-1かつD1c-FU後。D1i-3はD3後、D1i-4はD1i-2後。D4は独立。D5が合流点。D6はD4後。旧形式JSONの変換は全てD1e担当(D1g/D1i-1は拒否のみ)。**Shared Effect**: GAP-14→D1l→D3e。**予防**: この直列を飛ばす並列発注はGR-PV-4違反([permanence-prevention](../reviews/2026-07-12-m2-permanence-prevention.md)、実装ガード16)。
