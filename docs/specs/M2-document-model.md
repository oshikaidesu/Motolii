...
| ID | 内容 | 依存 | 完了条件(概要) |
|---|---|---|---|
| D1 | motolii-doc: ドキュメントモデル(プロジェクト/Asset/BPM/トラック/**トラック項目=クリップ or グループ(再帰)**/クリップごとの順序付きエフェクトスタック/**TimeMap(恒等+定数速度)**/**変形(正準座標・アンカー・親参照予約)**/クリッピングマスク設定(有効+モード)/キーフレーム)+ serde + バージョン/マイグレーション枠組み | 凍結ゲート | 保存→読み込み→同一レンダ結果のroundtripテスト。グループのネストがroundtripで保存される。TimeMap・親参照フィールドがroundtripで保存される。**未知プラグインIDを含むプロジェクトがロード失敗せず警告+パススルーで開き、再保存で未知部分が失われない**(F-9) |
