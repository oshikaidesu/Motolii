...
着手条件: **タスク表の依存を満たした行から発注する**。現在、D2(#109)・D1f(#106)・D4(#123)は発注可。**D1i-2(#100)は完了・mainマージ済み**。**D1cフォローアップ(#101)は完了(本PR・mainマージ待ち)**。D1d(#105)・D1e(#104)は**#101のmainマージ**まで待つ(ResourceLimits/OpenModeの重複定義禁止)。D3(#110)は**D1i-2完了・mainマージ=第二凍結点**まで待つ(D1i-4のCI施行はD1i-2完了により着手可)。D4はDocumentスキーマ非接触の独立レーン(D6はD4完了後)
...
| D1d | ジャーナル(ガード3/4/6): レコードchecksum+世代salt・不正テール切捨て・UUID相互参照・リプレイ失敗フォールバック・ピン留め世代。#101のジャーナル総量/レコードpayload上限を使い、上限を別定義しない | D1c-FU(#101) | 壊れ方カタログの単体/注入テスト緑。破損テールは論理的に無視し、原本を切詰めない。プロセスロック/stale lock/read-only fallbackは未決のため本タスク外 |
| D1e | マイグレーション枠+量的不変条件+旧版コーパス(ガード8)。前方互換の版上げ経路。**第二監査分の個別migration(旧`timeline_start`形式・旧`path_ops`形式→新形式)を含む**。新版判定は#101の`OpenMode`、入力境界は同じ`ResourceLimits`を使う | **D1i-1かつD1c-FU(#101)**(migration枠の骨格設計のみD1a後に先行可。変換実装は移行先スキーマとI/O契約が確定した後) | in-place禁止・バックアップ・クリップ/トラック/キー数一致・goldenコーパス回帰。旧版コーパスに`timeline_start`世代・旧`path_ops`世代を含む |
...
| D4 | motolii-audio: 楽曲1本のSymphoniaデコード→PCM全展開キャッシュ + cpal出力 + リングバッファ/プロデューサスレッド(ミキサーなし)。アンダーラン時は不足フレーム数を記録し、実音声の供給済みサンプル数を無音充填数と分離して公開する | 凍結ゲート | 任意位置からのサンプル読み出しテスト、アンダーランなしの連続再生。アンダーランの無音充填で論理サンプル位置が進まないテスト |
...
**2026-07-12 第二監査によるフォローアップ**: D1a/D1b/D1cは「完了」だが、[第二実コード監査](../reviews/2026-07-12-code-audit-2nd-d1.md)がスキーマ・検証の穴S1〜S18を確認した(D1a: S1/S2/S5/S6/S16、D1b: S3/S4/S9、D1c: S10/S14)。**S1/S6/S16は2026-07-12ユーザー決定(3点とも案1、補正4点込み — 同文書決定節)**。スキーマ側フォローアップは**D1g/D1h/D1i-1〜D1i-4**(上表)として発注済み。D1c分(S10/S14)の責任は**D1c-FU(#101)**へ集約し、D1d(#105)・D1e(#104)はその型を利用する。未着手のD1f・D2・D3は同文書の該当行を各発注書へ織り込むこと。
