...
**【決定】ではない。** ユーザー指示により確定を保留。AE/Lottie前例の下書きと、Cavalry比較([pathop-ae-cavalry-comparison](../reviews/2026-07-12-pathop-ae-cavalry-comparison.md))を並べた比較候補。確定後に本節を【決定】へ昇格し、その写しとしてD1i-2実装に入る(GR-PV-1)。

予防GR-PV-1: **意味を先に焼き、実装・ゴールデンはその写し**。画素意味の変更は新variant(S16)。単位はMotolii正準空間へ写す(Lottieの%・度をそのままスキーマに入れない)。北極星はAEスタック型の閉集合(F-13)だが、**パラメータ意味の厚みはCavalry側が上回る箇所がある** — 片面オフセットだけ焼く等の早焼きを避けるため比較を先に置く。
