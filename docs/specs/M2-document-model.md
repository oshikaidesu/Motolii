...
- **D1m/D2→M3 CU-G03 durability順序（CU-G03D/CU-G03R完了）**: VS-1の製品編集は[CU-G03決定](../reviews/2026-07-26-cu-g03-edit-durability-ordering-decision.md)どおり、1 accepted action = 1 replay可能`Command`に限定してnon-live preflight→既存`JournalEdit` v2のedit-only durable commit→live single writer適用→revision→Transient selection reconcile→snapshot envelope 1回publishの順にする。Apply/Redoはforward、Undoはinverseを各1件記録し、reopen後の最終Document意味を一致させる。journal errorまたはpost-durable live failureではpublishせずsessionをpoisonし、recover/reopenまで編集を拒否する。Commit fsync後・catalog保存前に残ったcommitted Edit tailは[#369](https://github.com/oshikaidesu/Motolii/pull/369)でcatalog counterだけのmain fast pathから既存replayへ送る。複数command macro耐久、journal payload/min-reader、再起動後Undo履歴、selection永続化は本決定へ含めない。
