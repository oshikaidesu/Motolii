...
- **D2/U4b-0 follow-up（決定・未実装）**: explicit Add Position Keyだけがfresh `KeyframeId`を導入する。帯域外`allocate_keyframe_id`+`SetProperty`はpre-edit snapshotからのjournal replayでcounterを再現しないため製品経路から棄却し、Position専用forward/inverse Commandに完全old/new `DocParam`、added ID、1 IDの`StableIdReservation`を持たせる。same-timeは既存IDを返すno-op、Undo後counter非巻戻し、Redo同一ID。既存SetProperty wireへ必須fieldを足さず、curve内挿入は既存評価を保つ。exact Rust型・solver tolerance・journal version変更が必要かは実装前closed contractで再固定する（[歴史回収 §4](../reviews/2026-07-23-historical-d2-selection-timeline-lineage-recovery.md#4-u4b-0-durable-add-position-key再採択)）。
