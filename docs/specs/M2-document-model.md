...
| D1k | **完了**: radian+planar orthographic runtime camera、camera-bearing render入力を実装。aspect=Composition正本、FrameDesc mismatch typed reject、Draft整数除算がaspectを壊す時は入力desc不変、resolutionを重複保持せずSlint非依存 | D1k-S | [解凍記録](../reviews/2026-07-18-d1k-runtime-camera-thaw-spec.md)どおりのopaque API・必須render signature。center/corner/roll/NDC→Y-down px、極値比の有限性、typed invalid/aspect mismatch、全entry早期検証、graph camera伝播、`1920x1080/2=960x540`・`16x9/2=16x9`を自動審判。`cargo test --workspace`、plugin、purity、CAM-G0全緑。旧position/target/FOV shimなし、oracle不変 |
