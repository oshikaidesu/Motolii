...
| VSM-A0S | **仕様決定／実装待ち**: D1f/D1hのfirst-party既知表をimmutable Contract Catalogへ移し、intrinsic validation／prepared contract resolution／executor availabilityを三段化する。raw load/save/recoveryは構造保持、製品open/Writerはcatalog、graph/exportは検証済みruntimeを必須とする。raw recipeはmigrationで変更しない | [A0D](../reviews/2026-07-17-vism-a0d-contract-migration-ownership-decision.md), GR-PV | [A0S仕様](../reviews/2026-07-17-vism-a0s-contract-catalog-spec.md)の公開型・入口表・旧API処分・拒否fixture。実装順はA0I-1→A0I-2→A0I-3 |
