...
着手条件: Wave1〜4の実装とP1修復(#153/#154)は main 到達済み。再締結中に着手できるM2作業は下表の依存と状態に従い、現時点ではD3e、D1m、CAM-G0が`READY`。D5は新規発注ではなく、landed済み骨格の統合/E2E審判待ち。Transport方針【採択】正本=[先例調査](../reviews/2026-07-14-d5-transport-prior-art.md)。
...
| ID | 状態 | 内容 | 依存 | 完了条件(概要) |
|---|---|---|---|---|
| CAM-G0 | **READY（camera決定main到達、fixture未着手）**: camera接続前の既存`ViewportTransform` 2D出力を、変更可harness `crates/motolii-render/tests/cam_g0_planar_identity.rs`とsemantic oracle `crates/motolii-render/tests/oracles/cam_g0_planar_identity.tsv`で固定。harnessは未分類でAPI・fixture構築・runtime配線に追従可。oracleのみsemantic分類し、固定入力メタデータと期待RGBA bytesをレビュー可能なtextで保持。matching aspectの単純solid形状でGPU出力byteをoracleへ採取 | camera決定, GR-PV | 将来のfixture PRでharness+oracleを追加し、oracleのみ`classification.tsv`へsemantic登録する独立PR。golden policyがoracle期待値変更を拒否し、`cargo test --workspace`全緑 |
