...
ステータス: **ドラフト**(凍結ゲートで確定。**2026-07-12**: D1-prelude・色契約・LayerId・時刻serde・duration半開・D1再分割・補間/変形意味論・**PathOp意味論表は下書き**(AE/Lottie案+Cavalry比較待ち。**【決定】ではない**))
...
## PathOp意味論表(D1i-2 / **下書き・未確定** 2026-07-12)

**【決定】ではない。** ユーザー指示により確定を保留。比較材料: [pathop-ae-cavalry-comparison](../reviews/2026-07-12-pathop-ae-cavalry-comparison.md)(豊富さ・実声・**拡張=Cavalry / 意味=Lottie(枯れ)の3層**)。確定後に本節を【決定】へ昇格し、その写しとしてD1i-2実装に入る(GR-PV-1)。**昇格PRの完了条件**: 未確定マーカーは5箇所に分散している(本節見出し・冒頭ステータス行・D1i-2行・`schema.rs`のPathOp docコメント・[permanence-prevention](../reviews/2026-07-12-m2-permanence-prevention.md)の表)— 昇格時に**全箇所を同時更新**し、`git grep 下書き` にPathOp関連の残存が無いことを確認する(取りこぼし1つが「矛盾=バグ」(docs/README原則)になる)。

予防GR-PV-1: **意味を先に焼き、実装・ゴールデンはその写し**。画素意味の変更は新variant(S16)。単位はMotolii正準空間へ写す(Lottieの%・度をそのままスキーマに入れない)。**パス意味の正本候補はLottie(Web再現済みの枯れた技術)**。拡張の口(スタック・将来`PluginKind::PathOp`)はCavalry精神。Cavalryの厚い角は追加的取り込みかv2プラグインへ — 独自方言で差別化しない。人気投票では決めない。

出典(一次):
- Lottie Animation Community [Shapes 1.0 / 1.0.1](https://lottie.github.io/lottie-spec/1.0.1/specs/shapes/)(Trim Pathの式・Multiple Shapes・Pucker/Bloat追加)
- [lottie-docs Shapes](https://lottiefiles.github.io/lottie-docs/shapes/)(Bodymovin出力の`pb`/`zz`/`op`/`rd`/`tm`/`tw`/`rp`属性表)
- AE: パス演算子はライブで同一グループ内の上位パスに作用([creating shapes and masks](https://helpx.adobe.com/after-effects/using/creating-shapes-masks.html)およびパス演算子説明の系譜)
- Cavalry: [Path Offset](https://cavalry.studio/docs/nodes/behaviours/path-offset/) / [Wave](https://cavalry.studio/docs/nodes/behaviours/wave/) / [Bevel](https://cavalry.studio/docs/nodes/behaviours/bevel/) / [Stroke Trim](https://cavalry.studio/docs/nodes/utilities/stroke/) / [Travel Deformer](https://cavalry.studio/docs/nodes/behaviours/travel-deformer/) / [Noise](https://cavalry.studio/docs/nodes/behaviours/noise/) ほか — 詳細は比較メモ
...
| 適用順 | `modifiers[i]` は **index 0から順**(D1i-1)。Lottie内容リストの「逆順合成」とはスタック表現が違う — Motoliiはroot stackなので順方向を正とする案 |
| 複数輪郭 | 入力パス集合の**各輪郭に独立適用**(Trim `Sequential`のみ連結長で扱う)案。Cavalry Path Offsetは閉+開混在非対応 — 混在拒否を入れるかは未決 |
| 座標 | 正準空間(原点中央・Y-up・高さ=1)。距離・振幅・半径は正準長さ |
| 非有限 | NaN/Infはvalidate拒否(D1h)。範囲外は下表どおり拒否または宣言済みクランプ |
| 退化 | 空入力→空出力。頂点1以下のパスは恒等(変更しない)。自己交差は許容(幾何を黙って「直さない」) |
| 互換 | `algorithm_version`フィールドは焼かない(S16)。意味変更は新`op` tag / 新variant |
| ゴールデン | variantごと意味論ゴールデン(D1i-2、**意味【決定】後**)。更新禁止はD1i-4 |

### variant表(AE/Lottie寄せの下書き。Cavalry差分は右列)

| `op` | Lottie `ty` | パラメータ案(型・単位・範囲) | AE/Lottie意味案 | Cavalry比較(未決への入力) | v1で焼かない席(案) |
|---|---|---|---|---|---|
| `pucker_bloat` | `pb` | `amount: F64` ∈ **[-1, 1]**。0=恒等。+1=頂点が重心へ(Lottie +100%)。-1=重心から距離2倍(Lottie −100%)。接線は頂点と逆向きに補間([lottie-docs](https://lottiefiles.github.io/lottie-docs/shapes/)) | AE Pucker & Bloat | **専用なし**(Noise/Pinchは別)。AE前例維持が強い | なし |
| `zig_zag` | `zz` | `amount: F64` ≥ 0(正準・峰谷距離=Lottie `s`)。`ridges: F64` ≥ 0(セグメントあたりの峰数=Lottie `r`) | AE Zig Zag | [Wave](https://cavalry.studio/docs/nodes/behaviours/wave/)はSine/Square/Sawtooth/Triangle+Travel+Adaptive。Triangle≈zz。他modeを最初から持つか未決 | `point_type` / 波形`mode` |
| `offset` | `op` | `distance: F64`(正準。正=外向き、負=内向き=Lottie `a`) | AE Offset Paths | [Path Offset](https://cavalry.studio/docs/nodes/behaviours/path-offset/): Double Sided・開Cap・Rounded・閉開混在非対応。片面のみ焼きは薄い可能性 | `line_join` / `miter_limit` / sidedness / caps |
| `round_corners` | `rd` | `radius: F64` ≥ 0(正準=Lottie `r`) | AE Round Corners | [Bevel](https://cavalry.studio/docs/nodes/behaviours/bevel/): Fillet/Chamfer・点別半径・角度フィルタ。Fillet≈rd | Chamfer / per-point radius |
| `trim` | `tm` | `start`/`end`: F64 ∈ **[0, 1]**。`offset`: F64(**周回**)。`mode`: `parallel`\|`sequential` | AE Trim Paths | Cavalryは[Stroke.Trim](https://cavalry.studio/docs/nodes/utilities/stroke/)+[Travel Deformer](https://cavalry.studio/docs/nodes/behaviours/travel-deformer/)。幾何Trimか描画Trimか未決 | Travel別op / taper(描画側) |
| `twist` | `tw` | `angle: F64`(**ラジアン**) | AE Twist | 専用なし。Lattice/Four Pointは空間ワープ族で閉集合外案 | 永続`center: Vec2` |
| `wiggle` | (AE Wiggle Paths。Lottie 1.0コア外) | `amp`/`freq`/`seed` | AE Wiggle Paths | [Noise](https://cavalry.studio/docs/nodes/behaviours/noise/) as Deformerは厚いがスープ化(F-8逆)。意図単位Wiggle維持案 | Noise Typeスープ |
| `repeater` | `rp` | `copies`/`offset` + **`transform: Transform2D`**(席追加案・default恒等) | AE/Lottie Repeater | Duplicator+Staggerは別評価口(F-7)。PathOpに畳まない | コピー間opacityランプ |

### 確定前の未決(比較メモより。潰すまで実装しない)

1. Offsetの sidedness / 開パスCap / 閉開混在拒否
2. ZigZag固定か Wave mode か
3. RoundCorners=Filletのみか Bevel拡張か
4. Trimの所属(パス幾何 vs Stroke描画)と Travel の扱い
5. Twist中心・Wiggleアルゴリズム名の固定文言
6. Lattice/Pinch/PathfinderをPathOpに入れないことの明示文

### D1i-2実装への拘束(意味【決定】後に発効)

1. **【決定】表**と食い違う「便利なデフォルト」は禁止(H-3)
2. validateは決定表の拒否項目を型付きエラーにする(完了条件)
3. 意味論ゴールデンは決定表の境界を最低限カバーする
4. `Repeater.transform`席の追加は**追加的**(default恒等)とする案 — 確定時に追認
5. 本下書きのまま実装・ゴールデンを増やさない
...
| D1i-2 | **第二監査フォローアップ③b(S5)— PathOp意味論表と実装**: 仕様の「PathOp意味論表」を**【決定】へ昇格したあと**、validate+幾何実装+variantゴールデンを一致させる。**Repeater.transform席追加**(default恒等)。Wiggleアルゴリズム名は決定時に固定。比較材料: [AE×Cavalry](../reviews/2026-07-12-pathop-ae-cavalry-comparison.md)。**現状: 表は下書き・未確定** | D1i-1 | 意味論表が仕様に**【決定】**として存在する。validateが表の拒否項目で型付きエラー。variantごとの保護された意味論ゴールデン。`Repeater`に`transform`があり旧JSONはdefault恒等で読める |
...
並列レーン(2026-07-12改訂): **D1a**→(D1b/D1f と D8)→D1c→D1d。第二監査フォローアップは**D1g→D1h→D1i-1→D1i-2(直列)**。**D2はD1i-1後、D3はD1i-2後**、D3→D7。D1eはD1i-1後(migration枠の骨格設計のみ先行可)。D1i-3はD3後、D1i-4はD1i-2後。D4は独立。D5が合流点。D6はD4後。旧形式JSONの変換は全てD1e担当(D1g/D1i-1は拒否のみ)。**予防**: この直列を飛ばす並列発注はGR-PV-4違反([permanence-prevention](../reviews/2026-07-12-m2-permanence-prevention.md)、実装ガード16)。
...
13. **意味を先に仕様へ書く**(GR-PV-1) — PathOp意味論表は**下書き**(Cavalry比較込み)。**【決定】後**にD1i-2実装。表が無い/未確定のまま実装しない
