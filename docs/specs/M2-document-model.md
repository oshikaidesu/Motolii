...
| D1i-4 | **第二監査フォローアップ③d(S16執行)— ゴールデン更新禁止のCI検査**: CI規則「**画素意味を変えるPRは既存の意味論ゴールデンを更新できない — 新variant+新ゴールデンを追加する**」の機械検査(regenerateマーカー運用=台帳C-1系と整合させる) | D1i-2 | 意味論ゴールデンを書き換えるPRがCIで落ちる施行テスト(マーカー付き例外の経路込み) |

並列レーン: **D1a**→(D1b/D1e/D1f と D2/D8)→D1c→D1d。D1a→D3→D7。D4は独立。D5が合流点。D6はD4後。第二監査フォローアップは**D1g→D1h→D1i-1→D1i-2(直列)**。D2はD1i-1後、D3はD1i-2後。D1i-3(非PathOp演算ゴールデン)はD3後、D1i-4(CI検査)はD1i-2後。旧形式JSONの変換は全てD1e担当(D1g/D1i-1は拒否のみ)。
