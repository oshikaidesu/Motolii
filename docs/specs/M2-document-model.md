...
| ID | 内容 | 依存 | 完了条件(概要) |
|---|---|---|---|
| D1 | oc-doc: ドキュメントモデル(プロジェクト/Asset/BPM/トラック/**トラック項目=クリップ or グループ(再帰)**/クリップごとの順序付きエフェクトスタック/クリッピングマスク設定(有効+モード)/キーフレーム)+ serde + バージョン/マイグレーション枠組み | 凍結ゲート | 保存→読み込み→同一レンダ結果のroundtripテスト。グループのネストがroundtripで保存される |
