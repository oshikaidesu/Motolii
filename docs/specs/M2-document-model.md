...
並列レーン(2026-07-12改訂): **D1a**→(D1b/D1f と D8)→D1c→D1d。第二監査フォローアップは**D1g→D1h→D1i-1→D1i-2(直列)**。**D2はD1i-1後、D3はD1i-2後**、D3→D7。D1eはD1i-1後(migration枠の骨格設計のみ先行可)。D1i-3はD3後、D1i-4はD1i-2後。D4は独立。D5が合流点。D6はD4後。旧形式JSONの変換は全てD1e担当(D1g/D1i-1は拒否のみ)。**出戻り防止**: この直列を飛ばす並列発注はGR-RW-4違反([reviews/2026-07-12-rework-prior-art.md](../reviews/2026-07-12-rework-prior-art.md)、実装ガード16)。
...
### 出戻り防止(先人対応 → LLMガード / 2026-07-12)

正本: [reviews/2026-07-12-rework-prior-art.md](../reviews/2026-07-12-rework-prior-art.md)。落とし穴H-4。エージェント入口は[AGENTS.md](../../AGENTS.md)「出戻り防止」。

13. **恒久物への焼き込みには決定の逆リンクが要る**(GR-RW-1) — Documentスキーマ/ジャーナル/意味論ゴールデンを増やすPRは、本仕様の【決定】節・入場ゲート項目・ユーザー決定文書のどれかへリンクする。無いなら仕様改訂を先に
14. **意味論表が揃うまで完了にしない**(GR-RW-2) — 特にD1i-2。`cargo test`緑だけでタスク表を「完了」にしない(第二監査の教訓)
15. **形状変更と画素意味変更を混ぜない**(GR-RW-3) — 形状→D1e migration(または旧形式拒否+D1e担当の明示)。画素→新PathOp/演算variant。既存意味論ゴールデンの更新で通さない(D1i-4)
16. **D1i-2完了前にD3しない**(GR-RW-4) — 並列レーン表の直列依存を守る。スキーマ非接触のD4/D6のみゲート対象外のまま先行可
17. **migration/フォローアップPRにnon-goalsを書く**(GR-RW-5) — 対象チケット以外のスキーマ整理・ついで修正は混入させず分割(Git hash移行のnon-goals型)
