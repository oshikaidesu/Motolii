...
| D1n | **WAIT（D1m main待ち）**: `ProjectSession`にmain/journal/catalog exact bounded bytes SHA-256 + catalog参照generation SHA-256のtransient revisionを保持し、mutationの閉じたread/write set直前の楽観preconditionで非協調external changeをtyped reject。journal hashはD1d scan既読bytesを使う。watch/mtime/provider名はhintのみ、自動merge/overwriteなし | D1m, [external revision決定](../reviews/2026-07-16-m2-external-revision-decision.md), GR-PV | (1)same-size/mtime main差替えを検出しFS不変 (2)journal中間/ignored-tail・catalog・対象generationのreplace/appear/disappearをcomponent付き拒否 (3)local連続saveはrevision更新し成功 (4)precondition mismatch/malformed/limit error時write 0、detect可能なpost-write peer-wins raceはtyped error+recovery保持。Motolii-wins TOCTOU非保証を明記 (5)watch無しでも緑、journal二重readなし、generation hashは対象operationだけ (6)D1d/D1m/保護golden不変、workspace全緑 |
...
並列レーン(2026-07-16改訂): **D1a**→(D1b/D1f と D8)→D1c→D1c-FU(#101)→D1d→**D1m→D1n**。第二監査フォローアップは**D1g→D1h→D1i-1→D1i-2(直列)**。**D2はD1i-1後、D3はD1i-2後**、D3→D7。D1eはD1i-1かつD1c-FU後。D1i-3はD3後、D1i-4はD1i-2後。D4は独立。D5が合流点。D6はD4後。旧形式JSONの変換は全てD1e担当(D1g/D1i-1は拒否のみ)。**Shared Effect**: GAP-14→D1l→D3e。**保存所有権**: D1d→D1m→D1n。**予防**: この直列を飛ばす並列発注はGR-PV-4違反([permanence-prevention](../reviews/2026-07-12-m2-permanence-prevention.md)、実装ガード16)。
...
11. **クラウド同期フォルダの検出と外部変更** → provider/path検出口は**D1c**、保存前のexact revision審判は**D1n**、watch/競合表示UIはM3（watch/mtimeはhintで保存正本にしない）
12. **Undoの退行防止** → **D2**
...
- ~~クラウド同期/外部変更の保存前審判~~ → **【決定】D1n**(2026-07-16): D1m後にtransient optimistic revisionを実装。watch/mtimeを権威にせず、検出競合は無変更typed reject
