...
| D1l | **Document意味・評価は完了／lint enforcementだけGAP-23で再開**（`a23a4ad`、`74af37e`、docs follow-up `02192c2`）: Effect Definition/Use共有schema+inline migration。Definition台帳+Use参照、v2 lifecycle Command、Writer prepare、Copy Local、journal／Undoは実装・再締結済み。製品の新規Documentは`new_current()`で現行v5、`new_v1()`はv1 legacy専用。2026-07-16決定は`#[deprecated]`を棄却し`#[doc(hidden)]`+AST gateを採択したが、2026-07-23監査でlive codeのdeprecated属性と25 lint suppression残存を確認。作品形式やlifecycleを巻き戻さず、機構だけを修復する | D1e, D1f, D1i-2, **GAP-14**, [journal/Undo追補](../reviews/2026-07-15-d1l-journal-revert-boundary-decision.md), [constructor決定](../reviews/2026-07-16-d1l-current-document-constructor-decision.md), [lint競合追補](../reviews/2026-07-16-d1l-new-v1-lint-conflict-decision.md), [Unit 4L回収](../reviews/2026-07-23-historical-d1l-constructor-lint-lineage-recovery.md), GR-PV | (1)inline migrationでpixel/order/extra/要素数保持、旧reader拒否+再migration冪等 (2)ID一意・参照整合・欠落拒否 (3)共有/同layer複数Use roundtrip・rename/reorder参照不変 (4)target集合/timeline隣接非保存、Composite Set/Backdrop構文不能 (5)GAP-14 §4–§5全条件 (6)nested Keyframe Copy Localの固定順再採番・payload完全性・Undo干渉Reject (7)v1全Command corpusをD1e共用plannerでlossless replay、v1/v2混在 (8)Create/Link/Copy Local準備はDocument不変・raw caller採番なし (9)reservation閉集合6 variantだけcounter以外全文一致+非巻戻し、version/min不変、Redo全文一致 (10)Unlinkに偽reservationなし、全失敗時Document全文不変 (11)現行reader/writer/min=5、`new_current()` roundtrip、AST gate、v1〜v4と版偽装inline/hybridの型付き拒否は緑。ただしdoc-hidden／suppressionなしのenforcement完了条件はGAP-23へ移し、green lintだけで完了としない |
