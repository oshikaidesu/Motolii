...
| U2h | **U2h-1 SPLIT**（[implementation split決定](../reviews/2026-07-27-u2h-1-primary-selection-implementation-split-decision.md)で`U2h-1S` `DONE`、`U2h-1I`実装 `DONE`、`U2h-1P`はCU-106 primary slice待ち）/ **U2h-2 additive以後**: Selection/focus+essential command surface | U2h-1はU0c,U2a,U2b,U2c-1,U2c-4。U2h-2はU2h-1,U3a-1 | U2h-1は[歴史回収 §5](../reviews/2026-07-23-historical-d2-selection-timeline-lineage-recovery.md#5-u2h-1-host-transient-primary-selection再採択)どおりHost Transient `primary: Option<LayerId>`だけを所有し、recursive envelope存在oracle、Document+selection atomic publish、Document revisionとprojection generation分離、Apply/Undo/Redo後publish前reconcile、Redo非復元を実装する。`projection_generation`更新条件・reconcile後publish時点は[CU-104 selection publish envelope決定](../reviews/2026-07-27-cu-104-selection-publish-envelope-decision.md)で決定済み。`u64::MAX`枯渇境界（preflight typed拒否・action消費・publish 0）は[CU-104E projection generation枯渇境界決定](../reviews/2026-07-27-cu-104e-projection-generation-exhaustion-decision.md)で決定済み。実装分割は[U2h-1 primary selection implementation split決定](../reviews/2026-07-27-u2h-1-primary-selection-implementation-split-decision.md)で`U2h-1I`（publish経路）と`U2h-1P`（selection-only入力）へ分離したが、[selection入力到達性決定](../reviews/2026-07-27-u2h-1p-selection-input-reachability-decision.md)によりproducer-only U2h-1Pを停止し、CU-105再確認後のCU-106 primary sliceへ統合した。U2h-2でadditive/range/marquee/bounded AXへ拡張し、focusはhoverと分離する。全段でDocument/Undo/journal/second storeへselectionを入れない |
| U3a | **U3a-1 SPLIT**（[owner/visibility分割決定](../reviews/2026-07-26-u3a-1-headless-timeline-owner-visibility-split-decision.md)で`U3a-1S` owner/visibility `DONE`、`U3a-1I` headless実装 `DONE`）/ **U3a-2 windowedはG0-9待ち**: timeline projection/layout/hit-test+dense surface | U3a-1はU0a,U0b。U3a-2はU3a-1,G0-9 | U3a-1は[歴史回収 §6](../reviews/2026-07-23-historical-d2-selection-timeline-lineage-recovery.md#6-u3a-1-headless-timeline再採択)どおりtop-level Clip/Position keyを同じLayerId/KeyframeId/RationalTimeからread-only投影し、`(start,end,LayerId)` first-fit、viewport cull、Manhattan diamond優先hit-testをtoolkit/renderer非依存にする。GroupとData/Vec2Axes/LookAt/Followはtyped unsupported、fixed lane/f32 spikeをportしない。U3a-2で遠景density〜近景individual、direct wgpu+Vello、window present/input/WebView同居を比較する。既存1k/100kはcapacity証拠でありheadless正しさやD2の証明ではない |
...
PRODUCT-ASSET lane直近完了粒はdocs-only `U2h-1PR`（[selection入力到達性決定](../reviews/2026-07-27-u2h-1p-selection-input-reachability-decision.md)、`DONE`）。producer-only `U2h-1P`はCU-105再確認後のCU-106 primary sliceまで`WAIT`。次PRODUCT-ASSET判断は`CU-105`の`READY-RECHECK`であり、完了済みU3a-1Iとの差分を`PASS / REDUCE / STOP`で処分する。以後の旧列`G0-6H→U0e-3→U2c-3/5→U3a→U4a-*→U4c→U2c-2`は自動的な強制順ではない。未決の順序を追い越して
