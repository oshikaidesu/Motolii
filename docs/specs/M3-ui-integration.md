...
| U4b | **U4b-0 contract/code/main `DONE` / Inspector Position read-only row `DO`、typed intent `WAIT_TARGET` / U4b-1 INTERP-COMMAND `DONE / ACCEPTED`・P04-C2 diagnostic correction `DO` / popup terminal visual `WAIT_TARGET`**: keyframe作成・編集+区間Easing Graph View | U4b-0はU2a,M2-D2。U4b-1はU4a,U4b-0 | U4b-0は[closed contract](../reviews/2026-08-04-u4b0-durable-position-key-closed-contract.md)どおりexplicit Add Position KeyだけからPosition専用durable forward/inverse commandでfresh IDを1件導入した。same-time no-op、Const→1 Linear key、animated off-keyは既存solver+de Casteljauでcurve-preserving insert、Undo/Redo/journal v2で同じID、Auto Key無しoff-key編集はtyped rejectする。codeのbounded acceptanceとmain統合まで閉じ、帯域外allocate+SetPropertyを製品経路にしない。通常製品入口は[Inspector Position entry reclosure](../reviews/2026-08-04-inspector-position-key-product-entry-reclosure.md)でPosition行へ選定し、current-playhead carrierはcommit `75ccd5e7`で成立した。[direct-promotion contract](../reviews/2026-08-04-inspector-position-row-direct-promotion-contract.md)どおり`CU-0A08ITIA`は有限`Const(Vec2)`のX/Yまたはtag-only `Keyframes`の非数値`animated` presenceを読むread-only rowとして`DO`、typed Host intent/queue `CU-0A08ITIB`は`WAIT_TARGET`である。keyframeの評価・contents・value/time/ID/interp/countはAに含めない。Position outgoing Interp専用D2は[implementation acceptance](../reviews/2026-08-04-interp-command-d2-implementation-acceptance.md)どおりcommit `03667b7d`でDONE / ACCEPTED。[P04-C2 terminal-adoption amendment](../reviews/2026-08-04-p04-c2-easing-product-route-contract.md#6-2026-08-04-terminal-adoption-amendment)は既存diagnostic projectionの`SetPositionKeyInterp` label correctionだけを`DO`にした。ProductApp sole EventLoop/shared GpuCtxとG0-9 placement/session/Bezier PATTERNは固定したが、popup rendererの既存product targetが無いのでpartial React/IPC route、renderer port、popup visual codeは`WAIT_TARGET`である。高度型とUser settings preset永続化の**work**、manual gateは引き続き別である |
...
[P02-C3 native Timeline editor playhead contract](../reviews/2026-08-04-native-timeline-editor-playhead-contract.md)のnative ruler→private `ProductApp` editor playhead→native Timeline/Stage producer/carrierはcommit `75ccd5e7`でcode/main `DONE`となった（[implementation acceptance](../reviews/2026-08-04-native-timeline-editor-playhead-implementation-acceptance.md)）。親P02-C3は未完であり、Inspector Position は `CU-0A08ITIA` のread-only rowだけ`DO`、typed wiringは`CU-0A08ITIB WAIT_TARGET`のまま、focus/visible range、playback/audio Transport、snap、Document意味は本境界に入れない。Aは有限`Const(Vec2)`値またはtag-only `Keyframes` presenceを読むだけで、keyframe評価・contents・countは持たない。P04-C2の[ACTIVE-INTERVAL Position read-model contract](../reviews/2026-08-04-position-active-interval-read-model-contract.md)は[Stage transport Easing trigger consumer contract](../reviews/2026-08-04-stage-transport-easing-trigger-consumer-contract.md)どおりread-only projectionとしてDONE / ACCEPTED。outgoing `Interp`の`INTERP-COMMAND` nodeも[implementation acceptance](../reviews/2026-08-04-interp-command-d2-implementation-acceptance.md)どおりcommit `03667b7d`でDONE / ACCEPTEDとなった。producer/Host/React/popup/inputは`WAIT_TARGET`で製品Easing routeは未成立のため、親P04-C2は`TARGET_MISSING`のままである。historical [compiler rejection](../reviews/2026-08-04-position-active-interval-implementation-admissibility-rejection.md)はconsumer不在の観察として保存する。
