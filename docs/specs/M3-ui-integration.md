...
| U4b | **U4b-0/U4b-1 CORE DONE / Add Position Key製品接続・2-key→Easing popup通常window E2E DONE / Easing保存往復は継続**: keyframe作成・編集+区間Easing Graph View | U4b-0はU2a,M2-D2。U4b-1はU4a,U4b-0 | U4b-0はPosition専用Add command / inverse、pure prepare、same-time no-op、curve-preserving insertまで実装済み。[U4b-0P接続・E2E](../reviews/2026-08-02-u4b0p-product-add-position-key-connection-decision.md)で既存Inspector Position markの外観をactionとして接続し、0秒/5秒の2 key、Timeline `◆ 2`、既存Easing popup入口まで通常製品windowで確認。[U4b-1製品Easing接続](../reviews/2026-08-02-u4b1-product-easing-connection-implementation-decision.md)のD2 release、Undo/Redo、Save/reopenを同じ実projectで閉じる作業は継続する |
