...
| U1a | eguiアプリシェル+既存device共有+静止preview | U0a, U0b, G0-1, M2-D3 | Documentの同一frameをUI内へCPU copyなしで表示。UI threadからrenderを直接呼べず、native texture登録をframe loop内で行わない |
...
| U3a | toolkit非依存timeline layout/hit-test+単一wgpu面 | U0a, U0b | clips 1,000+keys 100,000の固定fixture/viewport/操作列ベンチ。遠景density・中景cluster・近景individualを同じtime range/stable ID投影から作り、zoom境界前後でselection/playhead/visible range不変。density pixelをDocument identityへ使わない。CIは基準比、実画面60fps閾値はG0-4後 |
...
→U0d-1→U0d-2→U0d-3→U2a-1→U1a-1→U1a-1b→U1a-2→U1b-1/2→U2b-1→U2c-*`
の順に意味・入力・編集・shell境界を閉じる。次にPR #184から生成機構だけを
`U0e-1/2`へ抽出し、G0-6Hの人間審判後にだけ`U0e-3`へ進む。その後
`U3a→U4a-*`を最初の製品縦切りへ合流させる。論理依存が並列を許す後続も、
...
15. **Rerun転移と接合部の後半発覚を枝番内で先に潰す**: [M3 / Rerun実装後半発覚プレモーテム](../reviews/2026-07-20-m3-rerun-late-discovery-premortem.md)に従い、U0e-2のfixture単一正本と保存形式確定、U1a-1bの色/alpha/display pool寿命、U1a-2のstable panel identity、U1b-2のpool generation、U3aのsemantic zoom境界を各枝番の負例へ含める。fixture manifestへU0b-1の所有層を与えず、registration/resource計器はtest-only accessorに限る。Rerun調査全体を無関係なUタスクの一括ゲートにせず、参照する枝番だけfile/API単位のjust-in-time transfer packetを持つ
