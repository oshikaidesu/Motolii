...
PRODUCT-ASSET lane直近完了粒はdocs-only `CU-G04S0`（VS-1 edit runtime session source判断の選定、`DONE`）。docs-only `CU-109SP`（`CU-109`/`CU-111`相互先後のP1 precedence）も`DONE`。CU-105/CU-106親は`SPLIT`し、producer-only `U2h-1P`はprimary consumer `CU-106P`へ統合して実consumer surfaceまで`WAIT`。次PRODUCT-ASSET `DO`はdocs-only `CU-G04S`（VS-1 edit runtime session source判断）。`CU-109`は`WAIT`。`U3a-2Q-V`は`WAIT`（actual consumer surface evidence 待ち）。以後の旧列`G0-6H→U0e-3→U2c-3/5→U3a→U4a-*→U4c→U2c-2`は自動的な強制順ではない。未決の順序を追い越して
