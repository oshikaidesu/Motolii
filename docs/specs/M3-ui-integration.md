...
CU-0A03/R0からCU-0A07C/R4Cまで完了した。R2Bは[#344](https://github.com/oshikaidesu/Motolii/pull/344)でEasing trigger/CSSを、R3Bは[#350](https://github.com/oshikaidesu/Motolii/pull/350)でR3AのKEYS/LAYERS JSX/CSSをbyte同一でproduct ownerへ移し、mockを単一package importのconsumerへ反転して二重copy 0を閉じた。R4Cは[#357](https://github.com/oshikaidesu/Motolii/pull/357)でInspectorをproduct単一ownerへ移した。CU-0A08ISは[Inspector read-model inventory](../reviews/2026-07-26-cu-0a08is-inspector-read-model-inventory.md)で全表示分類とfixture拒否契約を固定した。CU-0A08IPは`DONE`（fixture由来 read-only projection decoder、product-owned・非export、fixture/testのみ）。CU-G02の`U3a-1`選定は維持する。`U3a-1`は`SPLIT`、`U3a-1S`（owner/visibility docs decision）は`DONE`、`U3a-1I`（headless実装）は`DONE`。次のPRODUCT-ASSET粒は`CU-G03`（edit durability ordering decision）を`DO / SPEC`として選定し、既存D1m/D2/U2b-1 authorityへ照合するdecision/spec PRだけを許可する。CU-G03はCU-109 runtime配線、CU-G09、CU-101/102、CU-104、U2h-1実装を束ねない。`U4a-1`/`U4a-2`はU3a-1後続でもVS-1の未決順序を追い越さない。`U2h-1`はCU-104とU2c ownershipの整合まで未選択。`CU-0A08IT`は`WAIT`（`U4a-2` Direct製品入口待ち）。`U2c-2`はU4a-2（Direct）+ U4c（Advanced）待ち。Host transport、typed intent、JSX binding、`S`行、Rust/schema/plugin変更は非目標のままとする。Motolii Studio Previewはまだ未実装である。G0-6HはU0e-3だけを止める並行人間審判、G0-9DはDistribution Readyまで待つ。
