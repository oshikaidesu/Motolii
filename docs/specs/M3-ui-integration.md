...
PRODUCT-ASSET lane直近完了粒は`CU-0B03`。`CU-0A08BTI`のRectangle typed intentを、Browser一面のCSP offline bundleからnative `wry` WebView、Rust exact codec、instance/sequence gate、容量16のprivate inboxまで接続した。callback内D2、fixture caller、generic invoke、公開plugin APIは0である。token後続は`WAIT`。次は`CU-0B04N/R`と実製品Host再投影`CU-0B05`の最小製品経路だけを再締結し、`Browser typed intent→Host→Place→Stage/Timeline/Inspector→Undo`を貫通させるまで横展開しない。以後の旧列は自動的な強制順ではない。未決の順序を追い越して
