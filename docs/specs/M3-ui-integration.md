...
→U0d-1→U0d-2→U0d-3→U2a-1→U1a-1/2→U1b-1/2→U2b-1→U2c-*`
...
15. **Rerun転移と接合部の後半発覚を枝番内で先に潰す**: [M3 / Rerun実装後半発覚プレモーテム](../reviews/2026-07-20-m3-rerun-late-discovery-premortem.md)に従い、U0e-2のfixture単一正本、U1a-1の色/alpha/display pool寿命、U1a-2のstable panel identity、U1b-2のpool generation、U3aのsemantic zoom境界を各枝番の負例へ含める。Rerun調査全体を無関係なUタスクの一括ゲートにせず、参照する枝番だけfile/API単位のjust-in-time transfer packetを持つ
