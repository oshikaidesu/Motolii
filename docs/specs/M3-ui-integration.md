...
| G0-6 | 視覚言語tokenと認知審判 | **人間審判ACCEPT / U0e-3解禁** | [着手前決定§5](../reviews/2026-07-16-m3-preflight-decisions.md#5-g0-6-見た目はuxの投影として導出する)。U0e-1生成機構、U0e-2R固定React比較baseline、現行routeの5画面×6 variant=30 PNGとread-only再現checkまで完了。[G0-6H人間審判ACCEPT](../reviews/2026-07-29-g0-6h-human-acceptance-decision.md)でUI作者がlive `#plugin-browser-candidate`を最終承認し、`CU-0B01`をDONE、親`CU-0B02` / `U0e-3`を解禁した。[CU-0B02S分割決定](../reviews/2026-07-29-cu-0b02s-product-token-ownership-split-decision.md)後の次粒は`CU-0B02T`。派生bitmapの個別人間閲覧やlatest sourceとのbyte同一性は主張せず、旧generationは不変の回帰証拠として保存する。 |
...
PRODUCT-ASSET lane直近完了粒はdocs-only `CU-0B02S`。実装粒`CU-0A09B`も`DONE`。親`CU-0B02` / `U0e-3`はtoken supplier、React consumer、native consumer、component state、icon判断へ`SPLIT`し、次PRODUCT-ASSET `DO`はproduct-owned Dark DTCGとgenerator v2のRust/CSS/manifestだけを閉じる`CU-0B02T`。accepted routeのlegacy style scrape、parallel mock token supplier、native default visualsは後続R/N粒で一方向consumerへ反転し、`CU-0B02T`へ束ねない。Light/custom/high-contrast、`--object-1..6`、icon gridは未決のまま発明しない。親`CU-0A09`は残るR6 surfaceのため`SPLIT`し、BrowserはDONE、Inspector/KEYS-LAYERS/Easing triggerは各R5交換待ち。R6はdevelopment observer分離だけを所有し、最初のnon-mock runtime callerはH1b、通常製品windowはW0bが所有する。authority粒`CU-0A08ITP-P`と実装粒`CU-0A08ITP`、実装粒`CU-0A08BTP`（Browser read-only projection / JSX connection）と`CU-109`（edit durability runtime wiring、[#425](https://github.com/oshikaidesu/Motolii/pull/425)）も`DONE`。docs-only `CU-G04S`（session source / no-session / interim action disposition）と`CU-G04S0`（選定）、docs-only `CU-109SP`（`CU-109`/`CU-111`相互先後のP1 precedence）も`DONE`。CU-105/CU-106親は`SPLIT`し、producer-only `U2h-1P`はprimary consumer `CU-106P`へ統合して実consumer surfaceまで`WAIT`。docs-only `CU-110S`、docs-only `CU-110D`、docs-only `CU-107S`、docs-only `CU-107D`、docs-only `CU-107R`、docs-only `CU-107N`、docs-only `CU-107W`も`DONE`。G0-6H/CU-0B01は人間`ACCEPT`で`DONE`。`U3a-2Q-V`は`WAIT`（actual consumer surface evidence 待ち）。以後の旧列`G0-6H→U0e-3→U2c-3/5→U3a→U4a-*→U4c→U2c-2`は自動的な強制順ではない。未決の順序を追い越して
