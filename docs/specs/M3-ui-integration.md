...
| U4b | **U4b-0 CORE DONE / product connection未実装、U4b-1 contract DONE / CORE DO**: keyframe作成・編集+区間Easing Graph View | U4b-0はU2a,M2-D2。U4b-1はU4a,U4b-0 | U4b-0はPosition専用Add command / inverse、pure prepare、same-time no-op、curve-preserving insert、JournalEdit v2 oracleまで実装済み。[U4b-1 outgoing Interp契約](../reviews/2026-08-02-u4b1-outgoing-interp-command-contract-decision.md)で隣接left/right identity、左key outgoingだけのold/new command、stale拒否、active interval read projectionを固定した。次はCORE command/projection。popupは[native Easing popup受入契約](../reviews/2026-07-22-m3-native-easing-popup-acceptance.md)に従い、React trigger、native内容、Host lifecycle/User settingsへ分離する |
