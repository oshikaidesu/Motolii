...
| U4a | `NodeDesc`自動parameter panel。U4a-1は`motolii-ui::parameter_control`所有のtoolkit非依存model、U4a-2は生成panel・preview・gesture接続 | U2b, U0e, U1b | U4a-1のcrate root公開面は`HostParameterControl`、`ParameterControlSpec`、`ParameterControlError::UnsupportedValueType`、`map_parameter_control(&ParamDef)`の4項目だけに固定する。`F64`はdomainを保持、`Vec2`/`Vec3`/`Color`は対応、`AssetRef`は型付き拒否し、specは既存`motolii_doc::CommandKind::SetProperty`と`motolii_doc::ScalarPropertyId::EffectParam`だけを指す。全登録pluginの全保存paramを対応または拒否し、plugin ABI・serde・Document command意味・`DocumentCommandRequest`は変更しない。U4a-2は全保存paramが自動panelから編集可能なconformance、100回連続slider更新でUI送信がblockせず、操作中は最新generationだけをpreview表示し、確定値が一致し、gesture全体がUndo 1回 |
...
2026-07-30に[比較面のnative Timeline移管](../reviews/2026-07-30-native-timeline-product-asset-transfer-implementation-decision.md)も完了し、既存React `KeyToolsCandidate`とnative header/ruler/S-M rail/row/bar/key/playhead/labelを通常製品windowへ接続した。Document / selection / Undo ownerとvisible range停止線は変更しない。
