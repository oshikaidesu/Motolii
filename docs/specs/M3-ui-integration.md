...
ステータス: **ドラフト / 製品実装停止**([M2基盤再締結ゲート](../reviews/2026-07-15-m2-foundation-reclosure-gate.md)はmainで解除済み。次のM3入場PRで入場条件を最新mainへ再翻訳して確定する。#180/#191は隔離CIと空クレート骨格であり、M3入場完了を意味しない。INF-1と[M3着手前決定](../reviews/2026-07-16-m3-preflight-decisions.md)によるG0-2/G0-3/G0-4/G0-7の意味決定は記録済み)
...
A-1(→**egui-wgpuの既存device/native texture共有で構造的に解消**。[採用判断](../reviews/2026-07-18-m3-egui-selection.md)で実機確認)、D-3(OpenCut流用の期待値管理)。

## M3仕様確定ゲート(G0)

| ID | 内容 | 状態 | 確定条件 |
|---|---|---|---|
| G0-1 | egui採否と既存wgpu共有 | **完了** | [egui採用判断](../reviews/2026-07-18-m3-egui-selection.md)。Apple M4 / Metalでcore-first device、native texture、lifecycle、日本語IMEを実機確認 |
...
| G0-3 | plugin UIモデル(GAP-13) | **完了(縮小採用)** | [着手前決定§3](../reviews/2026-07-16-m3-preflight-decisions.md#3-g0-3-plugin-uiは表現を開き操作文法を閉じる)。v1 public契約はHost自動生成panelのみ。plugin所有toolkit code/native widget/自由wgpu UIは公開しない |
...
| G0-6 | 視覚言語tokenと認知審判 | **手順完了・目視待ち** | [着手前決定§5](../reviews/2026-07-16-m3-preflight-decisions.md#5-g0-6-見た目はuxの投影として導出する)。U0e-1/2は生成機構とreference fixtureに限定して先行可。具体token値と製品componentを入れるU0e-3はG0-6Hの人間審判まで待つ |
...
以下は**新しいM3入場PRの採択後**に適用する個別依存の原則である。G0自体はM3全コードを一括停止する門ではなく、各タスクは依存するG0/M2項目だけを満たせば着手できる。例: U1aはG0-2/G0-3を待たずに進められる。custom UI追加タスクはG0-3の判定後に初めて起票する。現在はM3入場PR待ちの停止線が優先し、U0〜U9を着手・Issue化しない。

## 方針(2026-07-18 改訂: UI基盤をeguiに変更)

- **UI基盤はegui**。[採用判断](../reviews/2026-07-18-m3-egui-selection.md)どおり、初期統合はegui/eframe/egui-wgpu/egui-winit 0.35、egui_tiles 0.16、wgpu 29の組合せをadapter内で固定する。versionはDocument/plugin契約へ出さない
- プレビューは同一device上の`Rgba8Unorm` `TextureView`を`egui_wgpu::Renderer::register_native_texture`へ登録して表示する。display poolの安定viewはpool生成時に一度だけ登録し、毎frame sampler/bind groupを作らない
...
- キーフレーム編集UIは、Flow/Alight Motion式の**Interval Easing Editor**と、実時間×実値のdock可能な**Graph View**を分ける。前者は現在の2キーフレーム区間の`Interp::Bezier{x1,y1,x2,y2}`または高度区間補間を編集するショートカット。後者はfocus中channelの複数key・複数区間を俯瞰し、key時刻・値・導出したtangentを編集する独立Viewである。Graph Viewの表示pan/zoom/normalize/snapshotをDocumentへ入れず、drag中にauto-fitせず、curve上へのkey追加で既存形状を変えない。参照と停止線は[Graph View参照・比較決定](../reviews/2026-07-19-graph-view-reference-decision.md)。**空間モーションパス(位置の2D曲線)は両者と別概念**で、v1コアには入れない(プラグイン領域/v1後半)
...
旧案の「3段構え」(自動生成 / toolkit所有UI / wgpu描画)は歴史資料にだけ残し、現在のactive roadmapへ自由UIを置かない。

## プラグインパネルの拡張方式(v1)

GAP-13は[M3着手前決定§3](../reviews/2026-07-16-m3-preflight-decisions.md#3-g0-3-plugin-uiは表現を開き操作文法を閉じる)で**縮小採用**として解消した。
...
3. **自由UIはv1公開契約にしない**: plugin所有のegui/native UI codeとwgpu UI textureは実装しない。専用データモデル編集環境級の需要はv1後に別境界として再審査する
...
この表のIDは能力単位であり、そのまま1 PRにしない。M3入場PRが採択したU0b〜U4aの実装は、[UIコンセプトから実装チケットへの分解](../reviews/2026-07-16-m3-ui-concept-to-tickets.md)の枝番を **1 Issue = 1 commit** で発注する。親IDの完了は必要な枝番がすべてmainへ到達した時だけ記録する。

**停止線**: [M2基盤再締結ゲート](../reviews/2026-07-15-m2-foundation-reclosure-gate.md)はmainで解除済みだが、下表**U0〜U9の全製品タスク**はM3入場PRの採択前に着手・Issue化しない。下表と並列レーンは、M3入場PRが最新mainへ再翻訳した場合だけ発注書として有効になる。main到達済みのU0a相当は依存方向CIと空クレート骨格であり、U0後続の解禁根拠にしない。

| ID | 内容 | 依存 | 完了条件(概要) |
|---|---|---|---|
| U0a | `motolii-ui`クレート骨格+UI toolkit依存方向CI | G0-1 | `motolii-ui`以外の製品クレートのegui/eframe/egui-winit/egui-wgpu/egui_tiles依存をCargo metadata検査が拒否。domain公開型へtoolkit型が無い。現行Slint骨格を同じPRで置換する |
| U0b | UI状態所有表+toolkit非依存domain intent | G0-2, M2-D2 | 代表操作をDocument/User settings/Workspace/Transientへ分類。toolkit型なしintentの単体テスト。恒久workspace形式はこのタスクで発明しない |
| U0c | input router+安定`CommandId`+event種別 | U0b, G0-2 | press/release/click/dragを区別し、IME preedit中のshortcut抑止を自動試験。shortcutを持つ全登録commandに安定IDがあり、機能crateのraw key/modifier分岐を拒否。物理入力からdomain intentまでtoolkit型なし |
| U0d | **全shortcutを変更できる**keymap base+user delta JSON永続化 | U0c | builtin base不変。全bindingの追加/置換/複数割当/無効化、documented JSON read/validate/write、roundtrip、version migration冪等、移行前原本と未知`CommandId`保持。既定を無効化→別bindingで同じintentを発行する全command conformance。競合/OS予約は型付き診断 |
| U0e | DTCG theme token generator+component state+icon体系+UI scale/high contrast | U0a。具体値と製品導入はG0-6H | token JSONからRust/egui adapterへ決定的生成し手編集を拒否。5 reference screenの人間審判後にだけ具体値を製品へ入れ、text/non-text/focus contrast、gradient許可list、component state、icon grid、motion数値検査を通す。UI scaleはUser settingsとしてDocument/Final不変、高コントラストはDark/Lightと直交するtoken variantとして同じstate matrixを通す |
...
| U0g | **英語正本localization catalog+font/language User settings** | U0a, U0b | `en-US`をsource/fallback/reference正本とし、登録message keyだけをUIへ表示。language packはcatalog追加だけで導入でき、UI code/Document/`CommandId`/plugin ID不変。Default/System/Custom UI font、technical font、CJK fallbackをrole分離し、欠落glyph/読込不能を診断して組み込みfontへfallback。font/language変更でDocument/Undo/Final不変。日本語UIは別patch、作品内日本語入力はU1dで独立受け入れ |
| U1a | eguiアプリシェル+既存device共有+静止preview | U0a, G0-1, M2-D3 | Documentの同一frameをUI内へCPU copyなしで表示。UI threadからrenderを直接呼べず、native texture登録をframe loop内で行わない |
...
| U1d | 日本語IME深部受け入れ | U1a, U0c, U0g | 英語UIのままpreedit下線、候補位置、変換中Enter非奪取、長文歌詞を対象OS実機で記録。日本語UI language packの完成を依存にせず、失敗時は入力経路の仕様改訂へ戻る |
...
| U2c | Direct/Tool/Advanced + 共通component/診断conformance harness | G0-7, U2b | 代表操作を存在する複数入口から実行し、同じDocument意味/Undo 1回/Cancel変更ゼロ。領域固有rejectionをTransient Diagnostic Envelopeへ適応し、Brief/Context/Inspectで同じreason/subject/factsを保持する。UIイベント列や診断をserializeしない。gray/dimだけのsilent disabled、外部検索必須、Document objectのUI文言依存、巨大error enum、診断componentの直接mutation、同じIntentの局所picker/popup、独自hover/focus/Cancel/error投影をfixtureで拒否する |
...
| U3a | toolkit非依存timeline layout/hit-test+単一wgpu面 | U0a, U0b | clips 1,000+keys 100,000の固定fixture/viewport/操作列ベンチ。CIは基準比、実画面60fps閾値はG0-4後 |
...
| U4b | keyframe編集+Interval Easing Editor | U4a | Preview直下のEasing iconはplayheadが隣接key間にある時だけ有効（key上・区間外は無効）。key clickでは開かない。single click=Interval Easing Editor、double click=単一◎markのお気に入りcurveを現在区間へ即適用し1 Undo（popupを残さない）。お気に入り変更はUser settingでDocument/Undo不変、最終使用へ自動追従しない。現在区間の補間切替/cubic-bezier 4値+presetが反映し、既存区間の非対象curve不変プロパティテスト。補間型ごとのhandle本数・対応parameter・表示条件・可動域／連動はスクリーンショット確認値と明示的な推定を観察台帳で区別してtoolkit非依存宣言へ固定し、`BeginHandleDrag→Preview→Commit/Cancel` IntentをReact/eguiで共用する。推定parameterをDocument／公開APIへ昇格させない。drag中Transient、pointer-up 1 command/1 Undo、Escape/capture loss変更ゼロ。正規化graph座標だけをstateに持ち、egui `Response`/`Rect`/px/DPI/DOM selectorをDocument・評価・Undoへ流さない |
...
| U4d | **Plugin Browser discovery+視覚preview+適用** | U0e, U2c, U4a | Browserを`Media / Plugins`の二面とし、Pluginは`All / Installed / Used / Recent`、User Collections、Search、Resultsを同じshellへ投影する。`Issues`は通常sourceへ常設せず、`Missing / Unavailable`はAll結果、検索、diagnosticから発見する。Sources／Collectionsは大きな上下余白を使わず同じ高密度行scaleへ揃える。既定は横長thumbnail+短い名前、thumbnail-onlyは正方形、listはHost taxonomy（例:`Effect › Light`）を表示しsegmentから絞れる。Search直下のkind tag行、正常な`READY / AVAILABLE`、card説明、provider/install path、反復`Apply`を置かない。逸脱だけを形+文言で示す。single click=選択/Preview、対象ObjectへのD&D=基本Commit、有効な選択中Objectへのdouble click/`Enter`=同じCommit Intentで1 Undo。対象不在・型不一致・利用不能はtyped reason、部分適用なし。短い説明は適用後のInspector Effect parameter領域だけに置き、Object Transform等を同時に並べない。view/thumbnail寸法/検索/Collection/hover/CancelはDocument・Undo不変、RecentはCommitだけ。Project openからinstall/実行せず、missing payload保持・無関係編集・必要export拒否を既存diagnosticへ接続する。thumbnail生成・検索・1000候補scrollでUI threadをblockしない |
| U4e | dock可能なmulti-key Graph View | U2a, U3b, U4b | Apple Motion型のparameter list+実時間×実値graphを持ち、focus中channelをprimary、他channelをcontextとして投影する。Frame Selected、pan/zoom、display normalize、snapshot、filterはDocument/Undo不変。key/handle dragはTransient→release 1 gesture/1 Undo、Esc/capture loss変更ゼロ。Shift=左右tangent分離、Ctrl/Cmd=角度固定で長さだけ、Alt=長さ固定で角度だけを同じIntentへ正規化する。curve上のkey追加はde Casteljau分割で追加前のshapeを保つ。drag中auto-fit禁止。absolute tangent、snapshot、view range、toolkit座標をDocument・公開APIへ追加しない |
| U5 | scrub/再生transport UI | U0c, U0e, U1b, U3b, M2-D5 | vsync暴走注入でもTransport同期不変。最新seekのみ表示。低速時のvarispeedは数値試験+別記の聴感確認 |
| U6 | Media Browser+素材preview内source range+Inbox受取+import/D&D配置 | U0e, U2b | Project Assetと外部filesystem候補を別popup・別top-level tabへ分けず、既存Browserの`Media`へ統合する。source railは`All Media / Project / 複数の登録folder / Collections / Recent`を持ち、検索・preview・選択を共有する。Resultsは`thumbnail-only / thumbnail+name / list`を選べ、全modeでMedia thumbnailを16:9枠へ`contain`表示し、自動crop・自動trim・縦横比変更をしない。filesystem scopeは表示形式と独立した`Browse folders / All files`とし、前者は現在階層直下、後者は現在階層配下のfileだけを再帰的に平坦表示する。標準単一選択、modifier追加／解除、Shift範囲選択に加えて明示的な`Select` modeを備え、checkbox形状と件数で複数選択を読めるようにする。Tagは事前作成できる常設boxとしてResults直上の横shelfへ置き、縦rail下へ隠さない。file／folderまたは複数選択をboxへD&Dすると一括追加、box clickで絞り込み、件数を表示し、幅超過は横scrollする。解除は逆dragでなく、選択後の`Tags…` menuへ現在Tagをcheck表示し、checked Tag clickで一括解除、unchecked Tag clickで追加する。folder Tagは子へ暗黙伝播させず、stable file／directory identityを参照するWorkspace metadata候補として同一identityのAll／folder結果へ一貫投影し、Document、Undo、実directory構造を変えない。表示mode、scope、Tag、検索、選択変更では素材In/Out、Document、Undo不変。`All Media`はcanonical file identityでdedupeし、Project登録済みなら同じ結果へ`IN PROJECT`等の関係を表示する。登録folderの検索・選択・preview・In/OutはDocument外で、`Add to Inbox`は未配置参照を受け取るだけとする。全Media候補をTag boxへD&D可能とし、folder以外は有効targetへのdropを基本配置Commit、有効な選択targetへのdouble click／`Enter`を同じIntentへ接続する。各cardやfooterへ`Place / Open / Apply`を重複常設せず、folderのdouble click／`Enter`はnavigationでUndo不変。Project登録済み素材の配置、または外部素材の取込＋配置を確定した時だけ、既存の`Clip.duration = out - in`と`TimeMap.source_start = in`へ1 command／1 Undoで変換する。同じrangeからの再配置は明示変更まで同じ結果になり、配置前のrange変更、対象外drop、CancelではDocument/Undo不変。target不在・型不一致・欠落/不正asset・空/逆転/素材尺外rangeはtyped error表示し、部分取込・部分配置せず、再帰走査・metadata取得・decodeをUI threadで行わない。InboxとCollectionsはasset所有者や全履歴にならず、参照だけを持ち処理後に外す。動画/SVGを配置し楽曲1本を設定 |
...
並列レーン（ゲート解除とM3入場PR後のみ）: U0aはmain到達済み。最初はU0b-1と、PR #184を証拠・抽出元にするU0e-1を並列にできる。U0e-1/2は生成機構とreference fixtureだけ先行し、具体値と製品componentはG0-6H後のU0e-3へ分ける。U0b後はU0c/U0g/U2a、G0-8+K1a後にU0f、U0c後はU0d。U1a+U0g後はU1d、U1a後はU1b/U2b。U1b+U0e+D1k/D3後にU1f。U1b+U1c+U5+K1d後にU1g、U0e+U0f+U1g後にU1h。U2b後はU2c/U3a/U4a/U6/U9a、U9a後にU9b、U9b後にU9c。U0d+U2a+U2c後にU2f、D1l+D3e+U3a後にU2g、U1f+U2c後にU2d、U2d+D3後にU2e、U2c+U4a後にU4c/U4d。M2-D3後はM4-K0を独立実施可能。U0e/U3a後にU3d。U3b後はU5/U7/U8aを並列。U8bはM4-K7c待ち。AG-3はAG-1/AG-2後のv1.x追加レーン。追加UI力学レーンはU0c+U2a/U2b/U2c+U3a後にU2h、U2h+U3a後にU3e。U1iは各providerの型付きsnapshot到達後、U3fはM4-K1b/K7/K8の該当provider到達後にだけ接続する。

## GR-UI審判割当表

| 規律 | 対象タスク | 自動審判 | 人間実機審判 |
|---|---|---|---|
| GR-UI-1 状態所有 | U0b, U0d, U0f, U0g, U1f, U1h, U1i, U2b, U2d, U2h, U3c, U3e, U3f, U9b, U9c | 状態分類fixture、font/language/Stage View/preview resource設定変更時のDocument/Final不変、provider/selection/search/readiness非Document検査、keymap/resource settings roundtrip、script/SVG source非Document検査 | workspace/script復元UXは保存方針決定後 |
| GR-UI-2 command境界 | U0c, U2a, U2b, U2c, U2d, U2e, U2f, U2g, U2h, U3b, U4a, U4b, U4c, U4d, U6, U8a, U9a, U9b, U9c, AG-3 | input→intent、入口意味同値、selection/target/definition/use ID、macro/merge/Undo property test、generator batch原子性、`&mut Document`依存検査 | — |
| GR-UI-3 thread/latest | U1a, U1b, U1e, U1f, U1g, U1h, U1i, U3e, U3f, U4a, U4d, U5, U6, U9b, AG-3 | non-blocking seek/parameter/overscan/activity/search/readiness/telemetry、deadline時の最新frame、generator実行中のUI応答、generation逆順、同期readback禁止 | 長時間scrub/parameter/camera drag、pressure縮退、generator cancelの体感 |
| GR-UI-4 単位 | U0e, U1e, U1f, U2b, U2d, U4a, U4b, U7 | UI scale/DPI注入時domain command一致、UI scale変更時のDocument/Stage View/Output Frame/Final不変、UI degree↔Document radian、RationalTime、Stage View非永続 | 100/125/150/200%での到達性、別monitor/DPI移動 |
| GR-UI-5 UI toolkit隔離 | U0a, U0b, U1f, U1i, U2c, U2h, U3a, U3e, U3f, U9a, U9b, U9c | Cargo metadata+公開型走査、provider/selection/search/readiness model、generator/SVG hookのruntime/egui非依存test、windowなしlayout/hit-test test | — |
...
| GR-UI-8 視覚認知 | U0e, U0g, U1f, U1h, U1i, U2d, U2e, U2f, U2g, U2h, U3d, U3e, U3f, U4a, U4c, U4d, U5, U6 | token生成差分、英語/pseudo-locale/font fallback、Dark/Light×通常/高コントラストstate matrix、Output Frame/Stage View/Camera/pressure warning/activity/selection/filtered selection/readiness/target-pick/Relative HUD/from-in接続状態、Plugin thumbnail/list/taxonomy/逸脱状態、raw color、contrast、icon/state、通常+lightness/CVD reference画像 | 5秒識別、英語/追加language packでの文字切れと所在、高コントラスト時の境界/focus/selection/disabled/warning、frame内外/Camera対Hand/pressure理由/activity/selection/readiness/通常drag対Relative/from対in、Plugin結果の視覚走査、grayscale/CVD、既存UIとの馴染み |
...
本節の「着手前」「最初」はM3入場を起点とする。入場PR採択前は、製品コード・公開API・永続形式を変更するスパイクを含めて発注しない。

受容側の対照先例は[reviews/2026-07-16-m3-ui-rapid-acceptance-prior-art.md](../reviews/2026-07-16-m3-ui-rapid-acceptance-prior-art.md): 第一部=プロダクト単位の受容事例(Flow/AviUtl2/VOICEVOX/FCPX両面等 — 界隈の期待リスト)、第二部=**業界収斂した操作語彙の台帳**(本節ガード5の「業界標準の操作」の具体的な中身)とUX原理の一次資料、第三部=後発の勝ち筋「どの操作も直感的」(Ableton先例のAEカウンター分解 — 直感性の要件「結果が100ms以内に見え可逆」は性能の関数であり、performance-modelが前提条件)。仮説メモであり本節のガード・完了条件は変更しない(転移候補はM3入場PRの再翻訳時に個別採択)。
...
2. **タイムライン・波形・グラフ類を項目ごとのegui widgetで組まない**: immediate modeでも10,000行の無仮想化はCPU負荷が増える。タイムラインは1枚のカスタムwgpu面をnative textureとして表示し、layout/hit-test/render modelをtoolkit非依存に置く。U3aでclips 1,000+keys 100,000の固定fixtureを測る。**60fpsはG0-4で基準機・操作列・p50/p95を決めた後の製品目標で、hardware未指定のCI閾値にしない**
