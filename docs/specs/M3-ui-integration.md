...
| G0-6 | 視覚言語tokenと認知審判 | **手順完了・目視待ち** | [着手前決定§5](../reviews/2026-07-16-m3-preflight-decisions.md#5-g0-6-見た目はuxの投影として導出する)。U0eは具体tokenと5 reference screenの人間審判まで待つ |
...
| U2c | Direct/Tool/Advanced + 共通component conformance harness | G0-7, U2b | 代表操作を存在する複数入口から実行し、同じDocument意味/Undo 1回/Cancel変更ゼロ。hidden helper/itemなし、未実装入口は明示。UIイベント列や入口種別をserializeしない。新規UIを[UI操作言語](../ui-interaction-language.md)のcomponent state matrixへ通し、同じIntentの局所picker/popup、独自hover/focus/Cancel/error投影、正規component迂回をfixtureで拒否する |
