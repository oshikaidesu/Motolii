...
| U4b | **U4b-0 contract/code/main `DONE` / Inspector Position product entry `WAIT_TARGET` / U4b-1 INTERP-COMMAND `DONE / ACCEPTED`・product route `WAIT_TARGET`**: keyframe作成・編集+区間Easing Graph View | U4b-0はU2a,M2-D2。U4b-1はU4a,U4b-0 | U4b-0は[closed contract](../reviews/2026-08-04-u4b0-durable-position-key-closed-contract.md)どおりexplicit Add Position KeyだけからPosition専用durable forward/inverse commandでfresh IDを1件導入した。same-time no-op、Const→1 Linear key、animated off-keyは既存solver+de Casteljauでcurve-preserving insert、Undo/Redo/journal v2で同じID、Auto Key無しoff-key編集はtyped rejectする。codeのbounded acceptanceとmain統合まで閉じ、帯域外allocate+SetPropertyを製品経路にしない。通常製品入口は[Inspector Position entry reclosure](../reviews/2026-08-04-inspector-position-key-product-entry-reclosure.md)でPosition行へ選定し、current-playhead carrierはcommit `75ccd5e7`で成立した。normal Position row/projectionとtyped Host intentが未成立のため`CU-0A08ITI`は`WAIT_TARGET`のまま。Position outgoing Interp専用D2は[implementation acceptance](../reviews/2026-08-04-interp-command-d2-implementation-acceptance.md)どおりcommit `03667b7d`でDONE / ACCEPTED。ただしproducer/Host/React/popup/inputは`WAIT_TARGET`で、親`P04-C2` / U4b-1 product routeは未完である。U4b-1は[native Easing popup受入契約](../reviews/2026-07-22-m3-native-easing-popup-acceptance.md)に従い、ReactはGraph iconとobject・channel・pressed/disabledのaccessible state、native wgpu popupはframe/preset/user library/form/curve/grid/handle/drag preview、Hostはanchor/z-order/focus/dismiss/DPIとUser settings codecを所有する。visible summary chromeは別のUI判断まで実装しない。区間内部だけ有効、drag中write 0、release 1 Undo、Esc/focus loss 0、左key outgoingだけを変更する。manual gateはM3最終までdeferred |
