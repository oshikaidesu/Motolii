...
CU-0A03/R0、CU-0A04/R1 Browser ownership、CU-0A05A/R2A mock-side extraction、CU-0A05B/R2B product ownership、CU-0A06A/R3A mock-side extractionは完了した。R2Bは[#344](https://github.com/oshikaidesu/Motolii/pull/344)でR2AのReact trigger/CSSをbyte同一でproduct ownerへ移し、mockを単一package importのconsumerへ反転し、mock/product二重copy 0を閉じた。fixed commitに存在するBrowserはfixed-source provenance、R2Aで新設されfixed commitに存在しないEasing triggerはproduct current-closure provenanceとする。popup/model、Host/User settings/D2接続、visible summary chromeは持ち込んでいない。R3 readiness再確認では独立KEYS/LAYERS source不在を確認したため、`CU-0A06A/R3A mock-side JSX/CSS extraction`→`CU-0A06B/R3B product ownership`へ分割した。R3Aは[#347](https://github.com/oshikaidesu/Motolii/pull/347)で6状態oracleを未変更sourceへ先に固定し、TimelineCandidate全体を移管せずtool panel JSX/CSSだけを分離した。R3B再確認では固定hash・hookless props-only境界と既存Easing ownership topologyの再利用可能性が成立したため、現在粒はCU-0A06Bの`DO`である。Motolii Studio Previewはまだ未実装である。G0-6HはU0e-3だけを止める並行人間審判、G0-9DはDistribution Readyまで待つ。
