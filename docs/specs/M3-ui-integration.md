...
| U5 | scrub/再生transport UI。`CU-210P` paused ruler seekは通常製品E2E `DONE`、playback / audio clock / continuous scrubは未完 | U0c, U0e, U1b, U3b, M2-D5 | vsync暴走／render遅延注入でもaudio device clockとTransport速度不変。videoは古い要求を捨て、最新seekだけ表示。preview都合の自動varispeed／pitch変更を拒否 |
