...
| U2b | **U2b-1完了 / U2b-2 Place決定・未実装**: normalized UI event→private single writer→`Arc<Document>`購読、次にRectangle Place | U1a, U2a。U2b-2はU2b-1/U2c-1 | U2b-1はApply/Undo/Redo成功時だけsnapshotをpublish。U2b-2は[歴史回収 §3](../reviews/2026-07-23-historical-d2-selection-timeline-lineage-recovery.md#3-u2b-2-place-product-core再採択)どおり、accepted terminal dropだけwriter snapshotのLayerIdTable clone→fresh live-next→既存AddTrackItem 1件→apply_macro 1回を同じprivate同期call stackで行う。start/preview/cancel/stale/duplicateはDocument/revision/history/ID/publish 0。top-level compatible selection、first Track top fallback、canonical Y-up、Rect size 0.2、playhead〜composition endをGUI command意味として固定し、appearance、journal durability、selection publish、D3/GPUは別ゲート |
...
CU-0A03/R0からCU-0A07C/R4Cまで完了した。R2Bは[#344](https://github.com/oshikaidesu/Motolii/pull/344)でEasing trigger/CSSを、R3Bは[#350](https://github.com/oshikaidesu/Motolii/pull/350)でR3AのKEYS/LAYERS JSX/CSSをbyte同一でproduct ownerへ移し、mockを単一package importのconsumerへ反転して二重copy 0を閉じた。fixed commitに存在するBrowserはfixed-source provenance、R2A/R3Aで新設されfixed commitに存在しないEasing triggerとKEYS/LAYERSはproduct current-closure provenanceとする。popup/model、Host/User settings/D2接続、visible summary chrome、Timeline本体は持ち込んでいない。R4 readinessではInspectorの独立React source不在とskeleton非採用を確認し、CU-0A07SでR4A unchanged-source oracle、R4B mock-side同形React化、R4C byte同一product ownershipへ分割した。R4Aは[#353](https://github.com/oshikaidesu/Motolii/pull/353)で5状態と主要操作のoracleを固定して完了し、R4Bは[#355](https://github.com/oshikaidesu/Motolii/pull/355)で固定mock内の同形React化と一方向legacy adapterを完了し、R4Cは[#357](https://github.com/oshikaidesu/Motolii/pull/357)でR4BのInspector JSX/CSSをbyte同一でproduct ownerへ移し、mockを単一package importのconsumerへ反転して二重copy 0を閉じた。現在粒はCU-0A08Iの`READY-RECHECK`である。Motolii Studio Previewはまだ未実装である。G0-6HはU0e-3だけを止める並行人間審判、G0-9DはDistribution Readyまで待つ。
