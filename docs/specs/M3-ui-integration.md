...
| G0-3 | plugin UIモデル(GAP-13) | **完了(縮小採用)** | [着手前決定§3](../reviews/2026-07-16-m3-preflight-decisions.md#3-g0-3-plugin-uiは表現を開き操作文法を閉じる)。v1 public契約はHost自動生成panelのみ。自由`.slint`/wgpu UIは延期 |
...
3. **自由UIはv1公開契約にしない**: plugin同梱`.slint`の実行時ロードとplugin所有wgpu UI textureは実装しない。専用データモデル編集環境級の需要はv1後に別境界として再審査する
...
| U0c | input router+安定`CommandId`+event種別 | U0b, G0-2 | press/release/click/dragを区別し、IME preedit中のshortcut抑止を自動試験。shortcutを持つ全登録commandに安定IDがあり、機能crateのraw key/modifier分岐を拒否。物理入力からdomain intentまでSlint型なし |
...
| U0e | DTCG theme token generator+component state+icon体系 | U0a。具体値と製品導入はG0-6H | token JSONからRust/Slintへ決定的生成し手編集を拒否。5 reference screenの人間審判後にだけ具体値を製品へ入れ、text/non-text/focus contrast、gradient許可list、component state、icon grid、motion数値検査を通す |
| U0f | **resource policy User settings model+永続化**: VRAM/RAM/disk予算preset/custom、preview解像度auto/fixed、縮退許可をDocument外で保持 | G0-2, G0-8, U0b, M4-K1a | (1)version付きroundtrip/migration冪等 (2)未知field原本保全 (3)設定変更でDocument serialize/journal/Undo不変 (4)custom hard capをK1aへ反映 (5)共有メモリ合算capの矛盾を型付き拒否 (6)Slint/backend型を保存形式へ入れない |
...
| GR-UI-5 Slint隔離 | U0a, U0b, U1f, U1i, U2c, U2h, U3a, U3e, U3f, U9a, U9b, U9c | Cargo metadata+公開型走査、provider/selection/search/readiness model、generator/SVG hookのruntime/Slint非依存test、windowなしlayout/hit-test test | — |
