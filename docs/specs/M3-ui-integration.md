...
PRODUCT-ASSET laneの本筋はU2b-2 Place接続である。BrowserのCSP offline bundleからnative `wry` WebView、Rust exact codec、instance/sequence/source gate、容量16のprivate inboxまでは`CU-0B03`で成立した。private Place coreでは、表示済みcameraに対するStage releaseのcanonical Y-up変換、clone上のfresh `LayerId`候補、live-next照合、journal→`apply_macro`一回、同じ`LayerId`のprimary publish、cancel/invalid時の変更0を自動試験で固定した。`CU-103`で正本`VectorRecipe::StandardShape::Rect`を不透明白fill・strokeなしとしてcanonical GPU renderへlowerし、fixture plugin代用0、非Rect typed rejectを閉じた。実MacでHTML5 dragがreleaseを消費し、egui比較baselineのStage terminalへ届かない停止点を再現したため、`CU-0B04S`でReact=start identity、Host private platform capture、native Stage=latest layout epoch hit-test/canonical変換へ再締結し、`CU-0B04P`でAppKit global pointer/button probe、generation state、release latch、active repaintを実装した。`CU-0B04N`では通常project sessionをdirect product Hostへ切り替え、同一device/queue・一つのtop-level SurfaceへVRAM display slotを直接sampleするnative StageとTimeline viewportを表示し、同じ最新layout epochからBrowser boundsとStage hit-test/canonical Y-up変換を接続した。`CU-0B04R`ではopaque Browser islandへstale/duplicate geometry epoch拒否、Host requested focus owner、single-shot initial projection readyを接続した。`CU-0B05`ではreload/process terminationをnew instance epochのBrowser再生成へ接続し、old callback拒否、stable source/latest layout/requested focus再投影、一回の自動回復と二回目Browser-only degradedを実Macで閉じた。`CU-107PV`では一active dragのproduction `Moved`をlatest layoutのoptional Stage NDCを持つprivate Host Transient previewへ非terminal配送した。`CU-107TC`では候補terminalをEscape / outside Stage / capture loss / noneへprivateな閉集合で排他網羅分類した。`CU-107AD`ではProduct Host発行のmonotonic generationとretired high-waterでstale / duplicate / replayを拒否し、none候補を一active drag高々一件へadmitした。`CU-107TD`ではadmit済みterminalだけをprivate `PendingStageDrop`へ一回配送した。次は親`CU-107`の4前提閉鎖確認で、D2への接続は後続`CU-110`が所有する。Timeline/InspectorのDocument projection、CU-111 Undo/Redo、実機WebView跨ぎdropからD2までの配送は未完であり、製品E2E完了とは数えない。token後続は`WAIT`。このまま`Browser typed intent→Host→Place→Stage/Timeline/Inspector→Undo`を貫通させるまで横展開しない。以後の旧列は自動的な強制順ではない。未決の順序を追い越して
