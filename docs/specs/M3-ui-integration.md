...
| U4b | **U4b-0/U4b-1 CORE DONE / product connection未実装**: keyframe作成・編集+区間Easing Graph View | U4b-0はU2a,M2-D2。U4b-1はU4a,U4b-0 | U4b-0はPosition専用Add command / inverse、pure prepare、same-time no-op、curve-preserving insertまで実装済み。[U4b-1 outgoing Interp契約](../reviews/2026-08-02-u4b1-outgoing-interp-command-contract-decision.md)どおり隣接left/right identity、左key outgoingだけのold/new command、stale拒否、active interval read projection、JournalEdit v2 replayまで実装済み。次は通常製品routeへAdd triggerとEasing trigger/native/Host/D2を分離接続する |
