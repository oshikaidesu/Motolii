...
| G0-9L | 主開発Macのlocal platform prerequisite evidence gate | **仕様決定 / 実機審判待ち** | [段階化決定](../reviews/2026-07-23-m3-g0-9-staged-platform-gates.md)の固定Mac構成で、同条件renderer比較、実IME/VoiceOver/focus、100回resize、capture、Surface lost、WKWebView content process crash、offlineとstale epoch拒否を一つのmanifestへ閉じる。実機合格後も、固定Macのplatform prerequisite evidenceだけを限定確定する。W0b、H1b、Motolii Studio Preview、window結合、G0-6H、G0-3/GAP-13は解禁・PASS・省略・迂回しない |
