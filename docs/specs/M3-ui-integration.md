...
| U4b | **U4b-0 contract/code/main `DONE` / Inspector Position product entry `WAIT_TARGET` / U4b-1 INTERP-COMMAND `DONE / ACCEPTED`・P04-C2 Easing product route `CONTRACT_CLOSED / IMPLEMENTATION_PENDING`**: keyframe作成・編集+区間Easing Graph View | U4b-0はU2a,M2-D2。U4b-1はU4a,U4b-0 | U4b-0は[closed contract](../reviews/2026-08-04-u4b0-durable-position-key-closed-contract.md)どおりexplicit Add Position KeyだけからPosition専用durable forward/inverse commandでfresh IDを1件導入した。same-time no-op、Const→1 Linear key、animated off-keyは既存solver+de Casteljauでcurve-preserving insert、Undo/Redo/journal v2で同じID、Auto Key無しoff-key編集はtyped rejectする。codeのbounded acceptanceとmain統合まで閉じ、帯域外allocate+SetPropertyを製品経路にしない。通常製品入口は[Inspector Position entry reclosure](../reviews/2026-08-04-inspector-position-key-product-entry-reclosure.md)でPosition行へ選定し、current-playhead carrierはcommit `75ccd5e7`で成立した。normal Position row/projectionとtyped Host intentが未成立のため`CU-0A08ITI`は`WAIT_TARGET`のまま。Position outgoing Interp専用D2は[implementation acceptance](../reviews/2026-08-04-interp-command-d2-implementation-acceptance.md)どおりcommit `03667b7d`でDONE / ACCEPTED。[P04-C2 Easing product route contract](../reviews/2026-08-04-p04-c2-easing-product-route-contract.md)はread-only Stage `{snapshot,subscribe,publish}` bridgeを変えず、React anchor/layout intent→separate surface-local Stage sender/inbox→Host re-derivation/session→native popup terminal admission→existing queue enum/action handling location→D2を`CONTRACT_CLOSED / IMPLEMENTATION_PENDING`へ閉じた。ReactはGraph iconとobject・channel・pressed/disabledのaccessible state、native wgpu popupはframe/preset/user library/form/curve/grid/handle/drag preview、Hostはanchor/z-order/focus/dismiss/DPIとUser settings codec ownerを維持する。visible summary chromeは別のUI判断まで実装しない。区間内部だけ有効、drag中write 0、value-changing release 1 Undo、same-value/Esc/focus loss 0、左key outgoingだけを変更する。basic preset/custom release以外、高度型とUser settings preset永続化の**work**は`WAIT_TARGET`、manual gateはM3最終までdeferred |
...
[P02-C3 native Timeline editor playhead contract](../reviews/2026-08-04-native-timeline-editor-playhead-contract.md)のnative ruler→private `ProductApp` editor playhead→native Timeline/Stage producer/carrierはcommit `75ccd5e7`でcode/main `DONE`となった（[implementation acceptance](../reviews/2026-08-04-native-timeline-editor-playhead-implementation-acceptance.md)）。親P02-C3は未完であり、normal Position rowとInspector wiringは`CU-0A08ITI WAIT_TARGET`のまま、focus/visible range、playback/audio Transport、snap、Document意味は本境界に入れない。P04-C2の[ACTIVE-INTERVAL Position read-model contract](../reviews/2026-08-04-position-active-interval-read-model-contract.md)は[Stage transport Easing trigger consumer contract](../reviews/2026-08-04-stage-transport-easing-trigger-consumer-contract.md)どおりread-only projectionとしてDONE / ACCEPTED。outgoing `Interp`の`INTERP-COMMAND` nodeも[implementation acceptance](../reviews/2026-08-04-interp-command-d2-implementation-acceptance.md)どおりcommit `03667b7d`でDONE / ACCEPTEDとなった。producer/Host/React/popup/inputは`WAIT_TARGET`で製品Easing routeは未成立のため、親P04-C2は`TARGET_MISSING`のままである。historical [compiler rejection](../reviews/2026-08-04-position-active-interval-implementation-admissibility-rejection.md)はconsumer不在の観察として保存する。
