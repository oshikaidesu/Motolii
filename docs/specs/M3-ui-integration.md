...
| U4b | keyframe編集+区間Easing Graph View | U4a | Preview直下のGraph iconはplayheadが隣接key間にある時だけ有効（key上・区間外は無効）。key clickでは開かない。single click=Graph View、double click=単一◎markのお気に入りcurveを現在区間へ即適用し1 Undo（popupを残さない）。お気に入り変更はUser settingでDocument/Undo不変、最終使用へ自動追従しない。現在区間の補間切替/cubic-bezier 4値+presetが反映し、既存区間の非対象curve不変プロパティテスト |
...
| GR-UI-8 視覚認知 | U0e, U0g, U1f, U1h, U1i, U2d, U2e, U2f, U2g, U2h, U3d, U3e, U3f, U4a, U4c, U5, U6 | token生成差分、英語/pseudo-locale/font fallback、Dark/Light×通常/高コントラストstate matrix、Output Frame/Stage View/Camera/pressure warning/activity/selection/filtered selection/readiness/target-pick/Relative HUD/from-in接続状態、raw color、contrast、icon/state、通常+lightness/CVD reference画像 | 5秒識別、英語/追加language packでの文字切れと所在、高コントラスト時の境界/focus/selection/disabled/warning、frame内外/Camera対Hand/pressure理由/activity/selection/readiness/通常drag対Relative/from対in、grayscale/CVD、既存UIとの馴染み |
