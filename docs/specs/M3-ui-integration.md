...
| U4b | **U4b-0 contract/code/main `DONE` / Inspector Position product entry `WAIT_TARGET` / U4b-1 INTERP-COMMAND `DONE / ACCEPTED`・P04-C2 Easing product route `DO`**: keyframe作成・編集+区間Easing Graph View | U4b-0はU2a,M2-D2。U4b-1はU4a,U4b-0 | U4b-0は[closed contract](../reviews/2026-08-04-u4b0-durable-position-key-closed-contract.md)どおりexplicit Add Position KeyだけからPosition専用durable forward/inverse commandでfresh IDを1件導入した。same-time no-op、Const→1 Linear key、animated off-keyは既存solver+de Casteljauでcurve-preserving insert、Undo/Redo/journal v2で同じID、Auto Key無しoff-key編集はtyped rejectする。codeのbounded acceptanceとmain統合まで閉じ、帯域外allocate+SetPropertyを製品経路にしない。通常製品入口は[Inspector Position entry reclosure](../reviews/2026-08-04-inspector-position-key-product-entry-reclosure.md)でPosition行へ選定し、current-playhead carrierはcommit `75ccd5e7`で成立した。normal Position row/projectionとtyped Host intentが未成立のため`CU-0A08ITI`は`WAIT_TARGET`のまま。Position outgoing Interp専用D2は[implementation acceptance](../reviews/2026-08-04-interp-command-d2-implementation-acceptance.md)どおりcommit `03667b7d`でDONE / ACCEPTED。[P04-C2 Easing product route contract](../reviews/2026-08-04-p04-c2-easing-product-route-contract.md)はread-only Stage snapshotを変えず、React anchor/layout intent→surface-local Host re-derivation/session→native popup terminal admission→既存queue/D2を`DO`へ閉じた。ReactはGraph iconとobject・channel・pressed/disabledのaccessible state、native wgpu popupはframe/preset/user library/form/curve/grid/handle/drag preview、Hostはanchor/z-order/focus/dismiss/DPIを所有する。visible summary chromeは別のUI判断まで実装しない。区間内部だけ有効、drag中write 0、release 1 Undo、Esc/focus loss 0、左key outgoingだけを変更する。basic preset/custom release以外、高度型とUser settings preset永続化は別authority、manual gateはM3最終までdeferred |
