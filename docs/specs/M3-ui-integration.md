...
| U4b | **U4b-0 Add Position Key決定・未実装 / U4b-1 Easing**: keyframe作成・編集+区間Easing Graph View | U4b-0はU2a,M2-D2。U4b-1はU4a,U4b-0 | U4b-0は[歴史回収 §4](../reviews/2026-07-23-historical-d2-selection-timeline-lineage-recovery.md#4-u4b-0-durable-add-position-key再採択)どおりexplicit Add Position KeyだけからPosition専用durable forward/inverse commandでfresh IDを1件導入する。same-time no-op、Const→1 Linear key、animated off-keyは評価値をcurve-preserving insert、on-key編集はvalueだけ、Auto Key無しoff-key編集はtyped reject。帯域外allocate+SetPropertyを製品経路にしない。U4b-1は[native Easing popup受入契約](../reviews/2026-07-22-m3-native-easing-popup-acceptance.md)に従い、ReactはGraph icon/現在値要約、native wgpu popupはframe/preset/user library/form/curve/grid/handle/drag preview、Hostはanchor/z-order/focus/dismiss/DPIとUser settings codecを所有する。区間内部だけ有効、drag中write 0、release 1 Undo、Esc/focus loss 0、左key outgoingだけを変更する |
...
CU-0A03/R0とCU-0A04/R1 Browser ownershipは完了し、現在orderは`CU-0A05/R2 Easing trigger ownership`である。R2は未実装。Motolii Studio Previewもまだ未実装である。G0-6HはU0e-3だけを止める並行人間審判、G0-9DはDistribution Readyまで待つ。
