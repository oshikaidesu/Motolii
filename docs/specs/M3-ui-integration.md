...
| U3b | timeline配置/移動/trim操作。親CU-201は`SPLIT` | U0c, U2b, U3a | [CU-201S](../reviews/2026-08-01-cu-201-u3b-move-trim-snap-responsibility-split-decision.md)でmove command spec/core、trim spec/core、snap spec、product gesture、random oracle、E2Eへ分割。moveは`SetClipStart`、trimは`TrimClipIn` / `TrimClipOut`へ固定し、COREまで実装済み。[CU-201N-S](../reviews/2026-08-01-cu-201n-s-timeline-snap-contract-decision.md)のframe grid / 別Clip edge snapと[CU-201P](../reviews/2026-08-01-cu-201p-native-timeline-interval-gesture-implementation-decision.md)のnative gestureを既存D2へ接続し、通常製品windowでmove / trim / Undo / Redo / reopenを観測済み。[CU-201R](../reviews/2026-08-01-cu-201r-timeline-interval-sequence-oracle-decision.md)は192-step列で重複0、move平行移動、全Undo / reopen初期bytes一致、Cancel queue 0を固定した。provider不在playhead、beat/user marker、snap settingは候補外。次は`CU-201E`でsame identity、保存interval、UI drag state非永続を通常製品receiptへ閉じる |
