...
ステータス: **ドラフト / 製品実装停止**([M2基盤再締結ゲート](../reviews/2026-07-15-m2-foundation-reclosure-gate.md)が別の再締結PRで解除された後に、入場条件を最新mainへ再翻訳して確定。#180/#191は隔離CIと空クレート骨格であり、M3入場完了を意味しない。INF-1と[M3着手前決定](../reviews/2026-07-16-m3-preflight-decisions.md)によるG0-2/G0-3/G0-4/G0-7の意味決定は記録済みで、ゲート解除後のM3入場PRで再翻訳する)
...
以下は**M2再締結解除と新しいM3入場PRの採択後**に適用する個別依存の原則である。G0自体はM3全コードを一括停止する門ではなく、各タスクは依存するG0/M2項目だけを満たせば着手できる。例: U1aはG0-2/G0-3を待たずに進められる。custom UI追加タスクはG0-3の判定後に初めて起票する。現在は後段の停止線が優先し、U0〜U9を着手・Issue化しない。
...
この表のIDは能力単位であり、そのまま1 PRにしない。再締結解除後のM3入場PRが採択したU0b〜U4aの実装は、[UIコンセプトから実装チケットへの分解](../reviews/2026-07-16-m3-ui-concept-to-tickets.md)の枝番を **1 Issue = 1 commit** で発注する。親IDの完了は必要な枝番がすべてmainへ到達した時だけ記録する。

**停止線**: [M2基盤再締結ゲート](../reviews/2026-07-15-m2-foundation-reclosure-gate.md)の発効中は、下表**U0〜U9の全製品タスク**について、個別依存が満たされても着手・Issue化しない。下表と並列レーンは、別PRでゲート解除後にM3入場PRが採択・再翻訳した場合だけ発注書として有効になる。main到達済みのU0a相当は依存方向CIと空クレート骨格であり、U0後続の解禁根拠にしない。
