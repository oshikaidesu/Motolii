...
PRODUCT-ASSET lane直近完了粒は`CU-0B03H`。`CU-0A08BTI`のRectangle typed intentを受けるBrowser一面のexact Web codec、CSP offline bundle、product mount、SHA-256 asset closureを完了した。次PRODUCT-ASSET `DO`は同じcodecをnative bounded inboxへ接続する`CU-0B03`。callback内D2、fixture caller、generic invoke、公開plugin APIを禁止する。token後続は`WAIT`、`CU-107PV`は実製品Host再投影`CU-0B05`待ち。`Browser typed intent→Host→Place→Stage/Timeline/Inspector→Undo`を貫通させるまで横展開しない。以後の旧列は自動的な強制順ではない。未決の順序を追い越して
