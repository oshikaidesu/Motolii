...
PRODUCT-ASSET lane直近完了粒はdocs-only `U3a-2S0`（発注依存証跡closure、`DONE`）。CU-105/CU-106親は`SPLIT`し、producer-only `U2h-1P`はprimary consumer `CU-106P`へ統合して実consumer surfaceまで`WAIT`。次PRODUCT-ASSET判断は`U3a-2S`の`DO`。以後の旧列`G0-6H→U0e-3→U2c-3/5→U3a→U4a-*→U4c→U2c-2`は自動的な強制順ではない。未決の順序を追い越して
