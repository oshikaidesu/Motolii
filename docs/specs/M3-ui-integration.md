...
| U4b | **U4b-0 contract/code/main `DONE` / Inspector Position read-only row `DONE / ACCEPTED`、typed intent `CONTRACT_CLOSED / DO` / U4b-1 INTERP-COMMAND `DONE / ACCEPTED`・P04-C2 diagnostic correction `DONE / ACCEPTED` / popup terminal visual `WAIT_TARGET`**: keyframe作成・編集+区間Easing Graph View | U4b-0はU2a,M2-D2。U4b-1はU4a,U4b-0 | U4b-0は[closed contract](../reviews/2026-08-04-u4b0-durable-position-key-closed-contract.md)どおりPosition専用durable command、same-time no-op、既存solver+de Casteljau、Undo/Redo/journal v2をcode/mainまで閉じた。通常製品入口はInspector Position行、current-playhead carrierはcommit `75ccd5e7`、read-only rowはcommit `2c20e88e`で成立した。[one-shot intent contract](../reviews/2026-08-04-inspector-position-key-one-shot-intent-contract.md)により`CU-0A08ITIB`はexact sequence-only private message、separate Host FIFO、current Rust primary/playhead、one queue action、existing prepare/commit/publishへ閉じて`DO`。target/time/value/version、generic key API、opacity gesture変更は含めない。Aはkeyframeの評価・contents・value/time/ID/interp/countを持たず、human visualはM3 final `EXTERNAL_GATE_PENDING`。Position outgoing Interp専用D2とdiagnostic correctionはDONE / ACCEPTED、既存product renderer targetの無いEasing popup visualは`WAIT_TARGET`のままである |
