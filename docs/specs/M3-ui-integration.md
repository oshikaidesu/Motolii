...
| U4b | **U4b-0 contract/code/main `DONE` / Inspector Position read-only row `DONE / ACCEPTED`、typed intent `WAIT_TARGET` / U4b-1 INTERP-COMMAND `DONE / ACCEPTED`・P04-C2 diagnostic correction `DONE / ACCEPTED` / popup terminal visual `WAIT_TARGET`**: keyframe作成・編集+区間Easing Graph View | U4b-0はU2a,M2-D2。U4b-1はU4a,U4b-0 | U4b-0は[closed contract](../reviews/2026-08-04-u4b0-durable-position-key-closed-contract.md)どおりexplicit Add Position KeyだけからPosition専用durable forward/inverse commandでfresh IDを1件導入した。same-time no-op、Const→1 Linear key、animated off-keyは既存solver+de Casteljauでcurve-preserving insert、Undo/Redo/journal v2で同じID、Auto Key無しoff-key編集はtyped rejectする。codeのbounded acceptanceとmain統合まで閉じ、帯域外allocate+SetPropertyを製品経路にしない。通常製品入口は[Inspector Position entry reclosure](../reviews/2026-08-04-inspector-position-key-product-entry-reclosure.md)でPosition行へ選定し、current-playhead carrierはcommit `75ccd5e7`で成立した。[implementation acceptance](../reviews/2026-08-04-inspector-position-row-implementation-acceptance.md)どおり`CU-0A08ITIA`はcommit `2c20e88e`で有限`Const(Vec2)`のX/Yまたはtag-only `Keyframes`の非数値`animated` presenceを同じinert read-only Position行へ投影し、code/main `DONE / ACCEPTED`。typed Host intent/queue `CU-0A08ITIB`は`WAIT_TARGET`である。keyframeの評価・contents・value/time/ID/interp/countはAに含めず、human visualはM3 final `EXTERNAL_GATE_PENDING`。Position outgoing Interp専用D2は[implementation acceptance](../reviews/2026-08-04-interp-command-d2-implementation-acceptance.md)どおりcommit `03667b7d`でDONE / ACCEPTED。[P04-C2 diagnostic correction acceptance](../reviews/2026-08-04-p04-c2-diagnostic-correction-implementation-acceptance.md)どおりlabel correctionもDONE / ACCEPTED。ProductApp sole EventLoop/shared GpuCtxとG0-9 placement/session/Bezier PATTERNは固定したが、popup rendererの既存product targetが無いのでpartial React/IPC route、renderer port、popup visual codeは`WAIT_TARGET`である。高度型とUser settings preset永続化の**work**、manual gateは引き続き別である |
...
[P02-C3 native Timeline editor playhead contract](../reviews/2026-08-04-native-timeline-editor-playhead-contract.md)のnative ruler→private `ProductApp` editor playhead→native Timeline/Stage producer/carrierはcommit `75ccd5e7`でcode/main `DONE`となった（[implementation acceptance](../reviews/2026-08-04-native-timeline-editor-playhead-implementation-acceptance.md)）。親P02-C3は未完である。Inspector Position は[implementation acceptance](../reviews/2026-08-04-inspector-position-row-implementation-acceptance.md)どおり `CU-0A08ITIA` のread-only rowだけcommit `2c20e88e`でcode/main `DONE / ACCEPTED`、typed wiringは`CU-0A08ITIB WAIT_TARGET`のまま、focus/visible range、playback/audio Transport、snap、Document意味は本境界に入れない。Aは有限`Const(Vec2)`値またはtag-only `Keyframes` presenceを読むだけで、keyframe評価・contents・countは持たず、human visualはM3 final `EXTERNAL_GATE_PENDING`である。P04-C2の[ACTIVE-INTERVAL Position read-model contract](../reviews/2026-08-04-position-active-interval-read-model-contract.md)は[Stage transport Easing trigger consumer contract](../reviews/2026-08-04-stage-transport-easing-trigger-consumer-contract.md)どおりread-only projectionとしてDONE / ACCEPTED。outgoing `Interp`の`INTERP-COMMAND` nodeも[implementation acceptance](../reviews/2026-08-04-interp-command-d2-implementation-acceptance.md)どおりcommit `03667b7d`でDONE / ACCEPTEDとなった。producer/Host/React/popup/inputは`WAIT_TARGET`で製品Easing routeは未成立のため、親P04-C2は`TARGET_MISSING`のままである。historical [compiler rejection](../reviews/2026-08-04-position-active-interval-implementation-admissibility-rejection.md)はconsumer不在の観察として保存する。
