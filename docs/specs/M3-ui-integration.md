...
PRODUCT-ASSET lane直近完了粒は実装`CU-0A08BTI`。Dark token authority `CU-0B02T`完了後、token後続`CU-0B02R/N/C/I`は`WAIT`へ戻した。`CU-0A08BTI`はRectangle Browser dragをdecode済みscoped identityのfrozen Host callbackへ接続し、ReactのDocument/selection/Undo正本0を維持した。次PRODUCT-ASSET `DO`はH1b Host接続契約`CU-0B03H`。`Browser typed intent→Host→Place→Stage/Timeline/Inspector→Undo`を貫通させるまでtoken/component/icon/provenanceを前へ追加しない。`CU-107PV`は既決の実製品Host再投影`CU-0B05`待ちで、test/dummy callerを到達性に数えない。親`CU-0A09`は残るR6 surfaceのため`SPLIT`し、BrowserはDONE、Inspector/KEYS-LAYERS/Easing triggerは各R5交換待ち。R6はdevelopment observer分離だけを所有し、最初のnon-mock runtime callerはH1b、通常製品windowはW0bが所有する。CU-109 edit durabilityはDONE。G0-9DはDistribution Readyまで待つ。以後の旧列`G0-6H→U0e-3→U2c-3/5→U3a→U4a-*→U4c→U2c-2`は自動的な強制順ではない。未決の順序を追い越して
