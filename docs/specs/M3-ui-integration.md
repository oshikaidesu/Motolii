...
| U4b | **U4b-0 CORE DONE / product connection未実装、U4b-1 Easing**: keyframe作成・編集+区間Easing Graph View | U4b-0はU2a,M2-D2。U4b-1はU4a,U4b-0 | [U4b-0 closed contract](../reviews/2026-08-02-u4b0-add-position-key-closed-contract-decision.md)どおりPosition専用command / inverse、pure prepare、same-time no-op、Const→Linear、animated curve-preserving insert、区間外canonical Hold、eval-owned Bezier split、JournalEdit v2 oracleまで実装済み。次は既存製品routeへのtyped intent接続。U4b-1は[native Easing popup受入契約](../reviews/2026-07-22-m3-native-easing-popup-acceptance.md)に従い、ReactはGraph iconとaccessible state、native wgpu popupはcurve内容、Hostはlifecycle/User settingsを所有。区間内部だけ有効、drag中write 0、release 1 Undo、Esc/focus loss 0、左key outgoingだけを変更する |
