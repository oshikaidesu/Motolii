...
| U4 | パラメータパネル + キーフレーム編集(値のキー打ち、補間切替)+ **区間イージングポップアップ(Flow/AM式、cubic-bezier 4値編集+プリセット。グラフビューは作らない)** | U1, U2 | キーフレーム編集・イージング変更がプレビューに反映される。100回連続のslider更新でUI送信がblockせず、操作中は最新generationだけをpreview表示し、確定値が一致し、gesture全体がUndo 1回 |
