...
ステータス: **ドラフト / UI責任境界・surface topology・G0-9段階化決定**([M2基盤再締結ゲート](../reviews/2026-07-15-m2-foundation-reclosure-gate.md)はmainで解除済み。[UI runtime責任境界](../ui-runtime-architecture.md)はReact chrome + native Stage/Timeline + headless interactionへ固定し、通常windowは[1 top-level wgpu Surface + 2 native viewport + opaque child WebView islands](../reviews/2026-07-21-ui-surface-topology-decision.md)へ固定した。React所有面は[直接移管契約](../reviews/2026-07-22-m3-react-product-asset-promotion-contract.md)R0〜R6に従うproduct-owned packageとmock consumer化だけ先行可。[G0-9段階化決定](../reviews/2026-07-23-m3-g0-9-staged-platform-gates.md)はfixed-Mac prerequisite evidence `G0-9L`とdistribution `G0-9D`を分離するが、local実機審判後も製品統合を解禁しない。現行mainのegui shell等は比較基準として保持する。plugin UI公開契約はG0-9と分離したG0-3 / GAP-13の判断まで停止する。toolkit/renderer非依存の状態所有、layout/hit-test、domain intent、Command境界、Rust coreは各既存依存に従い進行可)
...
| G0-9 | React chrome + native Stage/Timelineのsurface統合parent gate | **責任境界・topology決定 / platform受入継続** | [UI runtime責任境界](../ui-runtime-architecture.md)、[軸分離決定](../reviews/2026-07-22-m3-surface-extension-axis-separation.md)、[React直接移管契約](../reviews/2026-07-22-m3-react-product-asset-promotion-contract.md)、[surface topology決定](../reviews/2026-07-21-ui-surface-topology-decision.md)を正本とする。R0〜R6のproduct-owned React packageとmock consumer化は先行可。`G0-9L`合格後もparentは継続し、`G0-9D`まで全platform合格を名乗らない。plugin sandbox／公開契約はG0-3で別判定する |
| G0-9L | 主開発Macのlocal platform prerequisite evidence gate | **CU-0G02〜04B完了 / CU-0G05L判定中** | [段階化決定](../reviews/2026-07-23-m3-g0-9-staged-platform-gates.md)の固定Mac構成で、同条件renderer比較、実IME/VoiceOver/focus、100回resize、capture、注入Surface lost、WKWebView content process実終了/reload、offline既定拒否とstale epoch拒否は合格済み。CU-0G04BはL3の同一実window閉包へminimize/restore、fullscreen往復、1×1一時不可用、pointer capture/cancel/focus lossを追補し、Grok R2 P0/P1=0でACCEPTした。CU-0G05Lの反対側reviewと限定確定までG0-9L PASSを宣言しない。W0b、H1b、Motolii Studio Preview、window結合、G0-6H、G0-3/GAP-13は解禁・PASS・省略・迂回しない |
| G0-9D | Windows・追加hardwareを含むdistribution gate | **WAIT / HARDWARE** | Windows 10/11 WebView2/PMv2/MS-IME/NVDA/offline/`ProcessFailed`、異DPI・第二monitor・HDR/SDR・実lost、配布対象Macの再審判を別証拠で閉じる。Mac結果、synthetic scale、一次資料でPASSにしない。完了までDistribution Readyを名乗らない |
