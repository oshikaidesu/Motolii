...
PRODUCT-ASSET laneの本筋はU2b-2 Place接続である。BrowserのCSP offline bundleからnative `wry` WebView、Rust exact codec、instance/sequence/source gate、容量16のprivate inboxまでは`CU-0B03`で成立した。private Place coreでは、表示済みcameraに対するStage releaseのcanonical Y-up変換、clone上のfresh `LayerId`候補、live-next照合、journal→`apply_macro`一回、同じ`LayerId`のprimary publish、cancel/invalid時の変更0を自動試験で固定した。`CU-103`で正本`VectorRecipe::StandardShape::Rect`を不透明白fill・strokeなしとしてcanonical GPU renderへlowerし、fixture plugin代用0、非Rect typed rejectを閉じた。ただし実MacではHTML5 dragがreleaseを消費し、egui比較baselineのStage terminalへ届かないため、Browser inboxとPlace coreはまだ製品経路として貫通していない。`CU-0B04S`でReact=start identity、Host private platform capture、native Stage=latest layout epoch hit-test/canonical変換へ再締結し、`CU-0B04P`でAppKit global pointer/button probe、generation state、release latch、active repaintを実装した。次は`CU-0B04N`でnative Stage viewportとlatest-epoch hit-test/canonical変換を接続する。Timeline/Inspector製品projection、CU-111 Undo/Redo、実機WebView跨ぎdropは未完であり、製品E2E完了とは数えない。token後続は`WAIT`。このまま`Browser typed intent→Host→Place→Stage/Timeline/Inspector→Undo`を貫通させるまで横展開しない。以後の旧列は自動的な強制順ではない。未決の順序を追い越して
