...
| U4b | **U4b-0/U4b-1 CORE DONE / Easing製品接続は自動検証DONE・人間E2E待ち / Add trigger未実装**: keyframe作成・編集+区間Easing Graph View | U4b-0はU2a,M2-D2。U4b-1はU4a,U4b-0 | U4b-0はPosition専用Add command / inverse、pure prepare、same-time no-op、curve-preserving insertまで実装済み。[U4b-1 outgoing Interp契約](../reviews/2026-08-02-u4b1-outgoing-interp-command-contract-decision.md)どおり隣接left/right identity、左key outgoingだけのold/new command、stale拒否、active interval read projection、JournalEdit v2 replayまで実装済み。[製品Easing接続](../reviews/2026-08-02-u4b1-product-easing-connection-implementation-decision.md)で既存React trigger、Host identity/layout照合、受入済みnative popup、D2 release 1 commitを通常製品routeへ接続し、自動負例を通した。次はTransport/playheadとAdd Position Keyの製品triggerを接続し、2-key projectで通常Mac製品windowのEasing人間E2Eを閉じる |
