...
CU-0A03/R0からCU-0A06B/R3Bまで完了した。R2Bは[#344](https://github.com/oshikaidesu/Motolii/pull/344)でEasing trigger/CSSを、R3Bは[#350](https://github.com/oshikaidesu/Motolii/pull/350)でR3AのKEYS/LAYERS JSX/CSSをbyte同一でproduct ownerへ移し、mockを単一package importのconsumerへ反転して二重copy 0を閉じた。fixed commitに存在するBrowserはfixed-source provenance、R2A/R3Aで新設されfixed commitに存在しないEasing triggerとKEYS/LAYERSはproduct current-closure provenanceとする。popup/model、Host/User settings/D2接続、visible summary chrome、Timeline本体は持ち込んでいない。R4 readinessではInspectorの独立React source不在とskeleton非採用を確認し、CU-0A07SでR4A unchanged-source oracle、R4B mock-side同形React化、R4C byte同一product ownershipへ分割した。R4Aは[#353](https://github.com/oshikaidesu/Motolii/pull/353)で5状態と主要操作のoracleを固定して完了し、R4Bは[#355](https://github.com/oshikaidesu/Motolii/pull/355)で固定mock内の同形React化と一方向legacy adapterを完了し、現在粒はCU-0A07C/R4Cの`DO`である。Motolii Studio Previewはまだ未実装である。G0-6HはU0e-3だけを止める並行人間審判、G0-9DはDistribution Readyまで待つ。
