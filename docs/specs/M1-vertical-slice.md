...
- 具体形: サンプル `project.json`(`version` / `input`=実写動画 / `overlay.center`=`ParamSource::Keyframes`(x座標をベジェイージングで左→右) / `size` / `color`)を `oc-cli export-project` に渡し、mp4を書き出す。
...
- 実装状況: `ProjectV1`は`input`+`overlay`(ParamSource)を受理済み、`oc-eval`にベジェイージング実装済み、`oc-export`でmp4書き出し済み。**残り=この2レイヤー構成の正式サンプル同梱+E2Eゴールデン**(T10残タスク)。
