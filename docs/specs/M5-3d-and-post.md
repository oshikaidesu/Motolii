...
| P5 | 統合検証: 動画平面 + glTFメッシュ + DataTrack/ParamDriver駆動オーバーレイ + ポストを1シーンで書き出し。実動画トラッキングを使う場合はM4の解析プラグイン完了後に接続する | P2U, P3, P4 | E2Eゴールデンテスト + 実素材でのショット制作 |
...
並列レーン: P1→P2の後、P2D→P3とP2Uを並行できる。P4も独立、P5が合流点。P6は独立(歌詞プラグイン第1号の前提であり、M5内のどのタスクにも依存しない)。
