...
| P2D-RC0 | [Render Contribution証拠Wave親task](../reviews/2026-07-29-m5-render-contribution-evidence-wave.md): P2Dを置換せず、未知の空間表現が型付き要求とrender contributionを追加できる最小境界をA/B/Cの独立docs証拠へ分割する | M1-FREEZE-GATE, M2-D3 | A/B/Cのauthority、current-code fact、固有allowlist、STOP、後続統合停止線が登録され、公開API／Document／plugin契約／wire／実装を変更しない |
| P2D-RCA | Motolii authority/current-code-fact起点のRender Contribution契約draft | P2D-RC0 | 要求とcontribution、Host admission/resolve、alpha/refraction、failure、追加的進化、First Vismのconformance fixture役割を比較中draftへ記録。Vism/package/schema/公開traitは決めない |
| P2D-RCB | Rerun固定commit `954bf95a`の対象file/APIをMotolii問題起点で監査し、asset単位の転移分類を裁定 | P2D-RC0 | AGENTS.mdの6ラベル順、証明／非証明範囲、license/dependency、`DEPEND / VENDOR / PORT / PATTERN / REJECT`、Motolii oracleを記録 |
| P2D-RCC | ゲームエンジン一次資料のrender phase／transparent／refraction方式をMotolii fixtureへ翻訳 | P2D-RC0 | 三つ以上の公式一次資料群をversion付きで比較し、engine内部責任を要件化せずM5正例・負例へ翻訳 |
| P2D-RCI | A/B/C証拠の統合とRender Contribution最終decision。必要ならcontract／private spike／schema／First Vism fixtureを再分割 | P2D-RCA, P2D-RCB, P2D-RCC | **本WaveではWAIT**。Motolii authorityを正として証拠を統合し、公開境界または永続意味が未決なら実装解禁せずSTOP |
...
並列レーン: M2-D1j/D1k/D3とM3-U1f/U2dで2D world/camera/Stageを先に成立させ、K0は透過Stageと独立して進める。P0IとP1 importも互いおよびK0から独立し、P1合流後にP2。P0I→P7a、P2+P7a+K1→P7b→P7c→P7U。P2D-RC0→P2D-RCA/P2D-RCB/P2D-RCCは実装依存を解禁しないdocs証拠Waveとして並行し、統合P2D-RCIは三粒完了後も別発注とする。P2後はP2D/P2U/P3を依存に従って進め、P2U+P3後にP2R。P4はK0後、P6は独立、P5が合流点。world/cameraの成立をP1へ依存させない。
