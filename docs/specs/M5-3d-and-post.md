...
| P2D-RCA2 | Motolii authorityと現行code factだけによるRender Contribution境界比較 | P2D-RC0, P2D-RCE0 | **DO**。専用v2文書だけを変更し、親task §5の6論点、alpha／refraction比較、unknown／第二表現負例、事実／推論／未決を分離する。公開契約を決めない |
| P2D-RCB2 | 裁定済みRerun 6 assetの証拠転記 | P2D-RC0, P2D-RCE0 | **DO**。専用v2文書だけを変更し、A1〜A6の固定観察／比較軸／非証明／class／持込禁止を転記する。network、分類変更、Motolii要件追加をしない |
| P2D-RCC2 | 三engine capsuleのMotolii fixture軸比較 | P2D-RC0, P2D-RCE0 | **DO**。専用v2文書だけを変更し、三family×固定6 fixture候補、soft alpha／refraction証拠coverage、非証明／未決を分離する。方式や数値を焼かない |
| P2D-RCI | 新しい証拠grainの統合とRender Contribution最終decision。必要ならcontract／private spike／schema／First Vism fixtureを再分割 | P2D-RCA2, P2D-RCB2, P2D-RCC2 | **本WaveではWAIT**。旧P2D-RCA/B/Cを完了依存にせず、Motolii authorityを正として統合し、公開境界または永続意味が未決なら実装解禁せずSTOP |
...
並列レーン: M2-D1j/D1k/D3とM3-U1f/U2dで2D world/camera/Stageを先に成立させ、K0は透過Stageと独立して進める。P0IとP1 importも互いおよびK0から独立し、P1合流後にP2。P0I→P7a、P2+P7a+K1→P7b→P7c→P7U。P2D-RC0の共通基盤とP2D-RCE0の固定証拠取得／Rerun裁定は成立し、旧P2D-RCA/P2D-RCB/P2D-RCCを停止したまま、新P2D-RCA2/P2D-RCB2/P2D-RCC2を非交差allowlistで並列化する。統合P2D-RCIは三粒完了までWAITとする。P2後はP2D/P2U/P3を依存に従って進め、P2U+P3後にP2R。P4はK0後、P6は独立、P5が合流点。world/cameraの成立をP1へ依存させない。
