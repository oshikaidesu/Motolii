...
- v1は依然として汎用3Dシーンエディタではない。ユーザーが触るのは素材配置、共有`CompCamera`、遮蔽ポリシーであり、ライト階層・collection・constraint・複数camera等は持ち込まない
...
- `Layer Order`: 3D系LayerSourcePluginは`t`、`Quality`、`CompCamera`、同じworld transform、出力`FrameDesc`を受け取り、GPU上でRGBAテクスチャへ描く。他レイヤーとの遮蔽だけを通常Compositeで決める
...
| P2 | 単一XYZ世界: 全オブジェクトへZ/world transform/共有`CompCamera`を通し、`Layer Order`では投影後にpremultiplied RGBAをレイヤー順合成 | P1 | 2D画像・動画・glTFが同じZ値とcameraで視差する。Zを跨いでも遮蔽は従来C-4 fixtureとピクセル同一 |
| P2D | 拡張可能な遮蔽ポリシー境界+組み込み`Layer Order / Group Depth / AE-style Bins`。通常UIは`Z Occlusion` OFF/ON、Advancedは全ポリシーと明示`Depth Participant`を表示 | P1, P2, M2-D1, M3 | (1)同じ座標の2平面が`Layer Order`ではレイヤー順、`Group Depth`ではZ交差で前後反転 (2)3D-2D-3D fixtureで明示参加フラグだけがbinを分断 (3)effect/mask/typeは無言でbinを変えない (4)切替前後で座標/見かけ投影/Document子順/Undo/選択不変 (5)group外はピクセル不変 (6)opaque/cutout/soft alphaの対応・拒否が明示 (7)未知/非対応ポリシーを無言fallbackしない (8)Advanced非表示でも適用ポリシーとbin境界を識別可能 |
| P2U | Canvas transform toolの`Scale / Depth Move`分離。Scale handleとZ rail/axisを既存toolbar/Inspector/timeline語彙へ統合 | P2, M2-D2, M3 | (1)Scale dragはscaleだけ、Depth dragはposition.zだけをD2 command化 (2)各gestureがUndo 1回 (3)perspective/orthographic fixture (4)active toolを文字なし・grayscaleでも形/iconから識別 (5)DPI差で同じ正規化gestureが同じdomain値 (6)script不要で両channelへkeyframe可能 |
...
| P3 | `CompCamera`のドキュメントモデル統合(キーフレーム可能な共有カメラパラメータ) | P2, P2D, M2-D1 | カメラアニメーションのE2Eテスト。レイヤーごとのカメラを持てないこともスキーマ上確認 |
| P4 | ポストプロセスNode群: ブラー(+Z距離マスク)、色調整(リフト/ガンマ/ゲイン)、グレイン | 凍結ゲート | 各ノードのゴールデンテスト |
| P5 | 統合検証: 動画平面 + glTFメッシュ + DataTrack/ParamDriver駆動オーバーレイ + ポストを1シーンで書き出し。実動画トラッキングを使う場合はM4の解析プラグイン完了後に接続する | P2R, P3, P4 | E2Eゴールデンテスト + 実素材でのショット制作 |
...
並列レーン: P1→P2の後、P2D→P3とP2Uを並行でき、P2U+P3の後にP2R。P4も独立、P5が合流点。P6は独立(歌詞プラグイン第1号の前提であり、M5内のどのタスクにも依存しない)。
