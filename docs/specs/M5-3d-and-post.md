...
- **空間は1つだけ**(2026-07-14ユーザー決定)。2D画像、動画、テキスト、図形、glTF、点群を含む全オブジェクトが常に同じ正準XYZ世界、同じactive cameraから評価されたObservation、同じworld transformを使う。簡易表示と実depthで座標・アニメーション・親子関係を解釈し直さない
...
- v1は依然として汎用3Dシーンエディタではない。ユーザーが触るのは素材配置、M2/M3から常在するPlanar compatibility camera、遮蔽ポリシーである。Camera Object／Providerへ拡張しても、ライト階層・collection・camera cut・複数active camera等を同時に持ち込まない
...
- **Depth Move**: `position.z`だけを編集し、scaleは固定する。キャンバスはanchorから伸びるDepth rail/axis arrow、`Depth` icon、tabular数字の現在値/差分を使う
- active toolは形・icon・labelで区別し、色だけに依存しない。timeline/Inspectorでも平面配置の`Position X/Y`、前後配置の`Depth Z`、`Scale`を別group・別iconで識別可能にし、操作中の対象channelを示す
- `Depth Z`はZ方向の平行移動であり、`Rotation Z`はZ軸まわりの回転である。同じ`Z`名の三連controlや同じautomation channelへまとめず、Depthの操作だけで別の3D modeへ暗黙に切り替えない
- orthographic cameraではZ移動で見かけサイズは変わらないが、Z座標と遮蔽結果は変わる。Scaleへ自動変換せず、Depth Move toolのままDepth railと数値変化を示す
...
複数レイヤーのZ配置は数値欄、script、制御nullへ追い出さず、CanvasとTimelineに隣接する**Depth Rail**で直接編集する。これは初期セットアップ専用dialogではなく、現在時刻の評価済みdepthを再生中も表示するtool viewである。通常は明示iconで開閉し、選択変更だけで自動展開しない。選択レイヤーを「奥行き展開」して簡易parallaxを作る入口と、激しいZ animationを監視・編集する場所を同じ視覚言語へ統一する。

Depth Railを独立させるのはUI上の意味分離であり、Documentの正準XYZや通常transformを分裂させるためではない。`Position X/Y`はStage平面、`Depth Z`は前後配置として見せるが、確定値は同じ`position.z`へ書く。Depth専用field、Depth固有の所有者、3D専用Object、Depth Rail独自のanimation channelを追加しない。
...
- 同じZへ重なるmarkerは値を変えず件数付きstackとして表示する。hoverや選択だけで扇状展開せず、Timeline選択のstable IDをstack内のfocusとして識別する
- **Layer Order Distribute**: 同じparentを持つ選択レイヤーへ奥端・手前端の区間を明示し、authoring orderで等間隔配置する頻出操作。context menuではなく常設iconから開き、既定方向は現在の`Layer Order`で手前に合成されるレイヤーをcamera側へ割り当てる
...
- 補正は現在時刻の評価済みObservationと変形から解析的に求め、pixel readbackや前フレーム状態を使わない。orthographic cameraでは補正不要で、Zだけを変更する
...
## Camera Provider／Observationと空間rendererの分界

object/material/generatorとCamera Providerの追加はplugin境界へ開くが、world、active cameraの席、Observation配布、bounds／picking、depth参加境界はHostが所有する。コアを汎用3Dアプリ化せず、2Dモーショングラフィックの操作感を保つため、次の分界を守る。

- Host(Document/Render): active camera bindingと評価済みObservationの配布経路、単一の正準XYZ世界、全オブジェクトのworld transform、グループの遮蔽ポリシー、レイヤー順、bounds／picking参加境界、グループ/マスク/エフェクトの評価順、Quality、出力`FrameDesc`を所有する。現行`CompCamera`はPlanar互換実装として常在するが、将来Observation Contractそのものではない
- `Layer Order`: 現行LayerSourcePluginは`t`、`Quality`、具体`CompCamera`、同じworld transform、出力`FrameDesc`を受け取り、GPU上でRGBAテクスチャへ描く。この受け渡しは既存Planar／RGBA経路のコード事実であり、P3前に一般Observation APIへ見せかけない。P3後の空間rendererは具体provider IDを知らずtyped Observationを消費し、他objectとの遮蔽だけを通常Compositeで決める
...
| P2D-RC0 | [Render Contribution証拠Wave親task](../reviews/2026-07-29-m5-render-contribution-evidence-wave.md): P2Dを置換せず、未知の空間表現を検討する共通Host境界、固定語彙、引用anchor、code fact、非目標と証拠分離規律を一度だけ固定する | M1-FREEZE-GATE, M2-D3 | 共通基盤をleafが再解釈せず、取得／転記／比較／裁定を一発注へ束ねない。公開API／Document／plugin契約／wire／実装を変更しない |
| P2D-RCE0 | **DONE**。主担当Codexによる固定証拠取得／転移裁定: Rerun固定6 asset、Bevy 0.19、Godot 4.6、Unreal Engine 5.8を6 capsuleへ分離し、Rerun A1〜A5=`PATTERN`、A6=`REJECT`を裁定 | P2D-RC0 | source commit／URL、version／取得日、license、証明／非証明範囲、製品非import、削除条件を固定。後続leafはnetwork、repo横断探索、Rerun分類変更を行わない |
| P2D-RCA | **停止／後継P2D-RCA2登録済み**。旧Motolii authority/current-code-fact起点の一括契約draft | P2D-RC0 | Grok REJECT差分は不採用。旧IDを再発注せず、共通基盤とCodex固定比較軸だけを読む新IDへ置換 |
| P2D-RCB | **停止／後継P2D-RCB2登録済み**。旧Rerun 6 asset一括監査／転移裁定 | P2D-RC0 | 旧IDを再発注せず、主担当Codexが取得／裁定した固定証拠を分類変更なしで転記する新IDへ置換 |
| P2D-RCC | **停止／後継P2D-RCC2登録済み**。旧三engine一次資料取得／比較／fixture翻訳 | P2D-RC0 | 旧IDを再発注せず、provider family別capsuleをnetwork禁止でfixture軸比較する新IDへ置換 |
...
| P2D-RCC3-UNREAL | **停止／差分なし**。Unreal capsuleから固定5観測項目への転記 | P2D-RC0, P2D-RCE0, GR-D4 | Opus `DESIGN_STOP`。非記載fragmentを固定したP2D-RCC4-UNREALへ置換 |
| P2D-RCB4 | **停止／差分なし**。固定Rerun fragment IDのA1〜A6 matrix配置 | P2D-RC0, P2D-RCE0, GR-D4 | Opus command-oracle不備3回反復。修理済みrunnerのP2D-RCB5へ |
...
| P2D-RCD1A | public typed request／contribution API shape decision | P3 Observation decision、P2D-RCD1、P2D-RCT1、P2D-RCR1 | **WAIT**。P3前に`CompCamera`／private RCS1を公開形へしない |
| P2D-RCD1I | public typed seam実装 | P2D-RCD1A | **WAIT**。schema／package／First Vism分岐なし |
...
| P2D-RCF1I-BASE | F1／F5／F6 executable harness | P2D-RCF1、P2D-RCD1I | **WAIT**。private RCS1を昇格しない |
...
| P2D-RCR1 | scene-color／refraction入力契約 | P2D-RCD1、P2D-RCFP1S、M4-K0の既決RoD／RoI意味契約 | **DONE**。[input contract](../reviews/2026-07-29-m5-scene-color-input-contract-decision.md)でimmutable pre-requester snapshot、upstream order、K0 RoI、transparent-black domain外、whole-request failureを固定。K0 runtime spikeのmergeは本decisionの前提にしない |
| P2D-RCP1 | scene-color copy／subpass方式decision | P2D-RCR1、P2D-RCFP1F | **WAIT**。resource lifetime、同期、画面外sample、budget |
| P2D-RCBUD1 | cache key／resource budget統合 | P2D-RCD1、M4-K1、P2D-RCFP1F、P2D-RCP1 | **WAIT**。Host計上とcache完全性 |
...
並列レーン: M2-D1j/D1k/D3とM3-U1f/U2dで2D world/camera/Stageを先に成立させ、K0は透過Stageと独立して進める。P0IとP1 importも互いおよびK0から独立し、P1合流後にP2。P0I→P7a、P2+P7a+K1→P7b→P7c→P7U。P2Dの意味designは[締結地図](../reviews/2026-07-29-m5-render-contribution-design-closure-map.md)§2まで完了。RCD2IとRCFP1F evidenceは並列可、P3→RCD1A→RCD1I→RCF1I、RCFP1F→RCP1、M4-K1とRCFP1F／RCP1の合流後にRCBUD1とする。RCI §2.2のcamera／Observation非所有を保ち、P3の型は先取りしない。P2後はP2D/P2U/P3を依存に従って進め、P2U+P3後にP2R。P4はK0後、P6は独立、P5が合流点。world/cameraの成立をP1へ依存させない。
