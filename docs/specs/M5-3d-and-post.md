...
| P2D-RCA | **停止／再登録待ち**。旧Motolii authority/current-code-fact起点の一括契約draft | P2D-RC0 | Grok REJECT差分は不採用。共通基盤とCodex固定比較軸だけを読む単一比較grainとして、別ID／allowlistを登録するまで再発注しない |
| P2D-RCB | **停止／再登録待ち**。旧Rerun 6 asset一括監査／転移裁定 | P2D-RC0 | 主担当Codexが固定commitの証拠capsuleを一度だけ取得し、証拠後に転移分類を裁定する。裁定済み6ラベルを持つ別ID／allowlist登録前は発注しない |
| P2D-RCC | **停止／再登録待ち**。旧三engine一次資料取得／比較／fixture翻訳 | P2D-RC0 | 主担当Codexがprovider family別capsuleを固定し、network禁止の比較／fixture翻訳を別ID／allowlistで登録するまで再発注しない。三資料群要件はWave全体で満たす |
| P2D-RCI | 新しい証拠grainの統合とRender Contribution最終decision。必要ならcontract／private spike／schema／First Vism fixtureを再分割 | 再登録する後続grain | **本WaveではWAIT**。旧P2D-RCA/B/Cを完了依存にせず、Motolii authorityを正として統合し、公開境界または永続意味が未決なら実装解禁せずSTOP |
...
並列レーン: M2-D1j/D1k/D3とM3-U1f/U2dで2D world/camera/Stageを先に成立させ、K0は透過Stageと独立して進める。P0IとP1 importも互いおよびK0から独立し、P1合流後にP2。P0I→P7a、P2+P7a+K1→P7b→P7c→P7U。P2D-RC0の共通基盤とP2D-RCE0の固定証拠取得／Rerun裁定は成立したが、旧P2D-RCA/P2D-RCB/P2D-RCCは停止し、比較／転記／fixture翻訳を分離した別ID登録まで再発注しない。統合P2D-RCIも新しい依存登録までWAITとする。P2後はP2D/P2U/P3を依存に従って進め、P2U+P3後にP2R。P4はK0後、P6は独立、P5が合流点。world/cameraの成立をP1へ依存させない。
