...
- カメラはM5で初めて追加する3D機能ではない。[M2-D1j](M2-document-model.md)で全Compositionへ常在し、M3のOutput Frame操作から編集される。M5は同じ`CompCamera`のPerspective、Z/target/FOV等のAdvanced空間操作を既存Stageへ追加するだけで、レイヤー別camera、camera切替、グループ内camera、別3D viewportを作らない
...
| P1 | glTF読み込み(メッシュ・マテリアル最小限)+ OBJ→glTF変換パス。既存worldを所有せず、参加object/materialを供給する | 凍結ゲート | サンプルアセットの読み込みゴールデン。独自camera/worldを生成しない |
| P2 | **単一worldのM5拡張**: M2-D3で成立済みの2D=`z=0`+共有`CompCamera`へ、全objectのZ/world transformとP1 glTF参加を追加。`Layer Order`では投影後RGBAをauthoring order合成 | P1, M2-D1j, M2-D3 | 既存2D-only fixtureはM2出力とpixel同一。2D画像・動画・glTFが同じZ値/cameraで視差し、glTF importがworld/cameraの誕生条件でない。Zを跨いでもLayer Order遮蔽は従来C-4と同一 |
| P2D | 拡張可能な遮蔽ポリシー境界+組み込み`Layer Order / Group Depth / AE-style Bins`。通常UIは`Z Occlusion` OFF/ON、Advancedは全ポリシーと明示`Depth Participant`を表示 | P1, P2, M2-D1j, M3-U2c, M3-U4c, M4-K0 | (1)同じ座標の2平面が`Layer Order`ではレイヤー順、`Group Depth`ではZ交差で前後反転 (2)3D-2D-3D fixtureで明示参加フラグだけがbinを分断 (3)effect/mask/typeは無言でbinを変えない (4)切替前後で座標/見かけ投影/Document子順/Undo/選択不変 (5)group外はピクセル不変 (6)opaque/cutout/soft alphaの対応・拒否が明示 (7)未知/非対応ポリシーを無言fallbackしない (8)Advanced非表示でも適用ポリシーとbin境界を識別可能 (9)Unknown boundsをsort/cull根拠にせずFinalを切らない |
| P2U | Stage transform toolの`Scale / Depth Move`分離。Scale handleとZ rail/axisをM3-U2dのCamera/Object操作、既存toolbar/Inspector/timeline語彙へ統合 | P2, M2-D2, M3-U2d | (1)Scale dragはscaleだけ、Depth dragはposition.zだけをD2 command化 (2)各gestureがUndo 1回 (3)perspective/orthographic fixture (4)active toolを文字なし・grayscaleでも形/iconから識別 (5)DPI差で同じ正規化gestureが同じdomain値 (6)script不要で両channelへkeyframe可能 |
...
| P3 | **既存`CompCamera`のAdvanced空間操作**: Perspective切替、camera Z/target/FOV/roll、depth診断をM3 Stageへ追加。Document型や別cameraは増やさない | P2, M3-U2d, M3-U4c | M2-D1j camera animationのE2Eを2D+glTFで再利用。Output Frameの通常操作からAdvanced値を検査・編集でき、開閉でserialize不変。レイヤー別/別3D cameraを生成しない |
