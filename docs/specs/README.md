...
| [M4-cache-and-analysis.md](M4-cache-and-analysis.md) | ドラフト(K0 RoD/RoIは透過Stageと分離。旧K1をK1a〜K1dの台帳/並行store/階層退避/pressure制御へ分割。K2へShared Effect invalidationを追加。**着手前必読導線**からResourceLedger・surface import・portable実機gateを確認し、wgpu単独importやCPU downloadを完成扱いしない。凍結ゲートで確定) |
