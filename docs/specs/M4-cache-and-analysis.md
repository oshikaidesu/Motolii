...
`K1`は既存文書から参照されるキャッシュ基盤のumbrella IDで、実装PRの単位ではない。実装はK1a〜K1dへ分け、`M4-K1`という既存参照はこの4件のうち利用機能に必要な範囲を指す。
...
| K7 | グループ仮出力(ベイク): 選択グループの出力を時間範囲でキャッシュ(プリコンポ代替、コンセプト決定)。グループ内の編集で自動無効化。キャッシュ先はディスク階層を含む(全曲スクラブ前提、memory-model P4) | K1b, K1c, M2-D1 | ベイク後のスクラブがグループ再計算なしで行われ、グループ内編集で正しく無効化されるテスト |

並列レーン: K0はM2-D3後に独立実施できるが、M3-U1fの透過Stageをblockしない。K0→K1a→K1b→K1cと、K4は並行。K1c+K4→K1d。D3e+K1b→K2。K6は独立(UI接続はM3-U6と調整)。K7はK1b+K1c後。K3/K5は最終フェーズへ移動。
