...
- **キャッシュの前提(2026-07-10)**: 全レイヤー/ノードは最終的に2D合成パイプライン上の出力。`render(t)`が**前フレームの内部状態に依存しない**純関数であること([concept.md](../concept.md)の横断決定)。布・液体・パーティクル・2D剛体・ランタイム物理エンジン等の**逐次依存シミュレーションは設計上存在しない**(B-5と同型)。外部ベイク済みアニメは**入力アセット+パラメータ**としてキーに含め、再生は`t`の純関数として扱う
