...
| M5-C0 | PlanarとSpatialのObservation decision | Camera Provider決定を`REUSE`、`glam` private leaf | Host camera eval→typed Observation | provider 2種、3 OS golden、capability拒否、Planar pixel不変 | **意味決定DONE / schema・fixture・runtime WAIT**。[決定](reviews/2026-08-02-m5-c0-observation-contract-decision.md)。public API・M4 K1a前 |
