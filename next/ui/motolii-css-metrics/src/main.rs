...
//! 読んで呼び、JSON をファイルへ書くだけ。
//!
//! ```text
//! cargo run -p motolii-css-metrics -- inspector out/inspector.json
//! cargo run -p motolii-css-metrics -- browser   out/browser.json
//! cargo run -p motolii-css-metrics -- all       out/
//! cargo run -p motolii-css-metrics -- inspector out/i.json --viewport 520x900
//! ```
//!
//! `all` は既知の全 panel を既定 viewport で `<out>/<panel>.json` へ書く。
//! `--viewport WxH` は1枚指定のときだけ既定を上書きする(panel ごとに幅の
//! 要求が違うので `all` では個別の既定値をそのまま使う)。既知名以外を渡すと
//! html への直接パスとして扱う(`next/reference/mocks/` 以外の fixture を
//! 後で足すときのため)。

use std::path::{Path, PathBuf};

use motolii_css_metrics::{extract, PANELS};
...
fn main() {
    ...
    let (panel, out) = match positional.as_slice() {
        ...
        _ => {
            eprintln!(
                "usage: motolii-css-metrics <inspector|browser|all|<path.html>> <out.json|out/> [--viewport WxH]"
            );
            std::process::exit(2);
        }
    };
    ...
    if panel == "all" {
        ...
        for p in PANELS {
            ...
            report(
                p.name,
                run(&p.html_path(), viewport, &out_path),
                &mut failures,
            );
        }
    } else {
        ...
        match PANELS.iter().find(|p| p.name == panel) {
            Some(p) => {
                let viewport = viewport_override.unwrap_or(p.default_viewport);
                report(p.name, run(&p.html_path(), viewport, &out), &mut failures);
            }
