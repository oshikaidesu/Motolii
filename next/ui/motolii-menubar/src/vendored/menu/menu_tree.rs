...
// upstream path: src/widget/menu/menu_tree.rs
// License: MIT — Copyright (c) 2020 Kaiden42(全文は upstream LICENSE。MIT 条件により本ヘッダで著作権・許諾表示を保存する)
// motolii 変更点: fork API 追随1箇所 — `Shell::local` が `&mut Bus<A>` を取る新形へ(`temp_messages` を `Vec` から `iced_core::shell::Bus` へ)
        ...
        let mut run_op = |global_state: &mut GlobalState, tree: &mut Tree, op: &Op| {
            ...
            match op {
                ...
                Op::RedrawUpdate => {
                    ...
                    // `&mut Bus<A>` を取る(旧 upstream は `&mut Vec<_>`)— probe の E0308 その2。
                    let mut temp_messages = iced_core::shell::Bus::new();
