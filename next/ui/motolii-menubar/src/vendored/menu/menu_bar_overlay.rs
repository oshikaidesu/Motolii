// vendored: iced_aw menu widget — https://github.com/iced-rs/iced_aw
// rev 924be285b80339d969311c8dcc1ebff2e56e9dba(2026-07-31 "fix flake")
// upstream path: src/widget/menu/menu_bar_overlay.rs
// License: MIT — Copyright (c) 2020 Kaiden42(全文は upstream LICENSE。MIT 条件により本ヘッダで著作権・許諾表示を保存する)
// motolii 変更点: fork API 追随1箇所 — `Shell::local` が `&mut Bus<A>` を取る新形へ(`fake_messages` を `Vec` から `iced_core::shell::Bus` へ)
    ...
    fn update(
        ...
        match re {
            RecEvent::Event => {
                ...
                // motolii: fork(oshikaidesu/iced rev 73e686ee)の `Shell::local` は
                // `&mut Bus<A>` を取る(旧 upstream は `&mut Vec<_>`)— probe の E0308 その1。
                let mut fake_messages = iced_core::shell::Bus::new();
