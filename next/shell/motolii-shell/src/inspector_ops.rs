...
use motolii_store::{Intent, LayerAttrsPatch, LayerId, LayerTiming, Speed, TextStyleId};

use crate::inspector_pane::{FieldDraft, TransformField};
use crate::value_drag::ValueDragTarget;
use crate::{chrome, inspector_pane, timeline_pane, Message, Shell};
...
impl Shell {
    ...
    pub(crate) fn update_inspector(&mut self, message: inspector_pane::Message) -> Task<Message> {
        match message {
            ...
            inspector_pane::Message::ShapeFillInput(field, text) => {
                self.inspector_shape_fill_draft = Some(inspector_pane::ShapeFillDraft { field, text });
                Task::none()
            }
            inspector_pane::Message::ShapeFillSubmit(field) => {
                if self
                    .inspector_shape_fill_draft
                    .as_ref()
                    .is_some_and(|draft| draft.field == field)
                {
                    if let Err(error) = inspector_pane::commit_shape_fill(
                        &mut self.doc,
                        self.session.selection,
                        &mut self.inspector_shape_fill_draft,
                    ) {
                        self.status = Some(error);
                    }
                }
            ...
            inspector_pane::Message::ShapeFillGradient(index) => {
                if let Err(error) = inspector_pane::apply_shape_gradient(
                    &mut self.doc,
                    self.session.selection,
                    index,
                ) {
            ...
            // crate doc「S4」節参照)。
            inspector_pane::Message::Color(inspector_pane::color::Message::SwatchPressed(
                target,
    ...
    pub(crate) fn finish_text_style_field_drag(&mut self) {
        if let Err(error) = inspector_pane::finish_text_style_drag(
            &mut self.doc,
            &mut self.inspector_text_style_drag,
        ) {
            self.status = Some(error);
...
impl Shell {
    ...
    pub(crate) fn dispatch_inspector_ops(
        &mut self,
        message: Message,
        ...
        match message {
            Message::Inspector(msg) => {
                ...
                match &msg {
                    inspector_pane::Message::PointerMoved(point) => {
                        self.continue_value_drag(*point)
                    }
                ...
                match msg {
                    ...
                    inspector_pane::Message::Color(
                        inspector_pane::color::Message::ChannelDragPressed(target, channel),
                    ) => {
