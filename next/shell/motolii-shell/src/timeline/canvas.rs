...
pub(crate) fn draw(
    ...
    draw_ruler_ticks(pane, &mut frame, width, ruler_height);
    ...
    draw_time_bands(pane, &mut frame, width, rows_top, rows_bottom);
    ...
    for (index, row) in pane.rows.iter().enumerate() {
        ...
        frame.fill_text(canvas::Text {
            content: label,
            position: Point::new(start_x + pane.dims.spacing_s, row_top + row_height / 2.0),
            color: label_color,
...
fn draw_ruler_ticks(pane: &TimelinePane, frame: &mut canvas::Frame, width: f32, height: f32) {
...
/// (行方向ゼブラと同じ「交互」の言葉遣い)。
fn draw_time_bands(pane: &TimelinePane, frame: &mut canvas::Frame, width: f32, top: f32, bottom: f32) {
    ...
    while start_frame < pane.duration_frames {
        ...
        if segment_index % 2 == 1 {
            let x0 = frame_to_x(start_frame, width, pane.duration_frames);
            let x1 = frame_to_x(end_frame, width, pane.duration_frames).max(x0 + 1.0);
            frame.fill_rectangle(
                Point::new(x0, top),
