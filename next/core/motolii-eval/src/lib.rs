...
//!
//! **`ParamSource` / `DataTracks` / `DataTrack` は `next/` から未使用**(2026-08-20)。
//! 移植資産で試験付きなので消していないが、`ParamSource` は旧 workspace の
//! `ParamDriverPlugin`(4口の1つ)の形そのものである。**拡張口を設計する時に
//! これを既定の形として引き継がないこと** — 裁定13 は「2人目の利用者が現れるまで
//! 口を決めない」であり、口の形だけが先に入っているのは決めたことにならない。
//!
