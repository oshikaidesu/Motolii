...
script_mod! {
    ...
    // 字の大きさ — 役割は実質2段(本文/小)+値。Ableton と同じで、段が多いほど
    // 選び方が場当たりになる。Live 実測: 行の字のフルインク高 8pt ≈ フォント 11pt 相当
    // (「cap 8pt = フォント 8pt」と取り違えて3割小さくしたのが 2026-08-27 の誤り)。
    // 等幅(font_code)を許すのは**値**(数字・時刻・解像度)だけ。名前とラベルには使わない。
    // 薄インク(muted/faint)は本文サイズ以上でしか使わない — 小さい字を薄くすると読めない。
    let text = {
        xs: 8.5 * s
        sm: 10.0 * s
        base: 10.0 * s
