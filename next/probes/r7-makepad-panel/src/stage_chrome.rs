...
script_mod! {
    ...
    mod.widgets.StageChrome = set_type_default() do mod.widgets.StageChromeBase{
        ...
        stage_head := SolidView{width: Fill height: 26 flow: Right spacing: 6 align: Align{y: 0.5} padding: Inset{left: 8 right: 8} show_bg: true new_batch: true draw_bg.color: mod.tokens.face.panel
            ...
            camera := IconFlatButton{width: 28 height: 20 draw_bg.color: mod.tokens.face.well icon_walk: Walk{width: 13 height: 13} draw_icon +: {svg: crate_resource("self://resources/icons/camera.svg") color: mod.tokens.accent.on} on_click: || { ui.stage_mode.set_text("CAMERA") }}
        ...
        head_edge := SolidView{width: Fill height: 1 show_bg: true new_batch: true draw_bg.color: mod.tokens.face.down}
        stage_void := SolidView{width: Fill height: Fill flow: Down align: Align{x: 0.5 y: 0.5} padding: Inset{left: 8 right: 8 top: 8 bottom: 8} show_bg: true new_batch: true draw_bg.color: mod.tokens.face.panel
            comp_frame := SolidView{width: 722 height: 407 padding: 1 show_bg: true new_batch: true draw_bg.color: mod.tokens.face.down
                comp := SolidView{width: 720 height: 405 flow: Overlay show_bg: true new_batch: true draw_bg.color: #x000000
                    ...
                    stage_error := Label{width: Fill height: Fill align: Align{x: 0.5 y: 0.5} text: "" draw_text.color: mod.tokens.accent.on draw_text.text_style: theme.font_code{font_size: 10}}
                }
            }
        }
        band_edge := SolidView{width: Fill height: 1 show_bg: true new_batch: true draw_bg.color: mod.tokens.face.down}
        stage_band := SolidView{width: Fill height: 24 flow: Right spacing: 8 align: Align{y: 0.5} padding: Inset{left: 8 right: 8} show_bg: true new_batch: true draw_bg.color: mod.tokens.face.panel
            mode_well := ValueWell{
            ...
            off_frame_dot := SolidView{width: 4 height: 4 draw_bg.color: mod.tokens.accent.on}
