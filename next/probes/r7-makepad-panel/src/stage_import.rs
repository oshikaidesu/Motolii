...
unsafe fn import_iosurface_hal(
    ...
    // IOSurface-backed textures must not be Private: Private detaches the
    // texture from the surface backing and each device sees its own memory
    // (black stage). MTLStorageModeShared = 0 keeps the IOSurface as backing.
    let _: () = objc2::msg_send![desc_obj, setStorageMode: 0u64];
