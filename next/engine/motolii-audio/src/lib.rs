...
//! 移植)。旧 `crates/motolii-audio`(4,286行)+ 旧 `crates/motolii-transport`
//! (音声クロック部分のみ)からの移植であって再実装ではない。KNOWN.md
...
//!
//! **持ってこなかったもの(実デバイスを開く経路・shell結線は第2切片 A2)**:
//! - `device.rs`/`producer.rs` の cpal `Device`/`Stream` を実際に開く経路・
//!   バックグラウンドプロデューサスレッド — 音を実際に出すには要るが、
//!   「クロックが進む・止まる・seekする」の確認には要らない
//! - 旧 `motolii-transport` の `Transport`/`DrsController`/`PreviewSimulator` —
//!   映像フレームドロップ・適応解像度・`FramePlan`は音声クロックと別の関心事で、
//!   この束の対象外(`PlaybackClock`はTransportの音声クロック部分だけを継承する)
//!
