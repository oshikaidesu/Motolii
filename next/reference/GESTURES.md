...
> **注記(裁定217・2026-08-23)**: 裁定217 で判定基準が変わった。マウス完遂の正本は
> `axis/A02-mouse.tsv`。本表の「マウスのみ」は**穴を意味しない**(キーボード到達性の
> 欠如は穴として数えない — 主眼はマウス完遂そのものへ移った)。以下の表・判定は
> 裁定216 時点の一次資料として原文のまま残す。
