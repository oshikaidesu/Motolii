...
| 92 | 100行分のリネームを1回の操作でまとめてやろうとする(連番自動採番) | Layer menu/Timeline rail | `menu.rs:177-181` の `Auto-number Selected` → `selection.rs:407-408,444-464` → `batch_rename.rs:13-65`。選択中の生きたレイヤーをTimeline行順に並べ、ロック検査後に `apply_all` 1回で `Layer 001` 形式へ書く。`batch_rename` の2単体テストと `menu_drive` で検査 | 書ける |
...
6. #124 「上書き保存」という動詞(Save As と Save の区別)
7. #130 OSクローズボタン経路のdirty確認
8. #141 Export中のCancel到達性
9. #143 Export音声muxの結線状態
...
- **「複数選んで一度に直す」**(#87, #88): 100行という規模を要求されて初めて、
  ...
  共通 `apply_all` まで閉じ、#92 は Layer メニューから同じ選択集合を専用コンポーネントへ渡し、
  行順の自動採番と1回の `apply_all` まで閉じた。
