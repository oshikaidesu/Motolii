# 過去に測った事実(**現在の保証ではない。疑ってよい**)

## 読み方(2026-08-23 全面改訂)

**この見出しは以前「レーンは再検証しない」で、本文は「再検証にツール呼び出しを使うな」と
命じていた。撤回する。** 2026-08-23 の1日だけで、このファイルの記述は **25件が実装より古かった**
(`bm`/`matte` は既に消費済み・`parent` は前日に実装済み・行番号は分割で全部失効、等)。
**「疑うな」と書かれたファイルが、いちばん疑うべきファイルだった。**

**まず現在地を機械に聞くこと。** ここに書いてある事から始めない:

```bash
python3 scripts/plan_steps.py   "$(git rev-parse --show-toplevel)"   # 次にやる段階(唯一の進捗)
...
**このファイルと機械の出力が食い違ったら、機械が正しい。** 文書を直して進むこと
(止まらない・supervisor の裁定を待たない)。

**疑ってよい/疑うべき条件**:
- 行に日付が無い、または**2週間以上前**の日付
- `file:line` を含むのに `check_evidence.py` が緑にならない
- 「未実装」「無い」「ゼロ件」と書いてある — **この型が今日25件中の大半だった**

**確かめるのは安い。** `grep` 1回で済むことに、この文書の権威を使わない。

1行1事実、日付と出典を必ず添える。古くなった行は消さず「(失効 YYYY-MM-DD: 理由)」を付ける。

**2026-08-23 追記(KF レーン)**: 手書きの事実台帳は「監督が更新を忘れる」で
...
- 個別の穴・上流事実を1行ずつ検査化する当初案(`known_holes.rs`/
  `upstream_facts.rs`)は、機械抽出可能な範囲を広く取れる在庫表アプローチに
  総監督裁定で置き換えられた(2026-08-23、この節)。以下の「既知の穴」/
  「上流・依存」節は**索引未整理のまま残っている**(次のレーンへの宿題)

## 発注書に必ず入れる(2026-08-23 の教訓。飛ばすと再発する)

- **cargo は前景・`timeout 600000`。背景実行(`&`/`run_in_background`/`Monitor`)禁止** — 1日15件の自己停止が全部これ
- **OUTCOME は状態の変化で書く。「確認する」を成果物にしない**(裁定221)
- **既定の関門は `cargo check --tests`。`cargo test` は検収条件1点だけ**(裁定220)
- **テストは根拠を示してから。書いてよいのは検収条件・回帰の柵・後方互換の3型**(裁定218)
- **壁に当たっても上へ投げない。外部資料から決めて出典を書く**(裁定222)
- **自前で書く前に外を探す**(裁定215)。探索範囲を必ず併記する
- **退避の禁止 / 全件ステージの禁止(パス直指定)/ 緑になったら必ず記録する**
- **残った赤は無関係な物も RETURN に書く**(裁定208)

経緯: [教訓 2026-08-23](../../docs/reviews/2026-08-23-lessons.md)
...
- **stash 禁止**(worktree 間で `refs/stash` が共有される・裁定200)。
  **ベースラインと比べたい時は `git show HEAD:<path>` を使う** — 2026-08-23 に SP-3 レーンが
  比較目的で stash/pop を使った(被害は無かったが、同時に4本が未コミットで走っていた)/ Edit 直後の stale fingerprint は touch / CARGO_TARGET_DIR 共有禁止(後勝ち事故の実測あり)
