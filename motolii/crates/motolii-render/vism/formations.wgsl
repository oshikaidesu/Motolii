/*{
  ...
  "INPUTS": [
    { "NAME": "hold", "LABEL": "Hold", "TYPE": "float", "DEFAULT": 3.0, "MIN": 0.1, "MAX": 120.0 },
    { "NAME": "travel", "LABEL": "Travel", "TYPE": "float", "DEFAULT": 1.2, "MIN": 0.05, "MAX": 60.0 },
    { "NAME": "cell", "LABEL": "Cell", "TYPE": "float", "DEFAULT": 84.0, "MIN": 1.0, "MAX": 10000.0 },
    { "NAME": "radius", "LABEL": "Radius", "TYPE": "float", "DEFAULT": 420.0, "MIN": 1.0, "MAX": 10000.0 },
    { "NAME": "stagger", "LABEL": "Stagger", "TYPE": "float", "DEFAULT": 0.004, "MIN": 0.0, "MAX": 1.0 }
  ]
}*/
...
fn shape(i: u32, k: u32, n: u32, p: BlockParams) -> vec2f {
    switch i % 4u {
        case 0u: { return table(k, n, p.cell); }
        case 1u: { return circle(k, n, p.radius); }
        case 2u: { return helix(k, n, p.radius, p.cell); }
        default: { return grid(k, n, p.cell); }
    }
}

fn block(k: u32, p: BlockParams) -> Offset {
    let n = max(host.members, 1u);
    let period = p.hold + p.travel;
    // 番号で少し遅らせる(from center ではなく順): 群れが波のように移る。
    let t = max(host.time - f32(k) * p.stagger, 0.0);
    let i = u32(floor(t / period));
    let u = clamp((t - f32(i) * period - p.hold) / p.travel, 0.0, 1.0);
    let e = u * u * (3.0 - 2.0 * u);
    let here = shape(i, k, n, p);
    let next = shape(i + 1u, k, n, p);
    // 移る間は少し小さく薄く(手前を通る印象)。
