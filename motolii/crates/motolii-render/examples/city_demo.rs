...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (name, effects, params) in cases {
        ...
        doc.apply(Intent::SetComposition(Composition { width: w, height: h, fps: Fps::try_new(24, 1).unwrap(), duration_frames: 240, background: [0.0, 0.0, 0.0, 1.0], look: Default::default() }))?;
        let layer = LayerId(1);
        doc.apply_all([
