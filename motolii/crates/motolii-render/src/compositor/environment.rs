...
/// GPU に上げた環境(空)。放射輝度は背景用、照度は網の拡散照明用(上流 `Environment` の契約)。
...
/// 畳み込みの入力を先にここまで縮める。総当たりなので 64×32 × 32×16 = 100 万回で済ませる。
const CONVOLVE_INPUT_SIZE: (usize, usize) = (64, 32);
/// 鏡面の各段の寸法。段数は上流の `SPECULAR_LEVELS`。
const SPECULAR_SIZE: (usize, usize) = (128, 64);
/// 背景に敷く放射輝度の上限幅。8K の HDRI を Rgba16Float で持つと 256MB 食うので、ここで留める。
const MAX_RADIANCE_WIDTH: usize = 2048;
...
impl Compositor {
    ...
    pub(crate) fn upload_environment(
        ...
        let specular = re_renderer::environment::prefilter_specular(
            &small,
            CONVOLVE_INPUT_SIZE.0,
            CONVOLVE_INPUT_SIZE.1,
            SPECULAR_SIZE.0,
        ...
        let specular = self.upload_rgb_f16(
            &format!("{label} specular"),
            &specular,
            SPECULAR_SIZE.0 as u32,
            (SPECULAR_SIZE.1 * re_renderer::environment::SPECULAR_LEVELS) as u32,
        )?;
        ...
        let radiance = self.upload_rgb_f16(&format!("{label} radiance"), &radiance_rgb, radiance_w, radiance_h)?;
