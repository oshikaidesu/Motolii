// What a Block's motion entry means (the view's motion resource, written by the block programs):
// kind 0 a thing, 1 a connector between two things, 2 a rope.

// The view's motion (re_renderer's `motion_at` / `motion_len`) holds four vec4 per object —
...
fn motion_entry(slot: f32) -> u32 {
    ...
    if n == 0u || n * MOTION_STRIDE > motion_len() {
        return 0u;
    }
    return n;
}
...
fn motion_offset(slot: f32, world_position: vec3f) -> vec3f {
    ...
    let move_turn = motion_at(base);
    let centre_scale = motion_at(base + 1u);
    let axis_kind = motion_at(base + 2u);
    ...
    if axis_kind.w == 1.0 {
        ...
        return mix(move_turn.xyz, motion_at(base + 3u).xyz, t);
    }
    ...
    if axis_kind.w == 2.0 && (n + 1u) * MOTION_STRIDE <= motion_len() {
        let ab = axis_kind.xyz;
        let a = centre_scale.xyz;
        let b = a + ab;
        let t = clamp(dot(world_position - a, ab) / max(dot(ab, ab), 1e-6), 0.0, 1.0);
        let drawn = cubic(a, motion_at(base + 4u).xyz, motion_at(base + 5u).xyz, b, t);
        let live = cubic(a + move_turn.xyz, motion_at(base + 6u).xyz, motion_at(base + 7u).xyz, b + motion_at(base + 3u).xyz, t);
        return live - drawn;
    }
...
fn motion_tint(slot: f32) -> vec4f {
    let n = motion_entry(slot);
    if n == 0u || motion_at((n - 1u) * MOTION_STRIDE + 2u).w != 0.0 {
        return vec4f(1.0);
    }
    return motion_at((n - 1u) * MOTION_STRIDE + 3u);
}
