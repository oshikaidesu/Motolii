...
pub fn hook_stub(body: &str, stage_params: &str, names: &[String]) -> String {
    let mut stub = String::from(
        "struct FieldIn { frame_position: vec3f, normal: vec3f, params: array<vec4f, 3>, };\n\
         ...
         struct SurfaceIn { albedo: vec3f, normal: vec3f, view_dir: vec3f, world_position: vec3f, params: array<vec4f, 3>, };\n\
         fn shade_surface(albedo: vec3f, normal: vec3f, view_dir: vec3f, knobs: vec4f) -> vec3f { return albedo * knobs.x; }\n\
