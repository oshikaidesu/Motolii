...
pub fn settle(view: &StoreView<'_>, out: &mut Vec<ResolvedLayer>, t: RationalTime) -> Result<(), StoreError> {
    ...
    hand_out_stencils(view, out)?;
    snap_to_grids(view, out, t)?;
    snap_to_found_grids(view, out, t)?;
    apply_fields(view, out, t)?;
...
pub fn apply_fields(view: &StoreView<'_>, out: &mut [ResolvedLayer], t: RationalTime) -> Result<(), StoreError> {
    ...
    for i in 0..out.len() {
        ...
        let field = match view.value_at(layer.id, &PropertyId::new(FIELD)?, t)? {
        ...
        if !field_boxes.contains_key(&field) {
            ...
            let b = match found {
                Some(transform) => screen_box_of(view, field, transform, t)?,
                None => None,
            };
            field_boxes.insert(field, b);
        }
        let Some((f_lo, f_hi)) = field_boxes[&field] else { continue };
        let Some((lo, hi)) = screen_box_of(view, layer.id, layer.placement.transform, t)? else { continue };
        let centre = (lo + hi) * 0.5;
        // 箱までの距離(中にいれば 0)。
        let outside = (f_lo - centre).max(centre - f_hi).max(glam::Vec2::ZERO);
        let falloff = view.number(layer.id, FIELD_FALLOFF, 200.0, t)?.max(0.0) as f32;
        ...
        let scale = 1.0 + (view.number(layer.id, FIELD_SCALE, 1.0, t)? as f32 - 1.0) * strength;
        let opacity = 1.0 + (view.number(layer.id, FIELD_OPACITY, 1.0, t)? as f32 - 1.0) * strength;
        let away = centre - (f_lo + f_hi) * 0.5;
        let push = view.number(layer.id, FIELD_PUSH, 0.0, t)? as f32 * strength;
...
pub fn snap_to_grids(view: &StoreView<'_>, out: &mut [ResolvedLayer], t: RationalTime) -> Result<(), StoreError> {
    ...
    for layer in out.iter_mut() {
        ...
        let strength = view.number(layer.id, SNAP_TO_GRID, 0.0, t)?.clamp(0.0, 1.0) as f32;
        if strength <= 0.0 {
            continue;
        }
        let Some(parent) = view.attrs(layer.id)?.unwrap_or_default().parent else { continue };
        ...
        let size = hi - lo;
        let target_size = if view.choice(layer.id, SNAP_SIZE, t)? == 1 {
...
pub fn hand_out_stencils(view: &StoreView<'_>, out: &mut [ResolvedLayer]) -> Result<(), StoreError> {
    ...
    for layer in out.iter().filter(|l| l.blend_mode.is_stencil() && !l.ghost && l.copy == 0) {
        ...
        stencils.push((layer.id, meta.order, layer.clip_to_below, mode, view.attrs(layer.id)?.unwrap_or_default().parent));
    }
    ...
    for &(stencil, order, clipped, mode, parent) in &stencils {
        ...
        let base = if clipped { view.clipping_base(stencil)? } else { None };
        for i in 0..out.len() {
            let layer = &out[i];
            ...
            let mut guard = 0;
            while let Some(up) = view.attrs(*chain.last().unwrap_or(&layer.id))?.unwrap_or_default().parent {
                chain.push(up);
                guard += 1;
                if guard > 64 { break; }
            }
            let inside = if clipped {
                ...
                match found {
                    Some(top) => top != stencil && view.meta(top)?.is_some_and(|m| m.order < order),
                    None => false,
                }
            };
            if inside {
...
pub fn put_on_planes(view: &StoreView<'_>, out: &mut [ResolvedLayer], t: RationalTime) -> Result<(), StoreError> {
    ...
    for layer in out.iter() {
        ...
        while seen.insert(here) {
            ...
            if view.layout_display(here, t).unwrap_or(0) != 0 {
                plane = world.map(|w| centre(here, w));
            }
            if !flat_here {
                break;
            }
            match view.attrs(here)?.unwrap_or_default().parent {
...
pub fn put_backgrounds_behind(view: &StoreView<'_>, out: &mut [ResolvedLayer], t: RationalTime) -> Result<(), StoreError> {
    ...
    for layer in out.iter() {
        ...
        let mut next = view.attrs(layer.id)?.unwrap_or_default().parent;
    ...
    for layer in out.iter_mut() {
        if layer.source == crate::doc::store::LayerSource::Group {
            if let Some(order) = deepest.get(&layer.id).filter(|_| view.layout_display(layer.id, t).unwrap_or(0) != 0) {
