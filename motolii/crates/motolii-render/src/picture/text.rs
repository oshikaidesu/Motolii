...
pub fn glyph_offsets(view: &StoreView<'_>, layer: LayerId, t: RationalTime) -> Result<Option<std::sync::Arc<Vec<[f32; 2]>>>, StoreError> {
...
pub fn text_units(view: &StoreView<'_>, layer: LayerId, t: RationalTime) -> Result<Vec<[f32; 4]>, StoreError> {
...
pub fn flow_around(view: &StoreView<'_>, text: LayerId, t: RationalTime) -> Result<Option<std::sync::Arc<Vec<crate::picture::text_frame::Obstacle>>>, StoreError> {
    ...
    for layer in view.layers() {
        ...
        if to_text.is_none() {
            if crate::picture::resolve::text::resolved_text_document(view, text, t)?.and_then(|d| d.wrap_size).is_none() {
                return Ok(None);
            }
            to_text = Some(crate::picture::boxes::world_2d(view, text, t)?.inverse());
        }
        let to_text = to_text.unwrap_or(glam::Affine2::IDENTITY);
        let margin = view.number(layer, SHAPE_MARGIN, 0.0, t)?.max(0.0) as f32;
        let mut samples = crate::picture::motion_time::transition_samples(view, layer, t)?;
        if samples.is_empty() {
            samples.push((t, 1.0));
        }
...
pub fn measure_text(view: &StoreView<'_>, measure: Measure, width: Option<f32>, height: Option<f32>, t: RationalTime) -> [f32; 2] {
