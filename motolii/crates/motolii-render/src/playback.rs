...
use crate::doc::store::{Fps, RationalTime, StoreView};
...
pub struct CompositionTimebase {
    fps: Fps,
    duration_frames: i64,
}

impl CompositionTimebase {
    pub fn from_view(view: &StoreView<'_>) -> Option<Self> {
    ...
    pub fn frame_at(&self, sec: f64) -> i64 {
    ...
    pub fn time_at(&self, sec: f64) -> RationalTime {
...
impl PlaybackHealth {
    pub fn visible_label(&self) -> Option<&'static str> {
        match self {
...
pub type Clock = PlaybackController;

impl PlaybackController {
    pub fn from_view(view: &StoreView<'_>, fallback_duration_sec: f64) -> Self {
        let timebase = CompositionTimebase::from_view(view)
            .unwrap_or_else(|| CompositionTimebase::ordinary_default(fallback_duration_sec));
        let mut cache = AudioProgramCache::default();
        let (program, health) = match AudioProgram::from_view(view, &mut cache) {
    ...
    pub fn poll_audio(&self, view: &StoreView<'_>) {
    ...
    pub fn sync_view(&self, view: &StoreView<'_>) {
        ...
        let built = AudioProgram::from_view(view, &mut state.cache);
    ...
    fn audio_fingerprint(view: &StoreView<'_>) -> String {
        ...
        for layer in view.layers() {
            ...
            let Some(meta) = view.meta(layer).ok().flatten() else {
                continue;
            };
            let LayerSource::File { path, .. } = &meta.source else {
                continue;
            };
            ...
            let _ = write!(
                out,
                "l{}:{:?}:{:?}:{};",
                layer.0, meta.source, meta.timing, attrs.hidden as u8
            );
            for name in [
                property::LEVEL,
                property::PAN,
                ...
                if let Ok(property) = PropertyId::new(name) {
                    let _ = write!(
                        ...
                        "a{}:{:?};",
                        name,
                        view.property_source(layer, &property)
                    );
                }
            }
        }
    ...
    pub fn waveform_tracks(&self) -> Vec<WaveformTrack> {
        self.state
            .lock()
            .unwrap()
            .program
    ...
    pub fn composition_label(&self) -> String {
...
mod tests {
    ...
    fn composition_timebase_uses_document_fps_and_duration() {
        ...
        let timebase = CompositionTimebase::from_view(&doc.view()).unwrap();
    ...
    fn visual_fallback_keeps_transport_seekable_without_audio_or_device() {
        let timebase = CompositionTimebase::from_view(&document_at(30, 1, 300).view()).unwrap();
    ...
    fn document_sync_replaces_the_timebase_without_replacing_the_controller() {
        let clock = PlaybackController::from_view(&document_at(30, 1, 1_800).view(), 60.0);
        clock.seek(50.0);

        clock.sync_view(&document_at(24, 1, 240).view());
    ...
    fn frame_navigation_uses_the_composition_rate() {
        let clock = PlaybackController::from_view(&document_at(24, 1, 240).view(), 10.0);
        clock.seek_frame(37);
