...
fn eval_path_to_vector_path(path: &EvalPath) -> crate::doc::vector::Path {
    let vertices = path
        .vertices
        .iter()
        .map(|v| crate::doc::vector::Vertex {
            point: crate::doc::vector::Point {
                x: v.point[0],
                y: v.point[1],
            },
            in_tangent: crate::doc::vector::Point {
                x: v.in_tangent[0],
                y: v.in_tangent[1],
            },
            out_tangent: crate::doc::vector::Point {
                x: v.out_tangent[0],
                y: v.out_tangent[1],
            },
        })
        .collect();
    vec![crate::doc::vector::Contour {
        vertices,
        closed: path.closed,
    }]
}
