...
use super::{
    ...
    GpuResourceKey, GpuResourceVersion,
};

/// Concrete cross-contribution execution is separated from planning. The
/// executor receives only resolved logical resource keys and immutable payload.
...
pub(crate) enum EngineGpuOperation {
    /// Producer already materialized its backend payload while its concrete
    /// backend is still being moved behind the executor.
    Resident,
    Clip {
        source: GpuResourceKey,
        base: GpuResourceKey,
...
where
    X: EngineCrossExecutor<Error = EngineGpuBackendError>,
{
    ...
    fn execute_pass(
        ...
        match self.operations.get(key).copied() {
            ...
            Some(EngineGpuOperation::Clip { source, base, output, version }) => {
                self.cross.execute_clip(source, base, output, version)
            }
            Some(EngineGpuOperation::Matte { target, source, output, version, mode }) => {
                self.cross.execute_matte(target, source, output, version, mode)
            }
...
/// Resource-keyed adapter around the retained low-level Engine primitives.
/// Relationship discovery is complete before this point.
pub(crate) struct EngineCrossContext<'a> {
...
impl EngineCrossExecutor for EngineCrossContext<'_> {
    type Error = EngineGpuBackendError;
    ...
    fn execute_clip(
        ...
        let (_, source_value) = self
            .resources
            .get_any(source)
            .ok_or(EngineGpuBackendError::MissingCrossResource(source))?;
        let (_, base_value) = self
            .resources
            .get_any(base)
            .ok_or(EngineGpuBackendError::MissingCrossResource(base))?;
        let source_layer = source_value.layer.clone();
        let base_layer = base_value.layer.clone();
        let result = self
            .engine
            .clip_onto_base(base_layer, &source_layer.layer, &source_layer.passes)
            .map_err(|_| EngineGpuBackendError::CrossPrimitive)?
            .ok_or(EngineGpuBackendError::MissingCrossResource(output))?;
        self.resources.install(
            output,
            version,
            self.generation,
            super::ResidentCompositeLayer { layer: result },
        );
        Ok(())
    }
    ...
    fn execute_matte(
        ...
        let (_, target_value) = self
            .resources
            .get_any(target)
            .ok_or(EngineGpuBackendError::MissingCrossResource(target))?;
        let (_, source_value) = self
            .resources
            .get_any(source)
            .ok_or(EngineGpuBackendError::MissingCrossResource(source))?;
        ...
        let layer = self
            .engine
            .apply_matte(self.comp, self.camera, &target_layer, &source_layer, mode)
            .map_err(|_| EngineGpuBackendError::CrossPrimitive)?;
        let result = crate::render::compositor::LayerWithPasses {
            layer,
            passes: Vec::new(),
            padding: 0,
