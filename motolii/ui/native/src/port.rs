...
impl EditorRuntime{
    ...
    pub(crate) fn request(&mut self,j:J)->Result<(),String>{
        ...
        if !matches!(op,"previewProperties"|"previewColor"|"previewBlend"|"previewX"|"commitPreview"|"commitX"|"previewTimings"|"previewSequence"|"stageGesture"){self.cancel_preview();}
        match op{
            ...
            "animate"=>{self.animate=j["enabled"].as_bool().ok_or("Missing enabled")?;}
            ...
            "enableEffect"=>{let id=layer(&j)?;let effect=EffectId(integer(&j,"id")?as u32);let on=j["enabled"].as_bool().ok_or("Expected bool")?;if !self.doc.view().effects(id).map_err(e)?.iter().any(|x|x.id==effect){return Err("Effect missing".into())}let at=self.time()?;let edits=self.doc.place_checked(id,&PropertyId::effect_enabled(effect),Value::Bool(on),at,false).map_err(e)?;self.apply(edits)?;}
