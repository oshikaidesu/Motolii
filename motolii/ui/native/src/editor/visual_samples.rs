...
pub(crate) fn reply(doc:&Document, time:RationalTime, j:&J) -> Result<J,String> {
    ...
    let raster = match j["kind"].as_str() {
        ...
        Some("gradient")=>{
            ...
            let gradient=Gradient{ units: Default::default(), stop_ids: Vec::new(), next_stop_id: 0,kind,start:Point{x:-cos*extent,y:-sin*extent},end:Point{x:cos*extent,y:sin*extent},stops,blend};
