...
pub(crate) fn edits(doc:&Document, layer:LayerId, time:RationalTime, j:&J) -> Result<Vec<Intent>,String> {
    ...
    let resolved=motolii_render::picture::resolve::text::resolved_text_document(&view, layer,time).map_err(|e|e.to_string())?;
...
mod tests {
    ...
    fn kana_mark_is_one_selection_and_empty_text_can_choose_size() {
        ...
        assert_eq!(motolii_render::picture::resolve::text::resolved_text_document(&doc.view(), layer,time).unwrap().unwrap().styles[0].size,55.0);
    }
}
