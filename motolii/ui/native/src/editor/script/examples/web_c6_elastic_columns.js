// Codrops #6 — ElasticGridScroll (js/demo1/index.js): the grid's columns follow the scroll late, the farther from the centre the later.
...
for (let i = 0; i < COLS; i++) {
  const cells = [];
  for (let r = 0; r < ROWS; r++) cells.push(rectangle({ name: `Pic ${i}.${r}` }).fill(C[(i * 3 + r * 5) % C.length]));
