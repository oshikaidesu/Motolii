...
for (let i = 0; i < COLS * ROWS; i++) {
  const cell = rectangle({ name: `Pic ${i}` }).fill(C[(i * 5) % C.length]);
...
cells.forEach((c) => c.set("Horizontal Sizing", "Fill").set("Vertical Sizing", "Fill").set("Transition Duration", 0.7).set("Transition Easing", "Ease In Out"));
