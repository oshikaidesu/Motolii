// Codrops #5 化合 — the one element is a video clip and the frames it visits are photos (the Web moves a still between empty steps).
...
const frames = steps.map((s, i) => media(MAT + `photo${i + 1}.png`, { name: `Step ${i + 1}` }).set("Opacity", 0.35));
const one = media(MAT + "clip1.mp4", { name: "One" });
