...
lines.forEach((line, row) => {
  ...
  rowBox.key("Tilt X", at, -70, IO).key("Tilt X", at + SPEED, 0).key("Position Z", at, 400, IO).key("Position Z", at + SPEED, 0);
  for (const layer of [rowBox, ...words]) layer.projection("3D");
});
