...
pub(crate) struct CompositionTimebase {
    fps: Fps,
    duration_frames: i64,
}
...
impl CompositionTimebase {
    ...
    pub(crate) fn frame_at(&self, sec: f64) -> i64 {
    ...
    pub(crate) fn time_at(&self, sec: f64) -> RationalTime {
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub(crate) enum PlaybackHealth {
    AudioReady,
    VisualOnly(VisualFallback),
}

impl PlaybackHealth {
    pub(crate) fn visible_label(&self) -> Option<&'static str> {
        match self {
...
pub(crate) type Clock = PlaybackController;

impl PlaybackController {
    pub(crate) fn from_document(doc: &Document, fallback_duration_sec: f64) -> Self {
    ...
    pub(crate) fn health(&self) -> PlaybackHealth {
...
/// Timelineへ渡すimmutableなdata port。生成やcache規則はaudio側から出さない。
    pub(crate) fn waveform_tracks(&self) -> Vec<WaveformTrack> {
        self.state
            .lock()
            .unwrap()
            .program
    ...
    pub(crate) fn seek_frame(&self, frame: i64) {
        let seconds = self.state.lock().unwrap().timebase.seconds_at_frame(frame);
        self.seek(seconds);
    }

    pub(crate) fn current_time(&self) -> RationalTime {
        let now = self.now_sec();
        self.state.lock().unwrap().timebase.time_at(now)
    }

    pub(crate) fn frame_duration_sec(&self) -> f64 {
        self.state.lock().unwrap().timebase.frame_duration_sec()
    }

    pub(crate) fn format_timecode(&self) -> String {
        let now = self.now_sec();
    ...
    pub(crate) fn composition_label(&self) -> String {
