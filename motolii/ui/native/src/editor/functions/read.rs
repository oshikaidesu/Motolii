...
pub(crate) fn inspector_data_from_doc(view: &StoreView, layer: LayerId, t: RationalTime, catalog: &[EffectDescriptor]) -> InspectorData {
    ...
    let effects: Vec<EffectBlock> = attached_effects
        ...
        .map(|instance| {
            let id = instance.id;
            if let Some(kind) = crate::render::extensions::placement::kind(&instance.plugin_id) {
                ...
                let rows = kind.params.iter().filter(|p| p.shown(mode)).filter_map(|param| {
                    ...
                    let value = get(param.name).or_else(|| crate::render::extensions::placement::default_for(kind, param.name, mode)).unwrap_or_else(|| param.default_value());
                ...
                if view.meta(layer).ok().flatten().is_some_and(|m| m.source == crate::doc::store::LayerSource::Group) {
                    ...
                    for (_, child, name) in kids {
                        let Ok(prop) = PropertyId::effect_param(id, &format!("{}{}", crate::render::extensions::placement::SHARE_PREFIX, child.0)) else { continue };
                        let v = match view.value_at(layer, &prop, t).ok().flatten() { Some(Value::F64(v)) => v, _ => crate::render::extensions::placement::SHARE_DEFAULT };
                        let keyed = view.track(layer, &prop).ok().flatten().is_some();
