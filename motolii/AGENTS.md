...
作ってよいのは**編集の意味**と**エフェクトのデータ**だけ。描画は rerun、UI は Flutter の部品。**新規コードと検証の前に、同じ意味・機構の参考文献・既存実装・外部の定規を必ず探す**(現行repo・既決 → 依存／上流の取説・`reference/`・ソース・oracle → 公式規格・一次資料・製品先例)。**依存・fork元に同じ機構があるなら、そのソースを先に読む。**無ければ検索範囲を残し、自作testは借りた定規の写像だけにする。
