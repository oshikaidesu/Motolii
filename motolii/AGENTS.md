...
作ってよいのは**編集の意味**と**エフェクトのデータ**だけ。描画は rerun、UI は Flutter の部品。**順序は owner → 既決 → 上流の先例 → 型境界 → compile → 安い構造test → 実窓 → 最後に性能**。新規コードと検証の前に、同じ意味・機構の参考文献・既存実装・外部の定規を必ず探す(現行repo・既決 → 依存／上流の取説・`reference/`・ソース・oracle → 公式規格・一次資料・製品先例)。rerun/Flutter等の上流と同じ機構なら先にそのソースを読み、Motolii独自の抽象を足さず最も強い既存境界へ載せる。無ければ検索範囲を残し、自作testは借りた定規の写像だけにする。
...
- **手段は、明示された目的と確認済みの意向で測る**。指示された手段より明確に良い道が見えるなら、黙って従わず、実行前に案・根拠・代償を短く出す。ただし「相談」を許可取りに使わない — 判断材料を持っているのはこちら、決めるのはあなた。止まるのは、創作上の意味を新しく決める時・既決同士が衝突する時・上流にも先例がなく新architectureを発明する時・実窓の正否を人が裁定する時だけ。そこでは未決を埋めず、必要な裁定を残して独立な仕事へ進む。目的・範囲・重要な制約を変えるなら確認、依頼の中の軽微で戻せる改善は止めずに進める
- **長い作業を黙って進めない**。着手前に「何を・どの順で・どこまで」を 1〜2 行、区切りごとに結果を 1 行。build・計測・多 file の編集など数分かかる物は始めと終わりを必ず出す。調査器・profiler・test基盤の改善だけで10分進展が無ければ中断し、製品コードの問いへ戻る。testは原因探索の迷路にせず、コード/上流から確定した不変条件を固定する鍵にする。詰まったら詰まったと言う
- 編集状態は Document が持つ。書き込みは Intent 経由のみ。1つ直せば同族全部が直るcomponent/dataにする。同じ意味を2箇所へ書いたら未完
- 責任は入口→意味→評価→結果→試験まで完結。症状へpatchする前にownerを特定し、同じ境界違反が2件以上なら個別修正を止めてrootでcluster化する。誤ったownerをRust型で表現できるならruntime guardを足すのでなく型境界を切り、compile errorを移行表にする。後の統合・結線が要るなら未完。**並列の lane が同じ file を触る時点で失敗**(責任が集まっている印)。worktree で逃がさず、先にその file を data／manifest の口にして、以後は行を足すだけで載る形にする
