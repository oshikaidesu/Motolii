//! Vism の多段(ISF `PASSES`)が**順番どおりに全部走っている**ことを見張る。
//!
//! glow は 明部抽出 → 横ぼかし → 縦ぼかし → 合成 の4段。段が1つでも抜けたり
//! 順番が入れ替わると「光が元の形の外へ滲む」が起きない。出力が変わったかどうか
//! (presentable.rs)ではなく、**滲みの向きと量**を刺す。
...
/// 形の外(右と下)の明るさの合計。滲みが無ければ 0。
/// 層は左上に置くので、外側があるのは右と下。
fn halo(px: &[u8]) -> u32 {
    let mut sum = 0u32;
    for d in 0..3u32 {
        for t in 0..BOX {
...
fn glow_spreads_light_outside_the_shape() {
    ...
    assert!(
        ...
        "glow を積んでも形の外へ光が滲んでいない — 4段のどれかが走っていない\
         (明部抽出だけ / ぼかしが空振り / 合成が元絵のまま)。\
         **同じパスの中で書き込み先を読める形で束ねると、そのパスが丸ごと無効になる**"
    );
}
