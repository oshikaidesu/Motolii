...
//!
//! OWNS-JUSTIFICATION(D): 立証不足 — peak抽出アルゴリズム自体を他crateと比較
//! した記録が無い。decode経路(`FrameReader`)を再利用していることは明記されて
//! いるが、「なぜこのbucketing演算自体を自前で持つべきか」の検討記録は無い
//! (裁定215 棚卸し 2026-08-23 #15)。**捏造しない正直な記録**: 探した結果は
//! 「候補無し」— `symphonia` はdecodeのみでpeak抽出はアプリ側の仕事、専用の
//! waveform crateはエコシステム上マイナー(同棚卸し §4)。動いている実装は
//! 教義のために壊さない・借り換えない。
//!
//! ## 何をするか
