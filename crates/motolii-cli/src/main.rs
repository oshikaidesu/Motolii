use motolii_cli::{parse_args, Command, HELP};
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    match parse_args(std::env::args().skip(1))? {
        ...
        Command::ExportProject(args) => {
            motolii_media_tools_hint();
            let gpu = GpuCtx::new_headless()?;
            let prepared = motolii_cli::prepare_project_export(&args.project)?;
        ...
        Command::VerifyB4(args) => {
            motolii_media_tools_hint();
            let gpu = GpuCtx::new_headless()?;
            match motolii_cli::verify_b4_project_v1(
                &gpu,
                &args.project,
                args.tolerance,
                args.export_first,
                ...
                Err(motolii_cli::B4VerifyError::Mismatch { report, tolerance, .. }) => {
