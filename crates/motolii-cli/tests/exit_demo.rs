...
fn eased_center_track() -> KeyframeTrack {
...
fn exit_demo_video_bg_plus_eased_rect_matches_golden() {
    ...
    let _tracks = DataTracks::new();
    let center_param = DocParam::Keyframes(eased_center_track());
    ...
    for (idx, label) in samples {
        ...
        let center = match center_param {
            DocParam::Keyframes(ref k) => match k.eval(t) {
                Value::Vec2(v) => v,
