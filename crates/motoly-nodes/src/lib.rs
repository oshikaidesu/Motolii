...
use motoly_core::{premultiply_rgba_f32, FrameDesc, RationalTime};
use motoly_eval::{DataTracks, ParamSource, Value};
use motoly_gpu::GpuCtx;
use motoly_plugin::{
    ...
    pub fn render(
        ...
        let mut encoder = gpu
            ...
            .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                label: Some("motoly-nodes-filter"),
            });
        self.plugin
    ...
    pub fn new(gpu: &GpuCtx) -> Self {
        ...
        let pipeline_layout = gpu
            ...
            .create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
                label: Some("motoly-nodes-overlay-layout"),
        ...
        let pipeline = gpu
            ...
            .create_render_pipeline(&wgpu::RenderPipelineDescriptor {
                label: Some("motoly-nodes-overlay-pipeline"),
        ...
        let sampler = gpu.device.create_sampler(&wgpu::SamplerDescriptor {
            label: Some("motoly-nodes-overlay-sampler"),
    ...
    pub fn render(
        ...
        let uniform_buffer = gpu
            ...
            .create_buffer_init(&wgpu::util::BufferInitDescriptor {
                label: Some("motoly-nodes-overlay-uniform"),
        ...
        let bind_group = gpu.device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("motoly-nodes-overlay-bg"),
        ...
        let mut encoder = gpu
            .device
            .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                label: Some("motoly-nodes-overlay"),
            });
        {
            let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                label: Some("motoly-nodes-overlay-pass"),
...
impl CompositeNode {
    ...
    pub fn with_mode(gpu: &GpuCtx, mode: CompositeMode) -> Self {
        let shader = gpu
            .device
            .create_shader_module(wgpu::ShaderModuleDescriptor {
                label: Some("motoly-nodes-composite-blend"),
                source: wgpu::ShaderSource::Wgsl(include_str!("composite_blend.wgsl").into()),
            });
        let bind_group_layout =
            gpu.device
                .create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
                    label: Some("motoly-nodes-composite-bgl"),
                    entries: &[
                        texture_entry(0),
                        sampler_entry(1),
                        texture_entry(2),
        ...
        let pipeline_layout = gpu
            .device
            .create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
                label: Some("motoly-nodes-composite-layout"),
                bind_group_layouts: &[Some(&bind_group_layout)],
                immediate_size: 0,
            });
        let pipeline = gpu
            .device
            .create_render_pipeline(&wgpu::RenderPipelineDescriptor {
                label: Some("motoly-nodes-composite-pipeline"),
                layout: Some(&pipeline_layout),
                vertex: wgpu::VertexState {
                    module: &shader,
                    entry_point: Some("vs_main"),
        ...
        let sampler = gpu.device.create_sampler(&wgpu::SamplerDescriptor {
            label: Some("motoly-nodes-composite-sampler"),
            mag_filter: wgpu::FilterMode::Nearest,
            min_filter: wgpu::FilterMode::Nearest,
            ..Default::default()
        });
    ...
    fn render_with_encoder(
        ...
        let uniform_buffer = gpu
            .device
            .create_buffer_init(&wgpu::util::BufferInitDescriptor {
                label: Some("motoly-nodes-composite-uniform"),
                contents: bytemuck::bytes_of(&uniform),
                usage: wgpu::BufferUsages::UNIFORM,
            });
        let bind_group = gpu.device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("motoly-nodes-composite-bg"),
            layout: &self.bind_group_layout,
            entries: &[
                wgpu::BindGroupEntry {
                    binding: 0,
        ...
        if let Some(encoder) = encoder {
            ...
            let mut encoder = gpu
                .device
                .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                    label: Some("motoly-nodes-composite"),
                });
    ...
    fn encode_pass(
        ...
        let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
            label: Some("motoly-nodes-composite-pass"),
            color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                view: output_view,
                depth_slice: None,
                resolve_target: None,
