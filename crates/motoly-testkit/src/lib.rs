//! motoly-testkit: ゴールデン画像テストの共通部品。
//!
...
pub fn gpu_or_skip() -> Option<motoly_gpu::GpuCtx> {
    match motoly_gpu::GpuCtx::new_headless() {
        Ok(gpu) => Some(gpu),
        Err(e) => {
...
pub fn tmp_dir(tag: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("motoly-{tag}-{}", std::process::id()));
...
mod tests {
    ...
    fn temp_artifact_dir(test_name: &str) -> PathBuf {
        std::env::temp_dir().join(format!("motoly-testkit-{test_name}-{}", std::process::id()))
    }
