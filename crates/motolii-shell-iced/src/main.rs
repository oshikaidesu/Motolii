...
use motolii_shell_iced::{
    view, IntentLog, Launch, Message, NativePrompts, Outcome, ScreenshotRequest, Shell, StatusLog,
};
...
fn main() -> iced::Result {
    ...
    let screenshot = launch.screenshot;
    ...
    iced::application(
        move || {
            Host::new(
                logs.borrow_mut().take().unwrap_or_default(),
                screenshot.clone(),
            )
        },
        Host::update,
...
impl Host {
    fn new(logs: Logs, screenshot: Option<ScreenshotRequest>) -> Self {
        Self {
