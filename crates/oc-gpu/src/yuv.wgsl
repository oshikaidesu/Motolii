...
fn fs(in: VsOut) -> @location(0) vec4<f32> {
    ...
    let yv = textureLoad(y_tex, y_coord, 0).r * 255.0;
    let uv = textureLoad(u_tex, c_coord, 0).r * 255.0;
    let vv = textureLoad(v_tex, c_coord, 0).r * 255.0;

    let yl = (yv - params.y_off) * params.y_scale;
    let cb = (uv - 128.0) * params.c_scale;
