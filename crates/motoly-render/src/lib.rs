...
use motoly_gpu::{upload_rgba, GpuCtx};
use motoly_nodes::{create_rgba_render_target, CompositeNode, NodeError, OverlayNode, RectOverlay};
use motoly_plugin::TextureRef;
...
pub struct BackgroundTextureRequest<'a> {
    ...
    /// 既にGPU上にある背景RGBAテクスチャ。動画フレームはmotoly-gpuのYUV→RGBA変換後に渡す。
        ...
        match *step {
            ...
            RenderStep::OverlayRect {
                ...
                let output_texture = session
                    .ensure_target(gpu, desc, output.0, "motoly-render-graph-overlay")
                    .clone();
            ...
            RenderStep::CompositeNormal {
                ...
                let output_texture = session
                    .ensure_target(gpu, desc, output.0, "motoly-render-graph-output")
                    .clone();
...
fn render_frame_direct(
    ...
    let foreground = create_rgba_render_target(gpu, desc, "motoly-render-foreground");
    let output = create_rgba_render_target(gpu, desc, "motoly-render-output");
...
mod tests {
    use super::*;
    use motoly_core::{Fps, Quality, TimeMap};
    use motoly_gpu::download_rgba;
    use motoly_nodes::{CanonicalPoint, CanonicalSize};
