//! motoly-plugin: 静的リンク版のプラグインホスト契約。
//!
...
use motoly_core::{CompCamera, Fps, FrameDesc, RationalTime};
use motoly_eval::{DataTrack, Value};
...
pub enum PluginKind {
    /// 予約: デコード/メディアソース。M1ではmotoly-media境界として扱う。
    Input,
...
pub mod reference {
    ...
    fn clear_texture(
        ...
        let _pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
            label: Some("motoly-plugin-clear"),
