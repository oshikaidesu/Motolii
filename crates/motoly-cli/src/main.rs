use motoly_cli::{parse_args, Command, HELP};
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    match parse_args(std::env::args().skip(1))? {
        ...
        Command::ExportOverlay(args) => {
            ...
            let report = export_overlay_video(
                ...
                &ExportOverlayRequest {
                    ...
                    data_tracks: motoly_eval::DataTracks::new(),
                    qp0: args.qp0,
                },
            )?;
            println!(
        ...
        Command::ExportProject(args) => {
            motoly_media_tools_hint();
            let gpu = GpuCtx::new_headless()?;
            let prepared = motoly_cli::prepare_project_export(&args.project)?;
        ...
        Command::VerifyB4(args) => {
            motoly_media_tools_hint();
            let gpu = GpuCtx::new_headless()?;
            match motoly_cli::verify_b4_project_v1(
                &gpu,
                &args.project,
                args.tolerance,
                args.export_first,
                ...
                Err(motoly_cli::B4VerifyError::Mismatch { report, tolerance, .. }) => {
