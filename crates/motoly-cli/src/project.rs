...
use motoly_core::{ColorSpace, Fps, FrameDesc, PixelFormat, Quality, RationalTime};
use motoly_eval::{DataTrackId, DataTracks, ParamSource, Value};
use motoly_export::{export_overlay_video, ExportOverlayRequest, ExportReport};
use motoly_gpu::{GpuCtx, RgbaDownloader, YuvToRgba};
use motoly_media::{probe, FrameReader, MediaInfo};
use motoly_nodes::{ParamOverlayError, ParamRectOverlay};
use motoly_plugin::{
...
pub enum ProjectError {
    ...
    #[error(transparent)]
    Media(#[from] motoly_media::MediaError),
    #[error(transparent)]
    Plugin(#[from] motoly_plugin::PluginError),
    #[error(transparent)]
    Export(#[from] motoly_export::ExportError),
    #[error(transparent)]
    Overlay(#[from] ParamOverlayError),
    #[error(transparent)]
    Render(#[from] motoly_render::RenderError),
    #[error(transparent)]
    Gpu(#[from] motoly_gpu::GpuRuntimeError),
}
...
impl PreparedProject {
    ...
    pub fn render_export_frame_rgba(
        ...
        Ok(downloader.download(
            gpu,
            &rendered.texture,
            motoly_export::EXPORT_DOWNLOAD_TIMEOUT,
        )?)
    }
    ...
    pub fn decode_exported_frame_rgba(
        ...
        let frame = reader
            .next_frame()?
            .ok_or(ProjectError::Export(motoly_export::ExportError::InvalidRequest(
    ...
    fn read_source_frame(&self, export_index: usize) -> Result<motoly_core::CpuFrame, ProjectError> {
        let mut reader =
        ...
        for _ in 0..export_index {
            let _ = reader.next_frame()?.ok_or(ProjectError::Export(
                motoly_export::ExportError::InvalidRequest("input ended before expected frame"),
            ))?;
        }
...
mod tests {
    ...
    use motoly_eval::{Interp, Keyframe, KeyframeTrack};
    use motoly_nodes::{CanonicalPoint, CanonicalSize, RectOverlay};
    use motoly_plugin::reference::SINE_PARAM_DRIVER;
    ...
    fn export_frame_count_falls_back_to_duration_when_nb_frames_missing() {
        ...
        let info = MediaInfo {
            ...
            nb_frames: None,
            color_space: motoly_core::ColorSpace::Rec709Limited,
            rotation: 0,
        };
