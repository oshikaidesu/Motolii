...
空の`semantic`集合は拒否(空の禁止CIを運用に乗せない)。現行の意味論ゴールデンは D1i-2 の `d1i2_pathop_geometry.rs` と D1i-3 の `d1i3_*.rs`(BlendMode / LookAt・Follow / Bezier / Transform合成)。暫定は D7 の `d7_clipping_mask.rs`(C-1 sRGBブレンド依存のクリスタ合成検証)。
