...
use motolii_gpu::{
    BudgetCap, MemoryTier, ResourceAdmissionError, ResourceBudgets, ResourceGrant, ResourceLedger,
    ResourceOwner, ResourceRequest,
};

const CACHE_ENTRY: ResourceOwner = ResourceOwner::new("cache-entry");
const PREFETCH: ResourceOwner = ResourceOwner::new("prefetch");
...
fn resident(
    ...
    bytes: u64,
    pinned: bool,
) -> Result<Resident, ResourceAdmissionError> {
    Ok(Resident {
        tier,
        bytes,
        _grant: ledger.admit(ResourceRequest {
            owner: CACHE_ENTRY,
            tier,
            bytes,
            pinned,
...
) -> Result<Transfer, (Resident, ResourceAdmissionError)> {
...
fn destination_refusal_keeps_source_resident_and_accounted() {
    ...
    let source = resident(&ledger, MemoryTier::Vram, 80, true).unwrap();

    let (source, error) = match begin_transfer(&ledger, source, MemoryTier::Ram) {
    ...
    assert_eq!(
        ...
        ResourceAdmissionError::BudgetExceeded {
            ...
            used_bytes: 0,
            budget_bytes: 60,
            cap: BudgetCap::Tier(MemoryTier::Ram),
        }
    );
    assert_eq!(source.tier, MemoryTier::Vram);
    assert_eq!(ledger.snapshot().unwrap().vram.resident_bytes, 80);
    assert_eq!(ledger.snapshot().unwrap().ram.resident_bytes, 0);
    drop(source);
...
fn transfer_keeps_both_tiers_until_commit_and_abort_keeps_source() {
    ...
    let source = resident(&ledger, MemoryTier::Vram, 80, true).unwrap();
    let transfer = begin_transfer(&ledger, source, MemoryTier::Ram).unwrap();
    assert_eq!(ledger.snapshot().unwrap().vram.resident_bytes, 80);
    assert_eq!(ledger.snapshot().unwrap().ram.resident_bytes, 80);
    ...
    assert_eq!(destination.tier, MemoryTier::Ram);
    assert_eq!(ledger.snapshot().unwrap().vram.resident_bytes, 0);
    assert_eq!(ledger.snapshot().unwrap().ram.resident_bytes, 80);
    drop(destination);
    assert_eq!(ledger.snapshot().unwrap().ram.resident_bytes, 0);
...
fn duplicate_demand_is_single_flight_and_stale_completion_releases_reservation() {
    ...
    assert_eq!(ledger.snapshot().unwrap().ram.resident_bytes, 60);
