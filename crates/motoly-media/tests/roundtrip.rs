...
use motoly_media::{probe, read_frame_at, Encoder, FrameReader};
...
fn assert_frame_is(frame: &motoly_core::CpuFrame, index: i64) {
...
fn tmp_dir(tag: &str) -> std::path::PathBuf {
    let dir = std::env::temp_dir().join(format!("motoly-media-{tag}-{}", std::process::id()));
...
fn encode_probe_decode_seek_roundtrip() {
    ...
    // 最低バージョン検証が通ること(#5)
    motoly_media::verify_tool_versions().expect("ffmpeg/ffprobe version check");
