...
fn runtime_rejects_executor_without_contract() {
    ...
    executors.register_filter(&CLEAR_FILTER).unwrap();
...
fn runtime_rejects_version_and_descriptor_mismatch() {
    let mut version_contract =
        filter_contract("core.filter.clear", 2, CLEAR_FILTER.desc().params.clone());
    version_contract.node.display_name = CLEAR_FILTER.desc().display_name;
    version_contract.node.category = CLEAR_FILTER.desc().category;
    version_contract.node.tags = CLEAR_FILTER.desc().tags;
    let mut builder = PluginCatalogBuilder::new();
