...
fn delete_definition_while_used_is_rejected() {
    ...
    assert_eq!(
        ...
        CommandError::DefinitionInUse {
            id: s.d1.get(),
            use_ids: vec![s.u1.get(), s.u2.get(), s.u3.get()],
        }
    );
...
#[test]
fn remove_effect_create_inverse_rejects_when_definition_still_shared_and_leaves_document_unchanged()
{
    let s = shared_fixture();
    let before = s.doc.clone();
    ...
    let err = cmd
        .apply(&mut working)
        .expect_err("create inverse must reject while definition is shared");
    assert_eq!(
        err,
        CommandError::DefinitionInUse {
            id: s.d1.get(),
...
fn undo_copy_local_rejects_when_new_definition_still_shared() {
    ...
    let err = undo
        .apply(&mut working)
        .expect_err("shared new definition must reject undo");
    assert_eq!(
        err,
...
#[test]
fn migration_use_id_zero_without_counter_allocates_non_colliding_definition_id() {
    let raw = json!({
        "version": 1,
        "min_reader_version": 1,
        "composition": {
    ...
    let (doc, _) = migrate_bytes(&serde_json::to_vec(&raw).unwrap()).expect("migrate");
    let use_ = &doc.tracks[0].items[0].as_clip().unwrap().envelope.effects[0];
    assert_eq!(use_.id.get(), 0);
    assert_ne!(use_.definition_id.get(), 0);
    assert_eq!(use_.definition_id.get(), 1);
    doc.validate().unwrap();
}

#[test]
fn migration_preserves_three_inline_effects_order_and_extra() {
    let raw = json!({
        "version": 1,
        "min_reader_version": 1,
        "composition": {
        ...
        "tracks": [{
            ...
            "items": [{
                ...
                "envelope": {
                    "layer_id": 0,
                    "effects": [
                        {"id": 1, "plugin_id": "p.a", "vendor": "a"},
                        {"id": 2, "plugin_id": "p.b", "vendor": "b"},
                        {"id": 3, "plugin_id": "p.c", "vendor": "c"}
                    ],
                    "transform": {
                        "position": {"const": {"Vec2": [0.0, 0.0]}},
                        "anchor": {"const": {"Vec2": [0.0, 0.0]}},
    ...
    let (doc, _) = migrate_bytes(&serde_json::to_vec(&raw).unwrap()).unwrap();
    let effects = &doc.tracks[0].items[0].as_clip().unwrap().envelope.effects;
    assert_eq!(effects.len(), 3);
    assert_eq!(effects[0].id.get(), 1);
    assert_eq!(effects[1].id.get(), 2);
    assert_eq!(effects[2].id.get(), 3);
    let vendors = ["a", "b", "c"];
    for (i, use_) in effects.iter().enumerate() {
        let def = doc.effect_definition(use_.definition_id).unwrap();
        assert_eq!(
            def.plugin_id,
            format!("p.{}", vendors[i].chars().next().unwrap())
        );
        assert_eq!(
            def.extra.get("vendor"),
            Some(&JsonValue::String(vendors[i].into()))
        );
    }
}

#[test]
fn migration_nested_group_inline_effects() {
    let raw = json!({
        "version": 1,
        "min_reader_version": 1,
        "composition": {
        ...
        "tracks": [{
            ...
            "items": [{
                ...
                "envelope": {
                    "layer_id": 0,
                    "effects": [{"id": 10, "plugin_id": "p.group", "keep": true}],
                    "transform": {
                        "position": {"const": {"Vec2": [0.0, 0.0]}},
                        "anchor": {"const": {"Vec2": [0.0, 0.0]}},
                        "scale": {"const": {"Vec2": [1.0, 1.0]}},
                ...
                "children": [{
                    ...
                    "envelope": {
                        "layer_id": 1,
                        "effects": [{"id": 11, "plugin_id": "p.child", "keep": true}],
                        "transform": {
                            "position": {"const": {"Vec2": [0.0, 0.0]}},
                            "anchor": {"const": {"Vec2": [0.0, 0.0]}},
                            "scale": {"const": {"Vec2": [1.0, 1.0]}},
    ...
    let (doc, _) = migrate_bytes(&serde_json::to_vec(&raw).unwrap()).unwrap();
    assert_eq!(doc.effect_definitions.len(), 2);
    let TrackItem::Group(group) = &doc.tracks[0].items[0] else {
        panic!("expected group");
    };
    assert_eq!(group.envelope.effects[0].id.get(), 10);
    let TrackItem::Clip(clip) = &group.children[0] else {
        panic!("expected clip");
    };
    assert_eq!(clip.envelope.effects[0].id.get(), 11);
    doc.validate().unwrap();
}

#[test]
fn migration_rejects_hybrid_definition_id_and_inline_payload() {
    let raw = json!({
        "version": 1,
        "min_reader_version": 1,
        "composition": {
    ...
    let err = migrate_bytes(&serde_json::to_vec(&raw).unwrap()).expect_err("hybrid must fail");
    assert!(matches!(err, MigrateError::HybridEffectEntry { .. }));
}
