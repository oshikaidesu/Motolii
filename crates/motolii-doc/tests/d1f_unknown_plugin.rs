...
fn unknown_effect_plugin_id_loads_warns_and_roundtrips() {
    ...
    let effect = &clip.envelope.effects[0];
    assert_eq!(effect.plugin_id, "vendor.filter.glow_deluxe");
    assert_eq!(effect.effect_version, 3);
    assert_eq!(effect.params.get("amount"), Some(&DocParam::const_f64(0.5)));
    assert_eq!(effect.extra.get("vendor_flag"), Some(&json!(true)));
}
...
fn known_plugin_future_version_is_degraded_not_a_downgrade_error() {
    // core.filter.opacity は現行 version 1(motolii-plugin側 NodeDesc.version と同期)。
    ...
    if let TrackItem::Clip(clip) = &mut doc.tracks[0].items[0] {
        ...
        clip.envelope.effects.push(EffectInstance {
            ...
            params: BTreeMap::from([("amount".into(), DocParam::const_f64(0.9))]),
            extra: Map::new(),
        });
    }
...
fn raw_json_with_unknown_plugin_id_and_future_version_loads_and_preserves_extra() {
    ...
    let TrackItem::Clip(clip) = &roundtrip.tracks[0].items[0] else {
        panic!("expected clip");
    };
    assert_eq!(
        clip.envelope.effects[0].extra.get("vendor_only_field"),
