...
impl Document {
    ...
    pub fn effect_use_count(&self, definition_id: EffectDefinitionId) -> usize {
        fn count_in_items(items: &[TrackItem], definition_id: EffectDefinitionId) -> usize {
            let mut n = 0;
            for item in items {
                match item {
                    TrackItem::Clip(clip) => {
                        n += clip
                            ...
                            .effects
                            .iter()
                            .filter(|u| u.definition_id == definition_id)
                            .count();
                    }
                    TrackItem::Group(group) => {
                        n += group
                            ...
                            .filter(|u| u.definition_id == definition_id)
                            .count();
                        n += count_in_items(&group.children, definition_id);
                    }
                }
            }
            n
        ...
        self.tracks
            .iter()
            .map(|t| count_in_items(&t.items, definition_id))
            .sum()
    }
