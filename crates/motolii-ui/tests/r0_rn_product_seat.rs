...
fn read_project_json(path: &Path) -> Vec<u8> {
    fs::read(path).expect("read project.json")
}
...
#[test]
fn r1_browser_snapshot_projects_host_issued_rectangle_identity_without_document_write() {
    let _lock = HOST_TEST_LOCK.lock().expect("host test lock");
    let path = fixture_path("browser-rectangle-source");
    let project_json_before = read_project_json(&path);
    let host = host_create_for_test(&path).expect("create host");

    let snapshot = host_read_snapshot_for_test(host).expect("browser snapshot");
    assert_eq!(
        snapshot.browser_rectangle_scope_ref,
        format!("builtin-{host}")
    );
    assert_eq!(snapshot.browser_rectangle_item_id, "rectangle");
    assert_eq!(snapshot.revision, "0");
    assert_eq!(snapshot.projection_generation, "0");

    host_destroy_for_test(host).expect("destroy browser host");
    assert_eq!(
        read_project_json(&path),
        project_json_before,
        "Browser read projection must leave project.json bytes unchanged"
    );
}
...
fn r0_product_seat_preserves_snapshot_across_mount_resize_focus_unmount_remount() {
    let _lock = HOST_TEST_LOCK.lock().expect("host test lock");
    let path = fixture_path("lifecycle");
    let project_json_before = read_project_json(&path);
    let host = host_create_for_test(&path).expect("create host");
    ...
    assert_ne!(
        remount_stage, stage,
        "remount stage handle must differ from destroyed prior handle"
    );
    let remount =
    ...
    assert_eq!(
        read_project_json(&path),
        project_json_before,
        "healthy lifecycle must leave project.json bytes unchanged"
    );
}
...
fn r0_product_seat_rejects_stale_generation_without_document_write() {
    ...
    assert_eq!(after.layer_ids, before.layer_ids);
    host_destroy_for_test(host).expect("destroy stale host");
    assert_eq!(
        read_project_json(&path),
        project_json_before,
        "stale rejection must leave project.json bytes unchanged"
    );
}
...
fn r0_product_seat_rejects_incomplete_or_invalid_lifecycle_payloads() {
    let _lock = HOST_TEST_LOCK.lock().expect("host test lock");
    let path = fixture_path("invalid-lifecycle");
    let project_json_before = read_project_json(&path);
    let host = host_create_for_test(&path).expect("create host");
    let stage = host_register_stage_for_test(host).expect("register stage");
    ...
    assert_eq!(
        read_project_json(&path),
        project_json_before,
        "invalid lifecycle rejection must leave project.json bytes unchanged"
    );
}
...
fn r0_product_seat_rejects_single_host_invalid_path_and_late_handles() {
    ...
    let after_second = host_read_snapshot_for_test(host).expect("first host remains");
    ...
    let after_late = host_read_snapshot_for_test(host).expect("after late");
    assert_eq!(after_late.revision, baseline.revision);
    assert_eq!(
        after_late.projection_generation,
        baseline.projection_generation
    );
    assert_eq!(after_late.primary_layer_id, baseline.primary_layer_id);
    assert_eq!(after_late.layer_ids, baseline.layer_ids);
    ...
    assert_eq!(
        read_project_json(&path),
        project_json_before,
        "late/double-destroy/second-host rejection must leave project.json bytes unchanged"
    );
}
