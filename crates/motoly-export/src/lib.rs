//! motoly-export: M1の最小書き出しループ。
//!
//! 解析やCLIはまだ持たず、動画フレームをGPUでRGBA化し、motoly-renderの共通経路で
//! オーバーレイ合成して、motoly-media::Encoderへ流す。

use std::path::Path;
use std::time::Duration;

use motoly_core::{ColorSpace, FrameDesc, PixelFormat, Quality};
...
use motoly_render::{render_frame_with_background_texture, BackgroundTextureRequest, RenderSession};
