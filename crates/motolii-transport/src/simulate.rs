//! ヘッドレス向けプレビュー/再生シミュレータ(D5完了条件の機械判定)。
...
use crate::clock::drift_within_one_frame;
use crate::drs::{DrsConfig, FrameTiming};
use crate::{FramePlan, Transport};
...
impl PreviewSimulator {
    ...
    /// 1音声コールバック分を進め、供給済み列を記録する。
    pub fn tick_audio_callback(&mut self, device_wait_frames: u64) {
        self.transport
            .device_wait()
            .set_wait_frames(device_wait_frames);
        let mut buf = vec![0.0f32; self.chunk_frames];
        fill_or_silence(&self.ring_cons, &mut buf, &self.counters);
        self.supplied_trace
            .push(self.counters.frames_supplied());
    }

    /// レンダ1回(遅延`render_cost`をDRSへ記録)。
    pub fn tick_render(&mut self, render_cost: Duration) -> FramePlan {
        let plan = self
            .transport
            .next_frame_plan()
            .expect("valid transport state");
        self.transport.record_render_timing(FrameTiming {
            gpu: render_cost,
            cpu: Duration::ZERO,
            wall: render_cost,
        });
    ...
    pub fn run_half_speed_render(
        ...
        while elapsed < total_frames {
            if let Ok(chunk) = cache.read_frames(playhead, self.chunk_frames) {
                let _ = ring_prod.push_frames(&chunk);
            }
            self.tick_audio_callback(256);
            elapsed += self.chunk_frames as u64;
            samples_since_opportunity += self.chunk_frames as u64;

            let supplied_now = self.counters.frames_supplied();
            let advanced = supplied_now.saturating_sub(playhead);
            if advanced != self.chunk_frames as u64 {
                pcm_ok = false;
            }
            playhead = supplied_now;
...
mod tests {
    ...
    #[test]
    fn ten_minute_drift_within_one_frame() {
    ...
    fn half_speed_render_drops_video_keeps_audio_clock() {
        ...
        assert!(report.drs_downgraded || !sim.transport().drs().is_enabled());
    }
    ...
    #[test]
    fn timestamp_query_disabled_keeps_draft_quality() {
        let transport = test_transport_headless(48_000, false);
