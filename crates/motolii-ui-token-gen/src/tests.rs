...
#[test]
fn v2_product_profile_adds_deterministic_css_without_changing_v1() {
    let sources = vec![theme("fixture-dark", "0.125")];
    let v1 = generate_bundle(sources.clone()).unwrap();
    let v2 = generate_bundle_for(BundleProfile::V2Product, sources).unwrap();

    assert!(v1.tokens_css.is_none());
    assert!(String::from_utf8(v1.tokens_rs)
        .unwrap()
        .starts_with("// @generated by motolii-ui-token-gen v1;"));

    let css = String::from_utf8(v2.tokens_css.unwrap()).unwrap();
    assert!(css.starts_with("/* @generated by motolii-ui-token-gen v2;"));
    assert!(css.contains(":root[data-motolii-theme=\"fixture-dark\"]"));
    assert!(css.contains("--motolii-palette-accent: #2080ff40;"));
    assert!(css.contains("--motolii-layout-nested-gap: 12.5px;"));
    assert!(css.contains("--motolii-motion-delay: 200ms;"));
    assert!(css.contains("--motolii-motion-curve: cubic-bezier(0, -2, 1, 3);"));

    let manifest: serde_json::Value = serde_json::from_slice(&v2.manifest_json).unwrap();
    assert_eq!(manifest["generator"]["version"], 2);
    assert_eq!(manifest["outputs"][0], "tokens.rs");
    assert_eq!(manifest["outputs"][1], "tokens.css");
    assert_eq!(manifest["outputs"][2], "manifest.json");
}

#[test]
fn product_profile_directory_contract_is_read_only_and_profile_specific() {
    let directory = tempdir().unwrap();
    let sources = vec![theme("fixture-dark", "0.125")];
    generate_to_dir_for(BundleProfile::V2Product, sources.clone(), directory.path()).unwrap();
    let before = snapshot(directory.path());

    check_dir_for(BundleProfile::V2Product, sources.clone(), directory.path()).unwrap();
    assert_eq!(snapshot(directory.path()), before);
    assert!(matches!(
        check_dir_for(BundleProfile::V1Fixture, sources, directory.path()),
...
#[test]
fn product_profile_rejects_css_variable_collisions() {
    let collision = format!(
        r#"{{"$schema":"{SCHEMA_URI}","a-b":{{"c":{{"$type":"duration","$value":{{"value":0,"unit":"ms"}}}}}},"a":{{"b-c":{{"$type":"duration","$value":{{"value":0,"unit":"ms"}}}}}}}}"#
    );
    assert!(matches!(
        generate_bundle_for(
            BundleProfile::V2Product,
            vec![source("motolii-dark", &collision)]
        ),
        Err(Error::CssVariableCollision { .. })
    ));
}

#[test]
fn bundle_profile_is_closed() {
    assert_eq!(
        BundleProfile::parse("v1-fixture").unwrap(),
        BundleProfile::V1Fixture
    );
    assert_eq!(
        BundleProfile::parse("v2-product").unwrap(),
        BundleProfile::V2Product
    );
    assert!(matches!(
        BundleProfile::parse("v3"),
...
#[test]
fn committed_product_bundle_is_byte_exact_generator_output() {
    let product_root = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../ui/motolii-tokens");
    let source_path = product_root.join("sources/motolii-dark.json");
    let sources = vec![ThemeSource {
        id: "motolii-dark".to_owned(),
        bytes: fs::read(source_path).unwrap(),
    }];
    check_dir_for(
        BundleProfile::V2Product,
        sources,
        &product_root.join("generated"),
    )
    .unwrap();
}
