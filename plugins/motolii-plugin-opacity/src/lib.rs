//! `core.filter.opacity` v1 — premul RGBA 全体に `amount` を掛ける外部参照Filter。

use std::sync::OnceLock;

use motolii_plugin::{
    bytemuck, wgpu, F64Domain, FilterPlugin, GpuCtx, NodeDesc, ParamDef, PipelineCache,
    PipelineCacheKey, PluginContract, PluginError, PluginId, PluginKind, RenderCtx, ResolvedParams,
    TextureRef, Value, ValueType,
};
